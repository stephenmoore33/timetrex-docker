...
if ( version_compare( PHP_VERSION, '8.3.99', '>' ) == 1 ) {
	echo "You are currently using PHP v<b>" . PHP_VERSION . "</b> TimeTrex requires PHP <b>v8.3.x</b> or earlier!<br><br>\n";
	$redir = false;
}
...
echo " 9...";
@file_get_contents( 'http://coreapi.timetrex.com/pre_install.php?os=' . PHP_OS . '&php_version=' . PHP_VERSION . '&redir=' . (int)$redir . '&web_server=' . urlencode( substr( $_SERVER['SERVER_SOFTWARE'], 0, 20 ) ) . '&external_installer=' . $external_installer . '&url=' . urlencode( $_SERVER['HTTP_HOST'] . ':' . $_SERVER['SERVER_PORT'] . $_SERVER['REQUEST_URI'] ), 'r' );
...
if ( $redir == true ) {
	...
	echo "<meta http-equiv='refresh' content='0;url=../html5/?installer=1&disable_db=1&external_installer=" . $external_installer . "#!m=Install&a=license&external_installer=" . $external_installer . "'>";
} else {
	echo " FAILED!<br><br>\n";
