		...
		if ( $rl->check() == false ) {
			...
			if ( ( $permission->Check( 'document', 'view' ) || $permission->Check( 'document', 'view_own' ) || $permission->Check( 'document', 'view_child' ) || $permission->Check( 'document', 'view_private' ) )
					|| ( isset( $parent_object_type_id ) && $parent_object_type_id == 400 //400=Expense
							&& ( $permission->Check( 'user_expense', 'view_own' ) || $permission->Check( 'user_expense', 'view_child' ) || $permission->Check( 'user_expense', 'view' ) ) )
			) {
	...
	case 'punch_image':
		...
		if ( $rl->check() == false ) {
			...
			if ( $permission->Check( 'punch', 'view' )
				...
				$result = $api_f->stripReturnHandler( $api_f->getPunch( [ 'filter_data' => [ 'id' => $object_id ], 'filter_columns' => [ 'id' => true, 'user_id' => true ] ] ) ); //Only return minimum necessary columns so we aren't trying to parse custom fields and such.
				if ( isset( $result[0] ) && count( $result[0] ) > 0 ) {
					...
					if ( $file_name != '' || file_exists( $file_name ) ) {
						...
						$params['file'] = $file_name;
						//$params['ContentType'] = Misc::getMimeType( $file_name );
						$params['ContentType'] = 'image/jpeg'; //All punch images are JPEGs now, so just default to that. This is an optimization to save having to read the file, especially helpful for punch image reports.
						$params['ContentDisposition'] = [ getSendFileDownloadMode( $mode, HTTP_DOWNLOAD_INLINE ), basename( $file_name ) ];
						$params['cache'] = true;
					} else {
...
if ( isset( $params ) ) {
	if ( isset( $config_vars['other']['enable_xsendfile'] ) && $config_vars['other']['enable_xsendfile'] == true ) {
		Debug::Text( 'NOTICE: Downloading through X-SendFile...', __FILE__, __LINE__, __METHOD__, 10 );
		$params['xsendfile'] = true;
	}
