				...
				if ( $cc_obj->checkPasswordResetKey( $key ) == true ) {
					...
					if ( $password == $password2 ) {
						...
						if ( $cc_obj->isValid() ) {
							...
							Redirect::Page( 'https://coreapi.timetrex.com/store?step=login' );
						}
					} else {
	...
	case 'reset_password':
		...
		if ( $rl->check() == false ) {
			...
			if ( $cclf->getRecordCount() == 1 ) {
				...
				if ( $cc_obj->getStatus() == 10 ) { //10=Enabled.
					...
					Debug::Text( 'Found User, but its disabled! ', __FILE__, __LINE__, __METHOD__, 10 );
					$validator->isTrue( 'email', false, 'Email address was not found in our database (d)' );
				}
			} else {
				//Error
			...
			<div class="col-12">
				<div id="contentBox-ForgotPassword">
					...
					<div id="rowWarning" class="text-center">
						<?php echo TTi18n::getText( 'An email has been sent to' ) . ' <b>' . $email . '</b> ' . TTi18n::getText( 'with instructions on how to reset your password.' ); ?>
					</div>
				</div>
				<?php
				} else {
