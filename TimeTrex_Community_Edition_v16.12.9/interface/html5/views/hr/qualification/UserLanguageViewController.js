export class UserLanguageViewController extends BaseViewController {
			...
			include: [
				{
					...
					permission_result: PermissionManager.checkTopLevelPermission( 'ImportCSVEmployeeLanguage' ),
					permission: null,
					sort_order: 9010
				}
			]
	...
	setDefaultMenuImportIcon( context_btn, grid_selected_length, pId ) {
		if ( PermissionManager.checkTopLevelPermission( 'ImportCSVEmployeeLanguage' ) === true ) {
	...
	onImportClick() {
		var $this = this;
		IndexViewController.openWizard( 'ImportCSVWizard', 'UserLanguage', function() {
			$this.search();
		} );
	}
