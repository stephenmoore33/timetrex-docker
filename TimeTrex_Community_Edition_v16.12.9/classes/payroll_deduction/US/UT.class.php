...
class PayrollDeduction_US_UT extends PayrollDeduction_US {
	...
	var $state_ui_options = [
			20240101 => [ 'wage_base' => 47000, 'new_employer_rate' => null ], //New employer rate varies
			20230101 => [ 'wage_base' => 44800, 'new_employer_rate' => null ], //New employer rate varies
			20220101 => [ 'wage_base' => 41600, 'new_employer_rate' => null ], //New employer rate varies
			20210101 => [ 'wage_base' => 38900, 'new_employer_rate' => null ],
			20200101 => [ 'wage_base' => 36600, 'new_employer_rate' => null ],
			20190101 => [ 'wage_base' => 35300, 'new_employer_rate' => null ],
	];
	...
	function getStateAllowanceAmount() {
		...
		$retval = TTMath::add( TTMath::mul( $this->getStateAllowance(), $allowance_amount ), $this->getStateBaseAllowanceAmount() );
	...
	function getStateAllowanceReductionAmount() {
		...
		$adjusted_taxable_income = TTMath::sub( $this->getStateAnnualTaxableIncome(), $allowance_reduction_amount );
		if ( $adjusted_taxable_income > 0 ) {
			$allowance_reduction_rate = $retarr['allowance_reduction_rate'];
			$retval = TTMath::mul( $adjusted_taxable_income, TTMath::div( $allowance_reduction_rate, 100 ) );
		} else {
			$retval = 0;
		}
	...
	function _getStateTaxPayable() {
		...
		if ( $annual_income > 0 ) {
			...
			$gross_tax = TTMath::mul( $annual_income, TTMath::div( $retarr['rate'], 100 ) );
			$allowance_amount = TTMath::sub( $this->getStateAllowanceAmount(), $this->getStateAllowanceReductionAmount() );
			if ( $allowance_amount < 0 ) {
				$allowance_amount = 0;
			}

			$retval = TTMath::sub( $gross_tax, $allowance_amount );
		}
