...
class ChequeForms_Base {
    ...
    */
    function filterAmountWords( $value ) {
        if ( isset( $this->amount ) ) {
			$num_to_words_obj = ( new NumberToWords\NumberToWords() )->getNumberTransformer('en');

            $value =  str_pad( ucwords( $num_to_words_obj->toWords( floor($this->amount) ) ) .' ', 65, "-", STR_PAD_RIGHT );
        }
    ...
    */
    function filterAmountCents( $value ) {
        if ( isset( $this->amount ) ) {
            $value = TTMath::getAfterDecimal($this->amount) .'/100';
        }
        return $value;
    }
    ...
    */
    function filterAmountPadded( $value ) {
        if ( isset( $this->amount ) ) {
            $value = str_pad( TTi18n::MoneyFormat( $this->amount ), 12, '*', STR_PAD_LEFT);
        }
