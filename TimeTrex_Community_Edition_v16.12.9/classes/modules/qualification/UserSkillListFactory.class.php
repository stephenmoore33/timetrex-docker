 ...
 * @implements IteratorAggregate<UserSkillFactory>
 */
class UserSkillListFactory extends UserSkillFactory implements IteratorAggregate {
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$additional_order_fields = [ 'uf.first_name', 'uf.last_name', 'qf.name', 'qgf.name', 'df.name', 'bf.name', 'ugf.name', 'utf.name', 'proficiency_id' ];
		...
		if ( isset( $filter_data['proficiency'] ) && !is_array( $filter_data['proficiency'] ) && trim( $filter_data['proficiency'] ) != '' && !isset( $filter_data['proficiency_id'] ) ) {
		...
		$query .= ( isset( $filter_data['tag'] ) ) ? $this->getWhereClauseSQL( 'a.id', [ 'company_id' => TTUUID::castUUID( $company_id ), 'object_type_id' => 251, 'tag' => $filter_data['tag'] ], 'tag', $ph ) : null;
