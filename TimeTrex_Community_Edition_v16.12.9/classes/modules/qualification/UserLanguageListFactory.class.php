 ...
 * @implements IteratorAggregate<UserLanguageFactory>
 */
class UserLanguageListFactory extends UserLanguageFactory implements IteratorAggregate {
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$additional_order_fields = [ 'first_name', 'last_name', 'qualification', 'fluency_id', 'competency_id', 'qgf.name', 'default_branch', 'default_department', 'user_group', 'title' ];
		...
		if ( $order == null ) {
			$order = [ 'qualification' => 'asc', 'qgf.name' => 'asc' ];
			$strict = false;
		} else {
		...
		if ( isset( $filter_data['fluency'] ) && !is_array( $filter_data['fluency'] ) && trim( $filter_data['fluency'] ) != '' && !isset( $filter_data['fluency_id'] ) ) {
			$filter_data['fluency_id'] = Option::getByFuzzyValue( $filter_data['fluency'], $this->getOptions( 'fluency' ) );
		}
		if ( isset( $filter_data['competency'] ) && !is_array( $filter_data['competency'] ) && trim( $filter_data['competency'] ) != '' && !isset( $filter_data['competency_id'] ) ) {
		...
		$query .= ( isset( $filter_data['tag'] ) ) ? $this->getWhereClauseSQL( 'a.id', [ 'company_id' => TTUUID::castUUID( $company_id ), 'object_type_id' => 254, 'tag' => $filter_data['tag'] ], 'tag', $ph ) : null;
