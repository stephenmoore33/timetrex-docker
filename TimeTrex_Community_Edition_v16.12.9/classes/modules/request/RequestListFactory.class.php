 ...
 * @implements IteratorAggregate<RequestFactory>
 */
class RequestListFactory extends RequestFactory implements IteratorAggregate {
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			$rsf = new RequestScheduleFactory();
		}
		...
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			$query .= '		CASE WHEN rsf.id IS NOT NULL THEN rsf.start_date ELSE NULL END as request_schedule_start_date,
							CASE WHEN rsf.id IS NOT NULL THEN rsf.end_date ELSE NULL END as request_schedule_end_date, ';
		}

		$query .= '			a.user_id as user_id,
		...
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			$query .= ' LEFT JOIN ' . $rsf->getTable() . ' as rsf ON ( rsf.request_id = a.id AND rsf.deleted = 0 ) ';
		}

		$query .= '		LEFT JOIN ' . $uf->getTable() . ' as b ON ( a.user_id = b.id AND b.deleted = 0 )
		...
		if ( isset( $filter_data['start_date'] ) && !is_array( $filter_data['start_date'] ) && trim( $filter_data['start_date'] ) != '' ) {
			...
			$query .= ' AND ( a.date_stamp >= ? ';
			if ( isset( $filter_data['end_date'] ) && !is_array( $filter_data['end_date'] ) && trim( $filter_data['end_date'] ) != '' && getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
				$ph[] = $this->db->BindDate( (int)TTDate::parseDateTime( $filter_data['end_date'] ) ); //Should be end date.
				$ph[] = $this->db->BindDate( (int)TTDate::parseDateTime( $filter_data['start_date'] ) );
				$query .= ' OR ( rsf.start_date IS NOT NULL AND rsf.end_date IS NOT NULL AND rsf.start_date <= ? AND rsf.end_date >= ? ) )';
			} else {
				$query .= ' )';
			}
		}
		if ( isset( $filter_data['end_date'] ) && !is_array( $filter_data['end_date'] ) && trim( $filter_data['end_date'] ) != '' ) {
			...
			$query .= ' AND ( a.date_stamp <= ? ';
			if ( isset( $filter_data['start_date'] ) && !is_array( $filter_data['start_date'] ) && trim( $filter_data['start_date'] ) != '' && getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
				$ph[] = $this->db->BindDate( (int)TTDate::parseDateTime( $filter_data['end_date'] ) );
				$ph[] = $this->db->BindDate( (int)TTDate::parseDateTime( $filter_data['start_date'] ) ); //Should be start date.
				$query .= ' OR ( rsf.start_date IS NOT NULL AND rsf.end_date IS NOT NULL AND rsf.start_date <= ? AND rsf.end_date >= ? ) )';
			} else {
				$query .= ' )';
			}
