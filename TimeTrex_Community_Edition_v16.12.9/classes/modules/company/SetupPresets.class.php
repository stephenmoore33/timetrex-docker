...
class SetupPresets extends Factory {
	...
	function PayStubAccounts( $country = null, $province = null, $district = null, $industry = null ) {
			...
			switch ( $country ) {
				case 'ca':
					...
					*/
					...
					$cpp2_employee_psea_id = $this->createPayStubAccount(
							[
									'company_id'     => $this->getCompany(),
									'status_id'      => 10,
									'type_id'        => 20,
		...
		if ( $country == 'us' && $province != '' ) {
			...
			if ( in_array( $province, [ 'al', 'ca', 'co', 'de', 'ia', 'in', 'ky', 'md', 'mi', 'mo', 'oh', 'pa', 'wv', 'nj' ] ) ) {
				//CA=San Fransico, NJ=Newark, NY=NYC, New York City (But is handled differenly below)
				//CO=Flat-rate wage taxes are imposed in Denver and four other Colorado cities
				//WV=four cities in West Virginia.
				//OR=Transportation Tax (handled differently below)
				$this->createPayStubAccount(
						[
								'company_id'     => $this->getCompany(),
								'status_id'      => 10,
			...
			if ( in_array( $province, [ 'ny' ] ) ) {
				$this->createPayStubAccount(
						[
								...
								'ps_order'       => 206,
								'debit_account'  => '',
								'credit_account' => 'NY - New York City Income Tax' . $gl_account_payable_suffix, //2192
						]
				);
				...
				$this->createPayStubAccount(
						[
								...
								'ps_order'       => 206,
								'debit_account'  => '',
								'credit_account' => 'NY - Yonkers Income Tax' . $gl_account_payable_suffix, //2192
						]
				);
			}
			...
			if ( in_array( $province, [ 'co', 'ct', 'ma', 'nj', 'ny', 'or', 'wa', 'dc', ] ) ) { //CA is through State Disability (SDI), RI is through Temporary Disability Insurance.
				$this->createPayStubAccount(
						[
								'company_id'     => $this->getCompany(),
								'status_id'      => 10,
			...
			switch ( $province ) {
				...
				case 'mt': //montana
					...
					break;
				case 'ne': //nebraska
					//Unemployment Insurance - Employee (aka: SUIT)
					break;
				case 'nv': //nevada
					//Unemployment Insurance - Employee
					//Career Enhancement
				...
				case 'ri': //rhode island
					...
					//$this->createPayStubAccount( //Use standard "Unemployment Insurance" name for this instead.
					//		[
					//				'company_id'     => $this->getCompany(),
					//				'status_id'      => 10,
					//				'type_id'        => 30,
				...
				case 'wa': //washington
					...
					$this->createPayStubAccount(
							[
									...
									'status_id'      => 10,
									'type_id'        => 20,
									'name'           => strtoupper( $province ) . ' - CARES Fund',
									'ps_order'       => 211,
									'debit_account'  => '',
									'credit_account' => 'CARES Fund' . $gl_account_payable_suffix,
							]
					);
			...
			switch ( $country ) {
				case 'ca':
					...
					$form_t4_config = [
							...
							'income'       => [ //Employment Income (Box 14) - Should match Gross Pay on Remittance Summary Report (PD7A)
												...
												'exclude_pay_stub_entry_account' => [
														...
														//$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, 'Health Benefits Plan' ), //This references Extended Health Care (EHC) plans like Bluecross. https://help.timetrex.com/latest/professional/References/Canada-Taxation-of-Group-Benefits.htm (See private notes on that page)
														//$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, 'Dental Benefits Plan' ), ///This references Extended Health Care (EHC) plans like Bluecross. https://help.timetrex.com/latest/professional/References/Canada-Taxation-of-Group-Benefits.htm (See private notes on that page)
												],
							],
							...
							'employee_cpp2' => [ //Employee CPP2 (Box 16A)
												'include_pay_stub_entry_account' => [
														$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, 'CPP2' ),
												],
												'exclude_pay_stub_entry_account' => [],
							],
							'employer_cpp2' => [ //Employer CPP
												'include_pay_stub_entry_account' => [
														$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, 'CPP2 - Employer' ),
												],
												'exclude_pay_stub_entry_account' => [],
							],
							...
							'other_box'    => [ //Other Box
												...
												[
														'box'                            => 85, //(Code 85) - Employee-paid premiums for private health services plans, including plans like BlueCross. (Optional - But required for the employee to take advantage of the tax savings when they file tax return at year end), but may avoid requiring employee requiring supporting documents. - https://www.canada.ca/en/revenue-agency/services/tax/businesses/topics/payroll/benefits-allowances/private-health-services-plan-premiums.html
														'include_pay_stub_entry_account' => [
					...
					$form_roe_config = [
							...
							'other_psea_ids'              => [ //Other Monies for Box 17C
															   $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Termination Pay (In Lieu Of Notice)' ),
															   $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Bonus' ),
															   $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Severance Pay (Retiring Allowance)' ),
							],
							'absence_policy_ids'          => [],
					];

					$form_roe_config['absence_policy_ids'] = array_merge( $form_roe_config['absence_policy_ids'], (array)$this->getAbsencePolicyByCompanyIDAndName( '%Sick (PAID)' ) );
					$form_roe_config['absence_policy_ids'] = array_merge( $form_roe_config['absence_policy_ids'], (array)$this->getAbsencePolicyByCompanyIDAndName( '%Vacation (PAID)' ) );
					$form_roe_config['absence_policy_ids'] = array_merge( $form_roe_config['absence_policy_ids'], (array)$this->getAbsencePolicyByCompanyIDAndName( '%Statutory Holiday' ) );
	 ...
	 * @return bool
	 */
	function getCompanyDeductionByLegalEntityIDAndFuzzyName( $legal_entity_id, $name ) {
		$cdlf = TTnew( 'CompanyDeductionListFactory' ); /** @var CompanyDeductionListFactory $psealf */
		$cdlf->getByLegalEntityIdAndName( $legal_entity_id, $name );
		if ( $cdlf->getRecordCount() > 0 ) {
			return $cdlf->getCurrent()->getId();
		}
	...
	function createPayrollRemittanceAgency( $data ) {
		if ( is_array( $data ) ) {
			...
			if ( $praf->isValid() ) {
				...
				Debug::text( 'ERROR: Failed creating remittance agency...', __FILE__, __LINE__, __METHOD__, 10 );
			}
		}
	 ...
	 * @return bool
	 */
	function PayrollRemittanceAgencys( $country = null, $province = null, $district = null, $industry = null, $legal_entity_id = null, $filter_agency_id = null ) {
		global $config_vars;

		$country = strtolower( (string)$country );
		$province = strtolower( (string)$province );

		if ( $filter_agency_id != '' && !is_array( $filter_agency_id ) ) {
				...
				if ( $country != '' && $province == '' ) {
					...
					if ( isset( $agency_arr ) && is_array( $agency_arr ) ) {
						foreach ( $agency_arr as $agency_id => $agency_type_name ) {
							if ( empty( $filter_agency_id ) == false && in_array( $agency_id, (array)$filter_agency_id ) == false ) {
								Debug::text( '    Agency ID: ' . $agency_id . ' is not in filter list: ' . implode( ',', (array)$filter_agency_id ), __FILE__, __LINE__, __METHOD__, 10 );
								continue;
							}
							...
							switch ( substr( $agency_id, 0, 5 ) ) {
								case '10:CA': //Canada
									...
									break;
								case '10:US': //US
									if ( $agency_id == '10:US:00:00:0100' || $agency_id == '10:US:00:00:0110' ) { //Centers for Medicare & Medical Services (CMS.gov)
									...
									if ( defined( 'UNIT_TEST_MODE' ) && UNIT_TEST_MODE === true ) {
										...
										$primary_identification = '12-' . rand( 1000000, 9999999 );
					...
					if ( isset( $agency_arr ) && is_array( $agency_arr ) ) {
						foreach ( $agency_arr as $agency_id => $agency_type_name ) {
							...
							switch ( substr( $agency_id, 0, 5 ) ) {
								case '20:CA': //Canada
									if ( substr( $agency_id, -4 ) == '0040' || substr( $agency_id, -4 ) == '0110' ) { //Child Support, and EEO disable by default.
										$status_id = 20;                        //Disabled by default.
									}
								...
								case '20:US': //Federal Holidays observed by all states.
									if ( substr( $agency_id, -4 ) == '0040' || substr( $agency_id, -4 ) == '0050' || $agency_id == '20:US:CA:00:0110' ) { //Child Support, Retirement, EEO disable by default.
										$status_id = 20;                        //Disabled by default.
									}
		...
		if ( $lelf->getRecordCount() > 0 ) {
			foreach ( $lelf as $le_obj ) {
						...
						case 'ca':
							...
							$this->createCompanyDeduction(
									[
											...
											'payroll_remittance_agency_id'   => $this->getPayrollRemittanceAgencyByLegalEntityAndAgencyID( $le_obj->getId(), '10:CA:00:00:0010' ), //Must go before name
											'name'                           => 'CPP2 - Employee',
											'calculation_id'                 => 92, //CPP2 Formula
											'calculation_order'              => 81,
											'minimum_user_age'               => 18,
											'maximum_user_age'               => 70,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, 'CPP2' ),
											'include_pay_stub_entry_account' => [
													$psea_obj->getTotalGross(),
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, 'Life Insurance' ), //Employer contributions are taxable.
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, 'Accidental Death & Dismemberment' ), //Employer contributions are taxable.
							...
							$this->createCompanyDeduction(
									[
											...
											'name'                           => 'CPP2 - Employer',
											'calculation_id'                 => 10,
											'calculation_order'              => 86,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, 'CPP2 - Employer' ),
											'include_pay_stub_entry_account' => [ $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, 'CPP2' ), ],
											'user_value1'                    => 100,
									]
							);
						...
						case 'us':
							...
							$this->createCompanyDeduction(
									[
											...
											'payroll_remittance_agency_id'   => $this->getPayrollRemittanceAgencyByLegalEntityAndAgencyID( $le_obj->getId(), '10:US:00:00:0010' ), //Must go before name
											'name'                           => 'US - Federal Unemployment Insurance',
											'calculation_id'                 => 89, //89=US - Federal Unemployment Insurance (Employer)
											'calculation_order'              => 80,
					...
					if ( isset( $vacation_data['secondary_length_of_service'] ) && $vacation_data['secondary_length_of_service'] > 0 ) {
						$this->createCompanyDeduction(
								[
										...
										'maximum_length_of_service'         => ( $vacation_data['secondary_length_of_service'] ), //Used to minus - 0.001, but should no longer be necessary as of: a14acd8c
				...
				if ( $country == 'us' && $province != '' ) {
					...
					$company_state_unemployment_wage_base = $state_unemployment_wage_base = $pd_obj->getStateUIWageBase();
					...
					switch ( $province ) {
						...
						case 'ca': //california
							...
							$this->createCompanyDeduction(
									[
											...
											'user_value1'                    => 1.1, //Percent
											'user_value2'                    => 0, //WageBase - Removed in 01-Jan-2024.
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
						...
						case 'dc': //d.c.
							...
							$this->createCompanyDeduction(
									[
											...
											'calculation_order'              => 188,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employer' ),
											'user_value1'                    => 0.75, //Percent
											'user_value2'                    => 0, //WageBase - None
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
						...
						case 'ma': //massachusetts
							...
							$this->createCompanyDeduction(
									[
											...
											'calculation_order'              => 187,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employee' ),
											'user_value1'                    => 0.318, //Percent (Can be split with employer in all kinds of different ways)
											'user_value2'                    => $pd_obj->getSocialSecurityMaximumEarnings(), //WageBase - Matches Social Security
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
							...
							$this->createCompanyDeduction(
									[
											...
											'calculation_order'              => 188,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employer' ),
											'user_value1'                    => 0.00, //Percent (Can be split with employer in all kinds of different ways)
											'user_value2'                    => $pd_obj->getSocialSecurityMaximumEarnings(), //WageBase - Matches Social Security
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
						...
						case 'nj': //new jersey
							...
							$this->createCompanyDeduction(
									[
											...
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - Disability Insurance' ),
											'user_value1'                    => 0.20, //Percent
											'user_value2'                    => 161400, //WageBase - Updated: 16-Dec-2023 - Separate from Company wage base.
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
							...
							$this->createCompanyDeduction(
									[
											...
											'calculation_order'              => 186,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employee' ),
											'user_value1'                    => 0.14, //Percent
											'user_value2'                    => 161400, //WageBase - 2024
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
						...
						case 'ny': //new york
							...
							$this->createCompanyDeduction(
									[
											...
											'calculation_order'              => 186,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employee' ),
											'user_value1'                    => 0.455, //Percent
											'user_value2'                    => 87787, //WageBase - 2023
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
							...
							if ( !defined( 'UNIT_TEST_MODE' ) || UNIT_TEST_MODE === false ) { //When in unit test mode don't create NY city income taxes, as we will have to update all of our unit tests.
								...
								$this->createCompanyDeduction(
										[
												...
												'province'                       => strtoupper( $province ),
												'district'                       => 'NYC',
												'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - New York City Income Tax' ),
												'user_value1'                    => 10, //Single
												'user_value2'                    => 0, //0 Allowances
												'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
												'exclude_pay_stub_entry_account' => [
								...
								$this->createCompanyDeduction(
										[
												...
												'province'                       => strtoupper( $province ),
												'district'                       => 'Yonkers',
												'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - Yonkers Income Tax' ),
												'user_value1'                    => 10, //Single
												'user_value2'                    => 0, //0 Allowances
												'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
												'exclude_pay_stub_entry_account' => [
						...
						case 'pa': //pennsylvania
							...
							break;
						case 'ri': //rhode island
							//**Unemployement is called "Employment Security Tax"**, but we use the "Unemployement Insurance" pay stub account to be consistent with other states and automate the calculation.
							...
							//				'user_value1'                    => 0.00, //Percent
							//				'user_value2'                    => $state_unemployment_wage_base, //WageBase Rate for: 20240101
							//				'user_value3'                    => 0,
							//				'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
							//				'exclude_pay_stub_entry_account' => [
							//						$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
							...
							$this->createCompanyDeduction(
									[
											...
											'user_value2'                    => $state_unemployment_wage_base,
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
							...
							$this->createCompanyDeduction(
									[
											...
											'user_value1'                    => 1.10, //Percent
											'user_value2'                    => 84000, //WageBase - This is not the same as the unemployment wage base.
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
													$this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 10, 'Loan' ),
						...
						case 'wa': //washington
							...
							////Employment Admin Fund -- This is actually part of the UI tax rate?
							//$this->createCompanyDeduction(
							//		[
							//				'company_id'                     => $this->getCompany(),
							//				'legal_entity_id'                => $le_obj->getID(),
							...
							//				'type_id'                        => 10, //Tax
							//				'payroll_remittance_agency_id'   => $this->getPayrollRemittanceAgencyByLegalEntityAndAgencyID( $le_obj->getId(), '20:US:' . strtoupper( $province ) . ':00:0020' ), //Must go before name
							//				'name'                           => strtoupper( $province ) . ' - Employment Admin Fund - Employer',
							//				'calculation_id'                 => 15,
							//				'calculation_order'              => 186,
							...
							$this->createCompanyDeduction(
									[
											...
											'payroll_remittance_agency_id'   => $this->getPayrollRemittanceAgencyByLegalEntityAndAgencyID( $le_obj->getId(), '20:US:' . strtoupper( $province ) . ':00:0020' ), //Must go before name
											'name'                           => strtoupper( $province ) . ' - Paid Family and Medical Leave - Employer',
											'description'                    => '*NOTE: This should be 0.74% if the employer is paying 100% of premiums, or 0.211418% (28.57% of 0.74%) if the employer is paying 26.78% of premiums.',
											'calculation_id'                 => 15,
											'calculation_order'              => 188,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 30, strtoupper( $province ) . ' - Paid Family and Medical Leave - Employer' ),
							...
							$this->createCompanyDeduction(
									[
											...
											'type_id'                        => 10, //Tax
											'payroll_remittance_agency_id'   => $this->getPayrollRemittanceAgencyByLegalEntityAndAgencyID( $le_obj->getId(), '20:US:' . strtoupper( $province ) . ':00:0020' ), //Must go before name
											'name'                           => strtoupper( $province ) . ' - CARES Fund - Employee',
											'calculation_id'                 => 15,
											'calculation_order'              => 187,
											'pay_stub_entry_account_id'      => $this->getPayStubEntryAccountByCompanyIDAndTypeAndFuzzyName( 20, strtoupper( $province ) . ' - CARES Fund' ),
											'user_value1'                    => 0.58, //Percent
											'user_value2'                    => 0, //WageBase
											'user_value3'                    => 0,
											'include_pay_stub_entry_account' => [ $psea_obj->getTotalGross() ],
											'exclude_pay_stub_entry_account' => [
					...
					if ( in_array( $province, [
						...
						$this->createCompanyDeduction(
								[
										...
										'calculation_id'                 => 210, //210=US - State Unemployment Insurance (Employer)
				...
				case 'ca':
					$this->createRecurringHoliday(
							[
									...
									'day_of_month'       => 1,
									'month_int'          => 1,
									'always_week_day_id' => 2, //2=Next Week Day
							]
					);
					$this->createRecurringHoliday(
							[
									...
									//'day_of_month' => 1,
									//'month_int' => 1,
									'always_week_day_id' => 2, //2=Next Week Day (Always friday)
							]
					);
					$this->createRecurringHoliday(
							[
					...
					$this->createRecurringHoliday(
							[
									'company_id'         => $this->getCompany(),
									'name'               => strtoupper( $country ) . ' - Truth and Reconciliation Day',
									'type_id'            => 10,
									'special_day'        => 0,
									//'pivot_day_direction_id' => 0,
									//'week_interval' => 0,
					...
					$this->createRecurringHoliday(
							[
									...
									//'day_of_month' => 1,
									//'month_int' => 1,
									'always_week_day_id' => 2, //2=Next Week Day (Always on Monday)
							]
					);
					$this->createRecurringHoliday(
							[
...
//					);
					...
					//Federal reserve holiday, so banks are closed, but not all private businesses.
					$this->createRecurringHoliday(
							[
									'company_id'         => $this->getCompany(),
...
//			}
	 ...
	 * @return bool
	 */
	function getAccrualPolicyByCompanyIDAndName( $name ) {
		$filter_data = [
				'name'    => $name,
		];
	...
	function AccrualPolicy( $country = null, $province = null, $district = null, $industry = null ) {
			...
			if ( in_array( $province, [ 'bc' ] ) ) {
				$accrual_policy_id = $this->createAccrualPolicy(
						[
								'company_id'                   => $this->getCompany(),
								'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
								'name'                         => strtoupper( $province ) . ' - Sick',
								'type_id'                      => 20, //Calendar
								'apply_frequency_id'           => 20, //Annual
								'apply_frequency_hire_date'	   => false, //Must be Calendar year starting Jan 1st.
								'apply_frequency_month'		   => 1,
								'apply_frequency_day_of_month' => 1,
								'milestone_rollover_hire_date' => false,
				...
				if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
					$this->createAccrualPolicyMilestone(
							[
									...
									'length_of_service'         => 0,
									'length_of_service_unit_id' => 40, //Years
									'accrual_rate'              => ( 40 * 3600 ),
									'maximum_time'              => ( 40 * 3600 ),
									'rollover_time'             => ( 0 * 3600 ),
							]
					);
				}
			} else {
		...
		//United States
		if ( $country == 'us' && $province != '' ) {
			Debug::text( 'State: ' . $province, __FILE__, __LINE__, __METHOD__, 10 );
			//Sick
			$this->createAccrualPolicyAccount(
					[
							'company_id'                      => $this->getCompany(),
			...
			switch ( $province ) {
				case 'az': //Hourly Accrual: 1hr for every 30hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
									'name'                         => strtoupper( $province ) . ' - Sick',
									'type_id'                      => 30, //30=Hour Based
									'contributing_shift_policy_id' => $this->getContributingShiftPolicyByCompanyIDAndName( 'Regular Time + OT' ),
									'milestone_rollover_hire_date' => true,
									'minimum_employed_days'        => 0,
							]
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 30 ), 3 ), //1hr for every 30hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 40 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
						);
					}
					unset( $accrual_policy_id );
					break;
				case 'ca': //Hourly Accrual: 1hr for every 30hrs worked.
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 30 ), 3 ), //1hr for every 30hrs worked.
										'annual_maximum_time'       => ( 48 * 3600 ),
										'maximum_time'              => ( 48 * 3600 ),
										'rollover_time'             => ( 48 * 3600 ),
								]
						);
					}
					unset( $accrual_policy_id );
					break;
				case 'co': //Hourly Accrual: 1hr for every 30hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'ct': //Hourly Accrual: 1hr for every 40hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 40 ), 3 ), //1hr for every 40hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 40 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
					...
					unset( $accrual_policy_id );
					break;
				case 'dc': //Hourly Accrual: 1hr for every 37hrs worked. (Varies based on the number of employees though) This is the maximum for 100+ employees.
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 37 ), 3 ), //1hr for every 37hrs worked.
										'annual_maximum_time'       => ( 56 * 3600 ),
										'maximum_time'              => ( 9999 * 3600 ),
										'rollover_time'             => ( 9999 * 3600 ),
								]
						);
					}
					unset( $accrual_policy_id );
					break;
				case 'ma': //Hourly Accrual: 1hr for every 30hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'md': //Hourly Accrual: 1hr for every 30hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 30 ), 3 ), //1hr for every 30hrs worked.
										'annual_maximum_time'       => ( 64 * 3600 ),
										'maximum_time'              => ( 64 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
						);
					}
					unset( $accrual_policy_id );
					break;
				case 'me': //Hourly Accrual: 1hr for every 40hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'mi': //Hourly Accrual: 1hr for every 35hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 35 ), 3 ), //1hr for every 35hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 40 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
					...
					unset( $accrual_policy_id );
					break;
				case 'nj': //Hourly Accrual: 1hr for every 30hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 30 ), 3 ), //1hr for every 35hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 40 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
					...
					unset( $accrual_policy_id );
					break;
				case 'nm': //Hourly Accrual: 1hr for every 30hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'ny': //Hourly Accrual: 1hr for every 30hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'nv': //Hourly Accrual: 1hr for every 52hrs worked
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 52 ), 5 ), //1hr for every 52hrs worked.
										'annual_maximum_time'       => ( 9999 * 3600 ),
										'maximum_time'              => ( 9999 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
						);
					...
					unset( $accrual_policy_id );
					break;
				case 'or': //Hourly Accrual: 1hr for every 30hrs worked.
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					unset( $accrual_policy_id );
					break;
				case 'ri': //Hourly Accrual: 1hr for every 35hrs worked.
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 35 ), 3 ), //1hr for every 40hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 9999 * 3600 ),
										'rollover_time'             => ( 9999 * 3600 ),
								]
					...
					unset( $accrual_policy_id );
					break;
				case 'vt': //Hourly Accrual: 1hr for every 52hrs worked, up to a cap of 40hrs per year (40hrs can also rollover)
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
					...
					if ( TTUUID::isUUID( $accrual_policy_id ) && $accrual_policy_id != TTUUID::getZeroID() && $accrual_policy_id != TTUUID::getNotExistID() ) {
						$this->createAccrualPolicyMilestone(
								[
										...
										'length_of_service'         => 0,
										'length_of_service_unit_id' => 40, //Years
										'accrual_rate'              => round( ( 1 / 52 ), 3 ), //1hr for every 52hrs worked.
										'annual_maximum_time'       => ( 40 * 3600 ),
										'maximum_time'              => ( 40 * 3600 ),
										'rollover_time'             => ( 40 * 3600 ),
								]
					...
					unset( $accrual_policy_id );
					break;
				case 'wa': //Hourly Accrual: 1hr for every 40hrs worked.
					$accrual_policy_id = $this->createAccrualPolicy(
							[
									'company_id'                   => $this->getCompany(),
									'accrual_policy_account_id'    => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
	 ...
	 * @return array|bool
	 */
	function getAbsencePolicyByCompanyIDAndName( $name ) {
		$filter_data = [
				'name' => $name,
		];
		...
		if ( $country == 'ca' && $province != '' ) {
			...
			if ( $province == 'bc' ) {
				$this->createAbsencePolicy(
						[
								'company_id'  => $this->getCompany(),
								'name'        => strtoupper( $province ) . ' - Sick (PAID)',
								'pay_code_id' => current( $this->getPayCodeByCompanyIDAndName( 'Sick' ) ),
								'pay_formula_policy_id'     => $this->getPayFormulaPolicyByCompanyIDAndName( strtoupper( $province ) . ' - Sick (Daily Average)' ),
						]
				);
				$this->createAbsencePolicy(
						[
								'company_id'  => $this->getCompany(),
								'name'        => strtoupper( $province ) . ' - Sick (UNPAID)', //Generic UnPaid.
								'pay_code_id' => current( $this->getPayCodeByCompanyIDAndName( 'Sick (UNPAID)' ) ),
						]
				);
			} else {
				...
				$this->createAbsencePolicy(
						[
								'company_id'  => $this->getCompany(),
								'name'        => strtoupper( $province ) . ' - Sick (UNPAID)',
								'pay_code_id' => current( $this->getPayCodeByCompanyIDAndName( 'Sick (UNPAID)' ) ),
						]
				);
			}
	...
	function PayFormulaPolicy( $country = null, $province = null, $district = null, $industry = null ) {
		...
		if ( $country == 'ca' && $province != '' ) {
			...
			if ( $province != 'bc' ) { //BC Sick pay formula is handled in PayFormulaPolicyAdvanced() below.
				$this->createPayFormulaPolicy(
						[
								'company_id'                => $this->getCompany(),
								'name'                      => 'Sick',
	...
	/**
	 * This has to be called after Pay Codes and Contributing Shifts are created, as those are required for advanced pay formulas.
	 * @param null $country
	 * @param null $province
	 * @param null $district
	 * @param null $industry
	 * @return bool
	 */
	function PayFormulaPolicyAdvanced( $country = null, $province = null, $district = null, $industry = null ) {
		$country = strtolower( (string)$country );
		$province = strtolower( (string)$province );

		//Prevent it from running multiple times for the same country/province pair.
		if ( isset( $this->already_processed['pay_formula_policy_advanced'][$country . $province . $district . $industry] ) ) {
			Debug::text( 'Already ran for Country: ' . $country . ' Province: ' . $province . ' skipping...', __FILE__, __LINE__, __METHOD__, 10 );

			return true;
		} else {
			$this->already_processed['pay_formula_policy_advanced'][$country . $province . $district . $industry] = true;
		}
		...
		if ( $country == 'ca' && $province != '' ) {
			...
			if ( $province == 'bc' ) { //Make sure PayFormulaPolicy() above excludes creating "Sick" pay formula for BC.
				$this->createPayFormulaPolicy(
						[
								'company_id'                => $this->getCompany(),
								'name'                      => strtoupper( $province ) . ' - Sick (Daily Average)',
								'pay_type_id'               => 70, //Daily Average
								'wage_source_contributing_shift_policy_id' => $this->getContributingShiftPolicyByCompanyIDAndName( 'Regular Time + Paid Absence' ),
								'time_source_contributing_shift_policy_id' => $this->getContributingShiftPolicyByCompanyIDAndName( 'Regular Time + Paid Absence' ),
								'rate'						=> 0,
								'average_days' 				=> 30,
								'accrual_rate'              => -1.00, //Reduce accrual when this is used
								'accrual_policy_account_id' => $this->getAccrualPolicyAccountByCompanyIDAndName( 'Sick' ),
	...
	function PayCode( $country = null, $province = null, $district = null, $industry = null ) {
		...
		//Only create "Sick" pay code when processing for a specific province, as we have to ensure that the specific pay formula for Sick is created first.
		if ( $country != '' && $province != '' ) {
			$this->createPayCode(
					[
							'company_id'                => $this->getCompany(),
			...
			if ( in_array( $province, [ 'on' ] ) ) {
				$this->createContributingShiftPolicy(
						[
								'company_id'                      => $this->getCompany(),
								'name'                            => 'Regular Time + Vacation',
								'contributing_pay_code_policy_id' => $this->getContributingPayCodePolicyByCompanyIDAndName( 'Regular Time + Vacation' ),
						]
				);
			}
		}
				...
				default:
					...
					$this->createHolidayPolicy(
							[
									...
									'absence_policy_id'                     => $this->getAbsencePolicyByCompanyIDAndName( 'Statutory Holiday' ),
			...
			if ( in_array( $province, [ 'on' ] ) ) {
				$this->createHolidayPolicy(
						[
								...
								'absence_policy_id'                     => $this->getAbsencePolicyByCompanyIDAndName( 'Statutory Holiday' ),
								'recurring_holiday_id'                  => (array)$this->RecurringHolidaysByRegion( $country, $province ),
								'contributing_shift_policy_id'          => $this->getContributingShiftPolicyByCompanyIDAndName( 'Regular Time + Vacation' ),
								'eligible_contributing_shift_policy_id' => $this->getContributingShiftPolicyByCompanyIDAndName( 'Regular Time + OT' ),
						]
				);
			}
			...
			switch ( $country ) {
				...
				default:
					...
					$this->createPolicyGroup(
							[
									'company_id'                  => $this->getCompany(),
									'name'                        => strtoupper( $country ) . ' - Hourly Employees',
									...
									'accrual_policy'              => $this->getAccrualPolicyByCompanyIDAndName( strtoupper( $province ) . '%' ),
									...
									'absence_policy'              => $this->getAbsencePolicyByCompanyIDAndName( '%' ),
							]
					);
					break;
			}
	 ...
	 * @return bool
	 */
	function createKPIGroup( $data ) {
		if ( is_array( $data ) ) {
			$kgf = TTnew( 'KPIGroupFactory' ); /** @var KPIGroupFactory $kgf */
			$data['id'] = $kgf->getNextInsertId();
			$kgf->setObjectFromArray( $data );
			if ( $kgf->isValid() ) {
				return $kgf->Save( true, true );
			} else {
				//See if group already exists and return that ID instead.
				$kglf = TTNew( 'KPIGroupListFactory' ); /** @var PolicyGroupListFactory $kglf */
				$kglf->getByCompanyId( $this->getCompanyObject()->getID(), [ 'name' => ' = '. $this->db->qstr( $data['name'] ) ] );
				if ( $kglf->getRecordCount() == 1 ) {
					Debug::text( '  NOTICE: KPI Group already exists, using it!', __FILE__, __LINE__, __METHOD__, 10 );
					return $kglf->getCurrent()->getID();
				} else {
					Debug::text( '  ERROR: KPI Group is invalid!', __FILE__, __LINE__, __METHOD__, 10 );
				}
			}
		}
	...
	function createKPI( $data ) {
		if ( is_array( $data ) ) {
			$pgf = TTnew( 'KPIFactory' ); /** @var KPIFactory $pgf */
			$data['id'] = $pgf->getNextInsertId();
			$pgf->setObjectFromArray( $data );
			if ( $pgf->isValid() ) {
				return $pgf->Save( true, true );
			} else {
				Debug::text( '  ERROR: KPI is invalid!', __FILE__, __LINE__, __METHOD__, 10 );
			}
		}
	...
	function KPI( $country = null, $province = null, $district = null, $industry = null ) {
		$country = strtolower( (string)$country );
		$province = strtolower( (string)$province );

		//Prevent it from running multiple times for the same country/province pair.
		if ( isset( $this->already_processed['kpi'][$country . $province . $district . $industry] ) ) {
			Debug::text( 'Already ran for Country: ' . $country . ' Province: ' . $province . ' skipping...', __FILE__, __LINE__, __METHOD__, 10 );

			return true;
		} else {
			$this->already_processed['kpi'][$country . $province . $district . $industry] = true;

			$scale_1_to_3_rating_description = '1=Never; 2=Sometimes; 3=Often';
			$scale_1_to_5_rating_description = '1=Needs Improvement; 2=Meets Most Expectations; 3=Fully Meets and Sometimes Exceeds Expectations; 4=Consistently Exceeds Expectations; 5=Far Exceeds Expectations';

			$kpi_group = $this->createKPIGroup(
					[
							'company_id'                  => $this->getCompany(),
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Skill and proficiency in carrying out tasks. Produces high quality work',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
							'minimum_rate'                => 1,
							'maximum_rate'                => 5,
							'display_order' 			  => 1000,
					]
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Works efficiently to meet goals and objectives',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Communicates effectively with supervisor and peers',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Ability to work cooperatively as part of a team',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Reliability (attendance, punctuality, deadlines)',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Adeptness at problem solving, decision-making, and demonstrating good judgment',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Customer Relations – Responds appropriately to customers',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Good knowledge of our products and service',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Equipment and machinery skills',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Shows leadership and integrity',
							'description'                 => $scale_1_to_5_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Willingness to take on additional responsibilities',
							'description'                 => $scale_1_to_3_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
							'minimum_rate'                => 1,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Open and accessible to discussing new ideas and challenges',
							'description'                 => $scale_1_to_3_rating_description,
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Which goals did you meet in the last (quarter/year)?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was your greatest work accomplishment since your last review?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What are your most important goals for the next (quarter/six months/year)?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you have the resources/tools you need to perform your job effectively?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 2030,
					]
			);
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What is something that would help you do your job better?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What growth opportunities would you like to explore in order for you to reach your goals?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What elements of your job interest you the most?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What elements of your job interest you the least?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What gives you the greatest job satisfaction?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What elements of your job do you find the most challenging and why?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Are there areas where you feel you could improve?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What can I or your supervisor/manager do to help you better meet your goals?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What is your work-life balance currently like?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'How valued do you feel at the company?',
							'description'                 => '1=Not at all Valued; 2=Slightly Valued; 3=Satisfactorily Valued; 4=Very Valued',
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
							'minimum_rate'                => 1,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Are you comfortable with the company’s culture (work environment)? What improvements can be made?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you feel your team is working well collaboratively? Can you provide specific examples?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What are your ideal working conditions to be the most productive?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What skills do you have that you believe we could use more effectively?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What do you like most about working for this company?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'If you could change anything about your job or the company, what would you change?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you have any issues you’d like to address?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you have any questions for me (supervisor/manager) or any other comments?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Overall, how satisfied are you working here?',
							'description'                 => '1=Dissatisfied; 2=Somewhat Satisfied; 3=Satisfied; 4=Very Satisfied',
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
							'minimum_rate'                => 1,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What prompted you to start looking for another job?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What does your new position offer that influenced your decision to leave?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What could we have done better?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did you share your concerns with anyone at the company?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Were you given clear goals and objectives?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did you feel you had all the tools you needed to do your job well?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did you receive enough training to do your job effectively?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did you receive constructive feedback to help you improve your performance?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you think your achievements were recognized while working here?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'How would you describe the culture of our company?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What did you like best about your job?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What did you like least about your job?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What could we have done to keep you here?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What did you like best about working for the company?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What did you like least about working for the company?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What would make this company a better place to work?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'How likely are you to recommend this company to a friend looking for a job?',
							'description'                 => '1=Not at all likely; 2=Somewhat likely; 3=Very Likely; 4=Extremely Likely',
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
							'minimum_rate'                => 1,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'How would you rate your overall experience while working here?',
							'description'                 => '1=Dissatisfied; 2=Somewhat Satisfied; 3=Satisfied; 4=Very Satisfied',
							'group_id'                    => $kpi_group,
							'type_id'					  => 10, //10=Scale Rating
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What advice would you give to your replacement? Your team? Executive?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Do you have any questions or other comments?',
							'description'                 => '',
							'group_id'                    => $kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$incident_kpi_group = $this->createKPIGroup(
					[
							'company_id'                  => $this->getCompany(),
							'parent_id'					  => TTUUID::getZeroID(),
							'name'                        => 'Incident',
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was the date and time the incident occurred?',
							'description'                 => '',
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was the date and time the incident was reported?',
							'description'                 => '',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was this incident reported immediately to the employee\'s supervisor?',
							'description'                 => '',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was the type of incident?',
							'description'                 => 'Serious injury, serious incident, minor injury, potential for serious injury, property damage, production loss etc.',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4060,
			...
			$this->createKPI(
					[
							...
							'description'                 => 'Worksite, Department, Building, Address',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4070,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was the sequence of events that preceded the incident?',
							'description'                 => 'Describe what the employee saw, heard, smelled, felt or tasted immediately before the incident.',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4080,
			...
			$this->createKPI(
					[
							...
							'description'                 => 'Describe what the employee saw, heard, smelled, felt or tasted during the incident.',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4090,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What happened immediately after the incident?',
							'description'                 => '',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'List the names of any witnesses to the incident?',
							'description'                 => '',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What happened earlier in the day or anytime in past that led to the incident?',
							'description'                 => 'For example, training given or changes in equipment or procedures.',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4120,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Were any corrective actions identified and taken to prevent a recurrence?',
							'description'                 => 'Describe the corrective actions.',
							'group_id'                    => $incident_kpi_group,
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 4130,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'What was the type of accident?',
							'description'                 => 'Did it involve Equipment, Structural failure or collapse, Hazardous substance, Explosion, Fire. Flood etc.',
							'group_id'                    => [ $kpi_group ],
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 5000,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the incident result in a serious injury?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 5010,
					]
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the accident result in the death the employee?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Who was working with the injured person?',
							'description'                 => 'List names or say if employee was alone at the time.',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 5030,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was anyone else injured during the same incident?',
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did any environmental factors contribute to the accident? Which ones?',
							'description'                 => 'Light, heat, cold, noise, etc...',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 5050,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was there a tool or other equipment involved in the accident? Which ones?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 30, //30=Text
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was the employee observing normal safe operating procedures when the injury occurred?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was the employee properly attired when the accident happened?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did their clothing, hair or fingernails contribute to the accident or injury?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the accident involving lifting?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the injury result from a trip and fall?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Could this accident been avoided? How?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Was the employee directed to or taken for medical assistance? Where and by whom?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the employee receive first aid or medical treatment?  Where and by whom?',
							'description'                 => 'Include qualifications of person giving medical treatment.',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
							'display_order' 			  => 5140,
			...
			$this->createKPI(
					[
							'company_id'                  => $this->getCompany(),
							'name'                        => 'Did the employee require a hospital stay? How many days?',
							'description'                 => '',
							'group_id'                    => [ $kpi_group, $incident_kpi_group ],
							'type_id'					  => 20, //20=Yes/No
							'status_id'                   => 15, //15=Enabled (Optional)
	 ...
	 * @return bool
	 */
	function createUserDefault( $data ) {
		if ( is_array( $data ) ) {
			$udf = TTnew( 'UserDefaultFactory' ); /** @var PolicyGroupFactory $pgf */
			$data['id'] = $udf->getNextInsertId();
			$udf->setObjectFromArray( $data );
			if ( $udf->isValid() ) {
				return $udf->Save( true, true );
			}
		}
	 ...
	 * @return bool
	 */
	function UserDefaults( $country = null, $province = null, $district = null, $industry = null, $legal_entity_id = null ) {
		$country = strtolower( (string)$country );
		$province = strtolower( (string)$province );

		//If no UserObject is specified, skip creating UserDefaults until Quick Start wizard is launched on the users first login.
		if ( !is_object( $this->getUserObject() ) ) {
			Debug::text( '  NOTICE: No user object specified, unable to create User Defaults at this time...', __FILE__, __LINE__, __METHOD__, 10 );
			return false;
		}
		...
		if ( $lelf->getRecordCount() > 0 ) {
			//Get UserDefault defaults.
			$api_ud = new APIUserDefault;
			$user_default_defaults = $api_ud->stripReturnHandler( $api_ud->getUserDefaultDefaultData() ); //Don't include blank.

			foreach ( $lelf as $le_obj ) {
				Debug::text( 'Country: ' . $country . ' Legal Entity: ' . $le_obj->getLegalName() . ' ID: ' . $le_obj->getID(), __FILE__, __LINE__, __METHOD__, 10 );

				$upf = TTnew( 'UserPreferenceFactory' ); /** @var UserPreferenceFactory $upf */
				$time_zone = $upf->getLocationTimeZone( $country, $province );
				Debug::text( 'Time Zone: ' . $time_zone, __FILE__, __LINE__, __METHOD__, 9 );
				...
				if ( $country != '' && $province == '' ) {
					switch ( $country ) {
						...
						default:
							...
							$user_default_data = [
									'company_id'      => $this->getCompany(),
									'name'            => strtoupper( $country ) . ' - Default ('. $le_obj->getLegalName() .')',
									'legal_entity_id' => $le_obj->getId(),
									'city'            => $le_obj->getCity(),
					...
					$user_default_data = [
							'company_id'      => $this->getCompany(),
							'name'            => strtoupper( $province ) . ' - Default ('. $le_obj->getLegalName() .')',
							'legal_entity_id' => $le_obj->getId(),
							'city'            => $le_obj->getCity(),
							'country'         => $le_obj->getCountry(),
							'province'        => $le_obj->getProvince(),
					...
					$user_default_data['company_deduction'] = [
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'CA - Federal Income Tax' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'CPP - Employee' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'CPP - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'CPP2 - Employee' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'CPP2 - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'EI - Employee' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'EI - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Provincial Income Tax' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Workers Compensation' ),
					];
					...
					switch ( $province ) {
						...
						case 'nu':
						case 'nt':
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 0-4 Years' );
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 5+ Years' );
							break;
						case 'nb':
						case 'ns':
						case 'pe':
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 0-7 Years' );
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 8+ Years' );
							break;
						case 'yt':
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 0+ Years' );
							break;
						case 'sk':
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 0-9 Years' );
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 10+ Years' );
							break;
						case 'nl':
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 0-14 Years' );
							$user_default_data['company_deduction'][] = $this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Vacation Accrual - 15+ Years' );
							break;
					}
				} else if ( $country == 'us' && $province != '' ) {
					Debug::text( '  Province: ' . $province, __FILE__, __LINE__, __METHOD__, 10 );
					...
					$user_default_data['company_deduction'] = [
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'US - Federal Income Tax' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'US - Federal Unemployment Insurance' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'Social Security - Employee' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'Social Security - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'Medicare - Employee' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'Medicare - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), 'US - Federal Unemployment Insurance' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - State Income Tax' ),

							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Unemployment Insurance - Employer' ),
							$this->getCompanyDeductionByLegalEntityIDAndFuzzyName( $le_obj->getId(), strtoupper( $province ) . ' - Workers Compensation - Employer' ),
					];
				}

				if ( isset( $user_default_data ) ) {
					$user_default_data = array_merge( $user_default_defaults, $user_default_data );
					$user_default_id = $this->createUserDefault( $user_default_data );

					//Set Preference Notification defaults.
					if ( TTUUID::isUUID( $user_default_id ) ) {
						//Get default preference notification values.
						...
						//Default user_default already exists. Update user_default_preference_notification instead of creating new records.
						$udpnlf = TTnew( 'UserDefaultPreferenceNotificationListFactory' ); /** @var UserDefaultPreferenceNotificationListFactory $udpnlf */
						$udpnlf->getByUserDefaultId( $user_default_id );
						if ( $udpnlf->getRecordCount() > 0 ) {
							foreach ( $udpnlf as $udpn_obj ) { /** @var UserDefaultPreferenceNotificationFactory $udpn_obj */
								$udpn_obj->setStatus( $data[$udpn_obj->getType()]['status_id'] );
								$udpn_obj->setDevice( $data[$udpn_obj->getType()]['device_id'] );
								if ( $udpn_obj->isValid() ) {
									$udpn_obj->Save();
								} else {
									Debug::Text( 'Failed to update user default notification preference for company: ' . $this->getCompanyObject()->getID(), __FILE__, __LINE__, __METHOD__, 10 );
								}
							}
						}
					}
				}
				unset( $user_default_data, $user_default_id );
			}
	...
	function createPresets( $country = null, $province = null, $district = null, $industry = null, $flags = null, $legal_entity_id = null ) {
		...
		if ( $country == '' ) {
			...
			$this->AccrualPolicy();
			$this->PayFormulaPolicy(); //Must go before PayCode(), otherwise no pay formulas are mapped at all. See PayFormulaPolicyAdvanced() for when more complex pay formulas are created.
			$this->PayCode();
			$this->ContributingPayCodePolicy();
			$this->ContributingShiftPolicy();
			$this->PayFormulaPolicyAdvanced(); //This requires Contributing Shift Policies for advanced cases like OT averaging.
			...
			$this->UserDefaults( null, null, null, null, $legal_entity_id );
		} else if ( $country != '' && $province == '' ) {
			...
			$this->PayFormulaPolicy( $country ); //Must go before PayCode(), otherwise no pay formulas are mapped at all. See PayFormulaPolicyAdvanced() for when more complex pay formulas are created.
			$this->PayCode( $country );
			$this->ContributingPayCodePolicy( $country );
			$this->ContributingShiftPolicy( $country );
			$this->PayFormulaPolicyAdvanced( $country ); //This requires Contributing Shift Policies for advanced cases like OT averaging.
			...
			$this->UserDefaults( $country, null, null, null, $legal_entity_id );
		} else if ( $country != '' && $province != '' ) {
			...
			$this->PayFormulaPolicy( $country, $province ); //Must go before PayCode(), otherwise no pay formulas are mapped at all. See PayFormulaPolicyAdvanced() for when more complex pay formulas are created.
			...
			$this->PayFormulaPolicyAdvanced( $country, $province ); //This requires Contributing Shift Policies for advanced cases like OT averaging.
			...
			$this->KPI( $country, $province );

			$this->UserDefaults( $country, $province, null, null, $legal_entity_id );
		}
