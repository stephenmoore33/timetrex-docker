...
class LegalEntityFactory extends Factory {
	...
	protected $name_validator_regex = '/^[a-zA-Z0-9-_@&*+!:#;\/\\\ ,\.\'`"()\[\]|\x{0080}-\x{FFFF}]{1,250}$/iu'; //**Note also need to update LegalEntityFactory, CompanyFactory and website forms. -- Should be able to handle company names like: "Walmart #123 / Nike:Addidas; \ A&W, Inc."
	...
	private $add_presets;
	private $add_remittiance_source;
	...
	function _getSchemadata( ?array $filter = null ): ?TTS {
		...
		if ( empty( $filter ) || in_array( 'database', $filter, true ) ) {
			$schema_data->setColumns(
					TTSCols::new(
							...
							TTSCol::new( 'classification_code' )->setFunctionMap( 'ClassificationCode' )->setType( 'varchar' )->setIsNull( true ),
							TTSCol::new( 'legal_name' )->setFunctionMap( 'LegalName' )->setType( 'varchar' )->setIsNull( false ),
							TTSCol::new( 'trade_name' )->setFunctionMap( 'TradeName' )->setType( 'varchar' )->setIsNull( false ),
							...
							TTSCol::new( 'payment_services_status_id' )->setFunctionMap( 'PaymentServicesStatus' )->setType( 'integer' )->setIsNull( true ),
							TTSCol::new( 'payment_services_user_name' )->setFunctionMap( 'PaymentServicesUserName' )->setType( 'varchar' )->setIsNull( true ),
							TTSCol::new( 'payment_services_api_key' )->setFunctionMap( 'PaymentServicesAPIKey' )->setType( 'varchar' )->setIsNull( true ),
		...
		if ( empty( $filter ) || in_array( 'fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			$schema_data->setTabs(
					TTSTabs::new(
							TTSTab::new( 'tab_legal_entity' )->setLabel( TTi18n::getText( 'Legal Entity' ) )->setFields(
									new TTSFields(
											TTSField::new( 'id' )->setType( 'uuid' )->setVisible( [ 'UI' ], false ), //Hidden from UI, but visible to API and AI.
											TTSField::new( 'status_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Status' ) )->setDataSource( TTSAPI::new( 'APILegalEntity' )->setMethod( 'getOptions' )->setArg( 'status' ) ),
											TTSField::new( 'type_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Type' ) )->setDataSource( TTSAPI::new( 'APILegalEntity' )->setMethod( 'getOptions' )->setArg( 'type' ) ),
											TTSField::new( 'classification_code' )->setType( 'text' )->setLabel( TTi18n::getText( 'Classification Code' ) ),
											TTSField::new( 'legal_name' )->setType( 'text' )->setLabel( TTi18n::getText( 'Legal Name' ) ),
											TTSField::new( 'trade_name' )->setType( 'text' )->setLabel( TTi18n::getText( 'Trade Name' ) ),
											TTSField::new( 'short_name' )->setType( 'text' )->setLabel( TTi18n::getText( 'Short Name/Abbreviation' ) ),
											TTSField::new( 'address1' )->setType( 'text' )->setLabel( TTi18n::getText( 'Address (Line 1)' ) ),
											TTSField::new( 'address2' )->setType( 'text' )->setLabel( TTi18n::getText( 'Address (Line 2)' ) ),
											...
											TTSField::new( 'postal_code' )->setType( 'text' )->setLabel( TTi18n::getText( 'Postal/ZIP Code' ) ),
											TTSField::new( 'work_phone' )->setType( 'text' )->setLabel( TTi18n::getText( 'Phone' ) ),
											TTSField::new( 'fax_phone' )->setType( 'text' )->setLabel( TTi18n::getText( 'Fax' ) ),
											...
											TTSField::new( 'logo' )->setType( 'file' )->setLabel( TTi18n::getText( 'Logo' ) ),
									)
							),
							TTSTab::new( 'tab_payment_services' )->setLabel( TTi18n::getText( 'Payment Services' ) )->setDisplayOnMassEdit( false )->setFields(
									new TTSFields(
											TTSField::new( 'payment_services_status_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Status' ) )->setDataSource( TTSAPI::new( 'APILegalEntity' )->setMethod( 'getOptions' )->setArg( 'payment_services_status' ) ),
											TTSField::new( 'payment_services_user_name' )->setType( 'text' )->setLabel( TTi18n::getText( 'User Name' ) ),
											TTSField::new( 'payment_services_api_key' )->setType( 'text' )->setLabel( TTi18n::getText( 'API Key' ) ),
									)
							),
					)->addAudit(),
			);
		...
		if ( empty( $filter ) || in_array( 'search_fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setSearchFields(
					TTSSearchFields::new(
							TTSSearchField::new( 'company_id' )->setType( 'uuid' )->setColumn( 'a.company_id' )->setMulti( true ),
							...
							TTSSearchField::new( 'legal_entity_id' )->setType( 'uuid_list' )->setColumn( 'a.id' )->setMulti( true ),
							...
							TTSSearchField::new( 'legal_name' )->setType( 'text' )->setColumn( 'a.legal_name' ),
							TTSSearchField::new( 'trade_name' )->setType( 'text' )->setColumn( 'a.trade_name' ),
							...
							TTSSearchField::new( 'fax_phone' )->setType( 'phone' )->setColumn( 'a.work_phone' ),
		...
		if ( empty( $filter ) || in_array( 'api_methods', $filter, true ) ) {
			$schema_data->setAPIMethods(
					TTSAPIs::new(
							TTSAPI::new( 'APILegalEntity' )->setMethod( 'getLegalEntity' )
									->setSummary( 'Get legal entity records.' )
									->setArgs( [ 'data' => [ 'filter_data' => $schema_data->getSearchFields(), 'filter_columns' => $schema_data->getFields() ] ] )
									->setArgsModelDescription( 'Under the "data" arg there must be at least one or more "filter_data" elements specified, and one or more "filter_columns" elements which define the minimum necessary columns of data to return for carrying out other actions. For example if looking up the "id" of a record, just the "id" filter_columns needs to be specified with a value of true.' ),
							TTSAPI::new( 'APILegalEntity' )->setMethod( 'setLegalEntity' )
									->setSummary( 'Add or edit legal entity records. Will return the record UUID upon success, or a validation error if there is a problem.' )
									->setArgs( [ 'data' => $schema_data->getFields() ] ),
							TTSAPI::new( 'APILegalEntity' )->setMethod( 'deleteLegalEntity' )
									->setSummary( 'Delete legal entity records by passing in an array of UUIDs.' )
									->setArgs( new TTSFields(
													   TTSField::new( 'data' )->setType( 'multi-dropdown' )->setLabel( TTi18n::getText( 'IDs to Delete' ) )->setDataSource( TTSAPI::new( 'APILegalEntity' )->setMethod( 'getLegalEntity' ) ),
											   ) ),
							TTSAPI::new( 'APILegalEntity' )->setMethod( 'getLegalEntityDefaultData' )
									->setSummary( 'Get default legal entity data used for creating new legal entities. Use this before calling setLegalEntity to get the correct default data.' ),
					),
			);
		}
	...
	function getStoragePath( $legal_entity_id = null ) {
		...
		if ( $legal_entity_id == '' || TTUUID::isUUID( $legal_entity_id ) == false ) {
			return false;
		}
	...
	function Validate( $ignore_warning = true ) {
		...
		if ( $this->getLegalName() !== false ) {
			...
			if ( $this->Validator->isError( 'legal_name' ) == false ) {
				$this->Validator->isRegEx( 'legal_name',
										   $this->getLegalName(),
										   TTi18n::gettext( 'Legal Name contains invalid characters' ),
		...
		if ( $this->getTradeName() !== false ) {
			...
			if ( $this->Validator->isError( 'trade_name' ) == false ) {
				$this->Validator->isRegEx( 'trade_name',
										   $this->getTradeName(),
										   TTi18n::gettext( 'Trade Name contains invalid characters' ),
										   $this->name_validator_regex
				);
			}
		}
	...
	function getPaymentServicesAPIObject() {
		...
		if ( $this->getPaymentServicesStatus() == 10 && $this->getPaymentServicesUserName() != '' && $this->getPaymentServicesAPIKey() != '' ) {
			...
			$tt_ps_api = new TimeTrexPaymentServices(); //Unauthenticated connection for things like validating bank routing numbers.
		}
