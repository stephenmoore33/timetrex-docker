		...
		if ( empty( $filter ) || in_array( 'database', $filter, true ) ) {
			$schema_data->setColumns(
					TTSCols::new(
							...
							TTSCol::new( 'number' )->setFunctionMap( 'Number' )->setType( 'integer' )->setIsNull( false ),
							TTSCol::new( 'value' )->setFunctionMap( 'Value' )->setType( 'text' )->setIsNull( true ),
							TTSCol::new( 'extra_value' )->setFunctionMap( 'ExtraValue' )->setType( 'text' )->setIsNull( true ),
		...
		switch ( $name ) {
			case 'type':
				$retval = [
						...
						//
						50 => TTi18n::gettext( 'WebAuthn' ),

						//
						//Biometric data -- This is purged when employees are terminated.
						//
						//**NOTE: When changing these, also check APITimeClockUnAuthenticated->setUserIdentification(), and APIClientStationUnAuthenticated->setUserIdentification(), as we have security checks in place to lock down which type_id's can be enrolled.
						//
