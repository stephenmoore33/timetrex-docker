...
class RemittanceDestinationAccountFactory extends Factory {
	...
	private $enable_blank_remittance_source_account;
	...
	function _getSchemadata( ?array $filter = null ): ?TTS {
		...
		if ( empty( $filter ) || in_array( 'database', $filter, true ) ) {
			$schema_data->setColumns(
					TTSCols::new(
							...
							TTSCol::new( 'amount_type_id' )->setFunctionMap( 'AmountType' )->setType( 'integer' )->setIsNull( false ),
							TTSCol::new( 'amount' )->setFunctionMap( 'Amount' )->setType( 'numeric' )->setIsNull( false ),
							TTSCol::new( 'percent_amount' )->setFunctionMap( 'PercentAmount' )->setType( 'numeric' )->setIsNull( false ),
		...
		if ( empty( $filter ) || in_array( 'fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			$schema_data->setTabs(
					TTSTabs::new(
							TTSTab::new( 'tab_remittance_destination_account' )->setLabel( TTi18n::getText( 'Pay Methods' ) )->setFields(
									new TTSFields(
											...
											TTSField::new( 'status_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Status' ) )->setDataSource( TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getOptions' )->setArg( 'status' ) ),
											...
											TTSField::new( 'type_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Type' ) )->setDataSource( TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getOptions' )->setArg( 'type' ) ),
											TTSField::new( 'remittance_source_account_id' )->setType( 'single-dropdown' )->setLabel( TTi18n::getText( 'Remittance Source Account' ) )->setDataSource( TTSAPI::new( 'APIRemittanceSourceAccount' )->setMethod( 'getRemittanceSourceAccount' ) ),
											...
											TTSField::new( 'priority' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Priority' ) )->setDataSource( TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getOptions' )->setArg( 'priority' ) ),
											TTSField::new( 'amount_type_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Amount Type' ) )->setDataSource( TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getOptions' )->setArg( 'amount_type' ) ),
											...
											TTSField::new( 'value1_2' )->setType( 'text' )->setLabel( TTi18n::getText( 'Account Type' ) ),
		...
		if ( empty( $filter ) || in_array( 'search_fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setSearchFields(
					TTSSearchFields::new(
							...
							TTSSearchField::new( 'legal_entity_id' )->setType( 'uuid' )->setColumn( 'rsaf.legal_entity_id' )->setMulti( true ),
							TTSSearchField::new( 'remittance_source_account_id' )->setType( 'uuid' )->setColumn( 'a.remittance_source_account_id' )->setMulti( true ),
							...
							TTSSearchField::new( 'value2' )->setType( 'text' )->setColumn( 'a.value2' ),
		...
		if ( empty( $filter ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setAPIMethods(
					TTSAPIs::new(
							TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getRemittanceDestinationAccount' )
									->setSummary( 'Get remittance destination account records.' )
									->setArgs( [ 'data' => [ 'filter_data' => $schema_data->getSearchFields(), 'filter_columns' => $schema_data->getFields() ] ] )
									->setArgsModelDescription( 'Under the "data" arg there must be at least one or more "filter_data" elements specified, and one or more "filter_columns" elements which define the minimum necessary columns of data to return for carrying out other actions. For example if looking up the "id" of a record, just the "id" filter_columns needs to be specified with a value of true.' ),
							TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'setRemittanceDestinationAccount' )
									->setSummary( 'Add or edit remittance destination account records. Will return the record UUID upon success, or a validation error if there is a problem.' )
									->setArgs( [ 'data' => $schema_data->getFields() ] ),
							TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'deleteRemittanceDestinationAccount' )
									->setSummary( 'Delete remittance destination account records by passing in an array of UUIDs.' )
									->setArgs( new TTSFields(
													   TTSField::new( 'data' )->setType( 'multi-dropdown' )->setLabel( TTi18n::getText( 'IDs to Delete' ) )->setDataSource( TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getRemittanceDestinationAccount' ) ),
											   ) ),
							TTSAPI::new( 'APIRemittanceDestinationAccount' )->setMethod( 'getRemittanceDestinationAccountDefaultData' )
									->setSummary( 'Get default remittance destination account data used for creating new accounts. Use this before calling setRemittanceDestinationAccount to get the correct default data.' ),
					),
			);
		}
	...
	function _getFactoryOptions( $name, $params = null ) {
		...
		switch ( $name ) {
			...
			case 'amount_type':
				$retval = [
						10 => TTi18n::gettext( 'Percent of Net Pay' ),
						15 => TTi18n::gettext( 'Percent of Amount Remaining' ),
	 ...
	 */
	function getAmount() {
		return TTMath::removeTrailingZeros( $this->getGenericDataValue( 'amount' ), 2 );
	}
	...
	function getDisplayPercentAmount() {
		return TTMath::removeTrailingZeros( $this->getPercentAmount(), 0 ) . '%';
	}
	...
	function getValue3( $value = null ) {
		...
		$value = Misc::decrypt( $value, null, TTPassword::getPasswordSalt( ( is_object( $this->getUserObject() ) ? $this->getUserObject()->getCompany() : null ) ) );
	...
	function setValue3( $value ) {
		...
		if ( $value != '' ) { //Make sure we can clear out the value if needed. Misc::encypt() will return FALSE on a blank value.
			$encrypted_value = Misc::encrypt( $value, null, TTPassword::getPasswordSalt( ( is_object( $this->getUserObject() ) ? $this->getUserObject()->getCompany() : null ) ) );
	...
	function Validate( $ignore_warning = true ) {
		...
		if ( $this->getDeleted() == false ) {
			...
			// Percent
			if ( ( $this->getAmountType() == 10 || $this->getAmountType() == 15 ) && $this->getPercentAmount() != '' ) {
			...
			if ( $this->getAmountType() == 20 ) {
				...
				if ( $this->Validator->isError( 'percent_amount' ) == false && ( $this->getAmountType() == 10 || $this->getAmountType() == 15 ) ) {
			...
			if ( $this->getType() == 3000 ) {
				//Only require reauthentication if certain sensitive fields are changed.
				//   Enabling a record must be reauthenticated, as they could create a disabled record, then enable it otherwise.
				//   Creating any new record, or changing account numbers also require reauthentication.
				$data_diff = $this->getDataDifferences();
				if ( $this->isNew() == TRUE ||
						( ( $this->isDataDifferent( 'status_id', $data_diff ) == true && $this->getStatus() == 10 ) || $this->isDataDifferent( 'value3', $data_diff ) == true || $this->isDataDifferent( 'remittance_source_account_id', $data_diff ) == true )
					) {
					Debug::Text( '  NOTICE: Sensitive change, require reauthentication...', __FILE__, __LINE__, __METHOD__, 10 );
			...
			if ( PRODUCTION == true && $ignore_warning == false && $this->getStatus() == 10 && $this->getType() == 3000 && is_object( $this->getRemittanceSourceAccountObject() ) == true && is_object( $this->getRemittanceSourceAccountObject()->getLegalEntityObject() ) ) { //3000=EFT/ACH
				$le_obj = $this->getRemittanceSourceAccountObject()->getLegalEntityObject();

				if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL && is_object( $le_obj ) && is_object( $le_obj->getCompanyObject() ) && $le_obj->getCompanyObject()->getProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
					try {
		...
		$pstlf->getByRemittanceDestinationAccountId( $this->getId(), 1 ); //Limit 1
		if ( $pstlf->getRecordCount() > 0 ) {
			...
			$disallowed_edit_fields = [ 'type_id' ]; //'remittance_source_account_id'

			//There is a case where before a destination account is used, it changes from EFT/ACH to Check (this is intentionally allowed), so it has a bank account number associated with it that for some reason might not get cleared.
			//  Then as a check format it gets assigned to a pay stub, then the user tries to disable it. We need to make sure we don't check for value1,2,3 to change when its already a check format, but still check that the type doesn't change ever.
			if ( $this->getType() == 3000 ) {
				if ( $country == 'CA' ) {
					$disallowed_edit_fields[] = 'value1'; //US must be able to change account type.
				}
				$disallowed_edit_fields[] = 'value2';
				$disallowed_edit_fields[] = 'value3';
			}
		...
		unset( $pstlf, $disallowed_edit_fields, $changed_fields, $edited_fields_valid, $first_invalid_key );
	...
	function preValidate() {
		//Make sure if we are using a 'Check' format, we always clear bank format values, so we don't get into a situation where its a Check format and a bank account field is populated, which could cause validation errors.
		if ( $this->getType() == 2000 ) {
			$this->setValue1( '' );
			$this->setValue2( '' );
			$this->setValue3( '' );
				...
				if ( $include_columns == null || ( isset( $include_columns[$variable] ) && $include_columns[$variable] == true ) ) {
					...
					switch ( $variable ) {
						...
						case 'percent_amount':
							...
							break;
						case 'display_amount':
							if ( $this->getAmountType() == 10 || $this->getAmountType() == 15 ) { //Show percent sign at end, so the user can tell the difference.
								$data[$variable] = $this->getDisplayPercentAmount();
							} else {
