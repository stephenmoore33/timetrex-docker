 ...
 * @implements IteratorAggregate<PayStubEntryFactory>
 */
class PayStubEntryListFactory extends PayStubEntryFactory implements IteratorAggregate {
	...
	function getYTDAmountSumByUserIdAndEntryNameIdAndDate( $id, $entry_name_id, $date = null, $exclude_id = null, $where = null, $order = null ) {
		...
		//  However YTD amounts are fully based off the pay stub transaction date, so that should be used instead. Now that we don't handle advanced, the above shouldn't be an issue anymore.
		$query = '

					select	sum(amount) as amount, sum(units) as units, max( ytd_amount ) as ytd_amount
					from (
						select	sum(amount) as amount, sum(units) as units, max( ytd_amount ) as ytd_amount
						from	' . $this->getTable() . ' as a
								LEFT JOIN ' . $pslf->getTable() . ' as b ON ( a.pay_stub_id = b.id )
								LEFT JOIN ' . $pplf->getTable() . ' as c ON ( b.pay_period_id = c.id AND c.deleted = 0 )
						where	
							b.user_id = ?
							...
							AND ( a.deleted = 0 AND b.deleted = 0 )
						UNION
						select sum(amount) as amount, sum(units) as units, null as ytd_amount
		...
		Debug::text( 'YTD Sum for User ID: ' . $id . ' Entry Name ID: ' . $entry_name_id . ': Amount ' . $row['amount'] . ' Units: ' . $row['units'] .' YTD Amount: '. $row['ytd_amount'], __FILE__, __LINE__, __METHOD__, 10 );
	...
	function getAPIReportByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		*/
		...
		//Used by EEO government reports.
		if ( isset( $filter_data['pay_period_contains_date'] ) && !is_array( $filter_data['pay_period_contains_date'] ) && trim( $filter_data['pay_period_contains_date'] ) != '' ) {
			$ph[] = $ph[] = $this->db->BindTimeStamp( (int)TTDate::parseDateTime( $filter_data['pay_period_contains_date'] ) );
			$query .= ' AND bb.start_date <= ?';
			$query .= ' AND bb.end_date >= ?';
		}

		$query .= '
								AND (aa.deleted = 0 AND bb.deleted = 0 AND cc.deleted=0)
							group by aa.pay_stub_id, kk.id, kk.name, aa.pay_stub_entry_name_id, ii.type_id, ii.ps_order
							) a
						LEFT JOIN ' . $psf->getTable() . ' as b ON a.pay_stub_id = b.id
		...
		$query = '
					...
					from	(
							select aa.pay_stub_id as pay_stub_id,
								...
								hh.branch_id as branch_id,
								hh.department_id as department_id,
								hh.job_id as job_id,
								hh.job_item_id as job_item_id														
							from ' . $this->getTable() . ' as aa
							LEFT JOIN ' . $psaf->getTable() . ' as hh ON aa.pay_stub_amendment_id = hh.id
							LEFT JOIN ' . $psf->getTable() . ' as bb ON aa.pay_stub_id = bb.id
							LEFT JOIN ' . $uf->getTable() . ' as cc ON bb.user_id = cc.id
