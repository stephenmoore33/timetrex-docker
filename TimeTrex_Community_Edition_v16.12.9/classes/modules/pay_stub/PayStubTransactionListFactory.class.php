 ...
 * @implements IteratorAggregate<PayStubTransactionFactory>
 */
class PayStubTransactionListFactory extends PayStubTransactionFactory implements IteratorAggregate {
	...
	function getAPISummaryByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query .= ' AND ( lef.deleted = 0 AND rdaf.deleted = 0 AND a.deleted = 0 AND rsaf.deleted = 0 AND psf.deleted = 0 )
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$sort_column_aliases = [
				...
				'destination_user_full_name'     => 'uf.last_name'
		];
		...
		//NOTE: The per row sub-query was an optimization from a LEFT JOIN ( <sub-query> ) approach, which was much slower due to it having to
		//      search all transactions across all of time and filter out the rows that were not needed. This approach is much faster as it can use a index scan directly.
		$query = '
					SELECT	a.*,
							...
							( SELECT count(*) FROM ' . $this->getTable() . ' as pst_a LEFT JOIN ' . $psf->getTable() . ' as psf_b ON ( pst_a.pay_stub_id = psf_b.id ) WHERE a.pay_stub_id = pst_a.pay_stub_id AND psf_b.status_id = 25 AND pst_a.status_id != 10 AND pst_a.type_id = 10 AND pst_a.deleted = 0 ) as pay_stub_total_transactions_processed,
		...
		$query .= ( isset( $filter_data['created_by'] ) ) ? $this->getWhereClauseSQL( [ 'a.created_by', 'y.first_name', 'y.last_name' ], $filter_data['created_by'], 'user_id_or_name', $ph ) : null;
		$query .= ( isset( $filter_data['updated_by'] ) ) ? $this->getWhereClauseSQL( [ 'a.updated_by', 'z.first_name', 'z.last_name' ], $filter_data['updated_by'], 'user_id_or_name', $ph ) : null;
		$query .= ' AND ( a.deleted = 0 AND rsaf.deleted = 0 AND lef.deleted = 0 AND rdaf.deleted = 0 AND psf.deleted = 0 )';
		$query .= $this->getWhereSQL( $where );
		$query .= $this->getSortSQL( $order, $strict, $additional_order_fields );
