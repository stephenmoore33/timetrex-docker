...
class DemoData {
	...
	public $user_wage_groups;
	private $permission_presets;
	 ...
	 * @param string $company_id    UUID
	 * @param $type
	 * @param string|array $geo_fence_ids UUID
	 * @return bool
	 */
	 ...
	 * @param string $company_id    UUID
	 * @param $type
	 * @param string|array $branch_ids    UUID
	 * @param string|array $geo_fence_ids UUID
	 * @return bool
	 */
	 ...
	 * @return bool
	 */
	function createStation( $company_id, $type_id, $source = 'ANY', $station = 'ANY' ) {
	...
	function createAccrualPolicyAccount( $company_id, $type ) {
		...
		$apaf->setEnablePayStubBalanceDisplay( true );
		if ( $apaf->isValid() ) {
			$insert_id = $apaf->Save();
	 ...
	 * @param string $company_id       UUID
	 * @param $type
	 * @param string|array $taxes_policy_ids UUID
	 * @return bool
	 */
	 ...
	 * @return bool
	 */
	function createRegularTimePolicy( $company_id, $type, $contributing_shift_policy_id = 0, $pay_code_id = 0, $punch_tags = [] ) {
		...
		switch ( $type ) {
			case 10:
				...
				$rtpf->setCalculationOrder( 9910 );
				break;
			case 20: //Have a few extras with different names.
				$rtpf->setName( 'Regular Time (2)' );
				$rtpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$rtpf->setPayCode( $pay_code_id );
				$rtpf->setCalculationOrder( 9920 );
				break;
			case 21: //Have a few extras with different names.
				$rtpf->setName( 'Regular Time (3)' );
				$rtpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$rtpf->setPayCode( $pay_code_id );
				$rtpf->setCalculationOrder( 9921 );
				break;
			case 22: //Have a few extras with different names.
				$rtpf->setName( 'Regular Time (4)' );
				$rtpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$rtpf->setPayCode( $pay_code_id );
				$rtpf->setCalculationOrder( 9922 );
				break;
			case 30:
				//Setting punch tags requires the ID to already be set for CompanyGenericMapFactory.
				$insert_id = $rtpf->getNextInsertId();
				$rtpf->setId( $insert_id );

				$rtpf->setName( 'Regular Time (Tags)' );
				$rtpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$rtpf->setPayCode( $pay_code_id );
				$rtpf->setCalculationOrder( 9930 );
				$rtpf->setPunchTagSelectionType( 10 );
				$rtpf->setPunchTag( $punch_tags );
				break;
			case 999:
				$rtpf->setName( 'Regular Time (Catch All)' );
				$rtpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$rtpf->setPayCode( $pay_code_id );
				$rtpf->setCalculationOrder( 9990 );
				break;
		}
	 ...
	 * @return bool
	 */
	function createOverTimePolicy( $company_id, $type, $contributing_shift_policy_id = 0, $pay_code_id = 0, $punch_tags = [] ) {
		if ( $contributing_shift_policy_id === 0 ) {
			$contributing_shift_policy_id = TTUUID::getZeroID();
		}
		...
		switch ( $type ) {
			...
			case 40:
				//Setting punch tags requires the ID to already be set for CompanyGenericMapFactory.
				$insert_id = $otpf->getNextInsertId();
				$otpf->setId( $insert_id );

				$otpf->setName( 'Daily (>8hrs) [2]' );
				$otpf->setType( 10 );
				$otpf->setTriggerTime( ( 3600 * 8 ) );
				$otpf->setContributingShiftPolicy( $contributing_shift_policy_id );
				$otpf->setPayCode( $pay_code_id );

				$otpf->setPunchTagSelectionType( 20 );
				$otpf->setPunchTag( $punch_tags );
				break;
		}
	 ...
	 * @return bool
	 */
	function createPremiumPolicy( $company_id, $type, $contributing_shift_policy_id = 0, $pay_code_id = 0, $punch_tags = [] ) {
		if ( $contributing_shift_policy_id === 0 ) {
			$contributing_shift_policy_id = TTUUID::getZeroID();
		}
		...
		switch ( $type ) {
			...
			case 30:
				//Setting punch tags requires the ID to already be set for CompanyGenericMapFactory.
				$insert_id = $ppf->getNextInsertId();

				$ppf->setId( $insert_id );

				$ppf->setName( 'Evening [2]' );
				$ppf->setType( 20 );
				...
				$ppf->setPunchTagSelectionType( 10 );
				$ppf->setPunchTag( $punch_tags );
				break;
		}
	 ...
	 * @param $type
	 * @param $rate_type
	 * @param string|array $kpi_group_ids UUID
	 * @return bool
	 */
	...
	/**
	 * @param string $company_id           UUID
	 * @param string|array $meal_policy_ids      UUID
	 * @param string|array $exception_policy_id  UUID
	 * @param string|array $holiday_policy_ids   UUID
	 * @param string|array $over_time_policy_ids UUID
	 * @param string|array $premium_policy_ids   UUID
	 * @param string|array $rounding_policy_ids  UUID
	 * @param string|array $user_ids             UUID
	 * @param string|array $break_policy_ids     UUID
	 * @param string|array $accrual_policy_ids   UUID
	 * @param string|array $expense_policy_ids   UUID
	 * @param string|array $absence_policy_ids   UUID
	 * @param string|array $regular_policy_ids   UUID
	 * @return bool
	 */
	 ...
	 * @param $type
	 * @param string|array $user_ids         UUID
	 * @param string|array $client_group_ids UUID
	 * @return bool
	 */
	 ...
	 * @param $type
	 * @param string|array $invoice_district_ids UUID
	 ...
	 * @param null $payments
	 * @param string $user_ids            UUID
	 * @param string|array $shipping_policy_ids UUID
	 * @return bool
	 */
		...
		foreach ( $products as $key => $product_id ) {
			...
			if ( $plf->getRecordCount() > 0 ) {
				foreach ( $plf as $pf ) {
					...
					$transactions[$key]['amount'] = TTMath::mul( $transactions[$key]['unit_price'], $transactions[$key]['quantity'] );
				}
			}
			if ( isset( $payments ) ) {
				...
				$payments[$key]['amount'] = TTMath::MoneyRound( $transactions[$key]['amount'] );
				...
				if ( is_array( $taxes_arr ) ) {
					foreach ( $taxes_arr as $ptp_data ) {
						$payments[$key]['amount'] = TTMath::MoneyRound( TTMath::add( $payments[$key]['amount'], $ptp_data['amount'] ) );
					}
				}
			}
...
//					}
	 ...
	 * @param string|array $ethnic_group_ids UUID
	...
	function createUser( $company_id, $legal_entity_id, $type, $policy_group_id = null, $default_branch_id = null, $default_department_id = null, $default_currency_id = null, $user_group_id = null, $user_title_id = null, $ethnic_group_ids = null, $remittance_source_account_ids = null, $coordinates = null, $default_job_id = null, $default_job_item_id = null, $hire_date = null, $default_punch_tag_id = [], $custom_fields = null ) {
		...
		if ( $default_punch_tag_id === null ) {
			$default_punch_tag_id = TTUUID::getZeroID();
		}
		...
		$uf->setDefaultPunchTag( $default_punch_tag_id );
		...
		$uf->parseCustomFieldsFromArray( $custom_fields );
	...
	function createRecruitmentPortalConfig( $company_id ) {
		...
		$rpcf = TTnew('RecruitmentPortalConfigFactory');
		$rpcf->setObjectFromArray( $rpc_obj->getRecruitmentPortalConfigDefaultData() );
		if ( $rpcf->isValid() ) {
			$insert_id = $rpcf->Save();

			Debug::Text( '  Recruitment Portal Config ID: '. $insert_id, __FILE__, __LINE__, __METHOD__, 10 );
			return $insert_id;
		}

		Debug::Text( 'Failed Creating Recruitment Portal Config!', __FILE__, __LINE__, __METHOD__, 10 );
		return false;
	}
	...
	function createJobApplication( $job_applicant_id, $job_vacancy_id, $user_id ) {
		...
		$priority_ids = $jaf->getOptions( 'priority' );
		unset( $priority_ids[0] ); //Remove NONE as an option.
		$jaf->setPriority( array_rand( $priority_ids ) );
	 ...
	 * @param $company_id
	 * @param $user_id
	 * @param int $total_to_leave_unread
	 * @return bool
	 * @throws DBError
	 * @throws GeneralError
	 * @throws ReflectionException
	 */
	function setNotificationsAsRead( $company_id, $user_id, $total_to_leave_unread = 5 ) {
		$nlf = TTnew( 'NotificationListFactory' ); /** @var NotificationListFactory $nlf */
		$nlf->getByCompanyIdAndUserIdAndStatus( $company_id, $user_id, 10 );
		if ( $nlf->getRecordCount() > 0 ) {
			$i = 0;
			foreach ( $nlf as $n_obj ) { /** @var NotificationFactory $n_obj */
				if ( $i > ( $nlf->getRecordCount() - $total_to_leave_unread ) ) {
					break;
				}

				$n_obj->setStatus( 20 ); //Read
	...
	/**
	 * @param string $company_id     UUID
	 * @param string|array $child_user_ids UUID
	 * @return bool
	 */
		...
		switch ( $type ) {
			...
			case 30: //Try to match setType()
				...
				if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
					...
					$rsf->setPunchTag( $u_obj->getDefaultPunchTag() );
					$rsf->setAbsencePolicy( $absence_policy_id );
					$rsf->setOverlapType( 10 );
					if ( $rsf->isValid() ) {
	 ...
	 * @return bool
	 */
	function createPunchTagGroup( $company_id, $type, $parent_id = 0 ) {
		if ( $parent_id === 0 ) {
			$parent_id = TTUUID::getZeroID();
		}

		$ptg_obj = TTnew( 'PunchTagGroupFactory' ); /** @var PunchTagGroupFactory $ptg_obj */
		$ptg_obj->setCompany( $company_id );

		switch ( $type ) {
			case 10:
				$ptg_obj->setParent( $parent_id );
				$ptg_obj->setName( 'Policies' ); //ie: Shift Swap, Call-back, Inclement Weather, Overtime
				break;
			case 20:
				$ptg_obj->setParent( $parent_id );
				$ptg_obj->setName( 'Reasons' ); //ie: Traffic, Child Care, etc...
				break;
		}

		if ( $ptg_obj->isValid() ) {
			$insert_id = $ptg_obj->Save();
			Debug::Text( 'Punch Tag Group ID: ' . $insert_id, __FILE__, __LINE__, __METHOD__, 10 );

			return $insert_id;
		}

		Debug::Text( 'Failed Creating Punch Tag Group!', __FILE__, __LINE__, __METHOD__, 10 );
	 ...
	 * @return bool|string
	 */
	function createPunchTag( $company_id, $group_id, $manual_id, $name, $data ) {
		$pt_obj = TTnew( 'PunchTagFactory' ); /** @var PunchTagFactory $pt_obj */
		$pt_obj->setCompany( $company_id );
		$pt_obj->setGroup( $group_id );

		$pt_obj->setManualID( $manual_id );
		$pt_obj->setName( $name );

		$pt_obj->setStatus( 10 );

		//Because punch tags has sub-classes (CompanyGenericMapFactory) that depend on it, when adding a new record we need to make sure the ID is set first,
		$insert_id = $pt_obj->getNextInsertId();
		$pt_obj->setId( $insert_id );

		//User Group
		if ( isset( $data['user_group_selection_type'] ) ) {
			$pt_obj->setUserGroupSelectionType( $data['user_group_selection_type'] );
		} else {
			$pt_obj->setUserGroup( 10 );
		}
		if ( isset( $data['user_group_ids'] ) ) {
			$pt_obj->setUserGroup( $data['user_group_ids'] );
		}

		//User
		if ( isset( $data['include_user_ids'] ) ) {
			$pt_obj->setIncludeUser( $data['include_user_ids'] );
		} else {
			$pt_obj->setIncludeUser( false );
		}
		if ( isset( $data['exclude_user_ids'] ) ) {
			$pt_obj->setExcludeUser( $data['exclude_user_ids'] );
		} else {
			$pt_obj->setExcludeUser( false );
		}

		//Branch
		if ( isset( $data['branch_selection_type'] ) ) {
			$pt_obj->setPunchBranchSelectionType( $data['branch_selection_type'] );
		} else {
			$pt_obj->setPunchBranchSelectionType( 10 );
		}
		if ( isset( $data['branch_ids'] ) ) {
			$pt_obj->setPunchBranch( $data['branch_ids'] );
		}
		if ( isset( $data['exclude_default_branch'] ) ) {
			$pt_obj->setPunchExcludeDefaultBranch( $data['exclude_default_branch'] );
		} else {
			$pt_obj->setPunchExcludeDefaultBranch( 0 );
		}

		//Department
		if ( isset( $data['department_selection_type'] ) ) {
			$pt_obj->setPunchDepartmentSelectionType( $data['department_selection_type'] );
		} else {
			$pt_obj->setPunchDepartmentSelectionType( 10 );
		}
		if ( isset( $data['department_ids'] ) ) {
			$pt_obj->setPunchDepartment( $data['department_ids'] );
		}
		if ( isset( $data['exclude_default_department'] ) ) {
			$pt_obj->setPunchExcludeDefaultDepartment( $data['exclude_default_department'] );
		} else {
			$pt_obj->setPunchExcludeDefaultDepartment( 0 );
		}

		//Job
		if ( isset( $data['job_selection_type'] ) ) {
			$pt_obj->setPunchJobSelectionType( $data['job_selection_type'] );
		} else {
			$pt_obj->setPunchJobSelectionType( 10 );
		}
		if ( isset( $data['job_ids'] ) ) {
			$pt_obj->setPunchJob( $data['job_ids'] );
		}
		if ( isset( $data['exclude_default_job'] ) ) {
			$pt_obj->setPunchExcludeDefaultJob( $data['exclude_default_job'] );
		} else {
			$pt_obj->setPunchExcludeDefaultJob( 0 );
		}

		//Job Group
		if ( isset( $data['job_group_selection_type'] ) ) {
			$pt_obj->setPunchJobGroupSelectionType( $data['job_group_selection_type'] );
		} else {
			$pt_obj->setPunchJobGroupSelectionType( 10 );
		}
		if ( isset( $data['job_group_ids'] ) ) {
			$pt_obj->setPunchJobGroup( $data['job_group_ids'] );
		}

		//Job item
		if ( isset( $data['job_item_selection_type'] ) ) {
			$pt_obj->setPunchJobItemSelectionType( $data['job_item_selection_type'] );
		} else {
			$pt_obj->setPunchJobItemSelectionType( 10 );
		}
		if ( isset( $data['job_item_ids'] ) ) {
			$pt_obj->setPunchJobItem( $data['job_item_ids'] );
		}
		if ( isset( $data['exclude_default_job_item'] ) ) {
			$pt_obj->setPunchExcludeDefaultJobItem( $data['exclude_default_job_item'] );
		} else {
			$pt_obj->setPunchExcludeDefaultJobItem( 0 );
		}

		//Job Item Group
		if ( isset( $data['job_item_group_selection_type'] ) ) {
			$pt_obj->setPunchJobItemGroupSelectionType( $data['job_item_group_selection_type'] );
		} else {
			$pt_obj->setPunchJobItemGroupSelectionType( 10 );
		}
		if ( isset( $data['job_group_ids'] ) ) {
			$pt_obj->setPunchJobItemGroup( $data['job_item_group_ids'] );
		}

		//GEO Fence
		if ( isset( $data['geo_fence_ids'] ) ) {
			$pt_obj->setGEOFenceIds( $data['geo_fence_ids'] );
		}

		if ( $pt_obj->isValid() ) {
			$pt_obj->Save( true, true);
			Debug::Text( 'Punch Tag ID: ' . $insert_id, __FILE__, __LINE__, __METHOD__, 10 );

			return $insert_id;
		}

		Debug::Text( 'Failed Creating Punch Tag!', __FILE__, __LINE__, __METHOD__, 10 );
	...
	/**
	 * @param string $company_id  UUID
	 * @param string|array $group_ids   UUID
	 * @param $type
	 * @param string $currency_id UUID
	 * @return bool
	 */
	...
	function editSchedule( $id, $data = null ) {
		...
		if ( $slf->getRecordCount() == 1 ) {
			...
			if ( isset( $data['punch_tag_id'] ) ) {
				$s_obj->setPunchTag( $data['punch_tag_id'] );
			}
	...
	function editPunch( $id, $data = null ) {
		...
		if ( $plf->getRecordCount() == 1 ) {
			...
			if ( $p_obj->isValid() == true ) {
				...
				$p_obj->setUser( $p_obj->getPunchControlObject()->getUser() ); //Make sure we always set the UserID prior to setPunchObject() so PunchControlFactory also has the user_id.
				...
				if ( $p_obj->getPunchControlObject()->isValid() == true ) {
					...
					Debug::Text( 'Punch ID edited successfully! ID: ' . $id, __FILE__, __LINE__, __METHOD__, 10 );
					return true;
				} else {
					Debug::Text( 'ERROR: Failed validation when editing Punch ID: ' . $id, __FILE__, __LINE__, __METHOD__, 10 );
				}
			}
		}

		Debug::Text( 'ERROR: Punch ID does not exist to edit: ' . $id, __FILE__, __LINE__, __METHOD__, 10 );
		$plf->FailTransaction();
		$plf->CommitTransaction();
	...
	function createPunch( $user_id, $type_id, $status_id, $time_stamp, $data, $coordinates = null, $calc_total_time = true ) {
		...
		if ( $fail_transaction == false ) {
			...
			if ( isset( $data['punch_tag_id'] ) ) {
				$pcf->setPunchTag( $data['punch_tag_id'] );
			}
			...
			case 'UserSummaryReport':
				switch ( $type ) {
					case 10:
						...
						break;
					case 20:
						$rcf->setName( 'Hire Date (less than 5yrs)' );
					...
					case 200:
						...
						break;
					case 270:
						$rcf->setName( 'New Hires (less than 30 days)' );
	...
	function createRemittanceDestinationAccount( $user_id, $currency_id, $legal_entity_id, $remittance_source_account_id, $type_id, $percent_amount = null ) {
		...
		$amount_type_id = array_rand( $rdaf->getOptions( 'amount_type' ) ); //10=Percent of Net Pay, 15=Percent of Amount Remianing, or 20=Fixed

		$rdaf->setAmountType( $amount_type_id );
		if ( $amount_type_id == 10 || $amount_type_id == 15 ) {
		...
		switch ( $type_id ) {
			...
			case 130:
				$rdaf->setType( 2000 );
				$rdaf->setPriority( 1 );
				$rdaf->setAmountType( 10 ); //10=Percent of Net Pay
				$rdaf->setPercentAmount( $percent_amount );
			...
			case 230:
				$rdaf->setType( 2000 );
				$rdaf->setPriority( 1 );
				$rdaf->setAmountType( 15 ); //15=Percent of Remaining
				$rdaf->setPercentAmount( $percent_amount );
				$rdaf->setName( 'Checking Account ['. $type_id .']' );
				break;
		}
	 ...
	 * @return bool
	 */
	function createCustomField( $company_id, $name, $parent_table, $type_id, $meta_data = null ) {
		$cf = TTnew( 'CustomFieldFactory' ); /** @var CustomFieldFactory $cf */

		Debug::Text( 'Creating custom field.', __FILE__, __LINE__, __METHOD__, 10 );

		$cf->setCompany( $company_id );
		$cf->setName( $name );
		$cf->setParentTable( $parent_table );
		$cf->setType( $type_id );

		if ( $meta_data !== null ) {
			$cf->setCustomFieldMetaData( $meta_data );
		}

		if ( $cf->isValid() ) {
			$insert_id = $cf->Save();
			Debug::Text( 'Custom Field ID: ' . $insert_id, __FILE__, __LINE__, __METHOD__, 10 );

			return $insert_id;
		}

		Debug::Text( 'Failed Creating Custom Field!', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function generateData() {
		...
		if ( $company_id !== false ) {
			...
			//Create stations
			$this->createStation( $company_id, 10 ); //PC
			$this->createStation( $company_id, 26 ); //Mobile Browser
			$this->createStation( $company_id, 28 ); //Mobile App
			...
			for ( $i = 1; $i <= $this->getMaxRandomUsers(); $i++ ) {
			...
			$this->createAuthorizationHierarchyLevel( $company_id, $hierarchy_control_id, $user_ids[1], 2 );
			$this->createAuthorizationHierarchyLevel( $company_id, $hierarchy_control_id, $user_ids[2], 3 );
			$superior_user_ids = [ $current_user->getId(), $user_ids[1], $user_ids[2] ]; //Create an array of supervisor user_ids in level order so we can use them in createAuthorizations() lower down.
			//Debug::Arr( $superior_user_ids, 'Superior User IDs: ', __FILE__, __LINE__, __METHOD__, 10 );
			...
			if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE ) {
				...
				//Punch Tags
				$punch_tag_ids[] = $this->createPunchTag( $company_id, TTUUID::getZeroID(), 1, 'CallBack', [] );
				$punch_tag_ids[] = $this->createPunchTag( $company_id, TTUUID::getZeroID(), 2, 'Shift Swap', [] );
				$punch_tag_ids[] = $this->createPunchTag( $company_id, TTUUID::getZeroID(), 3, 'Inclement Weather', [] );
				$punch_tag_ids[] = $this->createPunchTag( $company_id, TTUUID::getZeroID(), 4, 'Overtime', [] );
			} else {
				$task_ids[] = TTUUID::getZeroID();
				$job_ids[] = TTUUID::getZeroID();
				$punch_tag_ids[] = TTUUID::getZeroID();
			}
			...
			if ( isset( $this->create_data['hr'] ) && $this->create_data['hr'] == true ) {
				...
				$kpi_all_ids[]['10'] = $this->createKPI( $company_id, 10, 10, [ TTUUID::getNotExistID() ] );
				$kpi_all_ids[]['10'] = $this->createKPI( $company_id, 20, 10, [ TTUUID::getNotExistID() ] );
				$kpi_all_ids[]['20'] = $this->createKPI( $company_id, 30, 20, [ TTUUID::getNotExistID() ] );
				$kpi_group1_ids[]['20'] = $this->createKPI( $company_id, 40, 20, [ TTUUID::getNotExistID(), $kpi_group_ids[0] ] );
				$kpi_group1_ids[]['10'] = $this->createKPI( $company_id, 50, 10, [ TTUUID::getNotExistID(), $kpi_group_ids[0] ] );
				$kpi_group2_ids[]['30'] = $this->createKPI( $company_id, 60, 30, [ TTUUID::getNotExistID(), $kpi_group_ids[1] ] );
				$kpi_group2_ids[]['30'] = $this->createKPI( $company_id, 70, 30, [ TTUUID::getNotExistID(), $kpi_group_ids[1] ] );
				...
				if ( getTTProductEdition() >= TT_PRODUCT_ENTERPRISE ) {
					...
					while ( $x <= 10 ) {
						...
						$job_vacancy_ids[] = $this->createJobVacancy( $company_id, $user_id, $user_title_ids[array_rand( $user_title_ids )], $branch_ids[array_rand( $branch_ids )], $department_ids[array_rand( $department_ids )] );
						$x++;
					}

					if ( count( $job_vacancy_ids ) > 1 ) {
						$j = 1;
						while ( $j <= 30 ) {
							$job_applicant_id = $this->createJobApplicant( $company_id );

							//Create random job applicant information and assign it to the applicant.
							$n = 1;
							while ( $n <= rand( 2, 5 ) ) {
							...
							//Job applicant randomly applies for vacancies.
							$y = 1;
							while ( $y <= rand( 2, 7 ) ) {
								$this->createJobApplication( $job_applicant_id, $job_vacancy_ids[array_rand($job_vacancy_ids)], $user_id );
								$y++;
							}
			...
			if ( isset( $this->create_data['punch'] ) && $this->create_data['punch'] == true ) {
				...
				foreach ( $user_ids as $user_id ) {
					...
					$user_random_punch_tag_ids = (array)array_flip( (array)array_rand( $punch_tag_ids, 2 ) );
							...
							if ( $punch_date >= $exception_cutoff_date && in_array( $user_id, $hierarchy_user_ids ) && date( 'w', $punch_date ) != 5 ) { //Make sure requests are only created when supervisors exist.
								if ( rand( 0, 99 ) < 30 ) { //30% chance
									...
									if ( rand( 0, 99 ) < 95 ) { //95% chance
										...
										if ( rand( 0, 99 ) < 95 ) { //95% chance
											$this->createAuthorization( 1010, $request_id, $superior_user_ids[1], true );
											if ( rand( 0, 99 ) < 95 ) { //95% chance
												$this->createAuthorization( 1010, $request_id, $superior_user_ids[0], true );
											}
										}
									}
							...
							if ( rand( 0, 99 ) < 60 ) { //60% chance (at least 3 out of 5 days)
								...
								$this->createPunchPair( $user_id,
														...
														[
																...
																'punch_tag_id'     => $punch_tag_ids[array_rand( $user_random_punch_tag_ids )],
		...
		//Mark most notifications as read for all users.
		foreach ( $user_ids as $user_id ) {
			$this->setNotificationsAsRead( $company_id, $user_id );
		}

		$custom_fields = [];

		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Text', 'ui_kit', 100 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Textarea', 'ui_kit', 110 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Integer', 'ui_kit', 400 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Decimal', 'ui_kit', 410 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Currency', 'ui_kit', 420 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Checkbox', 'ui_kit', 500 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Date', 'ui_kit', 1000 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Date Range', 'ui_kit', 1010 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Time', 'ui_kit', 1100 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Datetime', 'ui_kit', 1200 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Time Unit', 'ui_kit', 1300 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Single-select', 'ui_kit', 2100 );
		$custom_fields[] = $this->createCustomField( $company_id, 'Custom Multi-select', 'ui_kit', 2110 );
