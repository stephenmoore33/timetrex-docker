...
class CalculatePolicy {
	...
	private $pay_period_from_date_cache = null;
	...
	private $old_prev_user_date_total_end_time_stamp = null;   //Used to store the old value of prev_user_date_total_end_time_stamp, so we can revert back to it after calculating a regular time policy.
	...
	private $pay_formula_calculate_average_hourly_rates = false;

	private $is_calculating_eligible_for_holiday_policy_id = [];

	/**
	 * @var float|int
	 */
	private $holiday_after_days;
	/**
	 * @var int
	 */
	private $holiday_before_days;
	/**
	 * @var int
	 */
	private $pay_formula_average_days;
	/**
	 * @var array
	 */
	private $contributing_shift_policy_ids = [];
	/**
	 * @var array
	 */
	private $schedule_regular_time_policy_ids = [];
	/**
	 * @var array
	 */
	private $schedule_undertime_absence_policy_ids = [];
	/**
	 * @var array
	 */
	private $schedule_meal_time_policy_ids = [];
	/**
	 * @var array
	 */
	private $schedule_break_time_policy_ids = [];
	/**
	 * @var array
	 */
	private $contributing_pay_code_policy_ids = [];
	...
	function setUserObject( $obj ) {
		if ( is_object( $obj ) ) {
			...
			//Need to set the timezone as soon as the user object is specified, so when addPendingCalculationDate() is called they are in the proper timezone too.
			$this->setTimeZone();
		...
		if ( TTUUID::isUUID( $id ) && $id != TTUUID::getZeroID() && $id != TTUUID::getNotExistID() ) {
			if ( isset( $this->pay_periods[$id] ) && is_object( $this->pay_periods[$id] ) && $id == $this->pay_periods[$id]->getID() ) {
				...
				if ( $lf->getRecordCount() == 1 ) {
					$this->pay_periods[$id] = $lf->getCurrent(); //**NOTE: This is also populated in setPayPeriodFromDate().
	...
	function getPayPeriodScheduleObject( $id ) {
		if ( TTUUID::isUUID( $id ) && $id != TTUUID::getZeroID() && $id != TTUUID::getNotExistID() ) {
			if ( isset( $this->pay_period_schedules[$id] ) && is_object( $this->pay_period_schedules[$id] ) && $id == $this->pay_period_schedules[$id]->getID() ) {
				...
				$lf->getById( $id );
				if ( $lf->getRecordCount() == 1 ) {
					$this->pay_period_schedules[$id] = $lf->getCurrent(); //**NOTE: This is also populated in setPayPeriodFromDate().
	...
	function getIsDeletedData() {
		return $this->is_deleted_data;
	}

	/**
	 * Tracks any UDT records deleted OUTSIDE of the calculation class, so we can use it to turn on additional calculations that would otherwise be skipped.
	 * 		This is currently used when deleting absence records (UserDateTotal) in APIUserDateTotal.class.php. See $this->_calculate() comments regarding maximum daily time/UDT records.
	 * @param bool $bool
	 * @return bool
	 */
	function setIsDeletedData( $bool ) {
		$this->is_deleted_data = (bool)$bool;
	...
	function compactMealAndBreakUserDateTotalObjects( $user_date_total_rows ) {
		if ( is_array( $user_date_total_rows ) && count( $user_date_total_rows ) > 0 ) {
			...
			//
			//
			// **IMPORTANT: Meal/Break records (especially negative total time) should never make it out of the function, if they do then the downstream policy will just match it (ie: negative regular time, negative OT).
			foreach ( $user_date_total_rows as $key => $meal_break_udt_obj ) {
				//At the start of each loop, copy user_date_total_rows to tmp_user_date_total_rows so when a meal policy spans midnight and gets split into two
				// essentially causing nested meal policies compactions, they are adjusting from the last adjustment, and adjusting from the original record each time.
				// See support ticket #519788.
				// This occurs when a single regular time policy exists and is limited to a specific job, and the catch-all regular time policy does not exist (or is not assigned)
				//   Then the auto-deduct meal Allocation Type: At Active After Time is causing a 30min lunch to deduct from 11:59PM to 12:29AM (next day), with punches of 6:44PM to 8:12AM.
				$tmp_user_date_total_rows = $user_date_total_rows;

				if ( $meal_break_udt_obj->getObjectType() == 100 || $meal_break_udt_obj->getObjectType() == 110 ) {
					$min_user_date_total_rows_key = min( array_keys( $tmp_user_date_total_rows ) );
					Debug::Text( 'Found Meal/Break record... Key: ' . $key . ' Min Key: '. $min_user_date_total_rows_key .' Total Time: ' . $meal_break_udt_obj->getTotalTime() .' Start: '. TTDate::getDate('DATE+TIME', $meal_break_udt_obj->getStartTimeStamp() ) .' End: '. TTDate::getDate('DATE+TIME', $meal_break_udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );
					foreach ( $tmp_user_date_total_rows as $tmp_key => $tmp_udt_obj ) { /* @var UserDateTotalFactory $tmp_udt_obj */
						//Since legacy customers who upgraded from pre-UUID could have duplicate UUIDs for a break policy and a meal policy
						// we need to add the object type to the ID to avoid conflicts and mistakenly assuming a meal policy was already
						// processed when it was not, and a break policy of the same ID was processed instead.
						$source_object_composite_key = $meal_break_udt_obj->getObjectType() . $meal_break_udt_obj->getSourceObject();
						if ( !isset( $processed_keys[$source_object_composite_key][$tmp_key] )
								&& !in_array( $tmp_udt_obj->getObjectType(), [ 100, 101, 110, 111 ] )
								&& $meal_break_udt_obj->getBranch() == $tmp_udt_obj->getBranch()
								&& $meal_break_udt_obj->getDepartment() == $tmp_udt_obj->getDepartment()
								&& $meal_break_udt_obj->getJob() == $tmp_udt_obj->getJob()
								&& $meal_break_udt_obj->getJobItem() == $tmp_udt_obj->getJobItem()
								&& $meal_break_udt_obj->getPunchTag() == $tmp_udt_obj->getPunchTag()
								&& ( $meal_break_udt_obj->getTotalTime() < 0 && $tmp_udt_obj->getTotalTime() >= abs( $meal_break_udt_obj->getTotalTime() ) )
						) {
							Debug::Text( '  Found Related UDT: Key: ' . $tmp_key . ' Object Type: ' . $tmp_udt_obj->getObjectType() . ' Pay Code: ' . $tmp_udt_obj->getPayCode() . ' Total Time: ' . $tmp_udt_obj->getTotalTime() .' Start: '. TTDate::getDate('DATE+TIME', $tmp_udt_obj->getStartTimeStamp() ) .' End: '. TTDate::getDate('DATE+TIME', $tmp_udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );

							//Adjust corresponding record.
							//  Rather than just adjust the end timestamp, we need to insert the meal/break exactly where it should go, so other policies (ie: premiums) are calculated based on the exact right time.
							//  Therefore we need to adjust multiple records and even split or create new ones to "punch a hole" in an existing UDT record where the meal/break should be.
							if ( $meal_break_udt_obj->getGenericTempDataValue( 'source_object_allocation_type_id' ) == 100 && $tmp_udt_obj->getStartTimeStamp() != '' && $tmp_udt_obj->getEndTimeStamp() != '' && $meal_break_udt_obj->getStartTimeStamp() != '' && $meal_break_udt_obj->getEndTimeStamp() != '' ) {
								if ( TTDate::isTimeOverLap( $tmp_udt_obj->getStartTimeStamp(), $tmp_udt_obj->getEndTimeStamp(), $meal_break_udt_obj->getStartTimeStamp(), $meal_break_udt_obj->getEndTimeStamp() ) == true ) {
									Debug::Text( '    Meal/Break overlaps with this UDT record...', __FILE__, __LINE__, __METHOD__, 10 );
									$split_time_stamp_range = TTDate::splitDateRangeAtMidnight( $tmp_udt_obj->getStartTimeStamp(), $tmp_udt_obj->getEndTimeStamp(), $meal_break_udt_obj->getStartTimeStamp(), $meal_break_udt_obj->getEndTimeStamp() );

									if ( count( $split_time_stamp_range ) > 1 ) {
										$n = 0;
										foreach ( $split_time_stamp_range as $tmp_split_range ) {
											if ( $tmp_split_range['start_time_stamp'] != $meal_break_udt_obj->getStartTimeStamp() && $tmp_split_range['end_time_stamp'] != $meal_break_udt_obj->getEndTimeStamp() ) {
												if ( $n == 0 ) {
													$tmp_split_key = $tmp_key; //First loop iteration update the original record.
												} else {
													$tmp_split_key = ( $min_user_date_total_rows_key - $n );
												}
												$user_date_total_rows[$tmp_split_key] = clone $tmp_udt_obj; //Clone the object so we don't modify the original one.

												$user_date_total_rows[$tmp_split_key]->setStartTimeStamp( $tmp_split_range['start_time_stamp'] );
												$user_date_total_rows[$tmp_split_key]->setEndTimeStamp( $tmp_split_range['end_time_stamp'] );
												$user_date_total_rows[$tmp_split_key]->setTotalTime( ( $tmp_split_range['end_time_stamp'] - $tmp_split_range['start_time_stamp'] ) );
												$user_date_total_rows[$tmp_split_key]->setActualTotalTime( $user_date_total_rows[$tmp_split_key]->getTotalTime() );

												//Make sure start/end types are assigned properly to split UDT records based on meal/breaks, otherwise using a normal type would be considered to be the start of a new shift.
												//  Especially for things like minimum shift time premium policies. Meal/Breaks should not trigger a new shift to start, but getShiftData() needs to know these are related to meal/breaks.
												if ( $tmp_split_range['start_time_stamp'] == $meal_break_udt_obj->getEndTimeStamp() ) {
													$user_date_total_rows[$tmp_split_key]->setStartType( $meal_break_udt_obj->getStartType() );
												} else if ( $tmp_split_range['end_time_stamp'] == $meal_break_udt_obj->getStartTimeStamp() ) {
													$user_date_total_rows[$tmp_split_key]->setEndType( $meal_break_udt_obj->getEndType() );
												}
												Debug::Text( '      TMP Split Range: Split Key: ' . $tmp_split_key . ' Start: ' . TTDate::getDate( 'DATE+TIME', $tmp_split_range['start_time_stamp'] ) . ' End: ' . TTDate::getDate( 'DATE+TIME', $tmp_split_range['end_time_stamp'] ) . ' Type: Start: ' . $user_date_total_rows[$tmp_split_key]->getStartType() . ' End: ' . $user_date_total_rows[$tmp_split_key]->getEndType(), __FILE__, __LINE__, __METHOD__, 10 );

												//Since a single UDT record can be split into multiple, we need to record the *original* one that we need to map too for regular time exclusivity reduction.
												$user_date_total_rows[$tmp_split_key]->setGenericTempDataValue( 'split_from_source_key', $tmp_key );

												$n++;
											}
											//else {
											//	//Skipping meal/break split element. Essentially where the "hole" goes.
											//	Debug::Text( '      Skipping Meal/Break overlap element: Start: ' . TTDate::getDate( 'DATE+TIME', $tmp_split_range['start_time_stamp'] ) . ' End: ' . TTDate::getDate( 'DATE+TIME', $tmp_split_range['end_time_stamp'] ), __FILE__, __LINE__, __METHOD__, 10 );
											//}
										}
										unset( $n, $tmp_split_range, $tmp_split_key );
									} else {
										unset( $user_date_total_rows[$tmp_key] ); //The meal/break time exactly matches the UDT record, so it needs to be adjusted to total_time=0, or just removed completely.
									}
									unset( $split_time_stamp_range );

									//Remove original meal/break row.
									unset( $user_date_total_rows[$key] );

									//Mark record as processed so we don't process it again.
									$processed_keys[$source_object_composite_key][$tmp_key] = true;
									break;
								} else {
									Debug::Text( '    Meal/Break *DOES NOT* overlap with this UDT record...', __FILE__, __LINE__, __METHOD__, 10 );
								}
							} else {
								...
								//If we don't have start/end times, fallback to original "compaction" method of just taking it from the end of the UDT record.
								$user_date_total_rows[$tmp_key]->setTotalTime( ( $user_date_total_rows[$tmp_key]->getTotalTime() + $meal_break_udt_obj->getTotalTime() ) );
								$user_date_total_rows[$tmp_key]->setActualTotalTime( $user_date_total_rows[$tmp_key]->getTotalTime() );
								if ( $user_date_total_rows[$tmp_key]->getEndTimeStamp() != '' ) {
									$user_date_total_rows[$tmp_key]->setEndTimeStamp( ( $user_date_total_rows[$tmp_key]->getEndTimeStamp() + $meal_break_udt_obj->getTotalTime() ) );
								}
						...
						//else {
						//	Debug::Text( '    Skipping UDT (processed?): ' . $tmp_key . ' Object Type: ' . $tmp_udt_obj->getObjectType() . ' Pay Code: ' . $tmp_udt_obj->getPayCode() . ' Total Time: ' . $tmp_udt_obj->getTotalTime() .' Start: '. TTDate::getDate('DATE+TIME', $tmp_udt_obj->getStartTimeStamp() ) .' End: '. TTDate::getDate('DATE+TIME', $tmp_udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );
						//}
					}
				}
				//else {
			...
			unset( $tmp_user_date_total_rows, $meal_break_udt_obj, $tmp_udt_obj, $processed_keys );
	...
	function isOverriddenUserDateTotalObject( $udt_obj, $check_src_object_id = false ) {
		if ( is_array( $this->user_date_total ) ) {
			...
			foreach ( $this->user_date_total as $key => $tmp_udt_obj ) {
				if ( TTUUID::isUUID( $key ) == true //Found positive time record (key=UUID), only positive ones can be overridden anyways. Minor optmization to check for is_numeric() before UUID which uses regex.
	...
	function insertUserDateTotal() {
		if ( is_array( $this->user_date_total ) && count( $this->user_date_total ) > 0 ) {
			...
			$this->compactUserDateTotalDataBasedOnTimeStamps();
			...
			foreach ( $this->user_date_total as $key => $udt_obj ) {
				...
				if ( ( is_numeric( $key ) && $key < 0 && $udt_obj->getObjectType() != 10 && ( $udt_obj->getTotalTime() != 0 || $udt_obj->getTotalTimeAmount() != 0 || $udt_obj->getNote() != '' || ( $udt_obj->getObjectType() == 5 && $udt_obj->getStartTimeStamp() != false && $udt_obj->getEndTimeStamp() != false ) ) )
					...
					//Debug::text( '  Inserting UDT: Key: '. $key .' Punch Control ID: '. $udt_obj->getPunchControlID() .'  Object Type: '. $udt_obj->getObjectType() .' Start Time: ' . TTDate::getDate( 'DATE+TIME', $udt_obj->getStartTimeStamp() ) . ' End Time: ' . TTDate::getDate( 'DATE+TIME', $udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );
					...
					//      This will also relax other validations like ones that do SQL checks related to child IDs (ie: user, punch_control, etc... )
				...
				//else {
				//	Debug::text('Skipping UserDateTotal entry... Key: '. $key .' Object Type: '. $udt_obj->getObjectType() .' Total Time: '. $udt_obj->getTotalTime(), __FILE__, __LINE__, __METHOD__, 10);
				//}
			}
			...
			foreach ( $slf as $key => $s_obj ) {
				if ( $s_obj->getStatus() == 20
					...
					if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
						...
						$udtf->setPayPeriod( ( is_object( $this->pay_period_obj ) ? $this->pay_period_obj->getId() : false ) ); //Optimization so setDateStamp() doesn't need to lookup the pay period.
						...
						$udtf->setPunchControlID( TTUUID::getZeroID() ); //Optimization we don't need to cast this in compactUserDateTotalDataBasedOnTimeStamps()
						...
						if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE ) {
							...
							$udtf->setPunchTag( $s_obj->getPunchTag() );
						}
						...
						$udtf->setHourlyRate( $this->getHourlyRate( $s_obj->getAbsencePolicyObject()->getPayFormulaPolicy(), $udtf->getPayCode(), $date_stamp, $udtf->getBaseHourlyRate(), $udtf ) );
						...
						$udtf->calcTotalTimeAmounts(); //Call this so TotalTimeAmount is calculated immediately, as we don't save these records until later.
			...
			foreach ( $slf as $s_obj ) {
				...
				if ( $schedule_status_id == false || (int)$s_obj->getStatus() == (int)$schedule_status_id ) {
					...
					if ( $in_schedule_diff !== false || $out_schedule_diff !== false ) {
						...
						$schedule_diff = (float)( $in_schedule_diff + $out_schedule_diff ); //Must be float, as inScheduleDifference() above can return 0.5
						if ( $schedule_diff === 0.0 ) { //Because $schedule_diff is a float.
	...
	function getScheduleData( $start_date = null, $end_date = null, $status_ids = null, $limit = null, $order = null ) {
		...
		Debug::text( 'Getting Schedule Data for Start Date: ' . TTDate::getDate( 'DATE+TIME', $start_date ) . ' End: ' . TTDate::getDate( 'DATE+TIME', $end_date ), __FILE__, __LINE__, __METHOD__, 10 );
		$slf->getByUserIdAndStartDateAndEndDateAndStatusIdAndExcludeId( $this->getUserObject()->getId(), ( $start_date - $this->schedule_policy_max_start_stop_window ), ( $end_date + $this->schedule_policy_max_start_stop_window ), $status_ids, array_keys( (array)$this->schedule ), $limit, null, null, [ 'a.date_stamp' => 'asc', 'a.start_time' => 'asc' ] );
	 ...
	 * @return mixed
	 */
	function compactUserDateTotalDataBasedOnTimeStamps() {
		if ( is_array( $this->user_date_total ) && count( $this->user_date_total ) > 0 ) {
			$continue_merging = true;
			$x = 0;
			while ( $x < 100 && $continue_merging == true ) { //This handles multi-layer merging. Merging UDT records, then merging them again up to 100 layers.
				$continue_merging = false;

				$prev_udt_obj = false;
				$prev_udt_key = false;

				$this->sortUserDateTotalData( $this->user_date_total, 'sortUserDateTotalDataByDateAndObjectTypeAndStartTimeStampAndID' ); //Sort UDT records so compaction can be done optimally, then again each time a merge occurs.
				foreach ( $this->user_date_total as $udt_key => $udt_obj ) { /* @var UserDateTotalFactory $udt_obj */
					//Debug::text( 'UDT X: '. $x .' Date Stamp: ' . TTDate::getDate( 'DATE', $udt_obj->getDateStamp() ) . ' Pay Code ID: ' . $udt_obj->getPayCode() . ' Object Type: ' . $udt_obj->getObjectType() . ' Total Time: ' . $udt_obj->getTotalTime() . ' Override: ' . (int)$udt_obj->getOverride(), __FILE__, __LINE__, __METHOD__, 10 );
					if ( is_object( $prev_udt_obj ) ) {
						//Match current UDT with previous UDT record to see if they match. This only works well if the sorting is exactly right (based on Object Type and StartTimestamp)
						//Debug::text( '  Current UDT: X: '. $x .' Key: ' . $udt_key . ' Punch Control ID: ' . $udt_obj->getPunchControlID() . '  Object Type: ' . $udt_obj->getObjectType() . ' Start Time: ' . TTDate::getDate( 'DATE+TIME', $udt_obj->getStartTimeStamp() ) . ' End Time: ' . TTDate::getDate( 'DATE+TIME', $udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );
						//Debug::text( '  Prev UDT: X: '. $x .' Key: ' . $prev_udt_key . ' Punch Control ID: ' . $prev_udt_obj->getPunchControlID() . ' Object Type: ' . $prev_udt_obj->getObjectType() . ' Start Time: ' . TTDate::getDate( 'DATE+TIME', $prev_udt_obj->getStartTimeStamp() ) . ' End Time: ' . TTDate::getDate( 'DATE+TIME', $prev_udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );

						if (
								$udt_obj->getObjectType() == $prev_udt_obj->getObjectType()
								&& $udt_obj->getObjectType() != 5 && $udt_obj->getObjectType() != 10 //Exclude 5=System, 10=Worked
								&& $udt_obj->getPayCode() == $prev_udt_obj->getPayCode()
								&& $udt_obj->getDateStamp() == $prev_udt_obj->getDateStamp()
								&& $udt_obj->getStartTimeStamp() == $prev_udt_obj->getEndTimeStamp() //No gap between previous one ending and next one starting.
								&& $udt_obj->getPunchControlID() == $prev_udt_obj->getPunchControlID() //Only ObjectType=10 would normally have punch_control_ids, so this check isn't really useful, as punch_control_ids will always be false.
								&& $udt_obj->getHourlyRate() == $prev_udt_obj->getHourlyRate()
								&& $udt_obj->getTotalTime() > 0 && $prev_udt_obj->getTotalTime() > 0 //Ignore auto-deduct meals/breaks with negative total time, as the start/end dates need to be handled in a special way.
								&& $udt_obj->getOverride() == false && $prev_udt_obj->getOverride() == false
								&& $udt_obj->getBranch() == $prev_udt_obj->getBranch()
								&& $udt_obj->getDepartment() == $prev_udt_obj->getDepartment()
								&& $udt_obj->getJob() == $prev_udt_obj->getJob()
								&& $udt_obj->getJobItem() == $prev_udt_obj->getJobItem()
								&& !( array_diff( $udt_obj->getPunchTag(), $prev_udt_obj->getPunchTag() ) || array_diff( $prev_udt_obj->getPunchTag(), $udt_obj->getPunchTag() ) ) //array_diff() just checks one direction, so need to swap a & b to check both directions.
						) {
							//Merge current UDT record into previous one.
							if ( $udt_obj->getEndTimeStamp() != '' && $prev_udt_obj->getEndTimeStamp() != '' ) {
								...
								//Handle cases where no start/end times exist. Such as two holiday policies on the same day with no schedule (so no start/end times exist)
								$prev_udt_obj->setTotalTime( $udt_obj->getTotalTime() + $prev_udt_obj->getTotalTime() );
							}
							//$prev_udt_obj->setQuantity( ( $prev_udt_obj->getQuantity() + $udt_obj->getQuantity() ) ); //Quantities aren't currently divided up with UDT records are split.
							//$prev_udt_obj->setBadQuantity( ( $prev_udt_obj->getBadQuantity() + $udt_obj->getBadQuantity() ) );

							if ( $udt_obj->getNote() != '' ) { //Append note if it exists on the record about to be merged.
								$prev_udt_obj->setNote( $prev_udt_obj->getNote() . "\n" . $udt_obj->getNote() );
							}

							Debug::text( '    Found UDT object to compact... Merging X: '. $x .' Key: ' . $udt_key . ' into: ' . $prev_udt_key . ' After UDT: Start Time: ' . TTDate::getDate( 'DATE+TIME', $prev_udt_obj->getStartTimeStamp() ) . ' End Time: ' . TTDate::getDate( 'DATE+TIME', $prev_udt_obj->getEndTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10 );

							$prev_udt_obj->setEnableCalcSystemTotalTime( false );
							if ( $prev_udt_obj->isValid() ) {
								$prev_udt_obj->calcTotalTimeAmounts(); //Call this so TotalTime, TotalTimeAmount is calculated immediately, as we don't save these records until later.
							}

							unset( $this->user_date_total[$udt_key] ); //Remove current UDT record now that it has been merged.
					...
					$prev_udt_obj = $udt_obj; /* @var UserDateTotalFactory $prev_udt_obj */
			...
			$this->sortUserDateTotalData( $this->user_date_total ); //Sort UDT records back to expected order for calculating all policies once done compacting them.
		}
	...
	function calculateBreakTimePolicy( $date_stamp, $force_proportional_distribution = null ) {
		...
		$udtlf = $this->filterUserDateTotalDataByPunchTypeIDs( $date_stamp, $date_stamp, [ 30 ] ); //Break rows only.
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			...
			foreach ( $udtlf as $udt_obj ) { /** @var UserDateTotalFactory $udt_obj */
				if ( $out_for_break == false && $udt_obj->getEndType() == 30 ) {
					$break_out_timestamp = $udt_obj->getEndTimeStamp();
					$out_for_break = true;
				} else if ( $out_for_break == true && $udt_obj->getStartType() == 30 ) {
					$break_punch_arr[$pair][20] = $break_out_timestamp;
					$break_punch_arr[$pair][10] = $udt_obj->getStartTimeStamp();

					$out_for_break = false;
					$pair++;
					unset( $break_out_timestamp );

					if ( $udt_obj->getStartType() == 30 && $udt_obj->getEndType() == 30 ) {
						$break_out_timestamp = $udt_obj->getEndTimeStamp();
						$out_for_break = true;
					}
				} else {
			...
			if ( isset( $break_punch_arr ) ) {
				...
				foreach ( $break_punch_arr as $time_stamp_arr ) {
					if ( isset( $time_stamp_arr[10] ) && !empty( $time_stamp_arr[10] ) && isset( $time_stamp_arr[20] ) && !empty( $time_stamp_arr[20] ) ) {
						if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
							$udtf = TTnew( 'UserDateTotalFactory' ); /** @var UserDateTotalFactory $udtf */
							$udtf->setUser( $this->getUserObject()->getId() );
							$udtf->setPayPeriod( ( is_object( $this->pay_period_obj ) ? $this->pay_period_obj->getId() : false ) ); //Optimization so setDateStamp() doesn't need to lookup the pay period.
							...
							$udtf->setPunchTag( $this->getUserObject()->getDefaultPunchTag() );
							...
							$udtf->setTotalTime( TTMath::sub( $time_stamp_arr[10], $time_stamp_arr[20] ) );
							...
							Debug::text( '   Adding UDT row for Break (Taken) Total Time: ' . $udtf->getTotalTime() .' UDT Total Time: '. $udtf->getTotalTime() .' Break Out: '. TTDate::getDate('DATE+TIME', $time_stamp_arr[20] ) .' In: '. TTDate::getDate('DATE+TIME', $time_stamp_arr[10] ), __FILE__, __LINE__, __METHOD__, 10 );
							$this->user_date_total_insert_id--;
						} else {
							Debug::text( 'ERROR: Duplicate starting ID for some reason! ' . $this->user_date_total_insert_id, __FILE__, __LINE__, __METHOD__, 10 );
						}

						$break_overall_total_time = TTMath::add( $break_overall_total_time, TTMath::sub( $time_stamp_arr[10], $time_stamp_arr[20] ) );
						$break_total_time_arr[] = TTMath::sub( $time_stamp_arr[10], $time_stamp_arr[20] );
					} else {
						Debug::text( ' Break Punches not paired, or invalid timestamps... Skipping!', __FILE__, __LINE__, __METHOD__, 10 );
					}
				}

				//Get the last punch in timestamp so we start auto-add/auto-deduct timestamps from this.
				//   This is handled below by findPunchDataGapAfterTimeStamp() now instead.
		...
		unset( $udtlf, $pair, $x, $out_for_break, $break_out_timestamp, $break_punch_arr, $break_pair_total_time, $time_stamp_arr );
		...
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			...
			$punch_data = $this->filterPunchDataByDateAndTypeAndStatus( $date_stamp );

			//Loop over all break policies calculating the pay codes, up until $maximum_time is reached.
			...
			if ( $day_total_time > 0 && is_array( $break_time_policies ) && count( $break_time_policies ) > 0 ) {
				...
				foreach ( $break_time_policies as $bp_obj ) {
					...
					switch ( $bp_obj->getType() ) {
						case 10: //Auto-Deduct
							...
							if ( $bp_obj->getIncludeBreakPunchTime() == true ) {
								$break_policy_time = ( TTMath::sub( $bp_obj->getAmount(), $eligible_break_total_time ) * -1 );
					...
					if ( $break_policy_time != 0 ) {
						$break_policy_total_time = TTMath::add( $break_policy_total_time, $break_policy_time );
						$worked_time_break_policy_adjustments = [];
						...
						if ( $day_total_time > 0 && is_array( $udtlf ) && count( $udtlf ) > 0 ) {
							...
							switch ( $allocation_type_id ) {
								case 10: //Proportional Distribution
									...
									foreach ( $udtlf as $udt_obj ) {
										...
										$udt_raw_break_policy_time = TTMath::mul( TTMath::div( $total_time, $day_total_time ), $break_policy_time );
										if ( $break_policy_time > 0 ) {
											$rounded_udt_raw_break_policy_time = floor( $udt_raw_break_policy_time );
											$remainder = TTMath::add( $remainder, TTMath::sub( $udt_raw_break_policy_time, $rounded_udt_raw_break_policy_time ) );
										} else {
								...
								case 100: //At Active After Time
									...
									foreach ( $udtlf as $udt_obj ) {
										...
										if ( $found_first_udt == true
												|| ( $last_punch_in_timestamp >= $udt_obj->getStartTimeStamp() && $last_punch_in_timestamp <= $udt_obj->getEndTimeStamp() )
												|| ( $last_punch_in_timestamp >= $punch_data[array_key_last($punch_data)]->getTimeStamp() ) //After last out punch.
										) {
											...
											$found_first_udt = true; //Must go below IF check above, otherwise it would always evaluate to true.
										}
							...
							Debug::text( '  Skipping break adjustment calculation due to day_total_time=0 or no UDT records... Day Total Time: '. $day_total_time, __FILE__, __LINE__, __METHOD__, 10 );
						}
						...
						foreach ( $worked_time_break_policy_adjustments as $udt_id => $worked_time_break_policy_adjustment ) {
							if ( isset( $this->user_date_total[$udt_id] ) ) {
								if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
									$tmp_worked_time_break_policy_adjustment = $worked_time_break_policy_adjustment;

									$g = 0;
									while( abs( $tmp_worked_time_break_policy_adjustment ) > 0 && $g < 10 ) {
										$find_gap_data = $this->findPunchDataGapAfterTimeStamp( $punch_data, $last_punch_in_timestamp, ( ( $bp_obj->getType() == 15 ) ? 20 : 10 ), $tmp_worked_time_break_policy_adjustment ); //Find gaps to fill.
										...
										$udtf->setGenericTempDataValue( 'source_object_allocation_type_id', $allocation_type_id ); //So we can easily test against this in compactMealAndBreakUserDateTotalObjects()
										...
										$udtf->setPunchTag( $this->user_date_total[$udt_id]->getPunchTag() );
										...
										if ( $bp_obj->getType() == 15 ) { //Auto-Add
											if ( $last_punch_in_timestamp != '' ) { //If the first punch is a Break IN and only a Normal OUT, $last_punch_in_timestamp will be NULL.
												$udtf->setStartTimeStamp( $find_gap_data['start'] );
												$udtf->setEndTimeStamp( ( $find_gap_data['start'] + abs( $find_gap_data['total_time'] ) ) );
												$last_punch_in_timestamp = $udtf->getEndTimeStamp();
											}
										} else { //Auto-Deduct
											if ( $last_punch_in_timestamp != '' ) { //If the first punch is a Break IN and only a Normal OUT, $last_punch_in_timestamp will be NULL.
												$start_time_stamp = $find_gap_data['start'];
												$end_time_stamp = ( $find_gap_data['start'] + abs( $find_gap_data['total_time'] ) );

												//Detect if we are near the DST transition, and move the lunch to whichever side is closest (earlier or later) to avoid falling in the DST transition period.
												$dst_offset = TTDate::isTimeOverlapsDSTTransition( $start_time_stamp, $end_time_stamp, true );

												$udtf->setStartTimeStamp( ( $start_time_stamp + $dst_offset ) );
												$udtf->setEndTimeStamp( ( $end_time_stamp + $dst_offset ) );

												unset( $start_time_stamp, $end_time_stamp, $dst_offset );

												$last_punch_in_timestamp = $udtf->getEndTimeStamp();
											}
										}
										//Debug::Text( 'G: '. $g . ' Break Policy: Start: '. TTDate::getDate('DATE+TIME', $udtf->getStartTimeStamp() ) .' End: '. TTDate::getDate('DATE+TIME', $udtf->getEndTimeStamp() ) .' Total Time: '. $find_gap_data['total_time'] .' TMP: Time To Use: '. $tmp_worked_time_break_policy_adjustment .' Actual Time To Use: '. $worked_time_break_policy_adjustment .' Remaining Break Time: '. $remaining_break_time, __FILE__, __LINE__, __METHOD__, 10 );

										$udtf->setQuantity( 0 );
										$udtf->setBadQuantity( 0 );
										$udtf->setTotalTime( $find_gap_data['total_time'] );
										...
										$tmp_worked_time_break_policy_adjustment -= $find_gap_data['total_time'];
										$g++;
									}
								} else {
									Debug::text( 'ERROR: Duplicate starting ID for some reason! ' . $this->user_date_total_insert_id, __FILE__, __LINE__, __METHOD__, 10 );
		...
		if ( ( $daily_total_time > 0 || $always_return_at_least_one == true )
			...
			if ( is_array( $schedule_policy_arr ) ) {
				foreach ( $schedule_policy_arr as $sp_obj ) {
					$schedule_policy_break_policies = $sp_obj->getBreakPolicy(); //Calling getBreakPolicy() triggers a SQL query to the child table, this works as an optimization.
					if ( is_array( $schedule_policy_break_policies ) && count( $schedule_policy_break_policies ) > 0 ) {
						$schedule_policy_break_time_policy_ids = array_merge( $schedule_policy_break_time_policy_ids, $schedule_policy_break_policies );
					}
				}
			...
			unset( $schedule_policy_arr, $schedule_policy_break_policies );
			...
			if ( is_array( $bplf ) && count( $bplf ) > 0 ) {
				...
				if ( isset( $retarr ) ) {
					...
					Debug::text( 'Forced to always return at least one... ID: '. $bp_obj->getID(), __FILE__, __LINE__, __METHOD__, 10 );
	...
	function getBreakTimePolicy() {
		...
		if ( is_array( $splf ) && count( $splf ) > 0 ) {
			foreach ( $splf as $sp_obj ) {
				$schedule_policy_break_policies = $sp_obj->getBreakPolicy(); //Calling getBreakPolicy() triggers a SQL query to the child table, this works as an optimization.
				if ( is_array( $schedule_policy_break_policies ) && count( $schedule_policy_break_policies ) > 0 ) {
					$schedule_break_time_policy_ids = array_merge( $schedule_break_time_policy_ids, $schedule_policy_break_policies );
				}
			}
			unset( $sp_obj, $schedule_policy_break_policies );
		}

		$bplf = TTnew( 'BreakPolicyListFactory' ); /** @var BreakPolicyListFactory $bplf */
		$bplf->getByPolicyGroupUserIdOrId( $this->getUserObject()->getId(), array_unique( $schedule_break_time_policy_ids ) );
	...
	function calculateMealTimePolicy( $date_stamp, $force_proportional_distribution = null ) {
		...
		$udtlf = $this->filterUserDateTotalDataByPunchTypeIDs( $date_stamp, $date_stamp, [ 20 ] ); //Lunch rows only.
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			$pair = 0;
			$x = 0;
			$out_for_lunch = false;
			...
			foreach ( $udtlf as $udt_obj ) { /** @var UserDateTotalFactory $udt_obj */
				if ( $out_for_lunch == false && $udt_obj->getEndType() == 20 ) {
					$lunch_out_timestamp = $udt_obj->getEndTimeStamp();
					$out_for_lunch = true;
				} else if ( $out_for_lunch == true && $udt_obj->getStartType() == 20 ) {
					$lunch_punch_arr[$pair][20] = $lunch_out_timestamp;
					$lunch_punch_arr[$pair][10] = $udt_obj->getStartTimeStamp();
					$out_for_lunch = false;
					$pair++;
					unset( $lunch_out_timestamp );

					if ( $udt_obj->getStartType() == 20 && $udt_obj->getEndType() == 20 ) {
						$lunch_out_timestamp = $udt_obj->getEndTimeStamp();
						$out_for_lunch = true;
					}
				} else {
			...
			if ( isset( $lunch_punch_arr ) ) {
				foreach ( $lunch_punch_arr as $time_stamp_arr ) {
					if ( isset( $time_stamp_arr[10] ) && !empty( $time_stamp_arr[10] ) && isset( $time_stamp_arr[20] ) && !empty( $time_stamp_arr[20] ) ) {
						...
						if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
							...
							Debug::text( '   Adding UDT row for Meal (Taken) Total Time: ' . $lunch_total_time .' UDT Total Time: '. $udtf->getTotalTime() .' Lunch Out: '. TTDate::getDate('DATE+TIME', $time_stamp_arr[20] ) .' In: '. TTDate::getDate('DATE+TIME', $time_stamp_arr[10] ), __FILE__, __LINE__, __METHOD__, 10 );
							$this->user_date_total_insert_id--;
						...
						Debug::text( ' Lunch Punches not paired or invalid timestamps... Skipping!', __FILE__, __LINE__, __METHOD__, 10 );
					}
				}
		...
		unset( $udtlf, $udtf, $pair, $x, $out_for_lunch, $lunch_out_timestamp, $lunch_punch_arr, $lunch_pair_total_time, $time_stamp_arr );
		...
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			...
			//Loop over all meal policies calculating the pay codes, up until $maximum_time is reached.
			...
			if ( $day_total_time > 0 && is_array( $meal_time_policies ) && count( $meal_time_policies ) > 0 ) {
				...
				foreach ( $meal_time_policies as $mp_obj ) {
					...
					if ( $last_punch_in_timestamp == false ) {
						//This is active after time from the first punch in the day, but if the employee has a split shift, or worked from 1A to 2A, then came in at 8A and worked to 5P.
						//   It would try to deduct the meal from a period when they employee wasn't working, and always fall back to proportional distribution.
						//if ( !isset( $shift_udt_objs ) ) {
						//	$shift_udt_objs = $this->getShiftStartAndEndUserDateTotal( $date_stamp, $date_stamp );
						//}
						//if ( is_array( $shift_udt_objs ) && isset( $shift_udt_objs['start'] ) ) {
						...
						//Get the time of day where the employee actually worked past the meal active after time (trigger time) -- ie: 6hrs.
						//  This will always return a time when the employee was actually working, so it properly skips unworked time in split shifts. See MealBreakPolicy_testAutoDeductMealPolicyAtActiveTimeAllocationE
						$active_after_udt_obj = $this->getUserDateTotalActiveAfter( $date_stamp, $date_stamp, $mp_obj->getTriggerTime(), $mp_obj->getAmount() );
						if ( isset( $active_after_udt_obj['start'] ) ) {
							$last_punch_in_timestamp = $active_after_udt_obj['start'];
						}
						unset( $active_after_udt_obj );
					}
					...
					switch ( $mp_obj->getType() ) {
						case 10: //Auto-Deduct
							...
							if ( $mp_obj->getIncludeLunchPunchTime() == true ) {
								$meal_policy_time = ( TTMath::sub( $mp_obj->getAmount(), $lunch_total_time ) * -1 );
					...
					if ( $meal_policy_time != 0 ) {
						$worked_time_meal_policy_adjustments = [];
						...
						if ( $day_total_time > 0 && is_array( $udtlf ) && count( $udtlf ) > 0 ) {
							...
							switch ( $allocation_type_id ) {
								case 10: //Proportional Distribution
									...
									foreach ( $udtlf as $udt_obj ) {
										...
										//Make sure we use bcmath() functions here to avoid floating point imprecision issues.
										$udt_raw_meal_policy_time = TTMath::mul( TTMath::div( $total_time, $day_total_time ), $meal_policy_time );
										if ( $meal_policy_time > 0 ) {
											$rounded_udt_raw_meal_policy_time = floor( $udt_raw_meal_policy_time );
											$remainder = TTMath::add( $remainder, TTMath::sub( $udt_raw_meal_policy_time, $rounded_udt_raw_meal_policy_time ) );
										} else {
								...
								case 100: //At Active After Time
									...
									foreach ( $udtlf as $udt_key ) {
										...
										//Get the next row object so we can see if this is a transfer punch.
										$next_udt_obj = null;
										if ( $udtlf->key() < ( $udtlf->count() - 1 ) ) { //Make sure we are not on the last row.
											$udtlf->next(); //Jump forward
											$next_key = $udtlf->current(); //Get current key
											$udtlf->seek( ( $udtlf->key() - 1 ) ); //Jump back to where we were so the foreach() can continue as normal.

											if ( isset( $this->user_date_total[$next_key] ) ) {
												$next_udt_obj = $this->user_date_total[$next_key];
											}
										}

										//Find the correct UDT record that spans the lunch start time.
										//
										//There can be cases where the employee punches in at 12AM -> 6:15AM, then 6PM to 6:30AM, and lunch falls after 6hrs (6AM -> 6:30AM).
										//  Its an impossible situation to deduct lunch from, but we have to do something, so fall back to proportional distribution. (or: Half of lunch needs to be deducted from 6AM -> 6:15AM, then the other half from 6PM to 6:15PM.)
										// So check to make sure the entire lunch can be deducted from the UDT record. Otherwise this can throw off overtime calculations.
										if ( $found_first_udt == true
												|| ( $last_punch_in_timestamp >= $udt_obj->getStartTimeStamp() && $last_punch_in_timestamp < $udt_obj->getEndTimeStamp() //2nd clause must be "<" rather than "<=", so punches like: 8A -> 12P, 12P -> 5P (lunch active after 4hrs), only match on the 2nd punch pair rather than the first. See: testAutoDeductMealPolicyAtActiveTimeAllocationD1()
														&& ( ( $udt_obj->getEndTimeStamp() - $last_punch_in_timestamp ) >= abs( $remaining_meal_policy_time ) || ( $udt_obj->getEndTimeStamp() == $last_punch_in_timestamp && abs( $udt_obj->getTotalTime() ) >= abs( $remaining_meal_policy_time ) ) || ( is_object( $next_udt_obj ) && $udt_obj->getEndTimeStamp() == $next_udt_obj->getStartTimeStamp() ) )
														) ) {
											...
											if ( $found_first_udt == true ) {
												...
												//Need to figure out if we use the start time stamp or end time stamp to adjust from
												if ( $udt_obj->getEndTimeStamp() != $last_punch_in_timestamp ) {
													$tmp_udt_total_time = ( $udt_obj->getEndTimeStamp() - $last_punch_in_timestamp ); //Total time from meal start time to end of UDT record.
												} else {
													$tmp_udt_total_time = $remaining_meal_policy_time;
												}
											}
							...
							Debug::text( '  Skipping meal adjustment calculation due to day_total_time=0 or no UDT records... Day Total Time: '. $day_total_time, __FILE__, __LINE__, __METHOD__, 10 );
						}
							...
							if ( isset( $this->user_date_total[$udt_id] ) ) {
								if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
									...
									if ( $mp_obj->getType() == 15 ) { //Auto-Add
										...
										if ( $last_punch_in_timestamp != '' ) { //If the first punch is a Lunch IN and only a Normal OUT, $last_punch_in_timestamp will be NULL.
											//When auto-deducting lunch, the start
											$start_time_stamp = $last_punch_in_timestamp;
											$end_time_stamp = ( $last_punch_in_timestamp + abs( $worked_time_meal_policy_adjustment ) );
	...
	function filterMealTimePolicy( $date_stamp, $daily_total_time = null, $type_id = null, $always_return_at_least_one = false ) {
		if ( ( $daily_total_time > 0 || $always_return_at_least_one == true )
			...
			if ( is_array( $schedule_policy_arr ) ) {
				foreach ( $schedule_policy_arr as $sp_obj ) {
					$schedule_policy_meal_policies = $sp_obj->getMealPolicy(); //Calling getMealPolicy() triggers a SQL query to the child table, this works as an optimization.
					if ( is_array( $schedule_policy_meal_policies ) && count( $schedule_policy_meal_policies ) > 0 ) {
						$schedule_policy_meal_time_policy_ids = array_merge( $schedule_policy_meal_time_policy_ids, $schedule_policy_meal_policies );
					}
				}
			...
			unset( $schedule_policy_arr, $schedule_policy_meal_policies );
			...
			if ( is_array( $mplf ) && count( $mplf ) > 0 ) {
				...
				if ( isset( $retarr ) ) {
					...
					Debug::text( 'Forced to always return at least one... ID: '. $mp_obj->getID(), __FILE__, __LINE__, __METHOD__, 10 );
	...
	function getMealTimePolicy() {
		...
		if ( is_array( $splf ) && count( $splf ) > 0 ) {
			...
			foreach ( $splf as $sp_obj ) {
				$schedule_policy_meal_policies = $sp_obj->getMealPolicy(); //Calling getMealPolicy() triggers a SQL query to the child table, this works as an optimization.
				if ( is_array( $schedule_policy_meal_policies ) && count( $schedule_policy_meal_policies ) > 0 ) {
					$schedule_meal_time_policy_ids = array_merge( $schedule_meal_time_policy_ids, $schedule_policy_meal_policies );
				}
			}
			unset( $sp_obj, $schedule_policy_meal_policies );
		}

		$mplf = TTnew( 'MealPolicyListFactory' ); /** @var MealPolicyListFactory $mplf */
		$mplf->getByPolicyGroupUserIdOrId( $this->getUserObject()->getId(), array_unique( $schedule_meal_time_policy_ids ) );
	...
	function calculateAbsenceTimePolicy( $date_stamp ) {
		...
		if ( is_array( $user_date_total_rows ) ) {
			foreach ( $user_date_total_rows as $udt_key => $udt_obj ) {
				...
				if ( !isset( $this->absence_policy[$udt_obj->getSourceObject()] ) ) {
					$this->absence_policy[$udt_obj->getSourceObject()] = $udt_obj->getSourceObjectObject(); //Cache the absence policy objects so they can be used in calculateAverageHourlyRates
				}

				$ap_obj = $this->absence_policy[$udt_obj->getSourceObject()];
				...
				if ( is_object( $ap_obj ) && !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
					...
					$udtf->setPunchTag( $udt_obj->getPunchTag() );
					...
					if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) {
						...
						//Check to see if punches exist on this day, then see if they overlap the absences.
						if ( is_array( $this->punch ) && count( $this->punch ) > 0 ) {
							//#2799 - In cases where an absence is from 8A-1:45P, and the employee works from 8A-1:45P as well.
							//    Since the above overlap, OT >8hrs/day is not calculating properly.
							$udt_time_stamps = $this->calculateUserDateTotalStartAndEndTimeStamps( $udt_obj );

							//See comment above about overlapping punches and absences. Therefore try to use $udt_time_stamps if they exist, otherwise fall back to $udt_obj timestamps.
							if ( isset( $udt_time_stamps['start_time_stamp'] ) && isset( $udt_time_stamps['end_time_stamp'] ) ) {
	...
	function getUnderTimeAbsenceTimePolicy() {
		...
		if ( is_array( $splf ) && count( $splf ) > 0 ) {
			Debug::text( 'Found schedule policy rows: ' . count( $splf ), __FILE__, __LINE__, __METHOD__, 10 );
			foreach ( $splf as $sp_obj ) {
				if ( $sp_obj->getFullShiftAbsencePolicyID() !== false && $sp_obj->getFullShiftAbsencePolicyID() != TTUUID::getNotExistID() && $sp_obj->getFullShiftAbsencePolicyID() != TTUUID::getZeroID() ) {
					$schedule_undertime_absence_policy_ids[] = $sp_obj->getFullShiftAbsencePolicyID();
				}

				if ( $sp_obj->getPartialShiftAbsencePolicyID() !== false && $sp_obj->getPartialShiftAbsencePolicyID() != TTUUID::getNotExistID() && $sp_obj->getPartialShiftAbsencePolicyID() != TTUUID::getZeroID() ) {
					$schedule_undertime_absence_policy_ids[] = $sp_obj->getPartialShiftAbsencePolicyID();
				}
			}
			unset( $sp_obj );
		}

		if ( count( $schedule_undertime_absence_policy_ids ) > 0 ) {
			$aplf = TTnew( 'AbsencePolicyListFactory' ); /** @var AbsencePolicyListFactory $aplf */
			$aplf->getByIdAndCompanyId( array_unique( $schedule_undertime_absence_policy_ids ), $this->getUserObject()->getCompany() );
			...
			foreach ( $this->regular_time_exclusivity_map as $exclusivity_data ) {
				foreach ( $exclusivity_data as $regular_udt_key => $reg_udt_key ) {
					...
					if ( isset( $this->user_date_total[$regular_udt_key] ) && isset( $this->user_date_total[$reg_udt_key] ) ) {
						...
						if ( $udt_obj->getObjectType() == 25 ) { //25=Absence
							...
							$udt_obj->calcTotalTimeAmounts(); //Calculate TotalTimeAmount.
		...
		if ( $udt_obj->getObjectType() == 50 ) {
			...
			if ( $this->prev_user_date_total_start_time_stamp == null ) {
				...
				if ( is_array( $plf ) && count( $plf ) > 0 ) {
					...
					//If no punches exist, and the UDT object comes from a schedule absence, use the UDT timestamps as they match the schedule already.
					// Otherwise, try to match the timestamps up with their first scheduled shift.
					if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) { //Absence likely came from a scheduled absence shift.
						$start_time_stamp = $udt_obj->getStartTimeStamp();
						$end_time_stamp = $udt_obj->getEndTimeStamp();
					} else {
						//Absence likely was manually added directly to timesheet, so it doesn't have start/end times. They could also have a working schedule shift that day too.
						...
						if ( is_array( $slf ) && count( $slf ) > 0 ) {
							...
							$start_time_stamp = TTDate::getBeginDayEpoch( $udt_obj->getDateStamp() ); //Fall back to starting at midnight.
							$end_time_stamp = ( $start_time_stamp + $udt_obj->getTotalTime() );
						}
					}
				}
			...
			//This has to be switched back to the start_time_stamp if the regular time policy gets skipped for any reason (ie: differential criteria)
			// Currently that is being done in calculateRegularTimePolicy()
			$this->old_prev_user_date_total_end_time_stamp = $this->prev_user_date_total_end_time_stamp;
	...
	function calculateRegularTimePolicy( $date_stamp, $maximum_daily_total_time = null ) {
		...
		//Loop over all regular time policies calculating the pay codes.
		...
		if ( is_array( $regular_time_policies ) && $total_regular_time_policies > 0 ) {
			...
			for ( $i = 0; $i <= $total_regular_time_policies; $i++ ) {
				...
				if ( $i == $total_regular_time_policies ) {
					...
					Debug::text( 'Reached last row, apply catch all: I: ' . $i . ' Total Reg Policies: ' . $total_regular_time_policies, __FILE__, __LINE__, __METHOD__, 10 );
					$rtp_obj = end( $regular_time_policies ); /** @var RegularTimePolicyFactory $rtp_obj */
					...
					$rtp_obj = current( $regular_time_policies ); /** @var RegularTimePolicyFactory $rtp_obj */
				...
				if ( is_array( $user_date_total_rows ) && count( $user_date_total_rows ) > 0 ) {
					foreach ( $user_date_total_rows as $udt_key => $udt_obj ) { /** @var UserDateTotalFactory $udt_obj */
						Debug::text( 'Regular Time Policy: ' . $rtp_obj->getName() . ' ID: ' . $udt_obj->getID() . ' Time: ' . $udt_obj->getTotalTime() . ' Pay Code: ' . $rtp_obj->getPayCode() . ' Quantity: ' . $udt_obj->getQuantity() . ' Bad Quantity: ' . $udt_obj->getBadQuantity() . ' Used Regular Time: ' . $covered_time, __FILE__, __LINE__, __METHOD__, 10 );

						//Figure out the timestamps early so we can determine if any timestamps overlap existing UDT records we have created.
						if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) {
							// Short circuit early if this UDT key has already been flagged as used and we are working on a meal/break record currently.
							//   Otherwise $udt_used_time_ranges are checked lower down.
							if ( isset( $udt_used_keys[$udt_key] ) && ( $udt_obj->getType() == 100 || $udt_obj->getType() == 110 ) ) {
								Debug::text( 'UDT row already used in another regular time policy! (a) ID: ' . $udt_key, __FILE__, __LINE__, __METHOD__, 10 );
								continue;
							}

							$udt_start_time_stamp = $udt_obj->getStartTimeStamp();
							$udt_end_time_stamp = $udt_obj->getEndTimeStamp();
						} else {
							//Since manual timesheet doesn't have known start/end time stamps, the $udt_used_time_ranges checked below isn't as helpful,
							//  have a less strict check on if the UDT record has been used already or not.
							if ( isset( $udt_used_keys[$udt_key] ) ) {
								Debug::text( 'UDT row already used in another regular time policy! (b) ID: ' . $udt_key, __FILE__, __LINE__, __METHOD__, 10 );
								continue;
							}

							//Manual timesheet entry, set bogus start/end times that don't overlap any punches that may exist.
							$udt_time_stamps = $this->calculateUserDateTotalStartAndEndTimeStamps( $udt_obj );
							$udt_start_time_stamp = $udt_time_stamps['start_time_stamp'];
							$udt_end_time_stamp = $udt_time_stamps['end_time_stamp'];
							unset( $udt_time_stamps );
							Debug::text( '        No Start/End TimeStamps specified, setting temporary ones: Start: ' . TTDate::getDate( 'DATE+TIME', $udt_start_time_stamp ) . ' End: ' . TTDate::getDate( 'DATE+TIME', $udt_end_time_stamp ), __FILE__, __LINE__, __METHOD__, 10 );
						}

						//Loop through all ranges to see if there is any overlap based on timestamps.
						//  We do check based on UDT key, but there are cases when a UDT record needs to be split for meal/breaks, which creates a temporary UDT key that is marked as used.
						//  But then for another regular time policy that pulls in the same UDT, it can get split as well as a new key and duplicating the temporary UDT record and doubling up on regular time.
						//There is also cases where a contributing shift policy may create temporary UDT records that are split at midnight, so checking based on used UDT key is not enough, and we have to check based on time range used instead.
						foreach( $udt_used_time_ranges as $udt_used_time_range ) {
							//Ignore Auto-Add Meal/Break UDT records, as those will always overlap.
							if ( $udt_obj->getOverride() == false && $udt_obj->getType() != 100 && $udt_obj->getType() != 110 && TTDate::isTimeOverLapStandard( $udt_start_time_stamp, $udt_end_time_stamp, $udt_used_time_range['start'], $udt_used_time_range['end'], false ) == true ) {
								//Debug::text( 'UDT row already used in another regular time policy time range! ID: ' . $udt_key .' Type: '. $udt_obj->getType() .' Current: '. TTDate::getDate('DATE+TIME', $udt_start_time_stamp ) .' - '. TTDate::getDate('DATE+TIME', $udt_end_time_stamp ) .' Range: '. TTDate::getDate('DATE+TIME', $udt_used_time_range['start'] ) .' - '. TTDate::getDate('DATE+TIME', $udt_used_time_range['end'] ), __FILE__, __LINE__, __METHOD__, 10 );
								continue 2; //Continue out of upper foreach($user_date_total_rows) loop, as we already used this UDT time range.
							}
						}

						if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
							$create_udt_record = false;
							if ( ( $rtp_obj->getBranchSelectionType() == 10 && $rtp_obj->getExcludeDefaultBranch() == false ) || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getBranchSelectionType(), $rtp_obj->getExcludeDefaultBranch(), $udt_obj->getBranch(), $rtp_obj->getBranch(), $this->getUserObject()->getDefaultBranch() ) ) {
								...
								if ( ( $rtp_obj->getDepartmentSelectionType() == 10 && $rtp_obj->getExcludeDefaultDepartment() == false ) || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getDepartmentSelectionType(), $rtp_obj->getExcludeDefaultDepartment(), $udt_obj->getDepartment(), $rtp_obj->getDepartment(), $this->getUserObject()->getDefaultDepartment() ) ) {
									...
									if ( $rtp_obj->getJobGroupSelectionType() == 10 || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getJobGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobObject() ) ) ? $udt_obj->getJobObject()->getGroup() : null ), $rtp_obj->getJobGroup() ) ) {
										...
										if ( ( $rtp_obj->getJobSelectionType() == 10 && $rtp_obj->getExcludeDefaultJob() == false ) || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getJobSelectionType(), $rtp_obj->getExcludeDefaultJob(), $udt_obj->getJob(), $rtp_obj->getJob(), $this->getUserObject()->getDefaultJob() ) ) {
											//Debug::text(' Shift Differential... Meets Job Criteria! Select Type: '. $rtp_obj->getJobSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
											if ( $rtp_obj->getJobItemGroupSelectionType() == 10 || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getJobItemGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobItemObject() ) ) ? $udt_obj->getJobItemObject()->getGroup() : null ), $rtp_obj->getJobItemGroup() ) ) {
												...
												if ( ( $rtp_obj->getJobItemSelectionType() == 10 && $rtp_obj->getExcludeDefaultJobItem() == false ) || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getJobItemSelectionType(), $rtp_obj->getExcludeDefaultJobItem(), $udt_obj->getJobItem(), $rtp_obj->getJobItem(), $this->getUserObject()->getDefaultJobItem() ) ) {
													...
													if ( $rtp_obj->getPunchTagGroupSelectionType() == 10 || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getPunchTagGroupSelectionType(), null, $udt_obj->getPunchTagGroupIds(), $rtp_obj->getPunchTagGroup() ) ) {
														//Debug::text(' Shift Differential... Meets Punch Tag Group Criteria! Select Type: '. $rtp_obj->getPunchTagGroupSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
														if ( ( $rtp_obj->getPunchTagSelectionType() == 10 && $rtp_obj->getExcludeDefaultPunchTag() == false ) || $this->contributing_shift_policy[$rtp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $rtp_obj->getPunchTagSelectionType(), $rtp_obj->getExcludeDefaultPunchTag(), $udt_obj->getPunchTag(), $rtp_obj->getPunchTag(), $this->getUserObject()->getDefaultPunchTag() ) ) {
															//Debug::text(' Shift Differential... Meets Punch Tag Criteria! Select Type: '. $rtp_obj->getPunchTagSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
															if ( $rtp_obj->getContributingPayCodePolicy() == TTUUID::getNotExistID() || ( isset( $this->contributing_pay_codes_by_policy_id[$rtp_obj->getContributingPayCodePolicy()] ) && in_array( $udt_obj->getPayCode(), $this->contributing_pay_codes_by_policy_id[$rtp_obj->getContributingPayCodePolicy()] ) ) ) {
																//Debug::text(' Shift Differential... Meets Contributing Pay Code Policy Criteria! Pay Code Policy: '. $otp_obj->getContributingPayCodePolicy(), __FILE__, __LINE__, __METHOD__, 10);
						...
						if ( $create_udt_record == true ) {
							...
							if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
								...
								$udtf->setStartType( $udt_obj->getStartType() );
								$udtf->setStartTimeStamp( $udt_start_time_stamp );
								$udtf->setEndType( $udt_obj->getEndType() );
								$udtf->setEndTimeStamp( $udt_end_time_stamp );
								...
								$udtf->setHourlyRate( $this->getHourlyRate( $rtp_obj->getPayFormulaPolicy(), $udtf->getPayCode(), $date_stamp, $udtf->getBaseHourlyRate(), $udtf ) );
								...
								if ( $this->isOverriddenUserDateTotalObject( $udtf ) == false ) {
									...
									if ( $udt_obj->getGenericTempDataValue('split_from_source_key') != '' ) {
										$tmp_udt_reduction_key = $udt_obj->getGenericTempDataValue('split_from_source_key');
									} else {
										$tmp_udt_reduction_key = $udt_key;
									}

									$this->regular_time_exclusivity_map[] = [ $tmp_udt_reduction_key => $this->user_date_total_insert_id ];
									Debug::text( '        Queuing reduction of Absence UDT Key: ' . $tmp_udt_reduction_key . '(' . $this->user_date_total_insert_id . ') from: ' . $udt_obj->getTotalTime() . ' to: ' . ( $udt_obj->getTotalTime() - $total_time ) . ' Total: ' . $udtf->getTotalTime(), __FILE__, __LINE__, __METHOD__, 10 );
...
//									}
								...
								$udt_used_keys[$udt_key] = true; //Always run this to prevent getting to the catch-all when override records exist.

								//Record the range that we have already covered so we don't duplicate regular time. This must not insert based on UDT key, as the same UDT key could get split multiple times.
								$udt_used_time_ranges[] = [ 'start' => $udtf->getStartTimeStamp(), 'end' => $udtf->getEndTimeStamp(), 'key' => $udt_key ];
								//Debug::text( '      Recording Used UDT Key: '. $udt_key .' Start: '. TTDate::getDate('DATE+TIME', $udtf->getStartTimeStamp() ) .' End: '. TTDate::getDate('DATE+TIME', $udtf->getEndTimeStamp() ) .' Total Time: '. $udtf->getTotalTime(), __FILE__, __LINE__, __METHOD__, 10 );
							} else {
								Debug::text( 'ERROR: Duplicate starting ID for some reason! ' . $this->user_date_total_insert_id, __FILE__, __LINE__, __METHOD__, 10 );
							}
							...
							//If the regular time policy is skipped due to differential criteria, switch this back to the old value before this policy started calculation, so it doesn't get advanced into the following days if many regular time policies exist but are skipped.
							$this->prev_user_date_total_end_time_stamp = $this->old_prev_user_date_total_end_time_stamp;
						}
					}
				}
	...
	function getRegularTimePolicy() {
		...
		if ( is_array( $splf ) && count( $splf ) > 0 ) {
			foreach ( $splf as $sp_obj ) {
				...
				if ( is_array( $sp_obj->getIncludeRegularTimePolicy() ) && count( $sp_obj->getIncludeRegularTimePolicy() ) > 0 ) {
					$schedule_regular_time_policy_ids = array_merge( $schedule_regular_time_policy_ids, (array)$sp_obj->getIncludeRegularTimePolicy() );
				}
			}
			unset( $sp_obj );
		}
		...
		$rtplf->getByPolicyGroupUserIdOrId( $this->getUserObject()->getId(), array_unique( $schedule_regular_time_policy_ids ) );
	 ...
	 * @return bool
	 */
	function calculateAverageHourlyRates( $user_date_total_records ) {
		if ( is_array( $user_date_total_records ) && count( $user_date_total_records ) > 0 ) {
			Debug::text( 'Calculating Average Hourly Rates...', __FILE__, __LINE__, __METHOD__, 10 );
			foreach ( $user_date_total_records as $key => $udt_obj ) {
				if ( is_numeric( $key ) && $key < 0 && $udt_obj->getTotalTime() > 0 && ( $udt_obj->getObjectType() == 20 || $udt_obj->getObjectType() == 25 || $udt_obj->getObjectType() == 30 || $udt_obj->getObjectType() == 40 ) ) {
					//Debug::text('  Calculating UserDateTotal Entry!', __FILE__, __LINE__, __METHOD__, 10);

					$source_object_id = $udt_obj->getSourceObject();

					//Since absence policies are loaded ondemand in calculateAbsenceTimePolicy(), its possible they aren't loaded when this runs.
					//  So this ensures they do get loaded when necessary, but also utilizes any that may already be loaded.
					if ( $udt_obj->getObjectType() == 25 && !isset( $this->absence_policy[$source_object_id] ) ) { //25=Absence
						$this->absence_policy[$source_object_id] = $udt_obj->getSourceObjectObject();
					}

					//Only recalculate rates if we are actually using averaging. Otherwise we calculate them in calculateOverTime()/calculatePremiumTime instead.
					if ( ( $udt_obj->getObjectType() == 20 && isset( $this->regular_time_policy[$source_object_id] ) && $this->isPayFormulaPolicyAveraging( $this->regular_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode() ) == true ) ) {
						$udt_obj->setHourlyRate( $this->getHourlyRate( $this->regular_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $this->getBaseHourlyRate( $this->regular_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getBaseHourlyRate(), null, [ 20, 25, 100, 110 ], $udt_obj->getObjectType() ) ) );
						$udt_obj->setHourlyRateWithBurden( $this->getHourlyRateWithBurden( $this->regular_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getHourlyRate() ) );

						$udt_obj->setEnableCalcSystemTotalTime( false );
						$udt_obj->calcTotalTimeAmounts(); //Call this so TotalTimeAmount is calculated immediately, as we don't save these records until later.
					} else if ( ( $udt_obj->getObjectType() == 25 && isset( $this->absence_policy[$source_object_id] ) && $this->isPayFormulaPolicyAveraging( $this->absence_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode() ) == true ) ) {
						$udt_obj->setHourlyRate( $this->getHourlyRate( $this->absence_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $this->getBaseHourlyRate( $this->absence_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getBaseHourlyRate(), null, [ 20, 25, 100, 110 ], $udt_obj->getObjectType() ) ) );
						$udt_obj->setHourlyRateWithBurden( $this->getHourlyRateWithBurden( $this->absence_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getHourlyRate() ) );

						$udt_obj->setEnableCalcSystemTotalTime( false );
						$udt_obj->calcTotalTimeAmounts(); //Call this so TotalTimeAmount is calculated immediately, as we don't save these records until later.
					} else if ( ( $udt_obj->getObjectType() == 30 && isset( $this->over_time_policy[$source_object_id] ) && $this->isPayFormulaPolicyAveraging( $this->over_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode() ) == true ) ) {
						$udt_obj->setHourlyRate( $this->getHourlyRate( $this->over_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $this->getBaseHourlyRate( $this->over_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getBaseHourlyRate(), $this->contributing_shift_policy[$this->over_time_policy[$source_object_id]->getContributingShiftPolicy()], [ 20, 25, 100, 110 ], $udt_obj->getObjectType() ) ) );
						$udt_obj->setHourlyRateWithBurden( $this->getHourlyRateWithBurden( $this->over_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getHourlyRate() ) );

						$udt_obj->setEnableCalcSystemTotalTime( false );
						$udt_obj->calcTotalTimeAmounts(); //Call this so TotalTimeAmount is calculated immediately, as we don't save these records until later.
					} else if ( ( $udt_obj->getObjectType() == 40 && isset( $this->premium_time_policy[$source_object_id] ) && $this->isPayFormulaPolicyAveraging( $this->premium_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode() ) == true )) {
						//  We do need to calculate these for premium time, for cases like California where if an employee doesn't get a lunch break and gets paid 1hr at their weighted average rate including OT.
						$udt_obj->setHourlyRate( $this->getHourlyRate( $this->premium_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $this->getBaseHourlyRate( $this->premium_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getBaseHourlyRate(), $this->contributing_shift_policy[$this->premium_time_policy[$source_object_id]->getContributingShiftPolicy()], [ 20, 25, 30, 100, 110 ], $udt_obj->getObjectType() ) ) );
						$udt_obj->setHourlyRateWithBurden( $this->getHourlyRateWithBurden( $this->premium_time_policy[$source_object_id]->getPayFormulaPolicy(), $udt_obj->getPayCode(), $udt_obj->getDateStamp(), $udt_obj->getHourlyRate() ) );

						$udt_obj->setEnableCalcSystemTotalTime( false );
						$udt_obj->calcTotalTimeAmounts(); //Call this so TotalTimeAmount is calculated immediately, as we don't save these records until later.
					} else {
						Debug::text('  WARNING: Policy likely does not exist. ID: '. $source_object_id .' ObjectType: '. $udt_obj->getObjectType(), __FILE__, __LINE__, __METHOD__, 10);
					}
				}
				//else {
	 ...
	 * @return bool
	 */
	function calculateOverTimePolicyForTriggerTime( $date_stamp, $current_trigger_time_arr, $prev_policy_data, $previous_end_time_stamp ) {
		$retval = [ 'result' => false, 'end_time_stamp' => false ];
		if ( isset( $this->over_time_policy[$current_trigger_time_arr['over_time_policy_id']] ) ) {
			...
			$otp_obj = $this->over_time_policy[$current_trigger_time_arr['over_time_policy_id']]; /** @var OverTimePolicyFactory $otp_obj */
			...
			if ( is_array( $user_date_total_rows ) && count( $user_date_total_rows ) > 0 ) {
				...
				Debug::text( '  Total UDT Rows: ' . count( $user_date_total_rows ) .' Current Trigger TimeStamp: ' . TTDate::getDate( 'DATE+TIME', $current_trigger_time_arr['start_time_stamp'] ), __FILE__, __LINE__, __METHOD__, 10 );

				//Ensure that we never have OT policies overlap, so when one stops, the next can't start before it. See comments below where we set: $retval = [ 'result' => false, 'end_time_stamp' => $udtf->getEndTimeStamp() ];
				if ( $previous_end_time_stamp !== false && $current_trigger_time_arr['start_time_stamp'] < $previous_end_time_stamp ) {
					Debug::text( '  Start TimeStamp is less than previous end timestamp, bumping to previous. Previous End Time Stamp: '. TTDate::getDate( 'DATE+TIME', $previous_end_time_stamp ) .' Current Time Stamp: '. TTDate::getDate( 'DATE+TIME', $current_trigger_time_arr['start_time_stamp'] ) , __FILE__, __LINE__, __METHOD__, 10 );
					$current_trigger_time_arr['start_time_stamp'] = $previous_end_time_stamp;
				}
				...
				foreach ( $user_date_total_rows as $udt_key => $udt_obj ) {
					...
					if ( isset( $this->over_time_recurse_map[$udt_key] ) ) {
						...
						$prev_recurse_udt_key = $this->over_time_recurse_map[$udt_key]; //Keep track or the original UDT key, so we can add that to our $this->over_time_recurse_map map below.
						...
						//Debug::text('  Recursion Switch to: UDT Row: ID: '. $udt_obj->getId() .' Object Type ID: '. $udt_obj->getObjectType() .' Pay Code: '. $udt_obj->getPayCode() .' Start Time: '. TTDate::getDate('DATE+TIME', $udt_obj->getStartTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10);
					}
					...
					$hourly_rate = $this->getHourlyRate( $otp_obj->getPayFormulaPolicy(), $otp_obj->getPayCode(), $date_stamp, $base_hourly_rate, $udt_obj );
					...
					if (
						...
						if ( $udt_overlap_time > 0 ) {
							...
							if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
								$create_udt_record = false;
								if ( ( $otp_obj->getBranchSelectionType() == 10 && $otp_obj->getExcludeDefaultBranch() == false ) || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getBranchSelectionType(), $otp_obj->getExcludeDefaultBranch(), $udt_obj->getBranch(), $otp_obj->getBranch(), $this->getUserObject()->getDefaultBranch() ) ) {
									...
									if ( ( $otp_obj->getDepartmentSelectionType() == 10 && $otp_obj->getExcludeDefaultDepartment() == false ) || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getDepartmentSelectionType(), $otp_obj->getExcludeDefaultDepartment(), $udt_obj->getDepartment(), $otp_obj->getDepartment(), $this->getUserObject()->getDefaultDepartment() ) ) {
										...
										if ( $otp_obj->getJobGroupSelectionType() == 10 || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getJobGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobObject() ) ) ? $udt_obj->getJobObject()->getGroup() : null ), $otp_obj->getJobGroup() ) ) {
											...
											if ( ( $otp_obj->getJobSelectionType() == 10 && $otp_obj->getExcludeDefaultJob() == false ) || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getJobSelectionType(), $otp_obj->getExcludeDefaultJob(), $udt_obj->getJob(), $otp_obj->getJob(), $this->getUserObject()->getDefaultJob() ) ) {
												//Debug::text(' Shift Differential... Meets Job Criteria! Select Type: '. $otp_obj->getJobSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
												if ( $otp_obj->getJobItemGroupSelectionType() == 10 || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getJobItemGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobItemObject() ) ) ? $udt_obj->getJobItemObject()->getGroup() : null ), $otp_obj->getJobItemGroup() ) ) {
													...
													if ( ( $otp_obj->getJobItemSelectionType() == 10 && $otp_obj->getExcludeDefaultJobItem() == false ) || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getJobItemSelectionType(), $otp_obj->getExcludeDefaultJobItem(), $udt_obj->getJobItem(), $otp_obj->getJobItem(), $this->getUserObject()->getDefaultJobItem() ) ) {
														...
														if ( $otp_obj->getPunchTagGroupSelectionType() == 10 || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getPunchTagGroupSelectionType(), null, $udt_obj->getPunchTagGroupIds(), $otp_obj->getPunchTagGroup() ) ) {
															//Debug::text(' Shift Differential... Meets Punch Tag Group Criteria! Select Type: '. $otp_obj->getPunchTagGroupSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
															if ( ( $otp_obj->getPunchTagSelectionType() == 10 && $otp_obj->getExcludeDefaultPunchTag() == false ) || $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $otp_obj->getPunchTagSelectionType(), $otp_obj->getExcludeDefaultPunchTag(), $udt_obj->getPunchTag(), $otp_obj->getPunchTag(), $this->getUserObject()->getDefaultPunchTag() ) ) {
																//Debug::text(' Shift Differential... Meets Punch Tag Criteria! Select Type: '. $otp_obj->getPunchTagSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
																if ( $otp_obj->getContributingPayCodePolicy() == TTUUID::getNotExistID() || ( isset( $this->contributing_pay_codes_by_policy_id[$otp_obj->getContributingPayCodePolicy()] ) && in_array( $udt_obj->getPayCode(), $this->contributing_pay_codes_by_policy_id[$otp_obj->getContributingPayCodePolicy()] ) ) ) {
																	//Debug::text(' Shift Differential... Meets Contributing Pay Code Policy Criteria! Pay Code Policy: '. $otp_obj->getContributingPayCodePolicy(), __FILE__, __LINE__, __METHOD__, 10);
																	$create_udt_record = true;
																}
															}
							...
							if ( $create_udt_record == true ) {
								if ( !isset( $this->over_time_trigger_time_exclusivity_map[$current_trigger_time_arr_trigger_time] ) || ( isset( $this->over_time_trigger_time_exclusivity_map[$current_trigger_time_arr_trigger_time] ) && in_array( $udt_key, $this->over_time_trigger_time_exclusivity_map[$current_trigger_time_arr_trigger_time] ) == false ) ) {
									...
									$udt_total_time_percent = ( $udt_obj->getTotalTime() > 0 ) ? TTMath::div( $udt_overlap_time, $udt_obj->getTotalTime() ) : 1; //Make sure we avoid a division by 0 here, it may happen during drag&drop.
									$udt_quantity = round( TTMath::mul( $udt_obj->getQuantity(), $udt_total_time_percent ), 2 );
									$udt_bad_quantity = round( TTMath::mul( $udt_obj->getBadQuantity(), $udt_total_time_percent ), 2 );
									...
									if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
										...
										$udtf->setHourlyRateWithBurden( $this->getHourlyRateWithBurden( $otp_obj->getPayFormulaPolicy(), $udtf->getPayCode(), $date_stamp, $udtf->getHourlyRate(), $udtf ) );
										...
										if ( $this->isOverriddenUserDateTotalObject( $udtf ) == false ) {
											...
											if ( $udt_obj->getID() == '' && ( in_array( $udt_obj->getObjectType(), [ 20, 25, 30, 100, 110 ] ) ) ) { //This needs to include all object_type IDs that can be included into overtime.
												...
												//Since we are two or more levels deep in our recursion, make sure our flat $this->over_time_recurse_map contains a mapping across all levels, and doesn't just jump from the first to the last.
												if ( isset( $prev_recurse_udt_key ) ) {
													$this->over_time_recurse_map[$prev_recurse_udt_key] = $this->user_date_total_insert_id;
												}
...
//											}
											...
											//Since OT policies are supposed to act like ratchets, and once in OT, it should never leave, there is a scenario
											//   where contributing shifts are time restricted and have a start/end time, which can cause OT to stop prematurely.
											//If this is the case, we need to return from this function as FALSE, so we treat it more like a differential criteria
											//and continue processing other OT policies (ie: weekly >40) which should apply for any remaining time after the time restriction.
											//  See OverTimePolicyTest->testTimeRestrictedHolidayAndWeeklyOverTimePolicyA()
											if ( $this->contributing_shift_policy[$otp_obj->getContributingShiftPolicy()]->isTimeRestricted() == true ) {
												$retval = [ 'result' => false, 'end_time_stamp' => $udtf->getEndTimeStamp() ];
											} else {
												$retval['result'] = true;
											}
										}
	...
	function calculateOverTimePolicy( $date_stamp, $trigger_time_arr, $maximum_daily_total_time = null ) {
		...
		if ( $total_over_time_policies > 0 && is_array( $trigger_time_arr ) && count( $trigger_time_arr ) ) {
			...
			$prev_end_time_stamp = false;
			$tmp_prev_end_time_stamp = false;
			...
			foreach ( $trigger_time_arr_keys as $key => $trigger_time_arr_trigger_time ) {
				...
				foreach ( $trigger_time_arr[$current_trigger_time_arr_trigger_time] as $current_trigger_time_arr ) {
			...
			foreach ( $trigger_time_arr_keys as $key => $trigger_time_arr_trigger_time ) {
				...
				foreach ( $trigger_time_arr[$current_trigger_time_arr_trigger_time] as $current_trigger_time_arr ) {
					...
					if ( $prev_policy_data == false
						...
						[ 'result' => $calculate_retval, 'end_time_stamp' => $tmp_prev_end_time_stamp ] = $this->calculateOverTimePolicyForTriggerTime( $date_stamp, $current_trigger_time_arr, $prev_policy_data, $prev_end_time_stamp );
				...
				//Only when finished calculating all OT policies at the same trigger time do we update the prev_end_time_stamp.
				// This is required to handle multiple OT policies with different differential criteria, where they all need to be calculated based on the original prev_end_time_stamp.
				$prev_end_time_stamp = $tmp_prev_end_time_stamp;
			}
	...
	function getOverTimePolicy() {
		...
		$otplf->getByPolicyGroupUserIdOrId( $this->getUserObject()->getId(), array_unique( $schedule_over_time_policy_ids ) );
	...
	function isSecondBiWeeklyOverTimeWeek( $date_stamp, $first_pay_period_start_date, $start_week_day_id = 0 ) {
		...
		$weekly_period_diff = (int)( $days_diff / 7 );
	...
	function getOverTimeTriggerTimeAdjustAmount( $start_date, $end_date, $otp_obj ) {
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL
				&& TTUUID::isUUID( $otp_obj->getTriggerTimeAdjustContributingShiftPolicy() ) && $otp_obj->getTriggerTimeAdjustContributingShiftPolicy() != TTUUID::getZeroID() && $otp_obj->getTriggerTimeAdjustContributingShiftPolicy() != TTUUID::getNotExistID() && isset( $this->contributing_shift_policy[$otp_obj->getTriggerTimeAdjustContributingShiftPolicy()] ) ) {

			// Previously we would just recalculate the entire week if adjust_total_time was *not* zero.
			// However there can be cases where a holiday has 8hrs of absence time, but the employee works it and that causes the holiday time to go away because they don't get holiday time when they work that day,
			// and now the overtime needs to be recalculated for the entire week again, but since the holiday time was removed adjust_total_time will now be 0hrs instead of 8hrs and it wouldn't trigger the retro-recalculation.
			//
			// So we either need to recalculate the entire week whenever OT policies have time adjustment policies specified at all, or we need to some way of detecting this scenario to trigger the recalculation as well.
			//   We should be able to detect this scenario when time exists in the pay code that is assigned to the overtime policy, since that tells us that it was in use during the week already.
			...
			$overtime_udt_records_exist = false;
			if ( $adjust_total_time == 0 ) {
				$overtime_udt_records_exist = ( count( $this->filterUserDateTotalDataByPayCodeIDs( $start_date, $end_date, $otp_obj->getPayCode() ) ) > 0 ) ? true : false;
			}

			if ( $adjust_total_time != 0 || $overtime_udt_records_exist == true ) {
				$this->addPendingCalculationDate( $start_date, $end_date );
			}

			Debug::text( '  Original: ' . $otp_obj->getTriggerTime() . ' Adjust by Time: ' . $adjust_total_time .' UDT records assigned to OT Pay Code: '. (int)$overtime_udt_records_exist, __FILE__, __LINE__, __METHOD__, 10 );
	...
	function getOverTimeTriggerArray( $date_stamp ) {
		...
		if ( is_array( $over_time_policies ) ) {
				...
				switch ( $otp_obj->getType() ) {
					...
					case 30: //Bi-Weekly (Switch this to Every X Weeks code below once more testing is done)
						...
						if ( $is_second_week == true ) {
							...
							$second_week_start_date = TTDate::getBeginWeekEpoch( $date_stamp, $this->start_week_day_id );
						} else {
							$first_week_start_date = TTDate::getBeginWeekEpoch( $date_stamp, $this->start_week_day_id );
						...
						//Debug::text( '   First Week: Date: ' . TTDate::getDate( 'DATE+TIME', $date_stamp ) . ' Start: ' . TTDate::getDate( 'DATE', $first_week_start_date ) . ' End: ' . TTDate::getDate( 'DATE', $first_week_end_date ) .' Start DoW: '. $this->start_week_day_id, __FILE__, __LINE__, __METHOD__, 10 );
						//Debug::text( '   Second Week: Date: ' . TTDate::getDate( 'DATE+TIME', $date_stamp ) . ' Start: ' . TTDate::getDate( 'DATE', $second_week_start_date ) . ' End: ' . TTDate::getDate( 'DATE', $second_week_end_date ), __FILE__, __LINE__, __METHOD__, 10 );
						...
						if ( $is_second_week == true ) {
							...
							Debug::text( '  In 2nd Week: Calculating OT for 1st week: Date: ' . TTDate::getDate( 'DATE+TIME', $date_stamp ) . ' Start: ' . TTDate::getDate( 'DATE', $first_week_start_date ) . ' End: ' . TTDate::getDate( 'DATE', $first_week_end_date ), __FILE__, __LINE__, __METHOD__, 10 );
							...
							Debug::text( '  In 1st Week: Calculating OT for 2nd week: Date: ' . TTDate::getDate( 'DATE+TIME', $date_stamp ) . ' Start: ' . TTDate::getDate( 'DATE', $second_week_start_date ) . ' End: ' . TTDate::getDate( 'DATE', $second_week_end_date ), __FILE__, __LINE__, __METHOD__, 10 );

							$this->getRequiredData( $second_week_start_date, $second_week_end_date, false ); //Make sure we get required data in the 2nd week so getOverTimeTriggerTimeAdjustAmount() can use it.
					...
					case 100: //Sat
						...
						$dow = $type_to_dow_map[(int)$otp_obj->getType()];
						if ( date( 'w', $date_stamp ) == $dow ) {
							$trigger_time = $otp_obj->getTriggerTime();
							$trigger_time -= $this->getOverTimeTriggerTimeAdjustAmount( TTDate::getBeginDayEpoch( $date_stamp ), TTDate::getEndDayEpoch( $date_stamp ), $otp_obj );
							Debug::text( '   DayOfWeek OT for DOW: '. $dow .' ... Daily Trigger Time: ' . $trigger_time, __FILE__, __LINE__, __METHOD__, 10 );
						} else {
							Debug::text( '   NOT DayOfWeek OT for DOW: '. $dow .'...', __FILE__, __LINE__, __METHOD__, 10 );
					...
					case 210: //Over Schedule (Weekly) / No Schedule
						...
						//When using Over Schedule (Weekly), it only considers committed shifts, so the user could have exceeded their schedule on the first day of the week
						// and therefore receive overtime. Then on the next day they work under their scheduled time for the day, the previous day needs to be recalculated to
						// remove the overtime. So basically we always need to recalculate the entire week for every punch when using this policy.
						// Because there is also a case where a schedule shift may be deleted from the middle/end of the week, and that could potentially cause overtime to trigger the day before.
						$this->addPendingCalculationDate( TTDate::getBeginWeekEpoch( $date_stamp, $this->start_week_day_id ), TTDate::getEndWeekEpoch( $date_stamp, $this->start_week_day_id ) );
						break;
				}
			...
			if ( isset( $trigger_time_arr ) ) {
				Debug::Arr($trigger_time_arr, 'Trigger Time Array: Date: '. TTDate::getDate( 'DATE', $date_stamp ), __FILE__, __LINE__, __METHOD__, 10);
		...
		if ( is_array( $this->exception_policy ) ) {
			...
			foreach ( $this->exception_policy as $ep_obj ) {
				...
				switch ( strtolower( $ep_obj->getType() ) ) {
					case 's1':    //Unscheduled Absence... Anytime they are scheduled and have not punched in.
						$this->calculateExceptionS1( $plf, $slf, $current_epoch, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 's2': //Not Scheduled
						$this->calculateExceptionS2( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions, $scheduled_id_cache );
						break;
					case 's3': //In Early
						$this->calculateExceptionS3( $plf, $slf, $user_id, $ep_obj, $date_stamp, $type_id, $current_exceptions, $scheduled_id_cache );
						break;
					case 's4': //In Late
						$this->calculateExceptionS4( $plf, $slf, $user_id, $ep_obj, $date_stamp, $type_id, $current_exceptions, $current_epoch, $scheduled_id_cache );
						break;
					case 's5': //Out Early
						$this->calculateExceptionS5( $plf, $slf, $user_id, $ep_obj, $date_stamp, $type_id, $current_exceptions, $scheduled_id_cache );
						break;
					case 's6': //Out Late
						$this->calculateExceptionS6( $plf, $slf, $user_id, $ep_obj, $date_stamp, $type_id, $current_exceptions, $current_epoch, $premature_delay, $scheduled_id_cache );
						break;
					case 'sb': //Not Scheduled Branch/Department
					case 'sc': //Not Scheduled Job/Task
						$this->calculateExceptionSBAndSC( $plf, $slf, $user_id, $ep_obj, $date_stamp, $type_id, $current_exceptions, $scheduled_id_cache );
					break;
					case 'st': //not scheduled tags
						$this->calculateExceptionST( $plf, $slf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions, $scheduled_id_cache );
						break;
					case 'g1':
						$this->calculateExceptionG1( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'm1': //Missing In Punch
						$this->calculateExceptionM1( $plf, $type_id, $current_epoch, $premature_delay, $user_id, $date_stamp, $ep_obj, $current_exceptions );
						break;
					case 'm2': //Missing Out Punch
						$this->calculateExceptionM2( $plf, $current_epoch, $premature_delay, $user_id, $date_stamp, $ep_obj, $current_exceptions );
						break;
					case 'm3': //Missing Lunch In/Out punch
						$this->calculateExceptionM3( $plf, $current_epoch, $premature_delay, $user_id, $date_stamp, $ep_obj, $current_exceptions );
						break;
					case 'm4': //Missing Break In/Out punch
						$this->calculateExceptionM4( $plf, $current_epoch, $premature_delay, $user_id, $date_stamp, $ep_obj, $current_exceptions );
						break;
					case 'c1': //Missed Check-in
						$this->calculateExceptionC1( $plf, $ep_obj, $premature_delay, $current_epoch, $user_id, $date_stamp, $type_id, $current_exceptions );
						break;
					case 'd1': //No Branch or Department
						$this->calculateExceptionD1( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'd2': //Not Allowed on Branch
						$this->calculateExceptionD2( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'd3': //Not Allowed on Department
						$this->calculateExceptionD3( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 's7': //Over Scheduled Hours
						$this->calculateExceptionS7( $plf, $slf, $date_stamp, $ep_obj, $user_id, $type_id, $current_exceptions );
						break;
					case 's8': //Under Scheduled Hours
						$this->calculateExceptionS8( $plf, $slf, $date_stamp, $ep_obj, $type_id, $current_epoch, $user_id, $current_exceptions );
						break;
					case 'o1': //Over Daily Time.
						$this->calculateExceptionO1( $plf, $date_stamp, $ep_obj, $user_id, $type_id, $current_exceptions );
						break;
					case 'o2': //Over Weekly Time.
					case 's9': //Over Weekly Scheduled Time.
						$this->calculateExceptionO2AndS9( $plf, $start_week_day_id, $ep_obj, $date_stamp, $user_id, $type_id, $current_exceptions );
						break;
					case 'l1': //Long Lunch
					case 'l2': //Short Lunch
						$this->calculateExceptionL1AndL2( $plf, $date_stamp, $user_id, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'l3': //No Lunch
						$this->calculateExceptionL3( $plf, $date_stamp, $user_id, $type_id, $current_epoch, $premature_delay, $ep_obj, $current_exceptions );
						break;
					case 'l4': //Late Lunch
						$this->calculateExceptionL4( $plf, $date_stamp, $user_id, $current_epoch, $premature_delay, $type_id, $ep_obj, $current_exceptions );
						break;
					case 'b1': //Long Break
					case 'b2': //Short Break
						$this->calculateExceptionB1AndB2( $plf, $date_stamp, $user_id, $ep_obj, $type_id, $current_exceptions, $scheduled_id_cache );
						break;
					case 'b3': //Too Many Breaks
					case 'b4': //Too Few Breaks
						$this->calculateExceptionB3AndB4( $plf, $date_stamp, $user_id, $type_id, $current_epoch, $premature_delay, $ep_obj, $current_exceptions, $scheduled_id_cache );
						break;
					case 'b5': //No Break
						$this->calculateExceptionB5( $plf, $date_stamp, $user_id, $type_id, $current_epoch, $premature_delay, $ep_obj, $current_exceptions );
						break;
					case 'v1': //TimeSheet Not Verified
						$this->calculateExceptionV1( $ep_obj, $current_epoch, $date_stamp, $user_id, $type_id, $current_exceptions );
						break;
					case 'j1': //Not Allowed on Job
						$this->calculateExceptionJ1( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'j2': //Not Allowed on Task
						$this->calculateExceptionJ2( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'j3': //Job already completed
						$this->calculateExceptionJ3( $plf, $date_stamp, $user_id, $ep_obj, $type_id, $current_exceptions );
						break;
					case 'j4': //No Job or Task
						$this->calculateExceptionJ4( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 't1': //Not Allowed on Punch Tag
						$this->calculateExceptionT1( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					case 't2': //No Punch Tag
						$this->calculateExceptionT2( $plf, $user_id, $date_stamp, $ep_obj, $type_id, $current_exceptions );
						break;
					default:
			...
			if ( is_array( $exceptions ) ) {
				if ( isset( $exceptions['create_exceptions'] ) && is_array( $exceptions['create_exceptions'] ) && count( $exceptions['create_exceptions'] ) > 0 ) {
					...
					foreach ( $exceptions['create_exceptions'] as $tmp_exception ) {
						...
						if ( $ef->isValid() ) {
							...
							Debug::text( '  Exception ID: ' . $ef->getId() .' Type: '. $ef->getType() .' Exception Policy: '. $tmp_exception['exception_policy_id'], __FILE__, __LINE__, __METHOD__, 10 );
							if ( $enable_premature_exceptions == true || ( isset( $tmp_exception['enable_email_notification'] ) && $tmp_exception['enable_email_notification'] == true ) ) {
								...
								if ( $eplf->getRecordCount() == 1 ) {
									...
									$ef->sendNotificationException( $this->getUserObject(), $date_stamp, ( isset( $tmp_exception['punch_obj'] ) ) ? $tmp_exception['punch_obj'] : null, ( isset( $tmp_exception['schedule_obj'] ) ) ? $tmp_exception['schedule_obj'] : null, $ep_obj );
								}
							} else {
				...
				if ( isset( $exceptions['delete_exceptions'] ) && is_array( $exceptions['delete_exceptions'] ) && count( $exceptions['delete_exceptions'] ) > 0 ) {
					...
					NotificationFactory::updateStatusByObjectIdAndObjectTypeId( 10, $exceptions['delete_exceptions'] ); //10=Exceptions, Mark any notifications linked to these exceptions as read.
				}
			...
			NotificationFactory::updateStatusByObjectIdAndObjectTypeId( 10, array_keys( $existing_exceptions ) ); //10=Exceptions, Mark any notifications linked to these exceptions as read.
	...
	/**
	 * Find the gap between punches after a certain time stamp for auto-add, or the non-gap for auto-deduct.
	 * @param $punch_data
	 * @param $filter_time_stamp
	 * @param $status_id
	 * @param $max_total_time
	 * @return array
	 */
	function findPunchDataGapAfterTimeStamp( $punch_data, $filter_time_stamp, $status_id, $max_total_time = null ) {
		//Loop through punches and find places to insert auto-deduct or auto-add meal/breaks
		$prev_punch_obj = null;
		$retval = [ 'start' => null, 'end' => null, 'is_last_punch' => false ];

		switch ( $status_id ) {
			case 10: //In
				//Find out punch after $time_stamp
				foreach( $punch_data as $punch_obj ) {
					if ( is_object( $prev_punch_obj ) ) {
						if ( empty( $filter_time_stamp ) && is_object( $prev_punch_obj ) && $punch_obj->getStatus() == 20 ) {
							$retval = [ 'start' => $prev_punch_obj->getTimeStamp(), 'end' => $punch_obj->getTimeStamp(), 'total_time' => ( $punch_obj->getTimeStamp() - $prev_punch_obj->getTimeStamp() ) ];
							break;
						} else if ( !empty( $filter_time_stamp ) && $punch_obj->getStatus() == 20 && $filter_time_stamp < $punch_obj->getTimeStamp() && $punch_obj->getTimeStamp() != $prev_punch_obj->getTimeStamp() ) {
							$retval = [ 'start' => $prev_punch_obj->getTimeStamp(), 'end' => $punch_obj->getTimeStamp(), 'total_time' => ( $punch_obj->getTimeStamp() - $prev_punch_obj->getTimeStamp() ) ];
							break;
						}
					}

					$prev_punch_obj = $punch_obj;
				}
				break;
			case 20: //Out
				//Find out punch after $time_stamp
				foreach( $punch_data as $punch_obj ) {
					if ( is_object( $prev_punch_obj ) ) {
						if ( empty( $filter_time_stamp ) && is_object( $prev_punch_obj ) && $punch_obj->getStatus() == 10 ) {
							$retval = [ 'start' => $prev_punch_obj->getTimeStamp(), 'end' => $punch_obj->getTimeStamp(), 'total_time' => ( $punch_obj->getTimeStamp() - $prev_punch_obj->getTimeStamp() ) ];
							break;
						} else if ( !empty( $filter_time_stamp ) && $punch_obj->getStatus() == 10 && $filter_time_stamp < $punch_obj->getTimeStamp() && $punch_obj->getTimeStamp() != $prev_punch_obj->getTimeStamp() ) {
							$retval = [ 'start' => $prev_punch_obj->getTimeStamp(), 'end' => $punch_obj->getTimeStamp(), 'total_time' => ( $punch_obj->getTimeStamp() - $prev_punch_obj->getTimeStamp() ) ];
							break;
						}
					}
		...
		//If we got to the last out punch and didn't find a gap, just use it.
		if ( !isset( $retval['start'] ) && isset( $punch_obj ) && $punch_obj->getStatus() == 20 ) { //20=Out
			$retval = [ 'start' => $punch_obj->getTimeStamp(), 'end' => ( ( $max_total_time > 0 ) ? ( $punch_obj->getTimeStamp() + $max_total_time ) : null ), 'is_last_punch' => true ];
		}

		//Never go before the current $time_stamp filter.
		if ( isset( $retval['start'] ) && $retval['start'] < $filter_time_stamp ) {
			$retval['start'] = $filter_time_stamp;

			//New start time could be after end time, so adjust end time if max_total_time is available to base it on.
			if ( $max_total_time != 0 && $retval['start'] >= $retval['end'] ) {
				$retval['end'] = ( $retval['start'] + $max_total_time );
			}
		}

		if ( isset( $retval['start'] ) && isset( $retval['end'] ) ) {
			$retval['total_time'] = ( $retval['end'] - $retval['start'] );
		} else if ( !isset( $retval['total_time'] ) ) {
			$retval['total_time'] = 0;
		}

		if ( isset( $max_total_time ) && isset( $retval['total_time'] ) && $retval['total_time'] > $max_total_time ) {
			$retval['end'] = ( $retval['start'] + $max_total_time );
			$retval['total_time'] = $max_total_time;
		}

		Debug::text( 'Found Punch Gap: Start: ' . TTDate::getDate('DATE+TIME', $retval['start'] ) .' End: '. TTDate::getDate('DATE+TIME', $retval['end'] ) .' Total Time: '. $retval['total_time'] .' Filter TimeStamp: '. TTDate::getDate( 'DATE+TIME', $filter_time_stamp ), __FILE__, __LINE__, __METHOD__, 10 );
		return $retval;
	}
	...
	function getContributingPayCodePolicy() {
		...
		if ( is_array( $csplf ) && count( $csplf ) > 0 ) {
			...
			//Find any regular/overtime/premium policies that use Pay Code Policy differential criteria and include those too.
			//  Otherwise calculateRegularTime/OverTime/Premium won't be able to find the cached $contributing_pay_code_policy records.
			$rtplf = $this->regular_time_policy;
			if ( is_array( $rtplf ) && count( $rtplf ) > 0 ) {
				foreach ( $rtplf as $rtp_obj ) {
					if ( $rtp_obj->getContributingPayCodePolicy() != TTUUID::getNotExistID() ) {
						$this->contributing_pay_code_policy_ids[] = $rtp_obj->getContributingPayCodePolicy();
					}
				}
				unset( $rtp_obj );
			}
			...
			if ( is_array( $otplf ) && count( $otplf ) > 0 ) {
				foreach ( $otplf as $otp_obj ) {
					if ( $otp_obj->getContributingPayCodePolicy() != TTUUID::getNotExistID() ) {
						$this->contributing_pay_code_policy_ids[] = $otp_obj->getContributingPayCodePolicy();
					}
				}
				unset( $otp_obj );
			}

			$ptplf = $this->premium_time_policy;
			if ( is_array( $ptplf ) && count( $ptplf ) > 0 ) {
				foreach ( $ptplf as $ptp_obj ) {
					if ( $ptp_obj->getContributingPayCodePolicy() != TTUUID::getNotExistID() ) {
						$this->contributing_pay_code_policy_ids[] = $ptp_obj->getContributingPayCodePolicy();
					}
				}
				unset( $ptp_obj );
			}

			$this->contributing_pay_code_policy_ids = array_unique( $this->contributing_pay_code_policy_ids );
	...
	function isPayFormulaPolicyAveraging( $pay_formula_policy_id, $pay_code_id ) {
		...
		if ( $pay_formula_policy_obj->getWageSourceType() == 30 || $pay_formula_policy_obj->getPayType() == 60 || $pay_formula_policy_obj->getPayType() == 70  ) {
	 ...
	 * @return bool
	 */
	function getPayFormulaPolicy( $date_stamp ) {
		//Get PayFormula policies and determine how many days we need to look behind in order to recalculate daily average hourly rates
		$this->pay_formula_average_days = 0;
		...
		if ( $this->pay_formula_policy == null || ( is_array( $this->pay_formula_policy ) && count( $this->pay_formula_policy ) == 0 ) ) {
			...
			if ( $pfplf->getRecordCount() > 0 ) {
				foreach ( $pfplf as $pfp_obj ) {
					...
					if ( $this->pay_formula_calculate_average_hourly_rates == false && $pfp_obj->getWageSourceType() == 30 ) {
						$this->pay_formula_calculate_average_hourly_rates = true;
					}

					if ( $pfp_obj->getAverageDays() > $this->pay_formula_average_days ) {
						$this->pay_formula_average_days = $pfp_obj->getAverageDays();
					}
				}

				//If any pay formulas use averaging types, calculate that number of days in the future.
				if ( $this->pay_formula_average_days > 0 && $this->getFlag( 'future_dates' ) == true ) {
					$this->addPendingCalculationDate( $date_stamp, TTDate::incrementDate( $date_stamp, $this->pay_formula_average_days, 'day' ) );
				}

				Debug::Text( 'Pay Formula Policy rows: ' . count( $this->pay_formula_policy ) .' Calc Avg Hourly Rates: '. (int)$this->pay_formula_calculate_average_hourly_rates .' Average Days: '. $this->pay_formula_average_days, __FILE__, __LINE__, __METHOD__, 10 );
	...
	/**
	 * @param int $date_stamp EPOCH
	 * @param ContributingShiftPolicyFactory $hour_contributing_shift_policy_obj
	 * @param int $hour_object_type_ids
	 * @param ContributingShiftPolicyFactory $wage_contributing_shift_policy_obj
	 * @return bool|float|int
	 */
	function getDailyHourlyRate( $date_stamp, $hour_contributing_shift_policy_obj, $hour_object_type_ids, $wage_contributing_shift_policy_obj, $pay_formula_policy_obj, $pay_code_id ) {
		$pay_code_daily_total_time = $this->getSumUserDateTotalData( $this->filterUserDateTotalDataByPayCodeIDs( $date_stamp, $date_stamp, $pay_code_id, [ 20, 25, 30, 40, 100, 110 ] ) );
		if ( $pay_code_daily_total_time > 0 ) {
			$total_daily_rate = 0;
			if ( $pay_formula_policy_obj->getPayType() == 60 ) { //60=Flat Daily Wage
				$uw_obj = $this->filterUserWage( $pay_formula_policy_obj->getWageGroup(), $date_stamp );
				if ( is_object( $uw_obj ) ) {
					$total_daily_rate = $uw_obj->getHourlyRate();
				} else {
					$total_daily_rate = $pay_formula_policy_obj->getRate();
				}
			} else if ( $pay_formula_policy_obj->getPayType() == 70 ) { //70=Daily Average
				$start_date = TTDate::incrementDate( $date_stamp, ( $pay_formula_policy_obj->getAverageDays() * -1 ), 'day' ); //X days *before* the current date.
				$end_date = TTDate::incrementDate( $date_stamp, -1, 'day' ); //Day before the current date.
				$this->getRequiredData( $start_date, $end_date );                                                                             //Use the end of the week date stamp.

				//Get total days with time over the averaging period.
				$total_days_with_time = count( (array)$this->getDayArrayUserDateTotalData( $this->filterUserDateTotalDataByContributingShiftPolicy( $start_date, $end_date, $hour_contributing_shift_policy_obj, [ 20, 25, 30, 40, 100, 110 ] ) ) );
				Debug::text( ' Total Days w/Time: ' . $total_days_with_time . ' Out of Total Overall Days: ' . $pay_formula_policy_obj->getAverageDays() .' Start: '. TTDate::getDate('DATE', $start_date ) .' End: '. TTDate::getDate('DATE', $end_date ), __FILE__, __LINE__, __METHOD__, 10 );

				if ( $total_days_with_time > 0 ) { //Handle average wages for negative values too.
					$total_wages = 0;

					$wage_udt_rows = $this->filterUserDateTotalDataByContributingShiftPolicy( $start_date, $end_date, $wage_contributing_shift_policy_obj, [ 20, 25, 30, 40, 100, 110 ] );
					if ( is_array( $wage_udt_rows ) && count( $wage_udt_rows ) > 0 ) {
						foreach ( $wage_udt_rows as $udt_obj ) {
							$total_wages = TTMath::add( $total_wages, $udt_obj->getTotalTimeAmount() );
						}
					}
					Debug::text( ' Total Wages: ' . $total_wages, __FILE__, __LINE__, __METHOD__, 10 );
					unset( $wage_udt_rows, $udt_obj );

					$total_daily_rate = TTMath::div( $total_wages, $total_days_with_time );
				}
			}

			if ( $total_daily_rate > 0 && $pay_code_daily_total_time > 0 ) {
				$daily_hourly_rate = TTMath::div( $total_daily_rate, TTDate::getHours( $pay_code_daily_total_time ) );
				Debug::text( 'Daily Total Time for Pay Code: ' . $pay_code_daily_total_time . ' Daily Rate: ' . $total_daily_rate . ' Daily Hourly Rate: ' . $daily_hourly_rate . ' DateStamp: ' . TTDate::getDate( 'DATE', $date_stamp ), __FILE__, __LINE__, __METHOD__, 10 );

				return $daily_hourly_rate;
			}
		} else {
			Debug::text( ' No time in this pay code: ' . $pay_code_id .' unable to calculate Daily Hourly Rate...', __FILE__, __LINE__, __METHOD__, 10 );
		}
	 ...
	 * @return bool|float|int
	 */
	function getAverageHourlyRate( $date_stamp, $hour_contributing_shift_policy_obj, $hour_object_type_ids, $wage_contributing_shift_policy_obj, $destination_object_type_id = 30 ) {
		//To determine average rate we need to seperate what hours are included and what dollars are included.

		$total_time = 0;
		$total_wages = 0;

		//NOTE: Its possible this might need to handle a daily averaging, weekly averaging and BiWeekly averaging in the future.
		//      If a customer in California for example needs averaging just over the same day for Daily OT, then over a week for Weekly OT, and over 2 weeks for BiWeekly OT.
		...
		if ( is_array( $hour_udt_rows ) && count( $hour_udt_rows ) > 0 ) {
			foreach ( $hour_udt_rows as $udt_obj ) {
				$total_time = TTMath::add( $total_time, $udt_obj->getTotalTime() );
			}
		}
		...
		if ( $total_time != 0 ) { //Handle average wages for negative values too.
			...
			if ( is_array( $wage_udt_rows ) && count( $wage_udt_rows ) > 0 ) {
				foreach ( $wage_udt_rows as $udt_obj ) {
					if ( $destination_object_type_id == 30 && $udt_obj->getObjectType() == 30 ) { //When calculating average rates on overtime, we need to use the base hourly rate from OT UDT records, to avoid pyramid calculating OT rates.
						...
						$tmp_wages = TTMath::mul( $udt_obj->getBaseHourlyRate(), TTDate::getHours( $udt_obj->getTotalTime() ) );
					} else {
						$tmp_wages = $udt_obj->getTotalTimeAmount();
					}

					//Debug::text('Adding wages: '. $tmp_wages .' Date: '. TTDate::getDate('DATE', $udt_obj->getDateStamp() ), __FILE__, __LINE__, __METHOD__, 10);
					$total_wages = TTMath::add( $total_wages, $tmp_wages );
				}
			}
			//Debug::text('Total Wages: '. $total_wages, __FILE__, __LINE__, __METHOD__, 10);
			unset( $wage_udt_rows, $udt_obj );

			$average_hourly_rate = TTMath::div( $total_wages, TTDate::getHours( $total_time ) );
	 ...
	 * @return bool|float|int
	 */
	function getBaseHourlyRate( $pay_formula_policy_id, $pay_code_id, $date_stamp, $contributing_pay_code_hourly_rate = false, $contributing_shift_policy_obj = null, $object_type_ids = null, $destination_object_type_id = null ) {
		//Don't cast to UUID as an optimization.
		...
		if ( is_object( $pay_formula_policy_obj ) ) {
			...
			switch ( $pay_formula_policy_obj->getWageSourceType() ) {
				...
				case 20: //Contributing Pay Code
					if ( is_numeric( $contributing_pay_code_hourly_rate ) ) {
						$tmp_hourly_rate = $contributing_pay_code_hourly_rate;
					}
					break;
				case 30: //Average Contributing Pay Codes
					if ( !empty( $destination_object_type_id ) ) { //If no destination object_type_id is specified, we aren't being called from calculateAverageHourlyRates(), so we can skip calculating all the average rates.
						if ( $pay_formula_policy_obj->getPayType() == 60 || $pay_formula_policy_obj->getPayType() == 70 ) {
							Debug::text( '  Daily Average Contributing Pay Codes... Determine Average Hourly Rate...', __FILE__, __LINE__, __METHOD__, 10 );
							$tmp_hourly_rate = $this->getDailyHourlyRate( $date_stamp, $this->contributing_shift_policy[$pay_formula_policy_obj->getTimeSourceContributingShiftPolicy()], $object_type_ids, $this->contributing_shift_policy[$pay_formula_policy_obj->getWageSourceContributingShiftPolicy()], $pay_formula_policy_obj, $pay_code_id );
							...
							$tmp_hourly_rate = $this->getAverageHourlyRate( $date_stamp, $this->contributing_shift_policy[$pay_formula_policy_obj->getTimeSourceContributingShiftPolicy()], $object_type_ids, $this->contributing_shift_policy[$pay_formula_policy_obj->getWageSourceContributingShiftPolicy()], $destination_object_type_id );
						}
					}
					break;
			}
	 ...
	 * @return int
	 */
	function getHourlyRate( $pay_formula_policy_id, $pay_code_id, $date_stamp, $base_hourly_rate, $udtf = null ) {
		//Don't cast to UUID as an optimization.
		//$pay_code_id = TTUUID::castUUID( $pay_code_id );
		//$pay_formula_policy_id = TTUUID::castUUID( $pay_formula_policy_id );
		...
		if ( is_object( $pay_formula_policy_obj ) ) {
			...
			$hourly_rate = $pay_formula_policy_obj->getHourlyRate( $tmp_hourly_rate, $udtf );
		}

		if ( isset( $this->pay_codes[$pay_code_id] ) && $this->pay_codes[$pay_code_id]->getType() == 30 && $hourly_rate > 0 ) { //Dock Pay
			$hourly_rate = TTMath::mul( $hourly_rate, -1 );
		}
	...
	function getHourlyRateWithBurden( $pay_formula_policy_id, $pay_code_id, $date_stamp, $base_hourly_rate = 0 ) {
		...
		$uw_obj = $this->filterUserWage( $pay_formula_policy_obj->getWageGroup(), $date_stamp );
		if ( is_object( $uw_obj ) ) {
			$hourly_rate = TTMath::mul( $base_hourly_rate, TTMath::add( TTMath::div( $uw_obj->getLaborBurdenPercent(), 100 ), 1 ) );
		}
	...
	function calculatePremiumTimePolicy( $date_stamp, $maximum_daily_total_time = null ) {
		...
		if ( is_array( $premium_time_policies ) && count( $premium_time_policies ) > 0 ) {
			foreach ( $premium_time_policies as $pp_obj ) { /** @var PremiumPolicyFactory $pp_obj */
				...
				if ( is_array( $user_date_total_rows ) && $user_date_total_rows_count > 0 ) {
					switch ( $pp_obj->getType() ) {
						...
						case 90: //Holiday (converts to Date/Time policy automatically)
							...
							if ( $pp_obj->isActive( TTDate::incrementDate( $date_stamp, -1, 'day' ), TTDate::incrementDate( $date_stamp, 1, 'day' ), $this ) ) {
									...
									(
										...
										if ( $pp_obj->getMinimumTime() > 0 || $pp_obj->getMaximumTime() > 0 ) {
											if ( $pp_obj->getMinMaxTimeType() == 30 ) { //30=Punch Pair - Resets Min/Max time each punch pair.
												//This needs to match actual punch pairs though, not UDT records which could later get compacted out
												// and also result in more premium UDT records if things like shifts are being split at certain times. (ie: 7A-3P, 3P-11P)
												// So make sure the Out timestamps match existing punch timestamps as well.
												//$premium_policy_daily_total_time = 0;

												if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) {
													$found_min_max_reset_matching_punch_pair = false;
													$punch_data = $this->filterPunchDataByDateAndTypeAndStatus( $date_stamp );
													if ( is_array( $punch_data ) && count( $punch_data ) > 0 ) {
														foreach ( $punch_data as $punch_obj ) {
															//Only check Out time stamp, because if we check in and out timestamps, it can match twice per punch pair instead of once.
															// Also we base the resulting premium policy UDT record on the end time stamp below, so it makes more sense to match on out timestamps here too.
															// Consider auto-deduct lunches, which are deducted from the end of the UDT record, so they will never match the end time stamp.
															// FIXME: This could cause problems with not enough time being calculated though if a UDT record is split, and part that matches is only a few minutes, and no minimum time is set, but just maximum time?
															//        Typically when customers are using this Min=1, Max=1 though, so total time doesn't matter.
															if ( $punch_obj->getStatus() == 10 && $punch_obj->getTimeStamp() == $udt_obj->getStartTimeStamp() ) { //Must use In punch time to match, as auto-deduct meal/breaks deducts from the end timestamp, and therefore would never match.
																$premium_policy_daily_total_time = 0;
																$found_min_max_reset_matching_punch_pair = true;
																break;
															}
														}
													}
													unset( $punch_data, $punch_obj );

													if ( $found_min_max_reset_matching_punch_pair == false ) {
														continue;
													}
												}
											} else {
											...
											if ( $pp_obj->getMinimumTime() > 0 ) {
												if ( $pp_obj->getMinMaxTimeType() == 30 ) { //30=Punch Pair - Resets Min/Max time each punch pair.
													...
													if ( $i == $user_date_total_rows_count && TTMath::add( $premium_policy_daily_total_time, $total_time ) < $pp_obj->getMinimumTime() ) {
														$total_time = TTMath::sub( $pp_obj->getMinimumTime(), $premium_policy_daily_total_time );
													}
												}
											}
											...
											if ( $pp_obj->getMaximumTime() > 0 ) {
												//Min & Max time is based on the shift time, rather then per punch pair time.
												if ( TTMath::add( $premium_policy_daily_total_time, $total_time ) > $pp_obj->getMaximumTime() ) {
													Debug::text( ' bMore than Maximum Time...', __FILE__, __LINE__, __METHOD__, 10 );
													$total_time = TTMath::sub( $total_time, TTMath::sub( TTMath::add( $premium_policy_daily_total_time, $total_time ), $pp_obj->getMaximumTime() ) );
												}
											}
										}
										...
										if ( $total_time != 0 ) { //Need to handle negative values too for things like lunch auto-deduct.
											...
											if ( $pp_obj->getType() == 100 ) {
												...
												if ( ( $pp_obj->getBranchSelectionType() == 10 && $pp_obj->getExcludeDefaultBranch() == false ) || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getBranchSelectionType(), $pp_obj->getExcludeDefaultBranch(), $udt_obj->getBranch(), $pp_obj->getBranch(), $this->getUserObject()->getDefaultBranch() ) ) {
													...
													if ( ( $pp_obj->getDepartmentSelectionType() == 10 && $pp_obj->getExcludeDefaultDepartment() == false ) || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getDepartmentSelectionType(), $pp_obj->getExcludeDefaultDepartment(), $udt_obj->getDepartment(), $pp_obj->getDepartment(), $this->getUserObject()->getDefaultDepartment() ) ) {
														...
														if ( $pp_obj->getJobGroupSelectionType() == 10 || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getJobGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobObject() ) ) ? $udt_obj->getJobObject()->getGroup() : null ), $pp_obj->getJobGroup() ) ) {
															...
															if ( ( $pp_obj->getJobSelectionType() == 10 && $pp_obj->getExcludeDefaultJob() == false ) || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getJobSelectionType(), $pp_obj->getExcludeDefaultJob(), $udt_obj->getJob(), $pp_obj->getJob(), $this->getUserObject()->getDefaultJob() ) ) {
																//Debug::text(' Shift Differential... Meets Job Criteria! Select Type: '. $pp_obj->getJobSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
																if ( $pp_obj->getJobItemGroupSelectionType() == 10 || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getJobItemGroupSelectionType(), null, ( ( is_object( $udt_obj->getJobItemObject() ) ) ? $udt_obj->getJobItemObject()->getGroup() : null ), $pp_obj->getJobItemGroup() ) ) {
																	...
																	if ( ( $pp_obj->getJobItemSelectionType() == 10 && $pp_obj->getExcludeDefaultJobItem() == false ) || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getJobItemSelectionType(), $pp_obj->getExcludeDefaultJobItem(), $udt_obj->getJobItem(), $pp_obj->getJobItem(), $this->getUserObject()->getDefaultJobItem() ) ) {
																		//Debug::text(' Shift Differential... Meets Task Criteria! Select Type: '. $pp_obj->getJobSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
																		if ( $pp_obj->getPunchTagGroupSelectionType() == 10 || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getPunchTagGroupSelectionType(), null, $udt_obj->getPunchTagGroupIds(), $pp_obj->getPunchTagGroup() ) ) {
																			//Debug::text(' Shift Differential... Meets Punch Tag Group Criteria! Select Type: '. $pp_obj->getPunchTagGroupSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
																			if ( ( $pp_obj->getPunchTagSelectionType() == 10 && $pp_obj->getExcludeDefaultPunchTag() == false ) || $this->contributing_shift_policy[$pp_obj->getContributingShiftPolicy()]->checkIndividualDifferentialCriteria( $pp_obj->getPunchTagSelectionType(), $pp_obj->getExcludeDefaultPunchTag(), $udt_obj->getPunchTag(), $pp_obj->getPunchTag(), $this->getUserObject()->getDefaultPunchTag() ) ) {
																				//Debug::text(' Shift Differential... Meets Punch Tag Criteria! Select Type: '. $pp_obj->getPunchTagSelectionType(), __FILE__, __LINE__, __METHOD__, 10);
																				if ( $pp_obj->getContributingPayCodePolicy() == TTUUID::getNotExistID() || ( isset( $this->contributing_pay_codes_by_policy_id[$pp_obj->getContributingPayCodePolicy()] ) && in_array( $udt_obj->getPayCode(), $this->contributing_pay_codes_by_policy_id[$pp_obj->getContributingPayCodePolicy()] ) ) ) {
																					//Debug::text(' Shift Differential... Meets Contributing Pay Code Policy Criteria! Pay Code Policy: '. $otp_obj->getContributingPayCodePolicy(), __FILE__, __LINE__, __METHOD__, 10);
																					$create_udt_record = true;
																				}
																			}
											...
											if ( $create_udt_record == true ) {
												...
												if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
													...
													if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) {
														...
														if ( isset( $found_min_max_reset_matching_punch_pair ) && $found_min_max_reset_matching_punch_pair == true ) { //Use start time when min/max resets at punch pair rather than end time, so the times make more sense with when they would actually occur.
															$udtf->setStartTimeStamp( $udt_obj->getStartTimeStamp() );
														} else {
													...
													$udtf->setHourlyRate( $this->getHourlyRate( $pp_obj->getPayFormulaPolicy(), $udtf->getPayCode(), $date_stamp, $udtf->getBaseHourlyRate(), $udtf ) );
							...
							unset( $udtlf, $udt_obj, $total_time );
							break;
									...
									if ( $maximum_time_worked_without_break > $pp_obj->getMaximumNoBreakTime() ) {
										...
										if ( $pp_obj->getMinimumTime() > 0 ) {
											if ( $pp_obj->getMinMaxTimeType() == 30 ) { //30=Punch Pair - Resets Min/Max time each punch pair.
												...
												if ( TTMath::add( $premium_policy_daily_total_time, $total_time ) < $pp_obj->getMinimumTime() ) {
													$total_time = TTMath::sub( $pp_obj->getMinimumTime(), $premium_policy_daily_total_time );
												}
											}
										}
											...
											foreach ( $user_date_total_rows as $udt_key => $udt_obj ) {
												...
												if ( $udt_obj->getStartTimeStamp() != '' && $udt_obj->getEndTimeStamp() != '' ) {
													...
													if ( $udt_obj->getStartTimeStamp() >= $current_shift_cutoff ) {
														...
														//$punch_total_time = TTMath::sub( $punch_pairs[$udt_obj->getPunchControlID()][1]['time_stamp'], $punch_pairs[$udt_obj->getPunchControlID()][0]['time_stamp']);
														$punch_total_time = $udt_obj->getTotalTime();
													} else {
													...
													if ( $pp_obj->getMinimumTime() > 0 || $pp_obj->getMaximumTime() > 0 ) {
														...
														if ( $pp_obj->getMinimumTime() > 0 ) {
															if ( $pp_obj->getMinMaxTimeType() == 30 ) { //30=Punch Pair - Resets Min/Max time each punch pair.
																...
																//for the day. If its applied to the last it will be just 1hr.
																//Min & Max time is based on the shift time, rather then per punch pair time.
																if ( ( $force_minimum_time_calculation == true || $x == $user_date_total_rows_count ) && TTMath::add( $premium_policy_daily_total_time, $punch_total_time ) < $pp_obj->getMinimumTime() ) {
																	$total_time = TTMath::sub( $pp_obj->getMinimumTime(), $premium_policy_daily_total_time );
																} else {
																	$total_time = $punch_total_time;
																}
								...
								if ( is_array( $shift_data ) ) {
									...
									foreach ( $shift_data as $shift_data_arr ) {
										...
										if ( $punch_total_time > $pp_obj->getMinimumShiftTime() ) {
											Debug::text( ' Shift exceeds minimum shift time... Shift Total Time: '. $punch_total_time, __FILE__, __LINE__, __METHOD__, 10 );
											continue;
										} else {
											$punch_total_time = TTMath::sub( $pp_obj->getMinimumShiftTime(), $punch_total_time );
										}
										...
										if ( $pp_obj->getMinimumTime() > 0 || $pp_obj->getMaximumTime() > 0 ) {
											...
											if ( $pp_obj->getMinimumTime() > 0 ) {
												if ( $pp_obj->getMinMaxTimeType() == 30 ) { //30=Punch Pair - Resets Min/Max time each punch pair.
													...
													//for the day. If its applied to the last it will be just 1hr.
													//Min & Max time is based on the shift time, rather then per punch pair time.
													if ( $x == $total_shifts && TTMath::add( $premium_policy_daily_total_time, $punch_total_time ) < $pp_obj->getMinimumTime() ) {
														$total_time = TTMath::sub( $pp_obj->getMinimumTime(), $premium_policy_daily_total_time );
													} else {
														$total_time = $punch_total_time;
													}
	...
	function getPremiumTimePolicy() {
		...
		$pplf->getByPolicyGroupUserIdOrId( $this->getUserObject()->getId(), array_unique( $schedule_premium_time_policy_ids ) );
	...
	function calculateAccrualPolicy() {
		...
		if ( isset( $first_date_stamp ) && isset( $last_date_stamp ) ) {
			...
			if ( isset( $new_and_absent_user_date_total_ids ) && count( $new_and_absent_user_date_total_ids ) > 0 ) {
				...
				if ( isset( $tmp_user_date_total ) ) {
					...
					$this->sortUserDateTotalData( $tmp_user_date_total, 'sortUserDateTotalDataByDateAndStartTimeStampAndObjectTypeAndID' ); //Sorting is inplace.

					foreach ( $tmp_user_date_total as $udt_key => $udt_obj ) { /** @var UserDateTotalFactory $udt_obj */
						//Debug::text('   UDT ID: '. $udt_obj->getID() .' Object Type ID: '. $udt_obj->getObjectType() .' Date: '. TTDate::getDate('DATE', $udt_obj->getDateStamp() ) .' Start Time: '. TTDate::getDate('DATE', $udt_obj->getStartTimeStamp() ), __FILE__, __LINE__, __METHOD__, 10);
						...
						if ( $this->isPayFormulaAccruing( $pay_formula_policy_obj ) == true ) {
							if ( isset( $this->accrual_time_exclusivity_map[$udt_key] ) ) {
								Debug::text( '  WARNING: Accrual already calculated on this ObjectType->PayCode->SourceObject combination, skipping... Based on UDT key: ' . $udt_obj->getId() . ' Object Type: '. $udt_obj->getObjectType() .' Total Time: '. $udt_obj->getTotalTime(), __FILE__, __LINE__, __METHOD__, 10 );
							} else {
								...
								$accrual_amount = TTMath::mul( $udt_obj->getTotalTime(), $pay_formula_policy_obj->getAccrualRate() );
								if ( $accrual_amount >= 0 ) {
								...
								if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL && $pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount() != TTUUID::getZeroID() ) {
									//Only adjust the amount here if there is a ThresholdFallback accural defined, otherwise the user could have a 8hr absence which is paid, but only 2hrs comes out of the accrual, causing a discrepancy.
									//  Which will cause a 2nd accrual entry for the fallback amount to be created below.
									$adjusted_amount_arr = $pay_formula_policy_obj->getAmountAfterBalanceThreshold( $this->getUserObject()->getID(), $accrual_amount );
									if ( isset( $adjusted_amount_arr['adjusted_amount'] ) ) {
										$accrual_amount = $adjusted_amount_arr['adjusted_amount'];
										$af->setAmount( $adjusted_amount_arr['adjusted_amount'] );
									}

									if ( isset( $adjusted_amount_arr['note'] ) ) {
										$af->setNote( $adjusted_amount_arr['note'] );
									}
								} else {
									$af->setAmount( $accrual_amount );
								}
								$af->setEnableCalcBalance( true );
								$af->setEnableStrictValidation( false ); //Optimize validation to reduce SQL queries.

								Debug::text( 'Adding Pay Formula Accrual Entry for: ' . $accrual_amount . ' Based on UDT key: ' . $udt_obj->getId() . ' Object Type: '. $udt_obj->getObjectType() .' Pay Code: ' . $udt_obj->getPayCode() . ' Source Object: ' . $udt_obj->getSourceObject(), __FILE__, __LINE__, __METHOD__, 10 );
								if ( $af->getAmount() != 0 && $af->isValid() ) {
									$af->Save();
									...
									if ( isset( $accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualPolicyAccount()][$date_stamp] ) ) {
										$accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualPolicyAccount()][$date_stamp] = TTMath::add( $accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualPolicyAccount()][$date_stamp], $accrual_amount );
									} else {
										$accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualPolicyAccount()][$date_stamp] = $accrual_amount;
									}
								} else {
									Debug::text( '  Accrual amount is 0 or record is invalid. Amount: ' . $af->getAmount(), __FILE__, __LINE__, __METHOD__, 10 );
								}

								//If any amount remaining after balance threshold adjustment, accrue that.
								if ( isset( $adjusted_amount_arr['amount_remaining'] ) && $adjusted_amount_arr['amount_remaining'] != 0 && $pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount() != TTUUID::getZeroID() ) {
									$af = TTnew( 'AccrualFactory' ); /** @var AccrualFactory $af */
									$af->setUser( $this->getUserObject()->getID() );
									$af->setAccrualPolicyAccount( $pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount() );
									$af->setTimeStamp( $udt_obj->getDateStamp() );
									$af->setUserDateTotalID( $udt_obj->getID() );
									...
									$accrual_amount = $adjusted_amount_arr['amount_remaining'];
									$af->setAmount( $accrual_amount );
									...
									Debug::text( 'Adding Fallback Pay Formula Accrual Entry for: ' . $accrual_amount . ' Based on UDT key: ' . $udt_obj->getId() . ' Pay Code: ' . $udt_obj->getPayCode() . ' Source Object: ' . $udt_obj->getSourceObject(), __FILE__, __LINE__, __METHOD__, 10 );
									if ( $af->getAmount() != 0 && $af->isValid() ) {
										$af->Save();

										//Record the exact date and accrual policy that each record is on, so we can properly account for the exact day when the maximum balance might be reached.
										if ( isset( $accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount()][$date_stamp] ) ) {
											$accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount()][$date_stamp] = TTMath::add( $accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount()][$date_stamp], $accrual_amount );
										} else {
											$accrual_pay_formula_arr[$pay_formula_policy_obj->getAccrualBalanceThresholdFallbackAccrualPolicyAccount()][$date_stamp] = $accrual_amount;
										}
									} else {
										Debug::text( '  Fallback Accrual amount is 0 or record is invalid. Amount: ' . $af->getAmount(), __FILE__, __LINE__, __METHOD__, 10 );
									}
								}
		...
		if ( is_array( $aplf ) && count( $aplf ) > 0 ) {
			...
			foreach ( $aplf as $ap_obj ) { /** @var AccrualPolicyFactory $ap_obj */
				...
				if ( isset( $accrual_pay_formula_arr[$ap_obj->getAccrualPolicyAccount()] ) ) {
					$original_balance = TTMath::add( $original_balance, TTMath::mul( TTMath::ArraySum( $accrual_pay_formula_arr[$ap_obj->getAccrualPolicyAccount()] ), -1 ) ); //Reverse the pay formula adjustment as if they didn't happen, so we can account for them on a day-by-basis below instead.
				}
				...
				if ( isset( $first_date_stamp ) && isset( $last_date_stamp ) ) {
					...
					foreach ( $this->dates['calculated'] as $date_stamp => $tmp ) {
						//Since this function is called after all other policies are calculated, the pay period could that of the last date calculated.
						// Therefore since we are looping over all dates again, we need to make sure the pay period is properly set for each date iteration.
						$this->setPayPeriodFromDate( $date_stamp ); //Should use begin day epoch.
						...
						if ( $ap_obj->getMinimumEmployedDays() == 0
							...
							$eligible_total_time = 0;
							$minimum_eligible_total_time = 0;
							$maximum_eligible_total_time = 0;

							if ( $ap_obj->getEligiblePeriod() != 0 ) {
								//Get eligiblity time over the specified period.
								$eligible_dates = $ap_obj->getEligibleDates( $date_stamp, 0, $this->pay_period_schedule_obj, $this->pay_period_obj );

								if ( isset( $eligible_dates['start_date'] ) && isset( $eligible_dates['end_date'] ) ) {
									$this->getUserDateTotalData( $eligible_dates['start_date'], $eligible_dates['end_date'] );
									Debug::Text( '  Eligiblity Period: ' . $ap_obj->getEligiblePeriod() . ' Start Date: ' . TTDate::getDate( 'DATE', $eligible_dates['start_date'] ) . ' End Date: ' . TTDate::getDate( 'DATE', $eligible_dates['end_date'] ), __FILE__, __LINE__, __METHOD__, 10 );

									$eligible_total_time = $this->getSumUserDateTotalData( $this->filterUserDateTotalDataByContributingShiftPolicy( $eligible_dates['start_date'], ( $date_stamp - 86400 ), $this->contributing_shift_policy[$ap_obj->getEligibleContributingShiftPolicy()], [ 20, 25, 30, 40, 100, 110 ] ) );
									$minimum_eligible_total_time = ( $eligible_total_time - $ap_obj->getMinimumEligibleTime() );
									$maximum_eligible_total_time = $eligible_total_time;
									Debug::Text( '  Eligible Time To Yesteday: ' . $eligible_total_time, __FILE__, __LINE__, __METHOD__, 10 );
								} else {
									Debug::Text( '  ERROR: No eligible dates...', __FILE__, __LINE__, __METHOD__, 10 );
								}
							}
							...
							if ( $ap_obj->isHourBasedLengthOfService() == true ) {
								...
								if ( $inception_total_time == false ) { //Try to only call to the DB once for the entire range.
									...
									$base_inception_total_time = $this->getSumUserDateTotalData( $this->filterUserDateTotalDataByContributingShiftPolicy( $ap_obj->getModifiedHireDate( $this->getUserObject() ), ( $first_date_stamp - 86400 ), $this->contributing_shift_policy[$ap_obj->getContributingShiftPolicy()], [ 20, 25, 30, 40, 100, 110 ] ) );
								}
								$additional_inception_total_time = $this->getSumUserDateTotalData( $this->filterUserDateTotalDataByContributingShiftPolicy( $first_date_stamp, $date_stamp, $this->contributing_shift_policy[$ap_obj->getContributingShiftPolicy()], [ 20, 25, 30, 40, 100, 110 ] ) );
								$inception_total_time = TTMath::add( $base_inception_total_time, $additional_inception_total_time );
							...
							$user_date_total_rows = $this->filterUserDateTotalDataByContributingShiftPolicy( $date_stamp, $date_stamp, $this->contributing_shift_policy[$ap_obj->getContributingShiftPolicy()], [ 20, 25, 30, 40, 100, 110 ] );
							if ( is_array( $user_date_total_rows ) && count( $user_date_total_rows ) > 0 ) {
								foreach ( $user_date_total_rows as $udt_obj ) {
									...
									//Handle eligibility criteria here.
									$udt_total_time = $udt_obj->getTotalTime();
									if ( $udt_total_time > 0 && $ap_obj->getEligiblePeriod() != 0 && ( $ap_obj->getMinimumEligibleTime() > 0 || $ap_obj->getMaximumEligibleTime() > 0 ) ) {
										if ( ( $minimum_eligible_total_time + $udt_total_time ) >= 0 ) { //>= 0 is required here so when the minimum time is exactly 40hrs and they worked exactly 40hrs in the week, it will still accrue.
											if ( $ap_obj->getMaximumEligibleTime() > 0 && ( $maximum_eligible_total_time + $udt_total_time ) > $ap_obj->getMaximumEligibleTime() ) {
												if ( $maximum_eligible_total_time < $ap_obj->getMaximumEligibleTime() ) {
													$udt_total_time = ( $ap_obj->getMaximumEligibleTime() - $maximum_eligible_total_time );
												} else {
													$udt_total_time = 0;
												}
											} else if ( $ap_obj->getMinimumEligibleTime() > 0 ) {
												$udt_total_time = min( $udt_total_time, ( $minimum_eligible_total_time + $udt_total_time ) );

												if ( $ap_obj->getMinimumEligibleApplyRetroactive() == true && $minimum_eligible_total_time < 0 ) {
													$udt_total_time += $ap_obj->getMinimumEligibleTime();
												}
											}
										} else {
											$udt_total_time = 0;
										}
										$minimum_eligible_total_time += $udt_obj->getTotalTime();
										$maximum_eligible_total_time += $udt_obj->getTotalTime();
									}
									...
									Debug::Text( '  Original Balance: ' . $original_balance . ' Adjustment: ' . $original_balance_adjustment . ' New Balance: ' . TTMath::add( $original_balance, $original_balance_adjustment ), __FILE__, __LINE__, __METHOD__, 10 );
									...
									if ( is_object( $milestone_obj ) ) {
										...
										if ( isset( $accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id] ) ) {
											$tmp_accrual_balance = TTMath::ArraySum( $accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id] );
										} else {
											$tmp_accrual_balance = 0;
										}

										$accrual_balance = TTMath::add( $original_balance, TTMath::add( $original_balance_adjustment, TTMath::add( $prev_original_balance_adjustment, $tmp_accrual_balance ) ) );
										...
										if ( $milestone_obj->getAnnualMaximumTime() > 0 ) {
											...
											$annual_accrued_amount = TTMath::add( $tmp_accrual_balance, $alf->getSumByUserIdAndAccrualPolicyAccountAndTypeAndStartDateAndEndDate( $this->getUserObject()->getId(), $ap_obj->getAccrualPolicyAccount(), [ 70, 76 ], $ap_obj->getCurrentMilestoneRolloverDate( $date_stamp, $this->getUserObject(), true ), $date_stamp ) );
										} else {
											$annual_accrued_amount = false;
										}
										Debug::Text( '	Current Balance: ' . $accrual_balance . ' (Original: ' . $original_balance . ' TMP: ' . $tmp_accrual_balance . ' UDT Total Time: ' . $udt_total_time . ') Accrual Policy: ' . $ap_obj->getName() . '(' . $ap_obj->getId() . ') Annual Accrued Amount: ' . $annual_accrued_amount, __FILE__, __LINE__, __METHOD__, 10 );
										...
										if ( ( $milestone_obj->getMaximumTime() == 0 && $milestone_obj->getAnnualMaximumTime() == 0 )
											...
											$accrual_amount = $ap_obj->calcAccrualAmount( $milestone_obj, $udt_total_time, 0 );

											if ( $accrual_amount > 0 ) {
												//If Annual Maximum time is set to 0, make that unlimited.
												if ( $milestone_obj->getAnnualMaximumTime() > 0 && TTMath::add( $annual_accrued_amount, $accrual_amount ) > $milestone_obj->getAnnualMaximumTime() ) {
													$accrual_amount = TTMath::sub( $milestone_obj->getAnnualMaximumTime(), $annual_accrued_amount );
												}

												//If Maximum time is set to 0, make that unlimited.
												if ( $milestone_obj->getMaximumTime() > 0 && TTMath::add( $accrual_balance, $accrual_amount ) > $milestone_obj->getMaximumTime() ) {
													$accrual_amount = TTMath::sub( $milestone_obj->getMaximumTime(), $accrual_balance );
												}

												//Debug::Text('	UDT Key: '. $udt_key .' Object Type ID: '. $udt_obj->getObjectType(), __FILE__, __LINE__, __METHOD__, 10);
												Debug::Text( '	Min/Max Adjusted Accrual Amount: ' . $accrual_amount . ' (Adjusted From: ' . $udt_total_time . ') Limits: Max Balance: ' . $milestone_obj->getMaximumTime() . ' Annual Max: ' . $milestone_obj->getAnnualMaximumTime() . ' Date: ' . TTDate::getDate( 'DATE', $date_stamp ), __FILE__, __LINE__, __METHOD__, 10 );
												...
												if ( isset( $accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id][$date_stamp] ) ) {
													$accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id][$date_stamp] = TTMath::add( $accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id][$date_stamp], $accrual_amount );
												} else {
													$accrual_compact_arr[$accrual_policy_account_id][$accrual_policy_id][$date_stamp] = $accrual_amount;
												}

												$prev_original_balance_adjustment = TTMath::add( $prev_original_balance_adjustment, $original_balance_adjustment );
								...
								if ( isset( $accrual_pay_formula_arr[$accrual_policy_account_id][$date_stamp] ) ) {
									//There is an absence on this day without any punches, so we still need to adjust the balance if the absence affected it, then move on to the next day.
									Debug::Text( '	No UDT records to calculate accrual on. Adjust the balance then move to the next day...', __FILE__, __LINE__, __METHOD__, 10 );
									$original_balance = TTMath::add( $original_balance, $accrual_pay_formula_arr[$accrual_policy_account_id][$date_stamp] );
									unset( $accrual_pay_formula_arr[$accrual_policy_account_id][$date_stamp] ); //Once its used, unset it so it can't be used again.
								}
							}
						} else {
		...
		if ( $current_employed_days >= $holiday_policy_obj->getMinimumEmployedDays()
			...
			if ( $holiday_policy_obj->getType() == 20 || $holiday_policy_obj->getType() == 30 ) {
				...
				if ( $holiday_policy_obj->getShiftOnHolidayType() > 0 ) {
					if ( isset( $this->contributing_shift_policy[$holiday_policy_obj->getEligibleContributingShiftPolicy()] ) ) { //Make sure eligible contributing shift policy is defined.
...
//						}
				...
				if ( $ignore_after_eligibility == true ) {
					...
					if ( $holiday_policy_obj->getMinimumWorkedAfterDays() > 0 && $holiday_policy_obj->getMinimumWorkedAfterPeriodDays() > 0 && isset( $this->contributing_shift_policy[$holiday_policy_obj->getEligibleContributingShiftPolicy()] ) ) {
						...
						} else if ( $holiday_policy_obj->getWorkedAfterScheduledDays() == 2 ) {  //Holiday Week Days
							//Start/End date should reflect weeks, no days here.
	...
	function getHolidayTime( $date_stamp, $holiday_policy_obj ) {
		if ( $holiday_policy_obj->getType() == 30 && isset( $this->contributing_shift_policy[$holiday_policy_obj->getContributingShiftPolicy()] ) ) { //Average
			...
			if ( $last_days_worked_count > 0 ) {
				$avg_seconds_worked_per_day = TTMath::div( $total_seconds_worked, $last_days_worked_count );
				...
				if ( is_object( $holiday_obj->getHolidayPolicyObject() )
					...
					if ( $holiday_time >= 0 ) {
						...
						if ( !isset( $this->user_date_total[$this->user_date_total_insert_id] ) ) {
							...
							$udtf->setHourlyRate( $this->getHourlyRate( $holiday_obj->getHolidayPolicyObject()->getAbsencePolicyObject()->getPayFormulaPolicy(), $udtf->getPayCode(), $date_stamp, $udtf->getBaseHourlyRate(), $udtf ) );
	...
	function getHolidayData( $start_date, $end_date, $enable_recalculate_holiday = true ) {
		...
		//  Since we don't know if the employee is working a shift near midnight in the week, we have to just always expand our holiday search by one day in either direction, just to cover our basis.
		$maximum_shift_time = (int)$this->pay_period_schedule_obj->getMaximumShiftTime();
		$start_date = TTDate::getBeginDayEpoch( TTDate::incrementDate( ( $start_date - $maximum_shift_time ), -1, 'day' ) );
		$end_date = TTDate::getBeginDayEpoch( TTDate::incrementDate( ( $end_date + $maximum_shift_time ), 1, 'day' ) );
	...
	function getHolidayPolicy( $date_stamp ) {
		...
		if ( $hplf->getRecordCount() > 0 ) {
			...
			foreach ( $hplf as $hp_obj ) {
				...
				if ( $hp_obj->getMinimumWorkedDays() > 0 && $hp_obj->getMinimumWorkedPeriodDays() > 0 && ( $hp_obj->getWorkedScheduledDays() == 1 || $hp_obj->getWorkedAfterScheduledDays() == 1 ) ) {
	...
	function getSumUserDateTotalData( $user_date_total_arr ) {
		...
		if ( is_array( $user_date_total_arr ) ) {
			foreach ( $user_date_total_arr as $udt_obj ) {
				//Debug::text( 'UDT: Date: '. TTDate::getDate('DATE', $udt_obj->getDateStamp() ) .' Object Type: '. $udt_obj->getObjectType() .' Total Time: ' . $udt_obj->getTotalTime(), __FILE__, __LINE__, __METHOD__, 10 );
	...
	function getShiftData( $user_date_total_arr, $epoch = null, $filter = null, $maximum_shift_time = null, $new_shift_trigger_time = null ) {
		...
		if ( is_array( $user_date_total_arr ) ) {
			...
			foreach ( $user_date_total_arr as $udt_key => $udt_obj ) {
				...
				//When there is recursive overtime calculations, a UDT record could get reduced to 0hrs. So we need to make sure those are skipped and don't cause new shifts to start due to a jump in end time.
				if ( $total_time == 0 && $udt_obj->getStartTimeStamp() == $udt_obj->getEndTimeStamp() ) {
					Debug::text('  Skipping UDT record due to total time=0 and start/end time match: '. $udt_key, __FILE__, __LINE__, __METHOD__, 10);
					continue;
				}
...
//				}
	...
	function sortUserDateTotalDataByDateAndObjectTypeAndStartTimeStampAndID( $a, $b ) {
		...
		//$a_date_stamp = $a->getDateStamp(); //Minor optimization by using variable instead of function call.
		//$b_date_stamp = $b->getDateStamp(); //Minor optimization by using variable instead of function call.
		$a_date_stamp = $a->data['date_stamp']; //Minor optimization by using variable instead of function call.
		$b_date_stamp = $b->data['date_stamp']; //Minor optimization by using variable instead of function call.

		if ( $a_date_stamp == $b_date_stamp ) {
			$a_object_type_id = $a->getObjectType();
			$b_object_type_id = $b->getObjectType();
			if ( $a_object_type_id == $b_object_type_id ) {

				$a_start_time_stamp = $a->getStartTimeStamp(); //**Must use getStartTimeStamp() for integer comparisons. Minor optimization by using variable instead of function call.
				$b_start_time_stamp = $b->getStartTimeStamp(); //**Must use getStartTimeStamp() for integer comparisons. Minor optimization by using variable instead of function call.
				if ( $a_start_time_stamp == $b_start_time_stamp ) {
					return ( $a->getID() < $b->getID() ) ? ( -1 ) : 1;
				} else {
					return ( $a_start_time_stamp < $b_start_time_stamp ) ? ( -1 ) : 1;
				}
			} else {
				return ( $a_object_type_id < $b_object_type_id ) ? ( -1 ) : 1;
			}
		} else {
			return ( $a_date_stamp < $b_date_stamp ) ? ( -1 ) : 1;
		}
	}
	 ...
	 * @return int
	 */
	function sortUserDateTotalDataByDateAndStartTimeStampAndObjectTypeAndID( $a, $b ) {
		//Sort order obtained from: getUserDateTotalData(), if changes are needed, change there too.
		//  Need to sort by date, then start_time_stamp, otherwise lunch/break auto-add policies will be considered in OT calculation after all regular time, and that can throw things off.
		//array( 'a.date_stamp' => 'asc', 'a.start_time_stamp' => 'asc', 'a.object_type_id' => 'asc', 'a.id' => 'asc' )
		//$a_date_stamp = $a->getDateStamp(); //Minor optimization by using variable instead of function call.
...
//			$b_object_type_id = ( $b->getObjectType() == 25 ) ? 20 : $b->getObjectType();
	 ...
	 * @return bool
	 */
	function sortUserDateTotalData( &$udtlf, $sort_function_name = 'sortUserDateTotalDataByDateAndStartTimeStampAndObjectTypeAndID' ) {
			...
			foreach ( $policy_arr as $policy_obj ) {
				...
				if ( isset( $this->contributing_shift_policy[$policy_obj->getContributingShiftPolicy()] ) ) {
					...
					if ( is_a( $policy_obj, 'PremiumPolicyFactory' ) ) {
						$order = (string)( 40 . str_pad( $policy_obj->getCalculationOrder(), 5, 0, STR_PAD_LEFT ) . str_pad( $policy_obj->getCreatedDate(), 11, 0, STR_PAD_LEFT ) . TTUUID::convertUUIDtoInt( $policy_obj->getID(), true ) );
					} else if ( is_a( $policy_obj, 'RegularPolicyFactory' ) ) {
						$order = (string)( 20 . str_pad( $policy_obj->getCalculationOrder(), 5, 0, STR_PAD_LEFT ) . str_pad( $policy_obj->getCreatedDate(), 11, 0, STR_PAD_LEFT ) . TTUUID::convertUUIDtoInt( $policy_obj->getID(), true ) );
					} else if ( is_a( $policy_obj, 'OvertimePolicyFactory' ) ) {
						$order = (string)( 30 . str_pad( 9999, 5, 0, STR_PAD_LEFT ) . str_pad( $policy_obj->getCreatedDate(), 11, 0, STR_PAD_LEFT ) . TTUUID::convertUUIDtoInt( $policy_obj->getID(), true ) );
					} else {
						$order = (string)( 99 . str_pad( 9999, 5, 0, STR_PAD_LEFT ) . str_pad( $policy_obj->getCreatedDate(), 11, 0, STR_PAD_LEFT ) . TTUUID::convertUUIDtoInt( $policy_obj->getID(), true ) );
					}
	 ...
	 * @param int $end_date        EPOCH
	 * @param ContributingShiftPolicyFactory $contributing_shift_policy_obj
				...
				if ( ( $object_type_ids == null || in_array( $udt_obj->getObjectType(), $object_type_ids ) ) ) {
					if ( ( $pay_code_ids == null || in_array( $udt_obj->getPayCode(), $pay_code_ids ) )
						...
						(
							...
							if ( $shift_data === null && $contributing_shift_policy_obj->isShiftDataRequired() == true ) {
...
//							Debug::text('Checking: Key: '. $udt_key .' UDT ID: '. $udt_obj->getID() .' Date Stamp: '. TTDate::getDate('DATE', $udt_obj->getDateStamp() ) .' Start Time: '. TTDate::getDate('DATE+TIME', $udt_obj->getStartTimeStamp() ) .' End Time: '. TTDate::getDate('DATE+TIME', $udt_obj->getEndTimeStamp() ) .' Pay Code ID: '. $udt_obj->getPayCode() .' Total Time: '. $udt_obj->getTotalTime() .'($'. (float)$udt_obj->getTotalTimeAmount().' Base Rate: '.(float)$udt_obj->getBaseHourlyRate().') Object Type ID: '. $udt_obj->getObjectType() .' SRC Object ID: '. $udt_obj->getSourceObject() .' ID: '. $udt_obj->getID(), __FILE__, __LINE__, __METHOD__, 10);
							...
							if ( getTTProductEdition() == TT_PRODUCT_COMMUNITY
									...
									(
											(
												$contributing_shift_policy_obj->doesApplyToAllDays( $udt_obj_date_stamp ) == true //Optimization for date checking.
												||
												(
											...
											&& $contributing_shift_policy_obj->isActiveDifferential( $udt_obj, $this->getUserObject() ) //This has to be separate from doesApplyToAllDays() above.
									)
							) {
								...
								if ( !isset( $already_split_key[$udt_key] ) && ( $contributing_shift_policy_obj->getIncludeShiftType() == 100 || $contributing_shift_policy_obj->getIncludeShiftType() == 150 ) ) {
									//Handle partial shifts here.
									...
									foreach ( $split_udt_obj_arr as $tmp_udt_key => $tmp_udt_obj ) {
										...
										//Make sure when splitting UDT records, we clear the cached $shift_data as well so that can be recalculated with the post-split UDT records.
										// This is critical for $this->getIncludeShiftType() == 150 and ContributingShiftPolicyFactory->isActiveFilterTime()
										$shift_data = null;
									}
									unset( $split_udt_obj_arr, $tmp_udt_key, $tmp_udt_obj );

									$this->sortUserDateTotalData( $this->user_date_total ); //When UDT records are split, we need to re-sort. Espescially important for calculating overtime policies.
								} else {
	 ...
	 * @return array
	 */
	function filterUserDateTotalDataByPayCodeIDs( $start_date, $end_date, $pay_code_ids = null, $object_type_ids = null ) {
		$udtlf = $this->user_date_total;
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			if ( $start_date == $end_date ) {
				$start_date = $end_date = TTDate::getMiddleDayEpoch( $start_date ); //If start/end date match avoid calling getMiddleDayEpoch() twice.
			} else {
				$start_date = TTDate::getMiddleDayEpoch( $start_date ); //Optimization - Move outside loop.
				$end_date = TTDate::getMiddleDayEpoch( $end_date );     //Optimization - Move outside loop.
			}
			...
			foreach ( $udtlf as $udt_key => $udt_obj ) {
				...
				if ( $udt_obj_date_stamp >= $start_date
						...
						&& ( $object_type_ids == null || in_array( $udt_obj->getObjectType(), (array)$object_type_ids ) )
				) {
					$retarr[$udt_key] = $udt_obj;
				}
			}
	 ...
	 * @return array|bool
	 */
	function getUserDateTotalActiveAfter( $start_date, $end_date, $active_after, $policy_total_time ) {
		$udtlf = $this->user_date_total;
		if ( is_array( $udtlf ) && count( $udtlf ) > 0 ) {
			//Since we need to fit auto-deduct meals into the shift time the employee actually worked, the meal has to end at the active time, and start 30/60mins before.
			//  For example if the meal was active after 6hrs and the shift was 8A -> 2P, the meal needs to be from 1P to 2P.
			$active_after = ( $active_after - $policy_total_time );

			$daily_total_time = 0;
			$active_after_remaining = $active_after;
			...
			foreach ( $udtlf as $udt_key => $udt_obj ) {
				...
				if ( $udt_obj_date_stamp >= $start_date
					...
					if ( $daily_total_time >= $active_after ) {
						//Get the exact time of day within the UDT object where the active_time is triggered.
						$active_after_start_time = ( $udt_obj->getStartTimeStamp() + $active_after_remaining );
						Debug::text( '  Found UDT at Active After Time: '. $daily_total_time .' Start Time: '. TTDate::getDate('DATE+TIME', $active_after_start_time ), __FILE__, __LINE__, __METHOD__, 10 );

						return [ 'start' => $active_after_start_time, 'udt_obj' => $udt_obj ];
					} else {
						$active_after_remaining -= $udt_obj->getTotalTime();
					}
				}
			}
		}
	...
	function getUserDateTotalData( $start_date = null, $end_date = null ) {
		...
		//If SORT order is changed, also change it in: sortUserDateTotalDataByDateAndStartTimeStampAndObjectTypeAndID()
		$udtlf->getByUserIdAndStartDateAndEndDateAndExcludeId( $this->getUserObject()->getId(), $start_date, $end_date, array_keys( (array)$this->user_date_total ), null, [ 'a.date_stamp' => 'asc', 'a.start_time_stamp' => 'asc', 'a.object_type_id' => 'asc', 'a.id' => 'asc' ] );
	...
	function getRequiredData( $start_date_stamp, $end_date_stamp, $enable_recalculate_holiday = true ) {
		...
		if ( count( $date_arr ) > 0 ) {
			...
			if ( is_array( $date_range ) ) {
				...
				if ( ( $this->isUserDateTotalData() == true || $this->isScheduleData() ) && $this->getFlag( 'regular' ) == true ) { //An absence policy could exist that calculates regular/overtime/premium, with no other time on the timesheet.
					$this->getRegularTimePolicy();
				}
				if ( ( $this->isUserDateTotalData() == true || $this->isScheduleData() ) && $this->getFlag( 'overtime' ) == true ) { //An absence policy could exist that calculates regular/overtime/premium, with no other time on the timesheet.
					$this->getOverTimePolicy();
				}
				if ( ( $this->isUserDateTotalData() == true || $this->isScheduleData() ) && $this->getFlag( 'premium' ) == true ) { //An absence policy could exist that calculates regular/overtime/premium, with no other time on the timesheet.
				...
				//  Need to check if UserDateTotal OR Schedule data exists, as the custom could have hour based accrual policies on absence time that is entered from the schedule.
				if ( ( $this->isUserDateTotalData() == true || $this->isScheduleData() ) && ( $this->getFlag( 'meal' ) == true || $this->getFlag( 'undertime_absence' ) == true || $this->getFlag( 'break' ) == true || $this->getFlag( 'regular' ) == true || $this->getFlag( 'overtime' ) == true || $this->getFlag( 'premium' ) == true || $this->getFlag( 'accrual' ) == true || $this->getFlag( 'holiday' ) == true || $this->getFlag( 'schedule_absence' ) == true ) ) {
				...
				if ( $this->getFlag( 'meal' ) == true || $this->getFlag( 'undertime_absence' ) == true || $this->getFlag( 'break' ) == true || $this->getFlag( 'regular' ) == true || $this->getFlag( 'overtime' ) == true || $this->getFlag( 'premium' ) == true || $this->getFlag( 'accrual' ) == true || $this->getFlag( 'holiday' ) == true || $this->getFlag( 'schedule_absence' ) == true ) {
					...
					$this->getPayFormulaPolicy( $start_date_stamp );
	...
	function addPendingCalculationDate( $start_date, $end_date = null ) {
		...
		if ( count( $pending_dates ) == 1 ) {
			...
			//Debug::Arr( array_map( ['TTDate','getISODateStamp'], $pending_dates ), 'Add Pending Dates: '. count($pending_dates), __FILE__, __LINE__, __METHOD__, 10);
		}

		if ( is_array( $pending_dates ) && count( $pending_dates ) > 0 ) {
			foreach ( $pending_dates as $tmp_date ) {
				if ( $tmp_date > 0 ) { //Make sure no bogus dates like -14400 or something has been passed in.
					...
					Debug::Text( '  Skipping bogus date: ' . $tmp_date, __FILE__, __LINE__, __METHOD__, 10 );
				}
			}
	...
	function setPayPeriodFromDate( $date_stamp ) {
		$date_stamp = TTDate::getBeginDayEpoch( $date_stamp ); //Make sure we always use begin day epoch, as we use it as a key in the cache.

		if ( isset( $this->pay_period_from_date_cache[$date_stamp] ) ) {
			$this->pay_period_obj = $this->getPayPeriodObject( $this->pay_period_from_date_cache[$date_stamp]['pay_period_id'] );
			$this->pay_period_schedule_obj = $this->getPayPeriodScheduleObject( $this->pay_period_from_date_cache[$date_stamp]['pay_period_schedule_id'] );
		} else {
			$pplf = TTnew( 'PayPeriodListFactory' ); /** @var PayPeriodListFactory $pplf */
			$pplf->getByUserIdAndEndDate( $this->getUserObject()->getId(), $date_stamp );
			if ( $pplf->getRecordCount() == 1 ) {
				...
				//Expand the pay period date range to individual cache records.
				foreach ( TTDate::getDatePeriod( $this->pay_period_obj->getStartDate(), $this->pay_period_obj->getEndDate(), 'P1D' ) as $tmp_date_stamp ) { //$tmp_date_stamp should always be begin day epoch.
					$this->pay_period_from_date_cache[$tmp_date_stamp] = [ 'pay_period_id' => $this->pay_period_obj->getId(), 'pay_period_schedule_id' => $this->pay_period_schedule_obj->getId() ];
				}
			} else {
			...
			//Populate pay period and pay period schedule cache while we have the data.
			if ( is_object( $this->pay_period_obj ) ) {
				$this->pay_periods[$this->pay_period_obj->getId()] = $this->pay_period_obj;
			}
			$this->pay_period_schedules[$this->pay_period_schedule_obj->getId()] = $this->pay_period_schedule_obj;
		}
	...
	private function _calculate( $date_stamp ) {
		...
		if ( is_object( $this->pay_period_schedule_obj )
			...
			//When using pay formula averaging, we need to always recalculate the days to the start of the week, so their average hourly rates can be updated.
			//  This is to handle the case where Mon & Tue have Daily OT that uses averaging, then Wed has no OT at all. Since Wed has no OT, getAverageHourlyRate() wouldn't be called, and it triggers recalculating other days in the week.
			//  So this will catch the case where getAverageHourlyRate() is not called, but days earlier in the week still need to be recalculated.
			if ( $this->pay_formula_calculate_average_hourly_rates == true ) {
				$this->addPendingCalculationDate( TTDate::getBeginWeekEpoch( $date_stamp, $this->start_week_day_id ), $date_stamp );
			}
			...
			if ( $this->getFlag( 'meal' ) == true || $this->getFlag( 'undertime_absence' ) == true || $this->getFlag( 'break' ) == true || $this->getFlag( 'regular' ) == true || $this->getFlag( 'overtime' ) == true || $this->getFlag( 'premium' ) == true || $this->getFlag( 'accrual' ) == true || $this->getFlag( 'holiday' ) == true || $this->getFlag( 'schedule_absence' ) == true ) {
				...
				//
				//  NOTE: What if an absence record as the only record on the day is deleted, there won't be UDT records to count in that case, and OT calculation on previous days won't get triggered (it would need to include absences to be affected).
				//        getIsDeletedData() should be used to trigger this if any records are deleted.
				...
				if ( count( $maximum_daily_total_time_udt ) > 0 || $this->getIsDeletedData() == true ) {
					...
					if ( $this->pay_formula_calculate_average_hourly_rates == true && ( $this->getFlag( 'absence' ) == true || $this->getFlag( 'regular' ) == true || $this->getFlag( 'overtime' ) == true || $this->getFlag( 'premium' ) == true ) ) {
						//Make sure we do this after all policies have been calculated.
						$this->calculateAverageHourlyRates( $this->user_date_total );
					}
				} else {
	 ...
	 *
	 * Max Retry of 9 with a 0.50 second sleep between each retry results in about 127 seconds total waiting until exit.
	 *    This should be fairly high by default to punches coming from a tablet or something don't get lost.
	 * @param bool $date_stamp
	 * @return bool
	 */
	function calculate( $date_stamp = false, $retry_max_attempts = 8, $retry_sleep = 0.50 ) {
		...
		//Lock recalculating timesheets for the user for any dates. As accrual_balance updating can cause serialization errors.
		// **IMPORTANT**: Must come before the transaction is started, and be released after its committed to avoid concurrent update failures.
		$this->getUserObject()->acquireAdvisoryLock( 'calculate_policy:'. $this->getUserObject()->getId(), false, $retry_max_attempts, $retry_sleep ); //Throws exception on failure.
		...
		try {
			...
			//If an exception is thrown, we need to fail the transaction and commit it to properly maintain the transaction depth with the transaction started above.
			$this->getUserObject()->FailTransaction();
			$this->getUserObject()->CommitTransaction();

			throw $e; //Re-throw exception so it can still be handled by the caller.
		}
	...
	function Save() {
		...
		try {
			//Disable audit log details for performance reasons, as its rarely needed when calculating timesheets.
			global $config_vars;
			$disable_audit_log_detail = $config_vars['other']['disable_audit_log_detail'] ?? false;
			...
			$config_vars['other']['disable_audit_log_detail'] = $disable_audit_log_detail; //Re-enable audit log details.
		} catch ( Exception $e ) {
			//If an exception is thrown, we need to fail the transaction and commit it to properly maintain the transaction depth with the transaction started in calculate().
			$this->getUserObject()->FailTransaction();
			$this->getUserObject()->CommitTransaction();
		...
		$this->getUserObject()->releaseAdvisoryLock( 'calculate_policy:'. $this->getUserObject()->getId() ); //Release after the commit to avoid concurrent update failures in higher isolation levels.
	 ...
	 * @param int|array $start_date This can be an array of dates.
	 * @param int|null $end_date
	 * @return false|mixed
	 * @throws ReflectionException
	 */
	static function reCalculateForJobQueue( $user_id, $flags, $start_date, $end_date = null ) {
		if ( TTUUID::isUUID( $user_id ) == false || $user_id == TTUUID::getZeroID() ) {
			return false;
		}

		if ( $flags == 'calcQuickExceptions' ) {
			$flags = [
					'meal'              => false,
		...
		if ( $ulf->getRecordCount() == 1 ) {
			$user_obj = $ulf->getCurrent(); /** @var $user_obj UserFactory */
			$transaction_function = function () use ( $user_obj, $flags, $start_date, $end_date ) {
				...
				if ( $cp->calculate( false, -1, 20 ) == true ) { //This sets timezone itself. -- 20s timeout, as it does a retryTransaction() below with 2 attempts. Want it to be shorter than the foreground APIPunch->setPunch() call of 60s.
					$cp->Save();
			...
			return $user_obj->RetryTransaction( $transaction_function, 2, 3 ); //Set retry_sleep this fairly high so real-time punches have a chance to get saved between retries.
		}
	 ...
	 * @param array $plf
	 * @param mixed $user_id
	 * @param bool|int $date_stamp
	 * @param mixed $ep_obj
	 * @param int $type_id
	 * @param array $current_exceptions
	 * @return array
	 */
	public function calculateExceptionT2( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, array $current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
				...
				if ( $p_obj->getStatus() == 10 && is_object( $p_obj->getPunchControlObject() ) ) {
					//If no Punch Tags are setup, ignore checking them.
					if ( empty( $p_obj->getPunchControlObject()->getPunchTag() )
							|| $p_obj->getPunchControlObject()->getPunchTag() == TTUUID::getZeroID()
							|| $p_obj->getPunchControlObject()->getPunchTag() == TTUUID::getNotExistID() ) {
						//Make sure at least one punch tag exists before triggering exception.
						$jlf = TTNew( 'PunchTagListFactory' ); /** @var PunchTagListFactory $jlf */
	 ...
	 * @param mixed $ep_obj
	 * @param int $type_id
	 * @param mixed $current_exceptions
	 * @return array
	 */
	public function calculateExceptionT1( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			...
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
					if ( is_object( $p_obj->getPunchControlObject() )
							&& is_array( $p_obj->getPunchControlObject()->getPunchTag() ) ) {

						//If the punch tag is all the same across many punches, don't look it up every time.
						$punch_tags = $p_obj->getPunchControlObject()->getPunchTag();

						//FIXME: Might be faster to loop over each $punch_tag and get it from SQL 1-by-1, as it can be cached that way. If more than 1 punch tag are assigned to a punch, the below method can't be cached at all.
						if ( !isset( $pt_obj ) || !( count( $punch_tags ) == 1 && $pt_obj->getId() == $punch_tags[0] ) ) {
							$ptlf->getById( $punch_tags );
						}

						if ( $ptlf->getRecordCount() > 0 ) {
							foreach ( $ptlf as $pt_obj ) { /** @var PunchTagFactory $pt_obj */
								if ( $pt_obj->isAllowedUser( $user_id, $p_obj->getPunchControlObject()->getBranch(), $p_obj->getPunchControlObject()->getDepartment(), $p_obj->getPunchControlObject()->getJob(), $p_obj->getPunchControlObject()->getJobItem() ) == false ) {
									$current_exceptions[] = [
											'user_id'             => $user_id,
											'date_stamp'          => $date_stamp,
											'exception_policy_id' => $ep_obj->getId(),
									...
									break; //Break so do not cause multiple identical exceptions for same punch.
								}
							}
						}
						//else {
						//	Debug::text( 'Punch Tag not found!', __FILE__, __LINE__, __METHOD__, 10 );
						//}
					}
				}
			}
			unset( $pt_obj, $ptlf, $p_obj, $punch_tags );
		}
	 ...
	 * @return array
	 */
	public function calculateExceptionJ4( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
				$add_exception = false;
	 ...
	 * @return array
	 */
	public function calculateExceptionJ3( array $plf, bool|int $date_stamp, mixed $user_id, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			$jlf = TTnew( 'JobListFactory' ); /** @var JobListFactory $jlf */
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
					if ( is_object( $p_obj->getPunchControlObject() )
						...
						if ( $jlf->getRecordCount() > 0 ) {
							$j_obj = $jlf->getCurrent(); /** @var JobFactory $j_obj */
	 ...
	 * @return array
	 */
	public function calculateExceptionJ2( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			$jlf = TTnew( 'JobListFactory' ); /** @var JobListFactory $jlf */
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
	 ...
	 * @return array
	 */
	public function calculateExceptionJ1( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			$jlf = TTnew( 'JobListFactory' ); /** @var JobListFactory $jlf */
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
					if ( is_object( $p_obj->getPunchControlObject() )
						...
						if ( $jlf->getRecordCount() > 0 ) {
							...
							if ( $j_obj->isAllowedUser( $user_id, $p_obj->getPunchControlObject()->getBranch(), $p_obj->getPunchControlObject()->getDepartment() ) == false ) {
								$current_exceptions[] = [
										'user_id'             => $user_id,
										'date_stamp'          => $date_stamp,
										'exception_policy_id' => $ep_obj->getId(),
	 ...
	 * @return array
	 */
	public function calculateExceptionV1( mixed $ep_obj, int $current_epoch, bool|int $date_stamp, mixed $user_id, int $type_id, mixed &$current_exceptions ): bool {
	 ...
	 * @param int $type_id
	 * @param int $current_epoch
	 * @param int $premature_delay
	 * @param mixed $ep_obj
	 * @param mixed $current_exceptions
	 * @return array
	 */
	public function calculateExceptionB5( array $plf, bool|int $date_stamp, mixed $user_id, int $type_id, int $current_epoch, int $premature_delay, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionB3AndB4( array $plf, bool|int $date_stamp, mixed $user_id, int $type_id, int $current_epoch, int $premature_delay, mixed $ep_obj, mixed &$current_exceptions, &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			...
			if ( isset( $break_punch_arr ) ) {
				...
				foreach ( $break_punch_arr as $pair => $time_stamp_arr ) {
					if ( isset( $time_stamp_arr[10] ) && isset( $time_stamp_arr[20] ) ) {
						$break_total_time = TTMath::sub( $time_stamp_arr[10], $time_stamp_arr[20] );
	...
	/**
	 * @param array $plf
	 * @param mixed $break_out_timestamp
	 * @param bool|int $date_stamp
	 * @param mixed $user_id
	 * @param mixed $ep_obj
	 * @param int $type_id
	 ...
	 * @return array
	 */
	public function calculateExceptionB1AndB2( array $plf, bool|int $date_stamp, mixed $user_id, mixed $ep_obj, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			//Get all break punches.
			$pair = 0;
			$x = 0;
	 ...
	 * @return array
	 */
	public function calculateExceptionL4( array $plf, bool|int $date_stamp, mixed $user_id, int $current_epoch, int $premature_delay, int $type_id, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			...
			if ( is_object( $meal_policy_obj ) || $meal_policy_obj === true ) {
				...
				$first_punch_obj = false;
				$last_punch_obj = false;
				$meal_punch = false;
				$meal_punch_time_stamp = false;
				$tmp_punch_total_time = 0;
				$tmp_punch_control_ids = [];
				foreach ( $plf as $p_obj ) {
					if ( !is_object( $first_punch_obj ) ) {
						$first_punch_obj = $p_obj;
					}

					if ( $meal_punch == false && $p_obj->getType() == 20 ) { //20 = Lunch
						Debug::text( 'Found meal Punch: ' . $p_obj->getTimeStamp(), __FILE__, __LINE__, __METHOD__, 10 );
						$meal_punch = true;
						$meal_punch_time_stamp = $p_obj->getTimeStamp();
						// break; //Don't break when lunch is detected, because we need to know if they have punched out to finish their shift or not for Late Lunch.
					}
				...
				if ( ( $meal_policy_obj === true || ( $p_obj->getTimeStamp() > ( $current_epoch - $premature_delay ) ) || ( $daily_total_time > $meal_policy_obj->getTriggerTime() && in_array( $meal_policy_obj->getType(), [ 15, 20 ] ) ) ) ) {
					//If the last punch is before the premature delay, make this a mature exception instead.
					//However if the employee has punched out for the end of their shift (Normal Out), then we don't need to mark it as pre-mature anymore.
					...
					if ( $type_id == 5
						...
						$type_id = 50;
					} else if ( $type_id == 50 && ( $daily_total_time == 0 && ( $current_epoch - $p_obj->getTimeStamp() ) < $meal_policy_obj->getTriggerTime() ) ) {
						//If the employee only has a single punch (no daily total time) and the difference between the current time and that punch time is less than the meal trigger time,
						// force it to be a pre-mature exception, as the meal trigger time has not passed yet.
						$type_id = 5;
					}

					//Once the employee punches out to end their shift, if they did not take a lunch at all, don't trigger Late Lunch, as the No Lunch should be triggered instead.
					if (
							(
									( $meal_punch == false && is_object( $p_obj ) && !( $p_obj->getType() == 10 && $p_obj->getStatus() == 20 ) && ( ( $p_obj->getTimeStamp() > ( $current_epoch - $premature_delay ) ) || $daily_total_time > ( $meal_policy_obj->getTriggerTime() + $ep_obj->getGrace() ) ) )
									||
									( $meal_punch == true && is_object( $first_punch_obj ) && $meal_punch_time_stamp > ( $first_punch_obj->getTimeStamp() + $meal_policy_obj->getTriggerTime() + $ep_obj->getGrace() ) )
							)
					) {
						Debug::text( 'Triggering Late Lunch exception! Type ID: '. $type_id, __FILE__, __LINE__, __METHOD__, 10 );
						$current_exceptions[] = [
								'user_id'             => $user_id,
								'date_stamp'          => $date_stamp,
								'exception_policy_id' => $ep_obj->getId(),
	 ...
	 * @return array
	 */
	public function calculateExceptionL3( array $plf, bool|int $date_stamp, mixed $user_id, int $type_id, int $current_epoch, int $premature_delay, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			//If they are scheduled or not, we can check for a meal policy and base our
			//decision off that. We don't want a No Lunch exception on a 3hr short shift though.
			//Also ignore this exception if the lunch is auto-deduct.
	 ...
	 * @return array
	 */
	public function calculateExceptionL1AndL2( array $plf, bool|int $date_stamp, mixed $user_id, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			...
			if ( isset( $lunch_punch_arr ) ) {
				...
				foreach ( $lunch_punch_arr as $pair => $time_stamp_arr ) {
					if ( isset( $time_stamp_arr[10] ) && isset( $time_stamp_arr[20] ) ) {
						$lunch_total_time = TTMath::sub( $time_stamp_arr[10], $time_stamp_arr[20] );
	 ...
	 * @return mixed
	 */
	public function calculateExceptionO2AndS9( array $plf, int $start_week_day_id, mixed $ep_obj, bool|int $date_stamp, mixed $user_id, int $type_id, mixed &$current_exceptions ): mixed {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionO1( array $plf, bool|int $date_stamp, mixed $ep_obj, mixed $user_id, int $type_id, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			//FIXME: Assign this exception to the last punch of the day, so it can be related back to a punch branch/department?
	 ...
	 * @param mixed $user_id
	 * @param mixed $current_exceptions
	 * @param mixed $first_punch_obj
	 * @return array
	 */
	public function calculateExceptionS8( array $plf, array $slf, bool|int $date_stamp, mixed $ep_obj, int $type_id, int $current_epoch, mixed $user_id, mixed &$current_exceptions ): bool {
	 ...
	 * @return array
	 */
	public function calculateExceptionS7( array $plf, array $slf, bool|int $date_stamp, mixed $ep_obj, mixed $user_id, int $type_id, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			//This ONLY takes in to account WORKED hours, not paid absence hours.
	 ...
	 * @return array
	 */
	public function calculateExceptionD3( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			$dlf = TTnew( 'DepartmentListFactory' ); /** @var DepartmentListFactory $dlf */
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
					if ( is_object( $p_obj->getPunchControlObject() )
							&& TTUUID::isUUID( $p_obj->getPunchControlObject()->getDepartment() ) && $p_obj->getPunchControlObject()->getDepartment() != TTUUID::getZeroID() && $p_obj->getPunchControlObject()->getDepartment() != TTUUID::getNotExistID() ) {
						//Found department punch, check department settings.

						//If the department is all the same across many punches, don't look it up every time.
						if ( !isset( $d_obj ) || ( $d_obj->getId() != $p_obj->getPunchControlObject()->getDepartment() ) ) {
							$dlf->getById( $p_obj->getPunchControlObject()->getDepartment() );
						}

						if ( $dlf->getRecordCount() > 0 ) {
							$d_obj = $dlf->getCurrent(); /** @var DepartmentFactory $d_obj */

							if ( $d_obj->isAllowedUser( $user_id, $p_obj->getPunchControlObject()->getBranch() ) == false ) {
								$current_exceptions[] = [
										'user_id'             => $user_id,
										'date_stamp'          => $date_stamp,
										'exception_policy_id' => $ep_obj->getId(),
							...
							//else {
							//	Debug::text('	 User allowed on Department!', __FILE__, __LINE__, __METHOD__, 10);
							//}
						} else {
							Debug::text( '	 Department not found!', __FILE__, __LINE__, __METHOD__, 10 );
						}
					} //else { //Debug::text('	   Not a Department Punch...', __FILE__, __LINE__, __METHOD__, 10);
				}
			}
			unset( $j_obj );
		}
	 ...
	 * @return array
	 */
	public function calculateExceptionD2( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 ) {
			$blf = TTnew( 'BranchListFactory' ); /** @var BranchListFactory $blf */
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getStatus() == 10 ) { //In punches
					if ( is_object( $p_obj->getPunchControlObject() )
							&& TTUUID::isUUID( $p_obj->getPunchControlObject()->getBranch() ) && $p_obj->getPunchControlObject()->getBranch() != TTUUID::getZeroID() && $p_obj->getPunchControlObject()->getBranch() != TTUUID::getNotExistID() ) {
						//Found branch punch, check branch settings.

						//If the branch is all the same across many punches, don't look it up every time.
						if ( !isset( $b_obj ) || ( $b_obj->getId() != $p_obj->getPunchControlObject()->getBranch() ) ) {
							$blf->getById( $p_obj->getPunchControlObject()->getBranch() );
						}

						if ( $blf->getRecordCount() > 0 ) {
							$b_obj = $blf->getCurrent(); /** @var BranchFactory $b_obj */

							if ( $b_obj->isAllowedUser( $user_id ) == false ) {
								$current_exceptions[] = [
										'user_id'             => $user_id,
										'date_stamp'          => $date_stamp,
										'exception_policy_id' => $ep_obj->getId(),
							...
							//else {
							//	Debug::text('	 User allowed on Branch!', __FILE__, __LINE__, __METHOD__, 10);
							//}
						} else {
							Debug::text( '	 Branch not found!', __FILE__, __LINE__, __METHOD__, 10 );
						}
					} //else { //Debug::text('	   Not a Branch Punch...', __FILE__, __LINE__, __METHOD__, 10);
				}
			}
			unset( $j_obj );
		}
	 ...
	 * @return array
	 */
	public function calculateExceptionD1( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
				$add_exception = false;
	 ...
	 * @return array
	 */
	public function calculateExceptionC1( array $plf, mixed $ep_obj, int $premature_delay, int $current_epoch, mixed $user_id, bool|int $date_stamp, int $type_id, mixed &$current_exceptions ): bool {
	 ...
	 * @return array
	 */
	public function calculateExceptionM4( array $plf, int $current_epoch, int $premature_delay, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionM3( array $plf, int $current_epoch, int $premature_delay, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionM2( array $plf, int $current_epoch, int $premature_delay, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionM1( array $plf, int $type_id, int $current_epoch, int $premature_delay, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
	 ...
	 * @throws DBError
	 */
	public function calculateExceptionG1( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 ) {
			foreach ( $plf as $p_obj ) {
				if ( $p_obj->getLongitude() != false && $p_obj->getLatitude() != false ) {
					$glf = TTnew( 'GEOFenceListFactory' ); /** @var GEOFenceListFactory $glf */

					//Check that GEO fences are assigned to cost centers first.
					// If no GEO fences are assigned to any of the cost centers, don't trigger the exception.
					// This prevents the user from having to assign at least one GEO fence to every cost center used at a certain level (ie: branches) when they turn G1 exception on.
					$glf->getByCompanyIdAndBranchAndDepartmentAndJobAndTaskAndPunchTag( $this->getUserObject()->getCompany(), $p_obj->getPunchControlObject()->getBranch(), $p_obj->getPunchControlObject()->getDepartment(), $p_obj->getPunchControlObject()->getJob(), $p_obj->getPunchControlObject()->getJobItem(), $p_obj->getPunchControlObject()->getPunchTag(), 1 );
					if ( $glf->getRecordCount() > 0 ) {
						$glf->getByCompanyIdAndGEOLocationAndBranchAndDepartmentAndJobAndTaskAndPunchTag( $this->getUserObject()->getCompany(), $p_obj->getLatitude(), $p_obj->getLongitude(), $p_obj->getPositionAccuracy(), $p_obj->getPunchControlObject()->getBranch(), $p_obj->getPunchControlObject()->getDepartment(), $p_obj->getPunchControlObject()->getJob(), $p_obj->getPunchControlObject()->getJobItem(), $p_obj->getPunchControlObject()->getPunchTag(), 1 );
						...
						Debug::text( ' No GEO fences assigned to the specified branch/department/job/task... Not triggering G1 exception.', __FILE__, __LINE__, __METHOD__, 10 );
					}
	 ...
	 * @param int $type_id
	 * @param mixed $current_exceptions
	 * @param mixed $punch_pair
	 * @return array
	 */
	public function calculateExceptionST( array $plf, array $slf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		if ( getTTProductEdition() >= TT_PRODUCT_CORPORATE && is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
			$prev_punch_time_stamp = false;
			foreach ( $plf as $p_obj ) {
				$punch_pairs[$p_obj->getPunchControlID()][] = [ 'status_id' => $p_obj->getStatus(), 'punch_control_id' => $p_obj->getPunchControlID(), 'time_stamp' => $p_obj->getTimeStamp() ];
				...
				if ( $p_obj->getStatus() == 10 ) {
					...
					if ( $p_obj->setScheduleID( $scheduled_id_cache[$p_obj->getID()] ) == true ) {
						if ( is_object( $p_obj->getPunchControlObject() )
								&& (
									//We do not trigger the exception if the schedule does not have punch tags.
										is_array( $p_obj->getScheduleObject()->getPunchTag() )
										&& !empty( array_merge( array_diff( $p_obj->getPunchControlObject()->getPunchTag(), $p_obj->getScheduleObject()->getPunchTag() ), array_diff( $p_obj->getScheduleObject()->getPunchTag(), $p_obj->getPunchControlObject()->getPunchTag() ) ) )

								)
						) {
							Debug::text( '	 Punch tag does not match scheduled punch tag: ', __FILE__, __LINE__, __METHOD__, 10 );
							$current_exceptions[] = [
									'user_id'             => $user_id,
									'date_stamp'          => $date_stamp,
									'exception_policy_id' => $ep_obj->getId(),
	 ...
	 * @return array
	 */
	public function calculateExceptionSBAndSC( array $plf, array $slf, mixed $user_id, mixed $ep_obj, bool|int $date_stamp, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
			$prev_punch_time_stamp = false;
			foreach ( $plf as $p_obj ) {
				$punch_pairs[$p_obj->getPunchControlID()][] = [ 'status_id' => $p_obj->getStatus(), 'punch_control_id' => $p_obj->getPunchControlID(), 'time_stamp' => $p_obj->getTimeStamp() ];
	 ...
	 * @return array
	 */
	public function calculateExceptionS6( array $plf, array $slf, mixed $user_id, mixed $ep_obj, bool|int $date_stamp, int $type_id, mixed &$current_exceptions, int $current_epoch, int $premature_delay, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
			$prev_punch_time_stamp = false;
			foreach ( $plf as $p_obj ) {
				$punch_pairs[$p_obj->getPunchControlID()][] = [ 'status_id' => $p_obj->getStatus(), 'punch_control_id' => $p_obj->getPunchControlID(), 'time_stamp' => $p_obj->getTimeStamp() ];
	 ...
	 * @param int $type_id
	 * @param mixed $current_exceptions
	 * @param array $tmp_exception
	 * @return array
	 */
	public function calculateExceptionS5( array $plf, array $slf, mixed $user_id, mixed $ep_obj, bool|int $date_stamp, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionS4( array $plf, array $slf, mixed $user_id, mixed $ep_obj, bool|int $date_stamp, int $type_id, mixed &$current_exceptions, int $current_epoch, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
			$prev_punch_time_stamp = false;
			foreach ( $plf as $p_obj ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionS3( array $plf, array $slf, mixed $user_id, mixed $ep_obj, bool|int $date_stamp, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		if ( is_array( $plf ) && count( $plf ) > 0 && is_array( $slf ) && count( $slf ) > 0 ) {
			//Loop through each punch, find out if they are scheduled, and if they are in early
			$prev_punch_time_stamp = false;
			foreach ( $plf as $p_obj ) {
	 ...
	 * @return array
	 */
	public function calculateExceptionS2( array $plf, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions, mixed &$scheduled_id_cache ): bool {
		...
		unset( $prev_punch_time_stamp, $p_obj );

		return true; //Don't unset the $schedule_id_cache until we have calculated all exception policies, otherwise the caching isn't as useful.
	}
	 ...
	 * @return mixed
	 */
	public function calculateExceptionS1( array $plf, array $slf, int $current_epoch, mixed $user_id, bool|int $date_stamp, mixed $ep_obj, int $type_id, mixed &$current_exceptions ): mixed {
