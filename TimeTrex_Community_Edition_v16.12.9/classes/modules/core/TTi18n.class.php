...
class TTi18n {
	...
	*/
	...
	static public function generateLocale( $locale_arg = null ) {
		...
		if ( !empty( $locale_arg ) && in_array( self::getLanguageFromLocale( $locale_arg ), self::getSupportedLanguagesArray(), true ) == false ) { //Make sure the language is a support language before adding it to the locale array.
			Debug::Text( 'Language is not supported, ignoring: ' . $locale_arg, __FILE__, __LINE__, __METHOD__, 10 );
			$locale_arg = null;
		}
	...
	static public function getLanguageFromLocale( $locale = null ) {
		...
		$language_arr = self::getSupportedLanguagesArray();

		if ( in_array( $language, $language_arr, true ) == true ) {
			return $language;
		}
...
//				}
	...
	static public function chooseBestLocale( $user_locale_pref = null ) {
		...
		if ( self::getLocaleCookie() != false ) {
			Debug::text( 'Using Language from cookie: ' . self::getLocaleCookie(), __FILE__, __LINE__, __METHOD__, 11 );
			$success = self::setLocale( self::getLocaleCookie(), $category );
			if ( self::getLanguage() != self::getLocaleCookie() ) {
				TTi18n::setLocaleCookie( self::getLanguage() ); //Cookie specified invalid language, so set it to something known to be good.
			}
		}
	...
	*/
	 ...
	 * @return string[]
	 */
	static public function getSupportedLanguagesArray() {
		$supported_langs = [ 'en', 'de', 'es', 'id', 'fr', 'hu', 'ro' ]; //See getLanguageArray() for list of beta/community languages.
	...
	static public function getLanguageArray() {
		...
		$supported_langs = self::getSupportedLanguagesArray();

		$beta_langs = [ 'de', 'es', 'id', 'fr', 'hu', 'ro' ];
		if ( PRODUCTION == false ) {
			//YI is for testing only.
			$beta_langs[] = 'yi';
		}
	...
	*/
	...
	public static function parseFloat( $value, $precision = 2 ) {
		if ( is_float( $value ) === true || is_int( $value ) === true ) {
			...
			$value = preg_replace( '/([\-\.,])(?=.*?\1)|(?<!^)-|[^-0-9\.,]|([\D]*$)/', '', $value );
	 ...
	 */
	public static function convertNoBreakSpace( $string ) {
		//Some thousands separator symbols can be a UTF-8 character NO-BREAK SPACE, which requires UTF-8 encoding on pay stubs to display, see: https://bugs.php.net/bug.php?id=77914
		// Since its really just a space anyways, lets try to convert it to an ascii space to avoid that problem.
		return str_replace("\xc2\xa0", "\x20", $string );
	}
	...
	public static function getThousandsSymbol() {
		if ( is_object( self::$number_formatter ) ) {
			return self::convertNoBreakSpace( self::$number_formatter->getSymbol( NumberFormatter::GROUPING_SEPARATOR_SYMBOL ) );
		} else {
			return ',';
		}
	}
	...
	static public function formatNumber( $number, $auto_format_decimals = false, $min_decimals = 2, $max_decimals = 4 ) {
		if ( !is_numeric( $number ) ) { //Make sure we still allow NULL or a blank string through.
			$number = 0;
		}
		...
		if ( is_object( self::$number_formatter ) ) { //Make sure INTL extension is installed before using it.
			if ( $auto_format_decimals == true ) {
				$number = TTMath::removeTrailingZeros( $number, $min_decimals );
				...
				return self::convertNoBreakSpace( self::$number_formatter->format( $number ) );
			} else {
			...
			if ( $auto_format_decimals == true ) {
				...
				$number = TTMath::removeTrailingZeros( $number, 0 );
	...
	static public function formatCurrency( $amount, $currency_code = null, $show_code = 0 ) {
		...
		if ( is_object( self::$currency_formatter ) ) { //Make sure INTL extension is installed before using it.
			...
			if ( $show_code == 0 ) {
				...
				return self::convertNoBreakSpace( str_replace( [ self::$currency_formatter->getSymbol( NumberFormatter::CURRENCY_SYMBOL ), $tmp_formatter->getSymbol( NumberFormatter::CURRENCY_SYMBOL ) ], self::getCurrencySymbol( $currency_code ), self::$currency_formatter->formatCurrency( $amount, $currency_code ) ) ); //This seemed to always show US$1.23, or CA$1.23
			} else {
				return $currency_code_left_str . self::convertNoBreakSpace( self::$currency_formatter->formatCurrency( $amount, $currency_code ) ) . $currency_code_right_str;
			}
		} else {
