...
class CurrencyFactory extends Factory {
	...
	function _getSchemadata( ?array $filter = null ): ?TTS {
		...
		if ( empty( $filter ) || in_array( 'database', $filter, true ) ) {
			$schema_data->setColumns(
					TTSCols::new(
							...
							TTSCol::new( 'symbol' )->setFunctionMap( 'Symbol' )->setType( 'varchar' )->setIsSynthetic( true ),
							TTSCol::new( 'iso_code' )->setFunctionMap( 'ISOCode' )->setType( 'varchar' )->setIsNull( false ),
							TTSCol::new( 'conversion_rate' )->setFunctionMap( 'ConversionRate' )->setType( 'numeric' ),
							TTSCol::new( 'auto_update' )->setFunctionMap( 'AutoUpdate' )->setType( 'smallint' ),
							TTSCol::new( 'actual_rate' )->setFunctionMap( 'ActualRate' )->setType( 'numeric' ),
							TTSCol::new( 'actual_rate_updated_date' )->setFunctionMap( 'ActualRateUpdatedDate' )->setType( 'integer' ),
							TTSCol::new( 'rate_modify_percent' )->setFunctionMap( 'RateModifyPercent' )->setType( 'numeric' ),
							TTSCol::new( 'is_default' )->setFunctionMap( 'Default' )->setType( 'smallint' )->setIsNull( false ),
							TTSCol::new( 'is_base' )->setFunctionMap( 'Base' )->setType( 'smallint' )->setIsNull( false ),
							TTSCol::new( 'round_decimal_places' )->setFunctionMap( 'RoundDecimalPlaces' )->setType( 'smallint' )->setIsNull( false ),
		...
		if ( empty( $filter ) || in_array( 'fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setTabs(
					TTSTabs::new(
							TTSTab::new( 'tab_currency' )->setLabel( TTi18n::getText( 'Currency' ) )->setFields(
									new TTSFields(
											TTSField::new( 'id' )->setType( 'uuid' )->setVisible( [ 'UI' ], false ),
											TTSField::new( 'status_id' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Status' ) )->setDataSource( TTSAPI::new( 'APICurrency' )->setMethod( 'getOptions' )->setArg( 'status' ) ),
											TTSField::new( 'iso_code' )->setType( 'text' )->setLabel( TTi18n::getText( 'ISO Currency' ) ),
											TTSField::new( 'name' )->setType( 'text' )->setLabel( TTi18n::getText( 'Name' ) ),
											TTSField::new( 'is_base' )->setType( 'checkbox' )->setLabel( TTi18n::getText( 'Base Currency' ) ),
											TTSField::new( 'is_default' )->setType( 'checkbox' )->setLabel( TTi18n::getText( 'Default Currency' ) ),
											TTSField::new( 'auto_update' )->setType( 'checkbox' )->setLabel( TTi18n::getText( 'Auto Update' ) ),
											TTSField::new( 'round_decimal_places' )->setType( 'integer' )->setLabel( TTi18n::getText( 'Decimal Places' ) )->setDataSource( TTSAPI::new( 'APICurrency' )->setMethod( 'getOptions' )->setArg( 'round_decimal_places' ) )
									)
							),
							TTSTab::new( 'tab_rates' )->setLabel( TTi18n::getText( 'Rates' ) )->setInitCallback( 'initSubCurrencyRateView' )->setDisplayOnMassEdit( false )->setSubView( true )
					)->addAudit()
			);
		}
		...
		if ( empty( $filter ) || in_array( 'search_fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setSearchFields(
					TTSSearchFields::new(
							...
							TTSSearchField::new( 'iso_code' )->setType( 'text_list' )->setColumn( 'a.iso_code' ),
							TTSSearchField::new( 'name' )->setType( 'text' )->setColumn( 'a.name' ),

							TTSSearchField::new( 'status' )->setType( 'text' )->setColumn( 'a.status_id' ),
		...
		if ( empty( $filter ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setAPIMethods(
					TTSAPIs::new(
							TTSAPI::new( 'APICurrency' )->setMethod( 'getCurrency' )
									->setSummary( 'Get currency records.' )
									->setArgs( [ 'data' => [ 'filter_data' => $schema_data->getSearchFields(), 'filter_columns' => $schema_data->getFields() ] ] )
									->setArgsModelDescription( 'Under the "data" arg there must be at least one or more "filter_data" elements specified, and one or more "filter_columns" elements which define the minimum necessary columns of data to return for carrying out other actions. For example if looking up the "id" of a record, just the "id" filter_columns needs to be specified with a value of true.' ),
							TTSAPI::new( 'APICurrency' )->setMethod( 'setCurrency' )
									->setSummary( 'Add or edit currency records. Will return the record UUID upon success, or a validation error if there is a problem.' )
									->setArgs( [ 'data' => $schema_data->getFields() ] ),
							TTSAPI::new( 'APICurrency' )->setMethod( 'deleteCurrency' )
									->setSummary( 'Delete currency records by passing in an array of UUIDs.' )
									->setArgs( new TTSFields(
													   TTSField::new( 'data' )->setType( 'multi-dropdown' )->setLabel( TTi18n::getText( 'IDs to Delete' ) )->setDataSource( TTSAPI::new( 'APICurrency' )->setMethod( 'getCurrency' ) ),
											   ) ),
							TTSAPI::new( 'APICurrency' )->setMethod( 'getCurrencyDefaultData' )
									->setSummary( 'Get default currency data used for creating new currencies. Use this before calling setCurrency to get the correct default data.' ),
					),
			);
		}
	...
	function getReverseConversionRate() {
		return TTMath::div( 1, $this->getConversionRate() );
	}
	...
	function getPercentModifiedRate( $rate ) {
		...
		return TTMath::mul( $rate, $percent );
	}
	...
	static function getCrossConversionRate( $src_rate, $dst_rate ) {
		if ( $src_rate == $dst_rate ) {
			...
			$retval = TTMath::mul( TTMath::div( 1, $src_rate ), $dst_rate );
		}
	...
	static function convert( $src_rate, $dst_rate, $amount, $round_decimal_places = 2 ) {
		if ( $src_rate == $dst_rate ) {
			...
			$base_amount = TTMath::mul( TTMath::div( 1, $src_rate ), $amount );
			$retval = round( TTMath::mul( $dst_rate, $base_amount ), (int)$round_decimal_places );
	 ...
	 */
	static function convertCurrency( $src_currency_id, $dst_currency_id, $amount = 1, $round_decimal_places = null ) {
		...
		if ( is_object( $src_currency_obj ) && is_object( $dst_currency_obj ) ) {
			return TTMath::removeTrailingZeros( self::Convert( $src_currency_obj->getConversionRate(), $dst_currency_obj->getConversionRate(), $amount, ( empty( $round_decimal_places ) ? $dst_currency_obj->getRoundDecimalPlaces() : $round_decimal_places ) ), $dst_currency_obj->getRoundDecimalPlaces() );
		}
						...
						if ( $clf->getRecordCount() > 0 ) {
							...
							if ( TTDate::getDays( ( time() - $latest_currency_rate_date ) ) > 30 ) {
								...
								foreach ( TTDate::getDatePeriod( TTDate::getMiddleDayEpoch( $latest_currency_rate_date ), TTDate::getMiddleDayEpoch( time() ), 'P1D' ) as $x ) {
									$crf->ExecuteSQL( 'INSERT INTO ' . $crf->getTable() . ' (id,currency_id,date_stamp,conversion_rate,created_date) VALUES(\'' . $crf->getNextInsertId() . '\',\'' . $active_currency_id . '\',\'' . $crf->db->BindDate( $x ) . '\', ' . $last_conversion_rate . ',' . time() . ')' );
								}
							} else {
								//for( $x = TTDate::getMiddleDayEpoch( $latest_currency_rate_date ); $x <= TTDate::getMiddleDayEpoch( time() ); $x += 86400 ) {
								foreach ( TTDate::getDatePeriod( TTDate::getMiddleDayEpoch( $latest_currency_rate_date ), TTDate::getMiddleDayEpoch( time() ), 'P1D' ) as $x ) {
