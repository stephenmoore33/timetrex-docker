...
class TTPassword {
	 ...
	 * @return string
	 */
	static function getPasswordSalt( $company_id = null ) {
		$retval = '';

		global $config_vars;
		if ( isset( $config_vars['other']['salt'] ) ) {
			if ( is_array( $config_vars['other']['salt'] ) ) {
				if ( isset( $config_vars['other']['salt'][$company_id] ) && $config_vars['other']['salt'][$company_id] != '' ) {
					$retval = $config_vars['other']['salt'][$company_id];
				} else {
					$retval = $config_vars['other']['salt'][TTUUID::getZeroID()] ?? '';
				}
			} else if ( is_string( $config_vars['other']['salt'] ) && $config_vars['other']['salt'] != '' ) {
		...
		//Debug::Text( 'Password Salt: '. $retval .' Company ID: '. $company_id, __FILE__, __LINE__, __METHOD__, 1 );
		return trim( $retval );
	}
	...
	static function encryptPassword( $password, $id1 = null, $id2 = null, $version = null ) {
		...
		switch ( (int)$version ) {
			case 2: //v2
				...
				$encrypted_password = '2:' . hash( 'sha512', self::getPasswordSalt( $id1 ) . TTUUID::convertUUIDtoInt( $id1 ) . TTUUID::convertUUIDToInt( $id2 ) . $password );
				break;
			case 3: //v3 that uses UUIDs
				$encrypted_password = '3:' . hash( 'sha512', self::getPasswordSalt( $id1 ) . TTUUID::castUUID( $id1 ) . TTUUID::castUUID( $id2 ) . $password );
				break;
			default: //v1
				//Case insensitive, uses sha1 and global salt.
				$encrypted_password = sha1( self::getPasswordSalt( $id1 ) . strtolower( $password ) );
				break;
		}
		unset( $password );
