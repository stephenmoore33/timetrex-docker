...
class APIBranch extends APIFactory {
	...
	function getOptions( $name = false, $parent = null ) {
		if ( $name == 'columns'
				&& ( !$this->getPermissionObject()->Check( 'branch', 'enabled' )
						|| !( $this->getPermissionObject()->Check( 'branch', 'view' ) || $this->getPermissionObject()->Check( 'branch', 'view_own' ) || $this->getPermissionObject()->Check( 'branch', 'view_child' ) ) ) ) {
			$name = 'list_columns';
		}
	...
	function getBranchDefaultData() {
		...
		$data = [
				...
				'include_user_default_branch_id' => true,
		];

		$data = $bf->getCustomFieldsDefaultData( $company_obj->getId(), $data );
	...
	function setBranch( $data, $validate_only = false, $ignore_warning = true ) {
		...
		if ( !$this->getPermissionObject()->Check( 'branch', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'branch', 'edit' ) || $this->getPermissionObject()->Check( 'branch', 'edit_own' ) || $this->getPermissionObject()->Check( 'branch', 'edit_child' ) || $this->getPermissionObject()->Check( 'branch', 'add' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'branch', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true )
								) ) {
	...
	function copyBranch( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getBranch( [ 'filter_data' => [ 'id' => $data ] ], true ) );
