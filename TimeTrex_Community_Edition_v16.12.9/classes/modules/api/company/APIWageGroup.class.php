...
class APIWageGroup extends APIFactory {
	...
	function getOptions( $name = false, $parent = null ) {
		if ( $name == 'columns'
				&& ( !$this->getPermissionObject()->Check( 'wage', 'enabled' )
	...
	function getWageGroup( $data = null, $disable_paging = false ) {
		...
		//Always show all Wage Groups, even if they only have "wage,view_child" permission, as there would be no way to have a "supervisor subordinate only" see wage groups otherwise.
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'wage', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true )
								) ) {
				...
				if ( $id != '' ) {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if ( $this->getPermissionObject()->Check( 'wage', 'delete' )
								|| ( $this->getPermissionObject()->Check( 'wage', 'delete_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true ) ) {
	...
	function copyWageGroup( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getWageGroup( [ 'filter_data' => [ 'id' => $data ] ], true ) );
