...
class APIPolicyGroup extends APIFactory {
	...
	function getOptions( $name = false, $parent = null ) {
		if ( $name == 'columns'
				&& ( !$this->getPermissionObject()->Check( 'policy_group', 'enabled' )
						|| !( $this->getPermissionObject()->Check( 'policy_group', 'view' ) || $this->getPermissionObject()->Check( 'policy_group', 'view_own' ) || $this->getPermissionObject()->Check( 'policy_group', 'view_child' ) ) ) ) {
			$name = 'list_columns';
		}
	...
	function setPolicyGroup( $data, $validate_only = false, $ignore_warning = true ) {
		...
		if ( !$this->getPermissionObject()->Check( 'policy_group', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'policy_group', 'edit' ) || $this->getPermissionObject()->Check( 'policy_group', 'edit_own' ) || $this->getPermissionObject()->Check( 'policy_group', 'edit_child' ) || $this->getPermissionObject()->Check( 'policy_group', 'add' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'policy_group', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true )
								) ) {
	...
	function deletePolicyGroup( $data ) {
		...
		if ( !$this->getPermissionObject()->Check( 'policy_group', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'policy_group', 'delete' ) || $this->getPermissionObject()->Check( 'policy_group', 'delete_own' ) || $this->getPermissionObject()->Check( 'policy_group', 'delete_child' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
				...
				if ( $id != '' ) {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if ( $this->getPermissionObject()->Check( 'policy_group', 'delete' )
								|| ( $this->getPermissionObject()->Check( 'policy_group', 'delete_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true ) ) {
	...
	function copyPolicyGroup( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getPolicyGroup( [ 'filter_data' => [ 'id' => $data ] ], true ) );
