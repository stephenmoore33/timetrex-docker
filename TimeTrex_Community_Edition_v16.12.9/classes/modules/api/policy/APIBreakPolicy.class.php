...
class APIBreakPolicy extends APIFactory {
	...
	function getOptions( $name = false, $parent = null ) {
		if ( $name == 'columns'
				&& ( !$this->getPermissionObject()->Check( 'break_policy', 'enabled' )
						|| !( $this->getPermissionObject()->Check( 'break_policy', 'view' ) || $this->getPermissionObject()->Check( 'break_policy', 'view_own' ) || $this->getPermissionObject()->Check( 'break_policy', 'view_child' ) ) ) ) {
			$name = 'list_columns';
		}
	...
	function setBreakPolicy( $data, $validate_only = false, $ignore_warning = true ) {
		...
		if ( !$this->getPermissionObject()->Check( 'break_policy', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'break_policy', 'edit' ) || $this->getPermissionObject()->Check( 'break_policy', 'edit_own' ) || $this->getPermissionObject()->Check( 'break_policy', 'edit_child' ) || $this->getPermissionObject()->Check( 'break_policy', 'add' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'break_policy', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true )
								) ) {
	...
	function deleteBreakPolicy( $data ) {
		...
		if ( !$this->getPermissionObject()->Check( 'break_policy', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'break_policy', 'delete' ) || $this->getPermissionObject()->Check( 'break_policy', 'delete_own' ) || $this->getPermissionObject()->Check( 'break_policy', 'delete_child' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
				...
				if ( $id != '' ) {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if ( $this->getPermissionObject()->Check( 'break_policy', 'delete' )
								|| ( $this->getPermissionObject()->Check( 'break_policy', 'delete_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true ) ) {
	...
	function copyBreakPolicy( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getBreakPolicy( [ 'filter_data' => [ 'id' => $data ] ], true ) );
