...
class APIHolidayPolicy extends APIFactory {
	...
	function getOptions( $name = false, $parent = null ) {
		if ( $name == 'columns'
				&& ( !$this->getPermissionObject()->Check( 'holiday_policy', 'enabled' )
						|| !( $this->getPermissionObject()->Check( 'holiday_policy', 'view' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'view_own' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'view_child' ) ) ) ) {
			$name = 'list_columns';
		}
	...
	function getHolidayPolicyDefaultData() {
		...
		$data = [
				...
				'holiday_display_days'             => 381, //366+ 1x Bi-Weekly pay period (14 days) + 1 day for margin of error = 381
		];
	...
	function setHolidayPolicy( $data, $validate_only = false, $ignore_warning = true ) {
		...
		if ( !$this->getPermissionObject()->Check( 'holiday_policy', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'holiday_policy', 'edit' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'edit_own' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'edit_child' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'add' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'holiday_policy', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true )
								) ) {
	...
	function deleteHolidayPolicy( $data ) {
		...
		if ( !$this->getPermissionObject()->Check( 'holiday_policy', 'enabled' )
				|| !( $this->getPermissionObject()->Check( 'holiday_policy', 'delete' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'delete_own' ) || $this->getPermissionObject()->Check( 'holiday_policy', 'delete_child' ) ) ) {
			return $this->getPermissionObject()->PermissionDenied();
		}
				...
				if ( $id != '' ) {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if ( $this->getPermissionObject()->Check( 'holiday_policy', 'delete' )
								|| ( $this->getPermissionObject()->Check( 'holiday_policy', 'delete_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy(), $lf->getCurrent()->getID() ) === true ) ) {
	...
	function copyHolidayPolicy( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getHolidayPolicy( [ 'filter_data' => [ 'id' => $data ] ], true ) );
