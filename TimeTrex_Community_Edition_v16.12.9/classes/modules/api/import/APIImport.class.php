...
class APIImport extends APIFactory {
	...
	function getImportObjects() {
		...
		if ( $this->getPermissionObject()->Check( 'company_tax_deduction', 'add' ) && $this->getPermissionObject()->Check( 'company_tax_deduction', 'edit' ) ) {
			$retarr['-1300-UserDeduction'] = TTi18n::getText( 'Tax / Deduction Employee Settings' );
		}
		if ( $this->getPermissionObject()->Check( 'accrual', 'add' ) && ( $this->getPermissionObject()->Check( 'accrual', 'edit' ) || $this->getPermissionObject()->Check( 'accrual', 'edit_child' ) ) ) {
			$retarr['-1400-Accrual'] = TTi18n::getText( 'Accruals' );
		}

		//Imported indirectly through Skills/Education, etc... below.
		//if ( $this->getPermissionObject()->Check( 'qualification', 'add' ) && ( $this->getPermissionObject()->Check( 'qualification', 'edit' ) || $this->getPermissionObject()->Check( 'qualification', 'edit_child' ) ) ) {
		//	$retarr['-1400-Qualification'] = TTi18n::getText( 'Qualifications' );
		//}
		if ( $this->getPermissionObject()->Check( 'user_contact', 'add' ) && ( $this->getPermissionObject()->Check( 'user_contact', 'edit' ) || $this->getPermissionObject()->Check( 'user_contact', 'edit_child' ) ) ) {
			$retarr['-1405-UserContact'] = TTi18n::getText( 'Employee Contacts' );
		}
		if ( $this->getPermissionObject()->Check( 'user_skill', 'add' ) && ( $this->getPermissionObject()->Check( 'user_skill', 'edit' ) || $this->getPermissionObject()->Check( 'user_skill', 'edit_child' ) ) ) {
			$retarr['-1410-UserSkill'] = TTi18n::getText( 'Employee Skills' );
		}
		if ( $this->getPermissionObject()->Check( 'user_education', 'add' ) && ( $this->getPermissionObject()->Check( 'user_education', 'edit' ) || $this->getPermissionObject()->Check( 'user_education', 'edit_child' ) ) ) {
			$retarr['-1420-UserEducation'] = TTi18n::getText( 'Employee Education' );
		}
		if ( $this->getPermissionObject()->Check( 'user_membership', 'add' ) && ( $this->getPermissionObject()->Check( 'user_membership', 'edit' ) || $this->getPermissionObject()->Check( 'user_membership', 'edit_child' ) ) ) {
			$retarr['-1430-UserMembership'] = TTi18n::getText( 'Employee Memberships' );
		}
		if ( $this->getPermissionObject()->Check( 'user_license', 'add' ) && ( $this->getPermissionObject()->Check( 'user_license', 'edit' ) || $this->getPermissionObject()->Check( 'user_license', 'edit_child' ) ) ) {
			$retarr['-1440-UserLicense'] = TTi18n::getText( 'Employee Licenses' );
		}
		if ( $this->getPermissionObject()->Check( 'user_language', 'add' ) && ( $this->getPermissionObject()->Check( 'user_language', 'edit' ) || $this->getPermissionObject()->Check( 'user_language', 'edit_child' ) ) ) {
			$retarr['-1450-UserLanguage'] = TTi18n::getText( 'Employee Languages' );
		}
	...
	function getRawData( $limit = null ) {
		...
		if ( is_object( $this->getMainClassObject() ) && in_array( get_class( $this->getMainClassObject() ),  [ 'ImportUser', 'ImportRemittanceDestinationAccount' ] ) ) {
			global $authentication;

			if ( is_object( $authentication ) === true && $authentication->isSessionReauthenticated( true ) === false ) { //Allow impersonation without reauthenticating, so support staff can import without needing a password.
	 ...
	 * @return array
	 */
	function exportImportResults( $format = 'csv', $data = null ) {
		return $this->exportRecords( $format, 'export_import_report_results', $data['result'], [], ( ( isset( $data['filter_columns'] ) ) ? $data['filter_columns'] : null ) );
	}
