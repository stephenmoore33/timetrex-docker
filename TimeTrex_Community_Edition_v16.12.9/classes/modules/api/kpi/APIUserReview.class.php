...
class APIUserReview extends APIFactory {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if (
								...
								(
										...
										|| ( $this->getPermissionObject()->Check( 'user_review', 'edit_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getColumn( 'user_id' ) ) === true ) //Make sure we check against the employee the review is assigned too.
										|| ( $this->getPermissionObject()->Check( 'user_review', 'edit_child' ) && $this->getPermissionObject()->isChild( $lf->getCurrent()->getColumn( 'user_id' ), $permission_children_ids ) === true ) //Make sure we check against the employee the review is assigned too.
								) ) {
				...
				if ( $id != '' ) {
					...
					if ( $lf->getRecordCount() == 1 ) {
						...
						if ( $this->getPermissionObject()->Check( 'user_review', 'delete' )
								|| ( $this->getPermissionObject()->Check( 'user_review', 'delete_own' ) && $this->getPermissionObject()->isOwner( $lf->getCurrent()->getCreatedBy() ) === true )
								|| ( $this->getPermissionObject()->Check( 'user_review', 'delete_child' ) && $this->getPermissionObject()->isChild( $lf->getCurrent()->getCreatedBy(), $permission_children_ids ) === true )
						) {
	...
	function copyUserReview( $data ) {
		...
		$src_rows = $this->stripReturnHandler( $this->getUserReview( [ 'filter_data' => [ 'id' => $data ] ], true ) );
