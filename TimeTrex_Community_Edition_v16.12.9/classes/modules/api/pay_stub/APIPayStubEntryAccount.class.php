...
class APIPayStubEntryAccount extends APIFactory {
	...
	function getPayStubEntryAccount( $data = null, $disable_paging = false ) {
		...
		//If they have permissions to add/edit/view PS amendments, also allow them to view pay stub accounts, as its mandatory for PS amendments.
		if (
				( !$this->getPermissionObject()->Check( 'pay_stub_account', 'enabled' )
					|| !( $this->getPermissionObject()->Check( 'pay_stub_account', 'view' ) || $this->getPermissionObject()->Check( 'pay_stub_account', 'view_child' ) ) )
			&&
				( !$this->getPermissionObject()->Check( 'pay_stub_amendment', 'enabled' )
		...
		//If they can view any pay stub account, allow to them to view all, since this isn't an object that a limited view makes sense.
