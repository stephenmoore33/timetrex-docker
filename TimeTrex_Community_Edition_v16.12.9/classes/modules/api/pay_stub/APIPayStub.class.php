...
class APIPayStub extends APIFactory {
	...
	function getPayStub( $data = null, $disable_paging = false, $format = false, $hide_employer_rows = true ) {
		...
		if ( $format == 'pdf' ) {
			...
			if ( $pslf->getRecordCount() > 0 ) {
				...
				//FIXME: If a pay stub has $0 net pay, but does have some deductions, it wouldn't have any transactions, so when using full-service payroll the deduction amounts would not be impounded.
				//       If it was processed with other pay stubs, it should still be fine, but if it was processed on its own, nothing would be impounded.
				//		 When CRA remittances are processed, the shortfall is detected and a debit initiated immediately, so the CRA remittance is always correct, but it runs the risk of it being late.
				// 		 We can't really make a $0 pay stub transaction, as that would seem pretty strange to the user, and would be displayed on their pay stub.
				//		 We can't de-couple the remittance report from processing transactions and instead wait until the pay stubs are marked as paid, as out-of-cycle pay stubs are run much before the pay period is closed. Also some customers do that much later in the payroll process, at which point it would be too late, or require a separate transaction.
				//			Though we could do some sort of variation of this, and re-run the remittance report when the pay period is closed, and try to detect a difference at that point? **Still wouldn't fix out-of-cycle pay stubs run at the beginning of the pay period.
				// 		 Best option would be to somehow handle remittance reports on a per pay stub basis, so we can easily detect if someone was missed and make a correction. This way we can do it when the pay stub is marked as paid, or transactions are processed.
						...
						foreach ( $row['entries'] as $pay_stub_entry ) {
							if ( (
								...
								if ( $pay_stub_entry_obj->isValid() == true ) {
									...
									$lf->Validator->isTrue( 'pay_stub_entry', false, TTi18n::getText( '%1 entry for amount: %2 is invalid', [ $tmp_pay_stub_entry_account_name, TTi18n::MoneyFormat( $pay_stub_entry['amount'] ) ] ) );
								}
							} else {
							...
							foreach ( $row['transactions'] as $pay_stub_transaction ) {
								...
								if ( $pst_obj->isValid() ) {
									...
									if ( isset( $pay_stub_transaction['deleted'] ) == false || $pay_stub_transaction['deleted'] == 0 ) {
										...
										$lf->Validator->isTrue( 'pay_stub_transaction', false, TTi18n::getText( '%1 transaction for amount: %2 is invalid', [ $tmp_remittance_destination_account_name, TTi18n::MoneyFormat( $pst_obj->getAmount() ) ] ) );
	...
	function generatePayStubs( $pay_period_ids, $user_ids = null, $enable_correction = false, $run_id = false, $type_id = 10, $transaction_date = null ) {
		...
		global $config_vars;
		$user_generic_status_batch_id = TTUUID::generateUUID(); //Use the same batch_id for all pay periods in this run.
		$is_job_queue_used = false;
			...
			foreach ( $pplf as $pay_period_obj ) { /** @var PayPeriodFactory $pay_period_obj */
				...
				if ( PRODUCTION == false || $pay_period_obj->isPreviousPayPeriodClosed() == true ) { //Allow generating pay stubs without closing each pay period when not in production.
					...
					if ( $total_pay_stubs > 0 ) {
						//Use job queue if more than one pay period is selected, or more than 200 pay stubs. Assume that if running multiple pay periods its likely a larger run.
						if ( ( !isset( $config_vars['other']['enable_job_queue'] ) || $config_vars['other']['enable_job_queue'] == true ) && ( $total_pay_stubs > 200 || $pplf->getRecordCount() > 1 ) ) {
							...
							$is_job_queue_used = true; //Need to set a flag when job queue is used, so we know if we should call SystemJobQueue::waitUntilBatchCompleted at the end or not.
						} else {
							$this->getProgressBarObject()->start( $this->getAPIMessageID(), $total_pay_stubs, null, TTi18n::getText( 'Generating Paystubs...' ) );
		...
		//Wait for job queue to finish. Put this at the end so it if multiple pay periods are being generated it waits at the end of them all, rather than each pay period separately which would be slower.
		if ( $is_job_queue_used == true && ( !isset( $config_vars['other']['enable_job_queue'] ) || $config_vars['other']['enable_job_queue'] == true ) ) {
			SystemJobQueue::waitUntilBatchCompleted( $user_generic_status_batch_id, $this->getAPIMessageID(), 5, 7200 ); //Run up to 2hrs.
		}

		//There could be some job queue runs and some non-job queue runs (ie: one pay period with 1 pay stub, and another pay period with 500 pay stubs).
		//  Therefore we need to do the SystemJobQueue::waitUntilBatchCompleted() above, and this below, so it merges status records if both happen to exist.
		if ( UserGenericStatusFactory::isStaticQueue() == true ) {
			...
			$ugsf->setBatchID( $user_generic_status_batch_id );
