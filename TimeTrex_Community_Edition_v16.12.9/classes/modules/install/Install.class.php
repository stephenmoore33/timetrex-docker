...
class Install {
	...
	function setDatabaseConnection( $db_obj ) {
		if ( is_object( $db_obj ) ) {
			if ( $db_obj instanceof ADOdbLoadBalancer ) {
				$this->temp_db = $db_obj->getConnection( 'write' );
	...
	function writeConfigFile( $new_config_vars ) {
		if ( is_writeable( CONFIG_FILE ) ) {
			...
			if ( is_object( $data ) && get_class( $data ) == 'PEAR_Error' ) {
				Debug::Arr( $data, 'ERROR: Unable to modify Config File!', __FILE__, __LINE__, __METHOD__, 9 );
				Debug::Arr( @file_get_contents( CONFIG_FILE ), 'Config File: ', __FILE__, __LINE__, __METHOD__, 9 );
			} else {
				global $config_vars;
	...
	function setVersions() {
		if ( is_array( $this->versions ) ) {
			...
			foreach ( $this->versions as $name => $value ) {
				if ( $name == 'system_version' ) {
					$original_value = SystemSettingFactory::getSystemSettingValueByKey( $name );
					Debug::text( 'System Version: Old: '. $original_value .' New: '. $value, __FILE__, __LINE__, __METHOD__, 9 );

					if ( $original_value != $value && Misc::MajorVersionCompare( APPLICATION_VERSION, $original_value, '>' ) ) {
			...
			$sjqf = TTnew('SystemJobQueueFactory');
			$sjqf->notifyWorkers( 'EXIT' ); //Force all QueueWorkers to exit immediately so they can be restarted with new code/schema.
			unset( $sjqf );

			if ( $send_notification == true ) {
				Debug::text( 'System version has changed, send notification to users...', __FILE__, __LINE__, __METHOD__, 9 );
				$this->sendNotification(); //Notify users of the new version.
			}
		}
	...
	*/
	...
	function checkTableExists( $table_name ) {
		...
		if ( $db_conn == false ) {
			Debug::text( 'WARNING: No database connection, unable to determine if table exists...', __FILE__, __LINE__, __METHOD__, 9 );
			return false;
		}
	...
	function checkSystemSettingTableExists() {
		global $config_vars;
		if ( $this->checkDatabaseExists( $config_vars['database']['database_name'] ?? null ) == true ) {
	...
	function getAllSchemaVersions( $group = [ 'A', 'B', 'C', 'D' ] ) {
		...
		if ( $handle = opendir( $dir ) ) {
			while ( false !== ( $file = readdir( $handle ) ) ) {
				[ $schema_base_name, $extension ] = explode( '.', $file );
	...
	function createSchemaRange( $start_version = null, $end_version = null, $group = null ) {
		global $cache, $config_vars, $PRIMARY_KEY_IS_UUID, $COMPANY_GENERIC_MAP_DELETED_COLUMN, $COMPANY_DEDUCTION_FILTER_DATE_TYPE_ID_COLUMN;
		...
		if ( is_array( $schema_versions ) && $total_schema_versions > 0 ) {
			...
			if ( $this->checkTableExists( 'system_setting' ) == true ) {
				if ( (int)SystemSettingFactory::getSystemSettingValueByKey( 'schema_version_group_A' ) < 1093 ) {
					Debug::Text( '  Upgrading database before company_deduction had filter_date_type_id column... Setting COMPANY_DEDUCTION_FILTER_DATE_TYPE_ID_COLUMN = FALSE', __FILE__, __LINE__, __METHOD__, 1 );
					$COMPANY_DEDUCTION_FILTER_DATE_TYPE_ID_COLUMN = false;
				}
				...
				if ( (int)SystemSettingFactory::getSystemSettingValueByKey( 'schema_version_group_A' ) < 1126 ) {
					Debug::Text( '  Upgrading database before company_generic_map had deleted column... Setting COMPANY_GENERIC_MAP_DELETED_COLUMN = FALSE', __FILE__, __LINE__, __METHOD__, 1 );
					$COMPANY_GENERIC_MAP_DELETED_COLUMN = false;
				}
	...
	*/
	...
	function checkPHPVersion( $php_version = null ) {
		...
		//**NOTE: Update checkStackComponentsOutOfDate() version as well. Preferrably at least a version before these get increases.
		$min_version = '8.0.0';  //Change install.php as well, as some versions break backwards compatibility, so we need early checks as well.
		$max_version = '8.3.99'; //Change install.php as well, as some versions break backwards compatibility, so we need early checks as well.
			...
			if ( is_array( $checksums ) ) {
				...
				foreach ( $checksums as $checksum_line ) {
					...
					if ( $i == 0 ) {
						...
						$split_line = explode( '|', $checksum_line ); //Files could have spaces in the name, so explode on the "|" instead.
						if ( is_array( $split_line ) ) {
							$checksum = trim( $split_line[0] );
							$file_name = Environment::getBasePath() . str_replace( '/', DIRECTORY_SEPARATOR, str_replace( './', '', trim( $split_line[1] ) ) );
	...
	function checkDatabaseVersion() {
		...
		if ( $this->getDatabaseType() == 'postgresql' ) {
			if ( $db_version == null || version_compare( $db_version, '11', '>=' ) == 1 ) { //v9.6 has JSONB support.
				return 0;
			}
		}
	...
	function getScheduleMaintenanceJobsCommand() {
		...
		if ( OPERATING_SYSTEM == 'LINUX' ) {
			...
			if ( $system_root != '' ) {
				...
				$command = $system_root . '\system32\schtasks /create /SC minute /TN timetrex_maintenance /DU 01:00 /TR ""' . Environment::getBasePath() . '..\php\php-win.exe" "' . Environment::getBasePath() . 'maint\cron.php""';
			}
		}
	...
	function checkPHPCLIBinary() {
		if ( $this->getPHPCLI() != '' ) {
			//Sometimes the user may mistakenly make the PHP CLI the directory, rather than the executeable itself. Make sure we catch that case.
			if ( @is_dir( $this->getPHPCLI() ) == false && @is_executable( $this->getPHPCLI() ) == true ) {
				return 0;
			} else {
				Debug::Text( 'NOTICE: PHP CLI Binary incorrect: Message: ' . Debug::getLastPHPErrorMessage(), __FILE__, __LINE__, __METHOD__, 10 );
			}
		}
	...
	function checkDiskSpace() {
		...
		//Some systems have disk_total_space() restricted, which would cause: disk_total_space() has been disabled for security reasons
	...
	function checkPHPCLIRequirements() {
		if ( $this->checkPHPCLIBinary() === 0 ) {
			...
			if ( function_exists( 'exec' ) ) {
				...
				$this->setExtendedErrorMessage( 'checkPHPCLIRequirements', 'Function exec() has been disabled.' );
			}
		}
	...
	function checkProductEditionMatch() {
																																																																																									$obj_class = "\124\124\114\x69\x63\x65\x6e\x73\x65"; $obj_function = "\x67\145\x74\x54\x54\120\162\x6f\144\x75\143\x74\105\x64\x69\x74\151\x6f\x6e"; @$obj = new $obj_class; $retval = $obj->{$obj_function}();
		if ( getTTProductEdition() == $retval ) {
			return true;
		}
	 ...
	 * @return void
	 */
	static function syncProductEdition() {
		if ( DEPLOYMENT_ON_DEMAND == false && DEMO_MODE == false && PRODUCTION == true ) {
			$clf = TTnew( 'CompanyListFactory' ); /** @var CompanyListFactory $clf */
			$clf->getAll();
			if ( $clf->getRecordCount() > 0 ) {
				foreach ( $clf as $c_obj ) {
					if ( $c_obj->getProductEdition() > getTTProductEdition() ) {
						$c_obj->setProductEdition( getTTProductEdition() );
						if ( $c_obj->isValid() ) {
							$c_obj->Save();
						}
					}
	 ...
	 * @return void
	 */
	static function checkApplicationVersionMatchesDatabase() {
		if ( DEMO_MODE == false && PRODUCTION == true ) {
			...
			if ( isset( $system_settings['system_version'] ) && DEPLOYMENT_ON_DEMAND == false && APPLICATION_VERSION != $system_settings['system_version'] ) {
				$notification_data = [
						...
						'priority_id'	 => 2, //High
						'type_id'        => 'system',
						'title_short'    => TTi18n::getText( 'WARNING: Application version does not match database version.' ),
						'body_short'     => TTi18n::getText( '%1 application version (%2) does not match database version (%3). Please re-run the %1 web installer to complete the upgrade process.', [ APPLICATION_NAME, APPLICATION_VERSION, $system_settings['system_version'] ] ),
						'body_long_html' => TTi18n::getText( '%1 application version (%2) does not match database version (%3). Please re-run the %1 web installer to complete the upgrade process.', [ APPLICATION_NAME, APPLICATION_VERSION, $system_settings['system_version'] ] ), //Use this to append email footer.
				];

				Notification::sendNotificationToAllUsers( 100, true, true, $notification_data, ( 7 * 86400 ) ); //Send to all companies as its an urgent issue that needs to be resolved ASAP.
			}
		}
	}
	 ...
	 * @return void
	 */
	static function checkApplicationOutOfDate() {
		if ( DEMO_MODE == false && PRODUCTION == true ) {
			$application_version_date_days_old = TTDate::getDays( ( time() - (int)APPLICATION_VERSION_DATE ) );

			if ( $application_version_date_days_old > 365 ) {
				global $config_vars;

				if ( $application_version_date_days_old > 365 && $application_version_date_days_old < 395 ) {
					//After 1yr, show message only to primary company
					$permission_check = 80;
					$primary_company_only = true;
				} else if ( $application_version_date_days_old > 395 && $application_version_date_days_old < 425 ) {
					//After 1yr + 30 days, show message only to primary company only to supervisors or higher permissions
					$permission_check = 40;
					$primary_company_only = true;
				} else if ( $application_version_date_days_old > 425 && $application_version_date_days_old < 455 ) {
					//After 1yr + 30 days, show message to all companies only to supervisors or higher permissions
					$permission_check = 40;
					$primary_company_only = false;
				} else {
					//After 1yr + 90 days, show message to all companies, all employees over level 30
					$permission_check = 30;
					$primary_company_only = false;
				}

				$notification_data = [
						'object_id'      => TTUUID::getNotExistID( 1050 ),
						'object_type_id' => 0,
						'priority_id'	 => 2, //High
						'type_id'        => 'system',
						'title_short'    => TTi18n::getText( 'WARNING: %1 version (v%2) is severely out of date.', [ APPLICATION_NAME, APPLICATION_VERSION ] ),
						'body_short'     => TTi18n::getText( '%1 version (v%2) is severely out of date and may no longer be supported! Please upgrade to the latest version as soon as possible to avoid security issues and invalid calculations.', [ APPLICATION_NAME, APPLICATION_VERSION ] ),
						'body_long_html' => TTi18n::getText( '%1 version (v%2) is severely out of date and may no longer be supported! Please upgrade to the latest version as soon as possible to avoid security issues and invalid calculations.', [ APPLICATION_NAME, APPLICATION_VERSION ] ), //Use this to append email footer.
				];

				if ( $primary_company_only == true ) {
					if ( isset( $config_vars['other']['primary_company_id'] ) ) {
						Notification::sendNotificationToAllUsers( $permission_check, true, true, $notification_data, ( 14 * 86400 ), $config_vars['other']['primary_company_id'] ); //Primary company, send notice more often.
					}
				} else {
					Notification::sendNotificationToAllUsers( $permission_check, true, true, $notification_data, ( 28 * 86400 ) ); //Send to all companies as its severely out-of-date and prior notices were ignored.
				}
			}
		}
	}
	 ...
	 * @return void
	 */
	static function checkDatabaseInSync() {
		if ( DEMO_MODE == false && PRODUCTION == true ) {
			$sslf = new SystemSettingListFactory();
			$system_settings = $sslf->getAllArray();
			unset( $sslf );

			//Check that installed application edition is community, but it has SQL schema for another edition.
			if ( ( getTTProductEdition() == TT_PRODUCT_COMMUNITY && isset( $system_settings['schema_version_group_B'] ) ) || ( getTTProductEdition() > TT_PRODUCT_COMMUNITY && !isset( $system_settings['schema_version_group_B'] ) ) || ( new Install() )->checkProductEditionMatch() == false ) {
				$notification_data = [
						'object_id'      => TTUUID::getNotExistID( 1060 ),
						'object_type_id' => 0,
						'type_id'        => 'system',
						'priority_id'	 => 2, //High
						'title_short'    => TTi18n::getText( 'WARNING: Database schema out of sync.' ),
						'body_short'     => TTi18n::getText( '%1 database schema is out of sync with edition and likely corrupt. Please contact your %1 administrator immediately.', APPLICATION_NAME ),
						'body_long_html' => TTi18n::getText( '%1 database schema is out of sync with edition and likely corrupt. Please contact your %1 administrator immediately.', APPLICATION_NAME ), //Use this to append email footer.
				];
	 ...
	 * @return void
	 */
	static function checkStackComponentsOutOfDate() {
		if ( DEMO_MODE == false && PRODUCTION == true && (int)SystemSettingFactory::getSystemSettingValueByKey( 'schema_version_group_A' ) >= 1122 ) {
			if ( version_compare( PHP_VERSION, '8.1.0', '<' ) == true || PHP_INT_SIZE === 4 ) { //Check for 32-bit installs as well, since accrual policies with length of service dates >2038 will fail.
				if ( OPERATING_SYSTEM == 'WIN' ) {
					$message = TTi18n::getText( 'System stack components are out of date and not supported with this version of %1! Please perform a manual upgrade to the latest version of %1 immediately!', APPLICATION_NAME );
				} else {
					$message = TTi18n::getText( 'System stack components (PHP/PostgreSQL) are out of date and not supported with this version of %1! Please upgrade them immediately!', APPLICATION_NAME );
				}

				$notification_data = [
						'object_id'      => TTUUID::getNotExistID( 1070 ),
						'object_type_id' => 0,
						'type_id'        => 'system',
						'priority_id'	 => 2, //High
						'title_short'    => TTi18n::getText( 'WARNING: System stack components out of date.' ),
						'body_short'     => $message,
						'body_long_html' => $message, //Use this to append email footer.
				];
	...
	static function sendNotification() {
		//Schema 1120=When notification table was created -- This is important in case a schema upgrade version fails, it won't try to send out a notification before the notification table exists.
		if ( DEMO_MODE == false && (int)SystemSettingFactory::getSystemSettingValueByKey( 'schema_version_group_A' ) >= 1122 ) {
			$link = 'https://coreapi.timetrex.com/h.php?id=changelog&v=' . APPLICATION_VERSION . '&e=' . getTTProductEdition();
			$notification_data = [
					'type_id'        => 'system',
					'object_id'      => TTUUID::getNotExistID( 1120 ),
					'priority_id'    => 5, //Normal
					'object_type_id' => 0,
					'title_short'    => TTi18n::getText( 'NOTICE: %1 has been upgraded.', APPLICATION_NAME ),
					'body_long_html' => TTi18n::getText( '%1 has been upgraded to v%2. Please see the <a href="%3">Release Notes</a> for important information regarding changes in this version.', [ APPLICATION_NAME, APPLICATION_VERSION, $link ] ),
					'body_short'     => TTi18n::getText( '%1 has been upgraded to v%2, click here to see the Release Notes for important information regarding changes in this version.', [ APPLICATION_NAME, APPLICATION_VERSION ] ),
					'payload'        => [ 'link_target' => '_blank', 'link' => $link ],
			];

			Notification::sendNotificationToAllUsers( 80, true, true, $notification_data, ( 7 * 86400 ) ); //Send to all companies, as all users should be made aware of version changes.
		} else {
			Debug::Text( 'WARNING: Unable to send notifications due to DEMO MODE or SQL schema version...', __FILE__, __LINE__, __METHOD__, 9 );
		}
	}
