 ...
 * @implements IteratorAggregate<MessageRecipientFactory>
 */
class MessageRecipientListFactory extends MessageRecipientFactory implements IteratorAggregate {
	...
	function getByCompanyIdAndUserIdAndMessageSenderIdAndStatus( $company_id, $user_id, $id, $status_id, $where = null, $order = null ) {
		...
		$query = '
					SELECT a.*, c.message_control_id as message_control_id
					FROM ' . $this->getTable() . ' as a
						LEFT JOIN ' . $uf->getTable() . ' as b ON a.user_id = b.id
						LEFT JOIN ' . $msf->getTable() . ' as c ON a.message_sender_id = c.id 
					WHERE
							b.company_id = ?
							AND a.user_id = ?
							AND a.status_id = ?
