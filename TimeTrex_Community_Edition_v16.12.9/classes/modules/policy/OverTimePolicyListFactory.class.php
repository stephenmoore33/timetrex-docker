 ...
 * @implements IteratorAggregate<OverTimePolicyFactory>
 */
class OverTimePolicyListFactory extends OverTimePolicyFactory implements IteratorAggregate {
	...
	function getByIdAndCompanyId( $id, $company_id, $where = null, $order = null ) {
		...
		if ( $order == null ) {
			$order = [ 'type_id' => 'asc', 'trigger_time' => 'desc' ];
			$strict = false;
		} else {
			$strict = true;
		}
	...
	function getByCompanyIdAndContributingShiftPolicyId( $id, $contributing_shift_policy_id, $where = null, $order = null ) {
		...
		$query = '
					...
					where	company_id = ?
						AND ( 	
								contributing_shift_policy_id in (' . $this->getListSQL( $contributing_shift_policy_id, $ph, 'uuid' ) . ') 
								OR
								trigger_time_adjust_contributing_shift_policy_id in (' . $this->getListSQL( $contributing_shift_policy_id, $ph, 'uuid' ) . ')
							)
						AND deleted = 0';
		$query .= $this->getWhereSQL( $where );
		$query .= $this->getSortSQL( $order, $strict );
	...
	function getByPolicyGroupUserId( $user_id, $where = null, $order = null ) {
		...
		if ( $order == null ) {
			$order = [ 'd.type_id' => 'asc', 'd.trigger_time' => 'desc' ];
			$strict = false;
		} else {
			$strict = true;
		}
	...
	function getByPolicyGroupUserIdOrId( $user_id, $id = null, $where = null, $order = null ) {
		...
		if ( $order == null ) {
			$order = [ 'type_id' => 'asc', 'trigger_time' => 'desc', 'is_policy_group' => 'asc', 'id' => 'asc' ]; //Order by is_policy_group last so policies coming from the policy group are returned last and therefore override duplicate policies from the schedule policy.
			$strict = false;
		} else {
			$strict = true;
		}
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query = '
					select	
							a.*,
							(
								CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $pgf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 110 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
									THEN 1
									ELSE
