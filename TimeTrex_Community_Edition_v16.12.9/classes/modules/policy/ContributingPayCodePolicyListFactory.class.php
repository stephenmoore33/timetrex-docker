 ...
 * @implements IteratorAggregate<ContributingPayCodePolicyFactory>
 */
	...
	function getByCompanyIdAndPayCodeId( $id, $pay_code_id, $where = null, $order = null ) {
		...
		$query = '
					...
					FROM 
							...
							' . $cgmf->getTable() . ' as c
					WHERE	a.company_id = ?
						AND ( a.id = c.object_id AND a.company_id = c.company_id AND c.object_type_id = 90 AND c.deleted = 0 )
					...
					select	
							...
							(
								...
								ELSE 
									...
									THEN 1
									ELSE 
										CASE WHEN EXISTS ( select 1 from ' . $rtpf->getTable() . ' as x where x.contributing_pay_code_policy_id = a.id and x.deleted = 0)
										THEN 1
										ELSE 
											CASE WHEN EXISTS ( select 1 from ' . $otpf->getTable() . ' as x where x.contributing_pay_code_policy_id = a.id and x.deleted = 0)
											THEN 1
											ELSE 
												CASE WHEN EXISTS ( select 1 from ' . $ppf->getTable() . ' as x where x.contributing_pay_code_policy_id = a.id and x.deleted = 0)
												THEN 1
												ELSE 0
												END
											END
							...
							from	' . $this->getTable() . ' as a
						LEFT JOIN ' . $cgmf->getTable() . ' as b ON ( a.id = b.object_id AND b.company_id = a.company_id AND b.object_type_id = 90 AND b.deleted = 0 )
