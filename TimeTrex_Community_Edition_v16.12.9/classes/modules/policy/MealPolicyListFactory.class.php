 ...
 * @implements IteratorAggregate<MealPolicyFactory>
 */
	...
	function getByPolicyGroupUserIdOrId( $user_id, $id = null, $where = null, $order = null ) {
		...
		$query = '
					...
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND b.company_id = c.company_id AND c.object_type_id = 150 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
	...
	function getByPolicyGroupUserId( $user_id, $where = null, $order = null ) {
		...
		$query = '
					...
					from	' . $pguf->getTable() . ' as a,
							...
							' . $this->getTable() . ' as d
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND c.company_id = b.company_id AND c.object_type_id = 150 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
						AND ( b.deleted = 0 AND d.deleted = 0 )
						';
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query = '
					select	
							a.*,
							(
								CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $pgf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 150 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
									THEN 1
									ELSE
										CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $spf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 155 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
										THEN 1 ELSE 0
										END
								END
							) as in_use,
