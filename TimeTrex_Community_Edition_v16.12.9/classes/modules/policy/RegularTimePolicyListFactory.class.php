 ...
 * @implements IteratorAggregate<RegularTimePolicyFactory>
 */
class RegularTimePolicyListFactory extends RegularTimePolicyFactory implements IteratorAggregate {
	 ...
	 * @return bool|RegularTimePolicyListFactory
	 */
	function getByCompanyIdAndContributingPayCodePolicyId( $company_id, $id, $limit = null, $where = null, $order = null ) {
		if ( $id == '' ) {
			return false;
		}
		...
		$query = '
					...
					where
						company_id = ?
						AND contributing_pay_code_policy_id in (' . $this->getListSQL( $id, $ph, 'uuid' ) . ')
						AND deleted = 0';
		$query .= $this->getWhereSQL( $where );
		$query .= $this->getSortSQL( $order, $strict );
	...
	/**
	 * @param string $company_id            UUID
	 * @param string $contributing_shift_id UUID
	 * @param string $pay_code_id           UUID
	 * @param string $pay_formula_policy_id UUID
	 * @param int $limit                    Limit the number of records returned
	 * @param array $where                  Additional SQL WHERE clause in format of array( $column => $filter, ... ). ie: array( 'id' => 1, ... )
	 ...
	 * @return bool|RegularTimePolicyListFactory
	 */
	function getByCompanyIdAndContributingShiftIdAndPayCodeIdAndPayFormulaPolicyId( $company_id, $contributing_shift_id, $pay_code_id, $pay_formula_policy_id, $limit = null, $where = null, $order = null ) {
		if ( $company_id == '' ) {
			return false;
		}

		if ( $contributing_shift_id == '' ) {
			return false;
		}
		...
		$query = '
					...
					where
						company_id = ?
						AND contributing_shift_policy_id in (' . $this->getListSQL( $contributing_shift_id, $ph, 'uuid' ) . ')
						AND pay_code_id in (' . $this->getListSQL( $pay_code_id, $ph, 'uuid' ) . ')
						AND pay_formula_policy_id in (' . $this->getListSQL( $pay_formula_policy_id, $ph, 'uuid' ) . ')
						AND deleted = 0';
		$query .= $this->getWhereSQL( $where );
	...
	function getByPolicyGroupUserId( $user_id, $where = null, $order = null ) {
		...
		$query = '
					...
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND b.company_id = c.company_id AND c.object_type_id = 110 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
	...
	function getByPolicyGroupUserIdOrId( $user_id, $id = null, $where = null, $order = null ) {
		...
		if ( $order == null ) {
			$order = [ 'calculation_order' => 'asc', 'name' => 'asc', 'is_policy_group' => 'asc', 'id' => 'asc' ]; //Order by is_policy_group last so policies coming from the policy group are returned last and therefore override duplicate policies from the schedule policy.
			$strict = false;
		} else {
			$strict = true;
		}
		...
		$query = '
					...
					from	' . $pguf->getTable() . ' as a,
							...
							' . $this->getTable() . ' as d
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND b.company_id = c.company_id AND c.object_type_id = 100 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
						AND ( b.deleted = 0 AND d.deleted = 0 )
					UNION
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query = '
					select	
							a.*,
							(
								CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $pgf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 100 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 ) THEN 1 ELSE 0 END
							) as in_use,
