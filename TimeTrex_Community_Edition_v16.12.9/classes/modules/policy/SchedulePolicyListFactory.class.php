 ...
 * @implements IteratorAggregate<SchedulePolicyFactory>
 */
class SchedulePolicyListFactory extends SchedulePolicyFactory implements IteratorAggregate {
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$additional_order_fields = [ 'd.name', 'c.name' ];

		$sort_column_aliases = [
				'full_shift_absence_policy'    => 'd.name',
				'partial_shift_absence_policy' => 'c.name',
		];
		...
		$query .= ( isset( $filter_data['meal_policy_id'] ) ) ? ' AND ( a.id in ( SELECT object_id FROM ' . $cgmf->getTable() . ' as k WHERE a.id = k.object_id AND k.company_id = a.company_id AND k.object_type_id = 155 AND k.deleted = 0 ' . $this->getWhereClauseSQL( 'k.map_id', $filter_data['meal_policy_id'], 'uuid_list', $ph ) . ' ) ) ' : null;
		$query .= ( isset( $filter_data['break_policy_id'] ) ) ? ' AND ( a.id in ( SELECT object_id FROM ' . $cgmf->getTable() . ' as l WHERE a.id = l.object_id AND l.company_id = a.company_id AND l.object_type_id = 165 AND l.deleted = 0 ' . $this->getWhereClauseSQL( 'l.map_id', $filter_data['break_policy_id'], 'uuid_list', $ph ) . ' ) ) ' : null;
