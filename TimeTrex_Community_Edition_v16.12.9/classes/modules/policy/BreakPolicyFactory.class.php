...
class BreakPolicyFactory extends Factory {
	...
	function _getSchemadata( ?array $filter = null ): ?TTS {
		...
		if ( empty( $filter ) || in_array( 'database', $filter, true ) ) {
			...
			$schema_data->setColumns(
					TTSCols::new(
							...
							TTSCol::new( 'trigger_time' )->setFunctionMap( 'TriggerTime' )->setType( 'integer' ),
							TTSCol::new( 'amount' )->setFunctionMap( 'Break Time' )->setType( 'integer' )->setIsNull( false ),
							TTSCol::new( 'auto_detect_type_id' )->setFunctionMap( 'AutoDetectType' )->setType( 'integer' )->setIsNull( false ),
							TTSCol::new( 'start_window' )->setFunctionMap( 'StartWindow' )->setType( 'integer' ),
							TTSCol::new( 'window_length' )->setFunctionMap( 'WindowLength' )->setType( 'integer' ),
							TTSCol::new( 'minimum_punch_time' )->setFunctionMap( 'MinimumPunchTime' )->setType( 'integer' ),
							TTSCol::new( 'maximum_punch_time' )->setFunctionMap( 'MaximumPunchTime' )->setType( 'integer' ),
							TTSCol::new( 'include_break_punch_time' )->setFunctionMap( 'IncludeBreakPunchTime' )->setType( 'smallint' ),
							TTSCol::new( 'include_multiple_breaks' )->setFunctionMap( 'IncludeMultipleBreaks' )->setType( 'smallint' ),
							TTSCol::new( 'pay_code_id' )->setFunctionMap( 'PayCode' )->setType( 'uuid' ),
							TTSCol::new( 'pay_formula_policy_id' )->setFunctionMap( 'PayFormulaPolicy' )->setType( 'uuid' ),
							TTSCol::new( 'branch_id' )->setFunctionMap( 'Branch' )->setType( 'uuid' ),
							TTSCol::new( 'department_id' )->setFunctionMap( 'Department' )->setType( 'uuid' ),
							TTSCol::new( 'job_id' )->setFunctionMap( 'Job' )->setType( 'uuid' ),
							TTSCol::new( 'job_item_id' )->setFunctionMap( 'JobItem' )->setType( 'uuid' ),
							TTSCol::new( 'description' )->setFunctionMap( 'Description' )->setType( 'varchar' ),
							TTSCol::new( 'allocation_type_id' )->setFunctionMap( 'AllocationType' )->setType( 'smallint' )
		...
		if ( empty( $filter ) || in_array( 'fields', $filter, true ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setTabs(
					TTSTabs::new(
							TTSTab::new( 'tab_break_policy' )->setLabel( TTi18n::getText( 'Break Policy' ) )->setFields(
									new TTSFields(
											...
											TTSField::new( 'type_id' )->setType( 'single-dropdown' )->setLabel( TTi18n::getText( 'Type' ) )->setDataSource( TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getOptions' )->setArg( 'type' ) ),
											TTSField::new( 'trigger_time' )->setType( 'text' )->setLabel( TTi18n::getText( 'Active After' ) )->setWidth( '100%' ),
											TTSField::new( 'amount' )->setType( 'text' )->setLabel( TTi18n::getText( 'Deduction/Addition Time' ) )->setWidth( '100%' ),
											TTSField::new( 'auto_detect_type_id' )->setType( 'single-dropdown' )->setLabel( TTi18n::getText( 'Auto-Detect Breaks By' ) )->setDataSource( TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getOptions' )->setArg( 'auto_detect_type' ) ),
											TTSField::new( 'minimum_punch_time' )->setType( 'text' )->setLabel( TTi18n::getText( 'Minimum Punch Time' ) )->setWidth( '100%' ),
											TTSField::new( 'maximum_punch_time' )->setType( 'text' )->setLabel( TTi18n::getText( 'Maximum Punch Time' ) )->setWidth( '100%' ),
											TTSField::new( 'start_window' )->setType( 'text' )->setLabel( TTi18n::getText( 'Start Window' ) )->setWidth( '100%' ),
											TTSField::new( 'window_length' )->setType( 'text' )->setLabel( TTi18n::getText( 'Window Length' ) )->setWidth( '100%' ),
											TTSField::new( 'include_break_punch_time' )->setType( 'checkbox' )->setLabel( TTi18n::getText( 'Include Any Punched Time for Break' ) ),
											TTSField::new( 'include_multiple_breaks' )->setType( 'checkbox' )->setLabel( TTi18n::getText( 'Include Multiple Breaks' ) ),
											TTSField::new( 'allocation_type_id' )->setType( 'single-dropdown' )->setLabel( TTi18n::getText( 'Allocation Type' ) )->setDataSource( TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getOptions' )->setArg( 'allocation_type' ) ),
		...
		if ( empty( $filter ) || in_array( 'api_methods', $filter, true ) ) {
			...
			$schema_data->setAPIMethods(
					TTSAPIs::new(
							TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getBreakPolicy' )
									->setSummary( 'Get break policy records.' )
									->setArgs( [ 'data' => [ 'filter_data' => $schema_data->getSearchFields(), 'filter_columns' => $schema_data->getFields() ] ] )
									->setArgsModelDescription( 'Under the "data" arg there must be at least one or more "filter_data" elements specified, and one or more "filter_columns" elements which define the minimum necessary columns of data to return for carrying out other actions. For example if looking up the "id" of a record, just the "id" filter_columns needs to be specified with a value of true.' ),
							TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'setBreakPolicy' )
									->setSummary( 'Add or edit break policy records. Will return the record UUID upon success, or a validation error if there is a problem.' )
									->setArgs( [ 'data' => $schema_data->getFields() ] ),
							TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'deleteBreakPolicy' )
									->setSummary( 'Delete break policy records by passing in an array of UUIDs.' )
									->setArgs( new TTSFields(
													   TTSField::new( 'data' )->setType( 'multi-dropdown' )->setLabel( TTi18n::getText( 'IDs to Delete' ) )->setDataSource( TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getBreakPolicy' ) ),
											   ) ),
							TTSAPI::new( 'APIBreakPolicy' )->setMethod( 'getBreakPolicyDefaultData' )
									->setSummary( 'Get default break policy data used for creating new break policies. Use this before calling setBreakPolicy to get the correct default data.' ),
					),
			);
		}
		...
		switch ( $name ) {
			...
			case 'auto_detect_type':
				$retval = [
						...
						20 => TTi18n::gettext( 'Punch Time (Proactive)' ), //Pro-actively attempts to detect break, required for break reminders.
						25 => TTi18n::gettext( 'Punch Time (Reactive)' ), //Re-Actively detects break, therefore break reminders can't work unless the employee manually forces the punch to break.
				];
				break;
	...
	*/
		...
		if ( $this->getDeleted() != true && $this->Validator->getValidateOnly() == false ) { //Don't check the below when mass editing.
			if ( $ignore_warning == false ) { //Don't check the below when mass editing, but must check when adding a new record..
				if ( abs( $this->getAmount() ) > 9000 ) { //Breaks longer than 2.5hrs should trigger warning
					$this->Validator->Warning( 'amount', TTi18n::gettext( 'Break Time may be too high' ) );
				}
			}
	...
	function addLog( $log_action ) {
		return TTLog::addEntry( $this->getId(), $log_action, TTi18n::getText( 'Break Policy' ) .': '. $this->getName(), null, $this->getTable(), $this );
	}
}
