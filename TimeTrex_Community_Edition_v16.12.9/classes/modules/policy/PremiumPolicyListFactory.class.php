 ...
 * @implements IteratorAggregate<PremiumPolicyFactory>
 */
class PremiumPolicyListFactory extends PremiumPolicyFactory implements IteratorAggregate {
	...
	function getByPolicyGroupUserId( $user_id, $where = null, $order = null ) {
		...
		$query = '
					...
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND b.company_id = c.company_id AND c.object_type_id = 120 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
	...
	function getByPolicyGroupUserIdOrId( $user_id, $id = null, $where = null, $order = null ) {
		...
		if ( $order == null ) {
			$order = [ 'type_id' => 'asc', 'is_policy_group' => 'asc', 'id' => 'asc' ]; //Order by is_policy_group last so policies coming from the policy group are returned last and therefore override duplicate policies from the schedule policy. );
			$strict = false;
		} else {
			$strict = true;
		}
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query = '
					select	
							a.*,
							(
								CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $pgf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 120 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
									THEN 1
									ELSE
										CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $spf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 125 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
										THEN 1 ELSE 0
										END
								END
							) as in_use,
