 ...
 * @implements IteratorAggregate<HolidayPolicyFactory>
 */
	...
	function getByPolicyGroupUserId( $user_id, $where = null, $order = null ) {
		...
		$query = '
					...
					where	a.policy_group_id = b.id
						AND ( b.id = c.object_id AND c.company_id = b.company_id AND c.object_type_id = 180 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND a.user_id = ?
	...
	function getByPolicyGroupCompanyIdAndUserIdOrAssignedToContributingShiftPolicy( $company_id, $user_id, $where = null, $order = null ) {
		...
		$query = '
					...
					from
							...
							' . $this->getTable() . ' as d
					where
						( b.id = c.object_id AND c.company_id = b.company_id AND c.object_type_id = 690 AND c.deleted = 0 )
						AND c.map_id = d.id
						AND b.company_id = ?
						AND ( b.deleted = 0 AND d.deleted = 0 )
					UNION ALL
	...
	function getAPISearchByCompanyIdAndArrayCriteria( $company_id, $filter_data, $limit = null, $page = null, $where = null, $order = null ) {
		...
		$query = '
					select	
							a.*,
							(
								CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $pgf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 180 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 ) 
								THEN 1
								ELSE
									CASE WHEN EXISTS ( select 1 from ' . $cgmf->getTable() . ' as w, ' . $cspf->getTable() . ' as v where w.company_id = a.company_id AND w.object_type_id = 690 AND w.map_id = a.id AND w.object_id = v.id AND w.deleted = 0 AND v.deleted = 0 )
									THEN 1 ELSE 0 END
								END
							) as in_use,
