...
class KPIReport extends Report {
		...
		switch ( $name ) {
			...
			case 'static_columns':
				$retval = [
					...
					'-1000-kpi.type'   => TTi18n::getText( 'KPI Type' ),
					'-1005-kpi.status' => TTi18n::gettext( 'KPI Status' ),
					'-1008-kpi.kpi_group' => TTi18n::gettext( 'KPI Group' ),
					...
					'-1050-user.employee_number'    => TTi18n::gettext( 'Employee #' ),
	...
	function _getData( $format = null ) {
		...
		$filter_data['permission_children_ids'] = $this->getPermissionObject()->getPermissionChildren( 'user_review', 'view', $this->getUserObject()->getID(), $this->getUserObject()->getCompany() );
		...
		$kpi_filter_data = $filter_data;
		unset( $kpi_filter_data['permission_children_ids'] ); //Don't need this for KPIs, as its based on the created_by field, and that isn't what is expected when running this report.
		$klf = TTnew( 'KPIListFactory' ); /** @var KPIListFactory $klf */
		$klf->getAPISearchByCompanyIdAndArrayCriteria( $this->getUserObject()->getCompany(), $kpi_filter_data );
						...
						foreach ( $kpi_rows as $key => $row ) {
							...
							if ( isset( $this->form_data['user_review'][$user_review_control_id][$row['kpi.id']] ) ) {
								...
								if ( $this->form_data['user_review'][$user_review_control_id][$row['kpi.id']]['type_id'] == 10 ) { //10=Scale
									$this->pdf->MultiCell( $column_widths['user_review.rating'], $row_cell_height, ( $row['user_review.rating'] != '' ) ? TTMath::removeTrailingZeros( $row['user_review.rating'], 0 ) : '--', 1, 'C', 0, 0, '', '', true, 0 );
