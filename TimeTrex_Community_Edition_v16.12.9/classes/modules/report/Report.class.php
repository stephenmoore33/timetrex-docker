...
class Report {
	...
	public $duplicate_value_ignored_columns = [ 'first_name' => true, 'last_name' => true, 'middle_name' => true, 'verified_time_sheet' => true, 'verified_time_sheet_date' => true, 'date_stamp' => true, 'start_date' => true, 'end_date' => true, 'start_time' => true, 'end_time' => true ];

	protected $maximum_memory_limit = false;
	protected $memory_per_row = 2000;
	protected $maximum_rows_exceeded = false;
	...
	protected $custom_column_dynamic_types = [
			400, //Integer
			410, //Decimal
			420, //Decimal
			1300, //Timeunit
	...
	protected function __getOptions( $name, $params = null ) {
		...
		switch ( $name ) {
			...
			case 'paycode_columns':
				...
				if ( function_exists('ReportPayCodeaddSortPrefix') == false ) {
					function ReportPayCodeaddSortPrefix( $i, $padding_length = 2 ) {
						return str_pad( $i, $padding_length, 0, STR_PAD_LEFT );
					}
				}
				...
				if ( $pclf->getRecordCount() > 0 ) {
					$padding_length = strlen( $pclf->getRecordCount() );
					$i = 0;
					foreach ( $pclf as $pc_obj ) {
						$retval['-' . $params . '19' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-pay_code:' . $pc_obj->getId() . '_time'] = $pc_obj->getName();
						if ( isset( $absence_pay_codes[$pc_obj->getId()] ) ) {
							$retval['-' . $params . '20' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-absence_taken_pay_code:' . $pc_obj->getId() . '_time'] = $pc_obj->getName() . ' (' . TTi18n::getText( 'Taken' ) . ')';
						}
						$retval['-' . $params . '29' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-pay_code:' . $pc_obj->getId() . '_wage'] = $pc_obj->getName() . ' - ' . TTi18n::getText( 'Wage' );
						$retval['-' . $params . '39' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-pay_code:' . $pc_obj->getId() . '_hourly_rate'] = $pc_obj->getName() . ' - ' . TTi18n::getText( 'Hourly Rate' );
						$retval['-' . $params . '49' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-pay_code:' . $pc_obj->getId() . '_wage_with_burden'] = $pc_obj->getName() . ' - ' . TTi18n::getText( 'Wage w/Burden' );
						$retval['-' . $params . '59' . ReportPayCodeaddSortPrefix( $i, $padding_length ) . '-pay_code:' . $pc_obj->getId() . '_hourly_rate_with_burden'] = $pc_obj->getName() . ' - ' . TTi18n::getText( 'Hourly Rate w/Burden' );
	...
	function setExecutionTimeLimit( $int = false ) {
		if ( $int === false ) {
			...
			if ( isset( $config_vars['other']['report_maximum_execution_limit'] ) && $config_vars['other']['report_maximum_execution_limit'] != '' ) {
				$int = $this->config['other']['maximum_execution_limit'] = (int)$config_vars['other']['report_maximum_execution_limit'];
			}
		}
	...
	function setExecutionMemoryLimit( $str = false ) {
		...
		if ( $str === false ) {
			...
			if ( isset( $config_vars['other']['company_report_maximum_memory_limit'] ) && isset( $config_vars['other']['company_report_maximum_memory_limit'][$company_id] ) && $config_vars['other']['company_report_maximum_memory_limit'][$company_id] != '' ) {
				$str = $config_vars['other']['company_report_maximum_memory_limit'][$company_id];
			} else if ( isset( $config_vars['other']['report_maximum_memory_limit'] ) && $config_vars['other']['report_maximum_memory_limit'] != '' ) {
				$str = $config_vars['other']['report_maximum_memory_limit'];
			}
		}
		$soft_memory_limit = Misc::getBytesFromSize( $str );

		$available_memory = Misc::getSystemMemoryInfo();

		if ( isset( $config_vars['other']['company_report_maximum_memory_limit_hard'] ) && isset( $config_vars['other']['company_report_maximum_memory_limit_hard'][$company_id] ) && $config_vars['other']['company_report_maximum_memory_limit_hard'][$company_id] != '' ) {
			$hard_memory_limit = Misc::getBytesFromSize( $config_vars['other']['company_report_maximum_memory_limit_hard'][$company_id] );
		} else if ( isset( $config_vars['other']['report_maximum_memory_limit_hard'] ) && $config_vars['other']['report_maximum_memory_limit_hard'] != '' ) {
			$hard_memory_limit = Misc::getBytesFromSize( $config_vars['other']['report_maximum_memory_limit_hard'] );
		} else {
			$hard_memory_limit = $available_memory;
		}

		if ( $hard_memory_limit < $soft_memory_limit ) {
			$soft_memory_limit = $hard_memory_limit;
		}

		if ( $available_memory < $soft_memory_limit ) {
			Debug::Text( 'Available memory is less than soft limit, reducing to: ' . ( $available_memory * 0.75 ) . ' Soft Limit was: ' . $soft_memory_limit, __FILE__, __LINE__, __METHOD__, 10 );
			$soft_memory_limit = ( $available_memory * 0.75 ); //75% of available memory.
			$hard_memory_limit = $available_memory; //100% of available memory
		}

		$this->maximum_memory_limit = $soft_memory_limit;

		Debug::Text( 'Setting hard memory limit to: ' . $hard_memory_limit . ' Soft Limit: ' . $soft_memory_limit . ' Available Memory: '. $available_memory .' Based on: ' . $str, __FILE__, __LINE__, __METHOD__, 10 );

		ini_set( 'memory_limit', $hard_memory_limit );

		return true;
	}

	/**
	 * Determines if we are still within memory limits.
	 * @param $disable_debug_log
	 * @return bool
	 */
	function isMemoryLimitValid( $disable_debug_log = false ) {
		if ( $this->maximum_memory_limit > 0 ) {
			$current_memory_usage = memory_get_usage();
			if ( $current_memory_usage > $this->maximum_memory_limit ) {
				Debug::Text( '  WARNING: LIMIT REACHED... Exceeded memory limit: ' . $this->maximum_memory_limit . ' Current Usage: ' . $current_memory_usage .' TimeStamp: '. TTDate::getDate('DATE+TIME', time() ), __FILE__, __LINE__, __METHOD__, 10 );

				return false;
			} else if ( $current_memory_usage > ( $this->maximum_memory_limit * 0.60 ) ) {
				if ( $disable_debug_log == false ) {
					Debug::Text( '  WARNING: LIMIT APPROACHING... Memory usage is over 60% of the limit: ' . $this->maximum_memory_limit . ' Current Usage: ' . $current_memory_usage . ' TimeStamp: ' . TTDate::getDate( 'DATE+TIME', time() ), __FILE__, __LINE__, __METHOD__, 10 );
				}
			}
		}
	...
	/**
	 * Sets the memory per row to ensure its not exhausted.
	 *   Run the report with all columns, all time periods, without any grouping, subtotaling or sorting, and get the peak memory divided by total rows actually returned in the report, then divide by total columns returned to get a per column bytes.
	 *      Since its the peak memory, no need to double it or anything.
	 *   Run it again with just a single column, and use that as the minimum per row memory.
	 *   ie: $this->setMemoryPerRow( max( 2060, ( 197 * count( $columns ) ) ) ); //MAX: 48K for each row @ 247 columns = 197 bytes per column, per row. 2060bytes per row at 1 column.
	 * @param $memory
	 * @return bool
	 */
	function setMemoryPerRow( $memory = 5000 ) {
		$this->memory_per_row = $memory;
	 ...
	 * @return int
	 */
	function getMemoryBasedRowLimit() {
		if ( empty( $this->maximum_memory_limit ) ) {
			$this->setExecutionMemoryLimit();
		}

		if ( !empty( $this->maximum_memory_limit ) && $this->memory_per_row > 0 ) {
			$memory_based_row_limit = (int)floor( $this->maximum_memory_limit / $this->memory_per_row );
		} else {
			$memory_based_row_limit = 50000; //Reasonable default if it can't be calculated otherwise.
		}

		Debug::Text( '  Memory Based Limit: '. $memory_based_row_limit .' rows... Based on: Memory Limit: '. $this->maximum_memory_limit .' Memory Per Row: '. $this->memory_per_row, __FILE__, __LINE__, __METHOD__, 10 );
		return $memory_based_row_limit;
	}

	/**
	 * Checks to make sure we haven't reached the memory based row limit when rows are returned from the SQL database.
	 * @param $total_rows
	 * @return bool
	 */
	function isMemoryBasedRowLimitValid( $total_rows ) {
		$memory_based_row_limit = $this->getMemoryBasedRowLimit();
		Debug::Text( '  Estimated Memory Usage: '. ceil( ( $total_rows * $memory_based_row_limit ) ), __FILE__, __LINE__, __METHOD__, 10 );

		if ( $memory_based_row_limit > 0 && $total_rows >= $memory_based_row_limit ) {
			Debug::Text( '    ERROR: Row Limit based on available memory: '. $memory_based_row_limit .' Total Rows returned: '. $total_rows, __FILE__, __LINE__, __METHOD__, 10 );
			return false;
		}
	...
	function setMaximumRowsExceeded( $value ) {
		$this->maximum_rows_exceeded = $value;
		return true;
	}

	function getMaximumRowsExceeded() {
		return $this->maximum_rows_exceeded;
	}


	/**
	 * Check to see if the maximum rows has been exceeded, and if being called from the API, exit early so we return an error rather than partial data like we would to a web browser.
	 * @return bool true if we want to continue processing with partial data, false if we want to cancel the report entirely.
	 */
	function handleMaximumRowsExceeded( $format ) {
		$retval = true;

		if ( $this->getMaximumRowsExceeded() == true ) {
			if ( $format == 'html' || $format == 'pdf' ) {
				global $authentication;
				if ( is_object( $authentication ) && $authentication->getClientID() == 'api' ) {
					Debug::Text( '  NOTICE: Maximum rows exceeded when call made from API. Aborting report to prevent partial data being returned.', __FILE__, __LINE__, __METHOD__, 10 );
					$retval = false;
				} else {
					Debug::Text( '  NOTICE: Maximum rows exceeded, but only HTML/PDF report, return partial data...', __FILE__, __LINE__, __METHOD__, 10 );
					$retval = true;
				}
			} else {
				//This is critical for payroll export reports, so they don't export partial data without the user being notified of it.
				Debug::Text( '  NOTICE: Maximum rows exceeded, but not HTML/PDF format, do not return partial data...', __FILE__, __LINE__, __METHOD__, 10 );
				$retval = false;
			}
		}
	 ...
	 * @return bool
	 */
	function isSystemLoadValid( $disable_debug_log = false ) {
		//Check system load and memory usage.
		if ( $this->isMemoryLimitValid( $disable_debug_log ) == false ) {
			return false;
		}

		$current_execution_time = ceil( ( ( microtime( true ) - $_SERVER['REQUEST_TIME_FLOAT'] ) * 1000 ) );
		if ( (int)ini_get('max_execution_time') != 0 && $current_execution_time > ( ( $this->config['other']['maximum_execution_limit'] * 1000 ) * 0.60 ) ) { //Make sure we ignore this when running from a QueueWorker or CLI that sets maximum execution time to 0.
			if ( $disable_debug_log == false ) {
				Debug::Text( '  WARNING: LIMIT APPROACHING... Execution Time is over 60% of the limit: ' . ( $this->config['other']['maximum_execution_limit'] * 1000 ) . ' Current Time: ' . $current_execution_time . ' TimeStamp: ' . TTDate::getDate( 'DATE+TIME', time() ), __FILE__, __LINE__, __METHOD__, 10 );
			}
		}

		if ( Misc::isSystemLoadValid( 1, $disable_debug_log ) == false ) {
			return false;
		}
	...
	function convertTimePeriodToStartEndDate( $time_period_arr, $prefix = null, $force_dates_for_pay_periods = false ) {
		...
		//Check if time period exceeds 6 months or more than 6 periods to log warning?
		if ( isset($retarr[$prefix . 'start_date']) && isset($retarr[$prefix . 'end_date']) && TTDate::getDays( abs( ( $retarr[$prefix . 'end_date'] - $retarr[$prefix . 'start_date'] ) ) ) > (31 * 6) ) {
			Debug::Text( '  NOTICE: Time Period exceeds 6 months, this could be a large report...', __FILE__, __LINE__, __METHOD__, 10 );
		}
		...
		if ( is_array( $this->data ) && is_array( $this->getSubTotalConfig() ) && count( $this->getSubTotalConfig() ) > 0 ) {
			...
			foreach ( $this->formatSubTotalConfig() as $k => $iteration_config ) {
				...
				if ( $i == 0 ) {
					...
					//If a user just adds every column as the sub-total, it can cause the actual number of rows to increase by orders of magnitude.
					//  Therefore skip any sub-total columns that don't reduce the number of rows by at least 10%. Otherwise whats the point of sub-totalling?
					//  ie: Sub-totaling on a value that is unique for every row.
					if ( $total_data_rows > 1000 && count( $tmp_sub_total_data ) > ( $total_data_rows * 0.90 ) ) {
						Debug::Text( '   NOTICE: Skipping SubTotal iteration: ' . $i . ' due to not reducing number of rows by at least 10%...', __FILE__, __LINE__, __METHOD__, 10 );
					} else {
						if ( $this->isSystemLoadValid() == true ) { //Since sub-totalling and greatly increase the number of rows in the report, check system log and memory on each iteration.
							...
							foreach ( $tmp_sub_total_data as $key => $data ) {
								if ( isset( $sub_total_data[$key] ) ) {
									...
									for ( $x = 0; $x <= $sub_total_data_count; $x++ ) {
										$new_key .= '_';
							...
							//Since each sub-total iteration adds more records to the final output, check to see if we've exceeded the maximum page limit, and stop once we have.
							if ( ( $total_data_rows + count( $sub_total_data ) ) >= ( $this->config['other']['maximum_page_limit'] * 25 ) ) {
								$this->setMaximumRowsExceeded( true );
								if ( $this->handleMaximumRowsExceeded( $format ) == true ) {
									Debug::Text( '  Sub-totalling exceeded maximum page limit (' . $this->config['other']['maximum_page_limit'] . '), no point in sub-totalling further...', __FILE__, __LINE__, __METHOD__, 10 );
									break;
								}
							}
						} else {
							//Since each sub-total iteration adds more records to the final output, we should check memory usage on each iteration so we can exit gracefully.
							Debug::Text( '   ERROR: SubTotal iteration: ' . $i . ' exceeded system load or memory usage, stopping report...', __FILE__, __LINE__, __METHOD__, 10 );
							return false;
						}
					}
				}
			...
			if ( $format == 'csv' || $format == 'json' || $format == 'raw' || $format == 'payment_services' ) { //Force specific field formats for exporting to CSV format.
				switch ( $type ) {
					case 'report_date':
						...
						break;
					case 'currency':
						$retval = TTMath::MoneyRound( $value, 2, $this->getCurrencyObject() ); //Set MIN decimals to 2 and max to the currency rounding.
						break;
					case 'percent':
					case 'numeric':
					case 'int':
	...
	function getOutput( $format = null ) {
		...
		if ( $this->isSystemLoadValid() == true ) {
			//Post-Group Filter - Put after grouping is handled, otherwise the user might get unexpected results based on the data they actually see.
			//  Also put this after sorting, so the post-group filter can be applied to the sorted data, for example to return the first row of the sorted data. (especially for returning random rows based on a random number generated for each row, then sorted on that.)
			//  This would be a slight performance impact as we are sorting all rows first, then possibily filtering out the majority of them. But its worth the additional functionality I believe.
			$this->calculateCustomColumnFilters( 31 );
		} else {
			return false;
		}
		...
		if ( $format == 'pdf' || $format == 'html' || $format == 'raw' || $format == 'payment_services' || stripos( $format, 'pdf_' ) !== false ) {  //Only total/subtotal for PDF/RAW formats.
			if ( $this->isSystemLoadValid() == true ) {
				$this->Total();
				if ( $this->subTotal( $format ) == false ) { //subTotal() can return false if it exceeds memory/load limits while running.
					return false;
				}
			} else {
				return false;
...
//							}
	...
	function _pdf_displayMaximumPageLimitError() {
		...
		//Go back to first page and add label so its obvious as soon as the user looks at the first page.
		$this->pdf->setPage( 1 );
		$this->pdf->setY( 15 );
		$this->pdf->setTextColor( 255, 0, 0 );
		$this->pdf->SetFont( $this->config['other']['default_font'], 'B', $this->_pdf_fontSize( 14 ) );
		$this->pdf->Cell( $this->pdf->getPageWidth(), $this->_pdf_fontSize( 10 ), TTi18n::getText( 'MAXIMUM PAGES EXCEEDED' ), 0, 0, 'C', 0, '', 1 );
	 ...
	 * @return bool
	 */
	function _pdf_displayMaximumRowsExceededError() {
		$this->pdf->AddPage();
		$this->pdf->Ln( $this->pdf->getPageHeight() / 2 );
		$this->pdf->setTextColor( 255, 0, 0 );
		$this->pdf->SetFont( $this->config['other']['default_font'], 'B', $this->_pdf_fontSize( 18 ) );
		$this->pdf->Cell( $this->pdf->getPageWidth(), $this->_pdf_fontSize( 10 ), TTi18n::getText( 'Maximum rows exceeded, some data may not be displayed.' ), 0, 0, 'C', 0, '', 1 );
		$this->pdf->Ln();
		$this->pdf->setTextColor( 0, 0, 0 );
		$this->pdf->SetFont( $this->config['other']['default_font'], 'B', $this->_pdf_fontSize( 8 ) );
		$this->pdf->Cell( $this->pdf->getPageWidth(), $this->_pdf_fontSize( 6 ), TTi18n::getText( 'Please narrow down the report criteria or time period and run the report again.' ), 0, 0, 'C', 0, '', 1 );
		$this->pdf->Ln( 100 );
		...
		$this->pdf->setTextColor( 255, 0, 0 );
		$this->pdf->SetFont( $this->config['other']['default_font'], 'B', $this->_pdf_fontSize( 14 ) );
		$this->pdf->Cell( $this->pdf->getPageWidth(), $this->_pdf_fontSize( 10 ), TTi18n::getText( 'MAXIMUM ROWS EXCEEDED' ), 0, 0, 'C', 0, '', 1 );
...
//							}
		...
		//In the case of AuditTrail report which is hard limited to 5000 rows, if that number is reached, display a message to the user so they know not all data is on display on the report.
		if ( $maximum_pages_exceeded == false && $this->getMaximumRowsExceeded() == true ) {
			$this->html .= '<tr class="blank-row"><td colspan="' . $columns_count . '"></td></tr>';

			$this->html .= '<tr>';
			$this->html .= '<td class="exceeded-error" colspan="' . $columns_count . '">';
			$this->html .= TTi18n::getText( 'Maximum rows exceeded, some data may not be displayed.' );
			$this->html .= '</td>';
			$this->html .= '</tr>';

			$this->html .= '<tr>';
			$this->html .= '<td class="exceeded-warning" colspan="' . $columns_count . '">';
			$this->html .= TTi18n::getText( 'Please narrow down the report criteria or time period and run the report again.' );
			$this->html .= '</td>';
			$this->html .= '</tr>';
	...
	function _pdf_Footer() {
		...
		$this->pdf->setY( ( $this->pdf->getPageHeight() - $margins['top'] - 10 ) ); //For some reason the bottom margin gets changed to 0 on the last page in TCPDF. So we can't use it here.
...
//		$css .= '.header tbody{font-size: 14px;}';
		...
		$css .= '.print-icon{ text-align: center; vertical-align:bottom; cursor: pointer;}';
		...
		$css .= '.icons{ vertical-align: bottom; width:44%; }';
		$css .= '.icons table{ float: right; text-align: right;}';
...
//		$css .= '.col-header-right{ vertical-align:top; }';
	...
	function _html_Initialize() {
		...
		if ( !$this->html ) {
			...
			$this->html .= '<link rel="stylesheet" href="' . Misc::getURLProtocol() . '://' . Misc::getHostName() . Environment::getDefaultInterfaceBaseURL() . 'html5/theme/default/css/report_wizard.css' . '" type="text/css">';
...
//			$this->html .= '<td class="col-header-right">';
			...
			//icons right
			$this->html .= '<td class="icons">';
			$this->html .= '<table>';
			$this->html .= '<tr>';
			if ( $this->getMaximumRowsExceeded() == true ) {
				$this->html .= '<td class="exceeded-error">MAXIMUM ROWS EXCEEDED</td>';
			}
			$this->html .= '<td class="print-icon" nowrap><span class="tticon tticon-print_black_24dp refresh-btn" style="padding-right: 10px; font-size: 35px!important;" onclick="' . "this.style.display='none';" . ' document.querySelector(\'#refresh-icon\').style.display = \'none\'; window.print();' . "this.style.display='inline-block';" . ' document.querySelector(\'#refresh-icon\').style.display = \'inline-block\';"></span> <span id="refresh-icon" onclick="refreshReport()" style="font-size: 35px!important; display: inline-block;" class="tticon tticon-refresh_black_24dp refresh-btn"></span></td>';
			$this->html .= '</tr>';
			$this->html .= '</table>';
			$this->html .= '</td>';
			...
			$this->html .= '<table>';
			$this->html .= '<tr><td nowrap>' . TTi18n::getText( 'Generated' ) . ': ' . TTDate::getDate( 'DATE+TIME', $this->start_time ) . '</td></tr>';
			$this->html .= '<tr><td nowrap>' . TTi18n::getText( 'Generated For' ) . ': ' . htmlspecialchars( $this->getUserObject()->getFullName(), ENT_QUOTES ) . '</td></tr>';
			$this->html .= '</table>';
			$this->html .= '</td>';
	...
	function _pdf_Table() {
		...
		if ( $maximum_pages_exceeded == false && $this->getMaximumRowsExceeded() == true ) {
			$this->_pdf_displayMaximumRowsExceededError();
		}
					...
					if ( in_array( $column, $columns ) ) {
						...
						switch ( $row['type'] ) {
							...
							case 31:
								...
								$columns_data = array_merge( $columns_data, (array)TTMathFormula::parseColumnsFromFormula( $row['definition'] ) );
								break;
						}

						$tmp_custom_column_records[$column] = $row; //Record to tempoary array so it can re-ordered below to match the display order in the report.
					}
				}
			}

			//Order custom columns in the order they are displayed in the report, so that is the order they are evaluated in as well.
			//  This gives us rudimentary control and ability to use the results of one custom column in another, especially with set_global_var() and get_global_var()
			foreach ($columns as $tmp_column) {
				if ( isset($tmp_custom_column_records[$tmp_column]) ) {
					$this->config['custom_column'][] = $tmp_custom_column_records[$tmp_column];
				}
			}
			unset( $columns, $tmp_column, $tmp_custom_column_records );
	...
	function calculateCustomColumns( $type_id ) {
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			...
			Debug::Text( ' Memory Usage: Current: ' . memory_get_usage() . ' Peak: ' . memory_get_peak_usage() .' Type ID: '. $type_id, __FILE__, __LINE__, __METHOD__, 10 );
		}
	 ...
	 * @return array|bool
	 */
	function getCustomFieldColumns( $prefix_begin_counter, $company_id, $parent_table, $no_prefix_parent_table, $custom_prefix = [] ) {
		$retarr = [];
		...
		if ( is_array( $tmp_custom_fields ) ) {
			...
			foreach ( $tmp_custom_fields as $custom_field_id => $column_info ) {
	 ...
	 * @return array
	 */
	function getDynamicCustomFieldColumns( $prefix_begin_counter, $company_id, $parent_table, $no_prefix_parent_table, $custom_prefix = [] ) {
		$retarr = [];

		//Must loop over just filtered columns from getCustomFields(), rather than all $this->>custom_fields, as this could be called multiple times like in TimeSheetSummaryReport and we need to be able to append just the filtered columns without appending all fields again.
		$tmp_custom_fields = $this->getCustomFields( $company_id, $parent_table, $no_prefix_parent_table, $custom_prefix );
	...
	function getStaticCustomFieldColumns( $prefix_begin_counter, $company_id, $parent_table, $no_prefix_parent_table, $custom_prefix = [] ) {
		...
		if ( is_array( $tmp_custom_fields ) ) {
			...
			//Must loop over just filtered columns from getCustomFields(), rather than all $this->>custom_fields, as this could be called multiple times like in TimeSheetSummaryReport and we need to be able to append just the filtered columns without appending all fields again.
			foreach ( $tmp_custom_fields as $custom_field_id => $column_info ) {
				if ( in_array( $column_info['type_id'], $this->custom_column_dynamic_types ) == false ) {
					$retarr[$custom_field_id] = $column_info['name'];
				}
			}
	...
	function getReportCustomFields() {
