...
$pre_upgrade_file_name = $config_vars['cache']['dir'] . DIRECTORY_SEPARATOR . 'PRE_UPGRADE.PHP';
...
//			}
	...
	if ( in_array( '--stage2', $argv ) ) {
		...
		*/
		...
		@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_stage2_requirements' ], 'pre_install.php' ), 'r' );
		if ( $install_obj->checkAllRequirements( false, [ 'file_checksums', 'php_cli_requirements', 'base_url', 'clean_cache' ] ) == 0 ) {
			...
			if ( $install_obj->getIsUpgrade() == true ) {
				if ( $install_obj->checkDatabaseExists( $config_vars['database']['database_name'] ) == true ) {
					...
					@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_new_schema' ], 'pre_install.php' ), 'r' );
					...
					//Send out notifications that upgrade is complete.
					Install::sendNotification();

					//Finish Stage2 completely.
					Debug::Text( 'Stage 2 Successfull!', __FILE__, __LINE__, __METHOD__, 10 );
					echo "Stage 2 Successfull!\n";
					@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_done' ], 'pre_install.php' ), 'r' );
	...
	if ( in_array( '--schema_only', $argv ) == false && in_array( '--stage2', $argv ) == false ) {
		...
		if ( $full_force === true || $ttsc->isNewVersionReadyForUpgrade( $force ) === true ) {
			...
			@file_get_contents( 'http://coreapi.timetrex.com/pre_install.php?v=' . $install_obj->getFullApplicationVersion() . '&os=' . PHP_OS . '&php_version=' . PHP_VERSION . '&web_server=' . urlencode( substr( $_SERVER['SERVER_SOFTWARE'], 0, 20 ) ) . '&page=unattended_upgrade', 'r' );

			$ttsc->getPreUpgradeStage( $pre_upgrade_file_name );
			if ( file_exists( $pre_upgrade_file_name ) && is_executable( $php_cli ) ) {
				echo "Running pre-upgrade stage...\n";
				@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_pre_upgrade_stage' ], 'pre_install.php' ), 'r' );

				$command = '"' . $php_cli . '" -d opcache.enable_cli=0 "' . $pre_upgrade_file_name .'" --config "' . CONFIG_FILE . '" --base_path "' . Environment::getBasePath() . '"';
				system( $command, $exit_code );
				Debug::Text( 'Running pre-Upgrade Stage... Command: ' . $command . ' Exit Code: ' . $exit_code, __FILE__, __LINE__, __METHOD__, 10 );
			...
			echo "Checking current version system requirements...\n";
			if ( $install_obj->checkAllRequirements( false, [ 'file_checksums', 'php_cli_requirements', 'base_url', 'clean_cache' ] ) == 0 ) {
				...
				@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_check_requirements_pass' ], 'pre_install.php' ), 'r' );
				...
				if ( file_exists( $upgrade_file_name ) || ( !is_soap_fault( $file_url ) && $file_url !== false && $file_url != '' ) ) {
					@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_download' ], 'pre_install.php' ), 'r' );
					...
					if ( file_exists( $upgrade_file_name ) && filesize( $upgrade_file_name ) > 0 ) {
						...
						if ( $zip_result === true ) {
							@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_unzip' ], 'pre_install.php' ), 'r' );
						...
						if ( file_exists( $upgrade_staging_dir ) ) {
							...
							if ( isset( $upgrade_staging_extract_dir ) ) {
								@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_rename_dir' ], 'pre_install.php' ), 'r' );
							...
							if ( file_exists( $latest_unattended_upgrade_tool ) ) {
								if ( is_executable( $php_cli ) ) {
									...
									if ( $exit_code == 0 ) {
										...
										@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_pre_requirements' ], 'pre_install.php' ), 'r' );
										...
										if ( $exit_code == 0 ) {
											...
											@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_new_requirements' ], 'pre_install.php' ), 'r' );

											moveUpgradeFiles( $upgrade_staging_latest_dir );

											@file_get_contents( 'http://coreapi.timetrex.com/' . URLBuilder::getURL( [ 'v' => $install_obj->getFullApplicationVersion(), 'page' => 'unattended_upgrade_launch_stage2' ], 'pre_install.php' ), 'r' );
											...
											if ( $exit_code == 0 ) {
												...
												//
												//  **IMPORTANT: Put any other logic in Stage2 command instead of here, as this will be executing code from the old version!
												//
