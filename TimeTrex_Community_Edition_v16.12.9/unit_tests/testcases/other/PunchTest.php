...
*/
	...
	*/
	...
	function testSplitAtMidnightBasicB() {
		...
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 6:00PM' ), [ 'branch_id' => $this->branch_id, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp2 . ' 2:00AM' ), [ 'branch_id' => $this->branch_id, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [], 'punch_tag_id' => [] ], true );
	...
	function testSplitAtMidnightBasicC() {
		...
		$dd->createPunch( $this->user_id, 20, 20, strtotime( $date_stamp . ' 10:30PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 10, strtotime( $date_stamp . ' 11:30PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp2 . ' 2:00AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testSplitAtMidnightBasicD() {
		...
		//Need to create the punches separately as createPunchPair won't split the punches.
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 5:30PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 20, strtotime( $date_stamp . ' 11:30PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 10, strtotime( $date_stamp2 . ' 12:30AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp2 . ' 2:30AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testSplitAtMidnightBasicE() {
		...
		//Need to create the punches separately as createPunchPair won't split the punches.
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 4:00PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp2 . ' 12:00AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testSplitAtMidnightBasicF() {
		...
		//Need to create the punches separately as createPunchPair won't split the punches.
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 12:00AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp . ' 8:00AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	/**
	 * @group Punch_testPunchEditingShiftDayChangeD
	 */
	function testPunchEditingShiftDayChangeD() {
		//Test to make sure when editing a Lunch In punch that spans midnight and is on the next day it does not switch the entire shift to that next day when shifts are assigned to the day they start on.
		global $dd;
		...
		//Just create out punch, 15.5hrs later. Threshold is 16hrs.
		$dd->createPunchPair( $this->user_id,
							  strtotime( $date_stamp2 . ' 3:30AM' ), //Normal Out
							  strtotime( $date_stamp2 . ' 7:00AM' ), //Normal Out
							  [
									  'in_type_id'    => 20,
									  'out_type_id'   => 10,
									  'branch_id'     => 0,
		...
		$this->assertCount( 2, $punch_arr[$date_epoch][0]['shift_data']['punch_control_ids'] );
		$this->assertEquals( $date_epoch, $punch_arr[$date_epoch][0]['date_stamp'] );
		$this->assertArrayNotHasKey( $date_epoch2, $punch_arr );

		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch2 );
		//Total Time - Day 1
		$this->assertEquals( 5, $udt_arr[$date_epoch][0]['object_type_id'] );
		$this->assertEquals( ( 12.5 * 3600 ), $udt_arr[$date_epoch][0]['total_time'] );

		//Total Time - Day 2
		$this->assertArrayNotHasKey( $date_epoch2, $udt_arr );

		//Edit punch to move out time into next day.
		$dd->editPunch( $punch_arr[$date_epoch][0]['shift_data']['punches'][2]['id'],
						[
								'time_stamp' => strtotime( $date_stamp2 . ' 3:30AM' ), //No actual change.
						] );

		$punch_arr = $this->getPunchDataArray( TTDate::getBeginDayEpoch( $date_epoch ), TTDate::getEndDayEpoch( $date_epoch2 ) );
		//print_r($punch_arr);

		//Make sure current day has totals, but the next day does not.
		$this->assertArrayHasKey( $date_epoch, $punch_arr, 'Punch have been moved to the next day most likely!' );
		$this->assertArrayNotHasKey( $date_epoch2, $punch_arr );
	...
	/**
	 * @group Punch_testPunchDeletingC
	 */
	function testPunchDeletingC() {
		global $dd;

		$this->createPayPeriodSchedule( 10, 57600, 0 ); //Day shift starts on., new_shift_trigger=0
		$this->createPayPeriods();
		$this->getAllPayPeriods();

		//$ppf->setStartDate( TTDate::incrementDate( TTDate::getMiddleDayEpoch( $this->pay_period_objs[0]->getStartDate() ), 1, 'day' ) );

		$date_epoch = TTDate::getMiddleDayEpoch( $this->pay_period_objs[0]->getEndDate() );
		$date_stamp = TTDate::getDate( 'DATE', $date_epoch );

		$date_epoch2 = TTDate::incrementDate( TTDate::getMiddleDayEpoch( $this->pay_period_objs[0]->getEndDate() ), 1, 'day' );
		$date_stamp2 = TTDate::getDate( 'DATE', $date_epoch2 );
		...
		//Close pay period so the above punches can never be changed or moved to another day.
		$this->pay_period_objs[0]->setStatus( 20 );
		$this->pay_period_objs[0]->Save( false );
		...
		$this->assertCount( 1, $punch_arr[$date_epoch2][1]['shift_data']['punches'] );
		$this->assertCount( 1, $punch_arr[$date_epoch2][1]['shift_data']['punch_control_ids'] );
		$this->assertEquals( $date_epoch2, $punch_arr[$date_epoch2][1]['date_stamp'] );
		...
		//Total Time
		$this->assertEquals( 5, $udt_arr[$date_epoch][0]['object_type_id'] );
		$this->assertEquals( ( ( 10 * 3600 ) - TTDate::getDSTOffset( $date_epoch, $date_epoch2 ) ), $udt_arr[$date_epoch][0]['total_time'] );

		//Delete first in punch on the 2nd day. No shifts should be re-assigned anywhere.
		$dd->deletePunch( $punch_arr[$date_epoch2][1]['shift_data']['punches'][0]['id'] );
		...
		$this->assertArrayNotHasKey( $date_epoch2, $punch_arr ); //No punches on the 2nd day now that its been deleted.
	...
	function testRoundingDayTotalNormalLunch() {
		...
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 8:03AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 20, strtotime( $date_stamp . ' 12:00PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 10, strtotime( $date_stamp . ' 12:58PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp . ' 5:12PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testRoundingDayTotalAutoAddLunchA3() {
		...
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 8:03AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 20, strtotime( $date_stamp . ' 12:00PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 20, 10, strtotime( $date_stamp . ' 1:02PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp . ' 5:12PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testRoundingDayTotalAutoDeductLunchA1() {
		...
		//Need to use createPunch() rather than createPunchPair() otherwise day total rounding doesn't happen properly due to createPunch() out-of-order punch creation.
		$dd->createPunch( $this->user_id, 10, 10, strtotime( $date_stamp . ' 8:03AM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
		//$dd->createPunch( $this->user_id, 20, 20, strtotime($date_stamp.' 12:00PM'), array('branch_id' => 0,'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ), TRUE );
		//$dd->createPunch( $this->user_id, 20, 10, strtotime($date_stamp.' 1:02PM'), array('branch_id' => 0,'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ), TRUE );
		$dd->createPunch( $this->user_id, 10, 20, strtotime( $date_stamp . ' 5:12PM' ), [ 'branch_id' => 0, 'department_id' => 0, 'job_id' => 0, 'job_item_id' => 0, 'punch_tag_id' => [] ], true );
	...
	function testScheduleMatchingH() {
		...
		$tmp_branch_id[] = $this->branch_id;
		$tmp_branch_id[] = $dd->createBranch( $this->company_id, 20 );
		$tmp_department_id[] = $dd->createDepartment( $this->company_id, 10 );
		$tmp_department_id[] = $dd->createDepartment( $this->company_id, 20 );
		...
		$this->createSchedule( $this->user_id, $date_epoch, [
				...
				'start_time'         => ' 8:00AM',
				'end_time'           => '1:00PM',
				'branch_id'          => $tmp_branch_id[0],
				'department_id'      => $tmp_department_id[0],
		] );
		...
		$schedule_b = $this->createSchedule( $this->user_id, $date_epoch, [
				...
				'start_time'         => ' 1:00PM',
				'end_time'           => '5:00PM',
				'branch_id'          => $tmp_branch_id[1],
				'department_id'      => $tmp_department_id[1],
		] );
	...
	/**
	 * @group Punch_testScheduleMatchingJ
	 */
	function testScheduleMatchingJ() {
		global $dd;
		...
		$punch_arr = $this->getPunchDataArray( TTDate::getBeginDayEpoch( $date_epoch ), TTDate::getEndDayEpoch( $date_epoch2 ) );
		//print_r($punch_arr);
		$this->assertCount( 2, $punch_arr );             //Make sure only one day exists.
		$this->assertCount( 1, $punch_arr[$date_epoch] );
		$this->assertCount( 2, $punch_arr[$date_epoch2] );
		$this->assertEquals( $date_epoch, $punch_arr[$date_epoch][0]['date_stamp'] );
		$this->assertEquals( $date_epoch2, $punch_arr[$date_epoch2][1]['date_stamp'] );
		...
		//2nd day - 1hr meal is deducted based on the 12A to 3A schedule shift.
		$this->assertEquals( 5, $udt_arr[$date_epoch2][0]['object_type_id'] );
		$this->assertEquals( ( 28740 ), $udt_arr[$date_epoch2][0]['total_time'] );
		$this->assertEquals( 100, $udt_arr[$date_epoch2][1]['object_type_id'] );
		$this->assertEquals( ( -2398 ), $udt_arr[$date_epoch2][1]['total_time'] );
		$this->assertEquals( 100, $udt_arr[$date_epoch2][2]['object_type_id'] );
		$this->assertEquals( ( -1202 ), $udt_arr[$date_epoch2][2]['total_time'] );
		$this->assertCount( 3, $udt_arr[$date_epoch2] );
	...
	function testDefaultPunchSettingsNoScheduleA() {
		...
		$this->assertEquals( [], $data['punch_tag_id'] );
	...
	function testDefaultPunchSettingsNoScheduleB() {
		...
		$tmp_department_id[] = $dd->createDepartment( $this->company_id, 10 );
		$tmp_department_id[] = $dd->createDepartment( $this->company_id, 20 );
		$this->user_id = $dd->createUser( $this->company_id, $this->legal_entity_id, 10, 0, $tmp_branch_id[0], $tmp_department_id[0] ); //Non-Admin user.
		...
		$this->assertEquals( $tmp_branch_id[0], $data['branch_id'], 'Branch' );
		$this->assertEquals( $tmp_department_id[0], $data['department_id'], 'department' );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_id'], 'job' );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_item_id'], 'task' );
		$this->assertEquals( [], $data['punch_tag_id'], 'task' );
	...
	function testDefaultPunchSettingsNoScheduleD() {
		...
		$this->assertEquals( $tmp_branch_id[0], $data['branch_id'] );
		$this->assertEquals( $tmp_department_id[0], $data['department_id'] );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_id'] );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_item_id'] );
		$this->assertEquals( [], $data['punch_tag_id'] );
	...
	function testDefaultPunchSettingsScheduleB() {
		...
		$this->assertEquals( $tmp_branch_id[1], $data['branch_id'] );
		$this->assertEquals( $tmp_department_id[1], $data['department_id'] );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_id'] );
		$this->assertEquals( TTUUID::getZeroID(), $data['job_item_id'] );
		$this->assertEquals( [], $data['punch_tag_id'] );
...
//		$meal_policy_id = $this->createMealPolicy( 10 ); //60min autodeduct
	...
	function testSQLUniqueConstraintError() {
		...
		//WHILE STILL INSIDE THE ORIGINAL PUNCH TRANSACTION -- Cause a unique constraint failure by trying to do a raw insert using the duplicate values.
		// **NOTE: This can behave differently if .ini file has: persistent_connections=TRUE
		// **IMPORTANT**: This likely is not creating a new connection, as pg_connect() requires:  PGSQL_CONNECT_FORCE_NEW flag to be sent to do that. See testAdvisoryLockSession() as an example of how to do it properly.
		...
		try {
			...
			Debug::text( 'SQL Exception: Code: ' . $e->getCode() . ' Message: ' . $e->getMessage(), __FILE__, __LINE__, __METHOD__, 10 );
			$this->assertEquals( -5, $e->getCode(), $e->getMessage() );
//			try {
...
//			}
	...
	function testStationCheckSourceANY() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, 'ANY', 'ANY' );
	...
	function testStationCheckSourceIPv4() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, $ipv4, 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	function testStationCheckSourceIPv4NetMaskA() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, '75.157.242.0/24', 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	function testStationCheckSourceIPv4NetMaskB() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, '127.0.0.0/8,75.157.242.0/24', 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	function testStationCheckSourceIPv6() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, $ipv6, 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	function testStationCheckSourceIPv6NetMaskA() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, '2001:569:f9f0:d900:218:f3ff:fe4e:/112', 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	function testStationCheckSourceIPv6NetMaskB() {
		...
		$station_id = $dd->createStation( $this->company_id, 10, '2005::/20,2001:569:f9f0:d900:218:f3ff:fe4e:/112', 'ANY' );
		Debug::Text( 'Station ID: ' . $station_id . ' User ID: ' . $this->user_id, __FILE__, __LINE__, __METHOD__, 10 );
		$slf = new StationListFactory();
		$slf->getById( $station_id );
		if ( $slf->getRecordCount() == 1 ) {
	...
	/**
	 * @group Punch_testPunchTagsAndRegularTimePolicyA
	 */
	function testPunchTagsAndRegularTimePolicyA() {
		if ( getTTProductEdition() <= TT_PRODUCT_PROFESSIONAL ) {
			return true;
		}

		global $dd; /** @var DemoData $dd */

		$branches = [];
		$branches[] = $dd->createBranch( $this->company_id, 30 ); //Toronto
		$branches[] = $dd->createBranch( $this->company_id, 40 ); //Vancouver

		$departments = [];
		$departments[] = $dd->createDepartment( $this->company_id, 10 ); //Sales
		$departments[] = $dd->createDepartment( $this->company_id, 20 ); //Construction
		...
		$punch_tag_group_normal_id = $dd->createPunchTagGroup( $this->company_id, 10, TTUUID::getZeroID() );
		$punch_tag_group_exclude_id = $dd->createPunchTagGroup( $this->company_id, 10, TTUUID::getZeroID() );

		$punch_tags = [];
		$punch_tags[] = $dd->createPunchTag( $this->company_id, $punch_tag_group_normal_id, 1, 'Include Toronto or Vancouver Branch', [ 'branch_selection_type' => 20, 'branch_ids' => $branches ] );
		$punch_tags[] = $dd->createPunchTag( $this->company_id, $punch_tag_group_normal_id, 2, 'Test include Group', [ 'branch_selection_type' => 20, 'branch_ids' => $branches ] );

		$exclude_punch_tags = [];

		$exclude_punch_tags[] = $dd->createPunchTag( $this->company_id, $punch_tag_group_exclude_id, 3, 'Include Sales Department', [ 'department_selection_type' => 20, 'department_ids' => $departments[0] ] );
		$exclude_punch_tags[] = $dd->createPunchTag( $this->company_id, $punch_tag_group_exclude_id, 4, 'Test Exclude', [ 'department_selection_type' => 20, 'department_ids' => $departments[0] ] );

		//Set user default punch tag. Used in an assert below that exclude user default punch tags in the policy.
		$ulf = TTnew( 'UserListFactory' ); /** @var UserListFactory $ulf */
		$ulf->getById( $this->user_id );
		if ( $ulf->getRecordCount() == 1 ) {
			$u_obj = $ulf->getCurrent(); /** @var UserFactory $u_obj */
			$u_obj->setDefaultPunchTag( [ $punch_tags[1] ] );
		...
		$regular_time_policy_id = $dd->createRegularTimePolicy( $this->company_id, 30, $this->policy_ids['contributing_shift_policy'][10], $this->policy_ids['pay_code'][100], $punch_tags );
		...
		$dd->createPolicyGroup( $this->company_id,
								...
								null,                       //Expense
								null,                       //Absence
								[ $regular_time_policy_id ] //Regular
		);
		...
		$dd->createPunchPair( $this->user_id,
							  ...
							  [
									  ...
									  'job_id'        => 0,
									  'job_item_id'   => 0,
									  'punch_tag_id'  => $punch_tags,
							  ],
							  true
		);

		//Test any punch tag to make sure it gets saved to user date total.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( $punch_tags, $udt_arr[$date_epoch][1]['punch_tag_id'] );

		//Get the regular time policy to update it with new punch tag settings.
		$rtplf = TTnew( 'RegularTimePolicyListFactory' ); /** @var RegularTimePolicyListFactory $rtplf */
		$rtplf->getById( $regular_time_policy_id );
		if ( $rtplf->getRecordCount() > 0 ) {
			$rtp_obj = $rtplf->getCurrent(); /** @var RegularTimePolicyFactory $rtp_obj */
		} else {
			$this->assertEquals( 1, $rtplf->getRecordCount() );
		}



		//Test policy that must include a certain punch tag.
		$date_epoch = TTDate::incrementDate( TTDate::getMiddleDayEpoch( TTDate::getBeginWeekEpoch( time() ) ), 1, 'day' );
		$date_stamp = TTDate::getDate( 'DATE', $date_epoch );

		$rtp_obj->setPunchTag( $punch_tags );
		$rtp_obj->setPunchTagSelectionType( 20 );
		if ( $rtp_obj->isValid() ) {
			$rtp_obj->Save( false );
		}
		...
		//Test policy allows any punch tag except certain punch tag.
		$date_epoch = TTDate::incrementDate( TTDate::getMiddleDayEpoch( TTDate::getBeginWeekEpoch( time() ) ), 2, 'day' );
		$date_stamp = TTDate::getDate( 'DATE', $date_epoch );

		$rtp_obj->setPunchTag( $exclude_punch_tags );
		$rtp_obj->setPunchTagSelectionType( 30 );
		if ( $rtp_obj->isValid() ) {
			$rtp_obj->Save( false );
		}
		...
		$dd->createPunchPair( $this->user_id,
							  ...
							  [
									  ...
									  'job_id'        => 0,
									  'job_item_id'   => 0,
									  'punch_tag_id'  => $exclude_punch_tags,
							  ],
							  true
		);

		//User Date total record should not exist as punch tag criteria failed.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( false, isset( $udt_arr[$date_epoch][1] ) );



		//Test policy requires a specific punch group and our punch tag uses that group id.
		$date_epoch = TTDate::incrementDate( TTDate::getMiddleDayEpoch( TTDate::getBeginWeekEpoch( time() ) ), 3, 'day' );
		$date_stamp = TTDate::getDate( 'DATE', $date_epoch );

		$rtp_obj->setPunchTag( $punch_tags );
		$rtp_obj->setPunchTagSelectionType( 10 );
		$rtp_obj->setPunchTagGroup( $punch_tag_group_normal_id );
		$rtp_obj->setPunchTagGroupSelectionType( 20 );
		if ( $rtp_obj->isValid() ) {
			$rtp_obj->Save( false );
		}
		...
		//Test regular policy contains punch tag group.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( $punch_tags, $udt_arr[$date_epoch][1]['punch_tag_id'] );



		//Opposite of above, user date total should not be created as punch group requirement fails on regular time policy.
		...
		//User date total should not be created as punch group requirement fails as punch tags with wrong group id used on regular time policy.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( false, isset( $udt_arr[$date_epoch][1] ) );



		//Test specific punch tags on policy but exclude user default punch tag.
		$date_epoch = TTDate::incrementDate( TTDate::getMiddleDayEpoch( TTDate::getBeginWeekEpoch( time() ) ), 5, 'day' );
		$date_stamp = TTDate::getDate( 'DATE', $date_epoch );

		$rtp_obj->setPunchTag( $punch_tags );
		$rtp_obj->setPunchTagSelectionType( 20 );
		$rtp_obj->setPunchTagGroupSelectionType( 10 ); //Any
		$rtp_obj->setExcludeDefaultPunchTag( 1 );
		if ( $rtp_obj->isValid() ) {
			$rtp_obj->Save( false );
		}
		...
		//No UDT records should exist as user default punch tag is excluded and the punch has it.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( false, isset( $udt_arr[$date_epoch][1] ) );
	}

	/**
	 * @group Punch_testPunchTagsAndOvertimeTimePolicyA
	 */
	function testPunchTagsAndOverTimePolicyA() {
		if ( getTTProductEdition() <= TT_PRODUCT_PROFESSIONAL ) {
			return true;
		}
		...
		$punch_tags = [];
		$punch_tags[] = $dd->createPunchTag( $this->company_id, TTUUID::getZeroID(), 10, 'Test Punch Tag', [] );

		$regular_time_policy_id = $dd->createRegularTimePolicy( $this->company_id, 30, $this->policy_ids['contributing_shift_policy'][10], $this->policy_ids['pay_code'][100], $punch_tags );
		$overtime_policy_id = $dd->createOverTimePolicy( $this->company_id, 40, $this->policy_ids['contributing_shift_policy'][10], $this->policy_ids['pay_code'][100], $punch_tags );
		...
		//Test any punch tag to make sure it gets saved to user date total from being passed through overtime policy.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( $punch_tags, $udt_arr[$date_epoch][2]['punch_tag_id'] );
		$this->assertEquals( 30, $udt_arr[$date_epoch][2]['object_type_id'] ); //Overtime Policy
	}

	/**
	 * @group Punch_testPunchTagsAndPremiumPolicyA
	 */
	function testPunchTagsAndPremiumPolicyA() {
		if ( getTTProductEdition() <= TT_PRODUCT_PROFESSIONAL ) {
			return true;
		}
		...
		$regular_time_policy_id = $dd->createRegularTimePolicy( $this->company_id, 30, $this->policy_ids['contributing_shift_policy'][10], $this->policy_ids['pay_code'][100], $punch_tags );
		$premium_policy_id = $dd->createPremiumPolicy( $this->company_id, 30, $this->policy_ids['contributing_shift_policy'][10], $this->policy_ids['pay_code'][100], $punch_tags );
		...
		//Test any punch tag to make sure it gets saved to user date total from being passed through premium policy.
		$udt_arr = $this->getUserDateTotalArray( $date_epoch, $date_epoch );
		$this->assertEquals( $punch_tags, $udt_arr[$date_epoch][2]['punch_tag_id'] );
		$this->assertEquals( 40, $udt_arr[$date_epoch][2]['object_type_id'] ); //Premium Policy
	}
}
