...
/**
 * @group USPayrollDeductionTest2022
 */
class USPayrollDeductionTest2022 extends PHPUnit\Framework\TestCase {
	...
	public function setUp(): void {
		...
		$this->tax_table_file = dirname( __FILE__ ) . '/USPayrollDeductionTest2022.csv';
		...
		foreach ( $test_rows as $row ) {
			...
			if ( $row['country'] != '' && $row['gross_income'] != '' ) {
				...
				if ( $row['federal_deduction'] != '' ) {
					...
					if ( $amount_diff > 5 ) { //This is dollars, not percent.
						if ( $amount_diff_percent > 5 && $amount_diff < 20 ) {
							$this->assertLessThan( 30, $amount_diff_percent ); //Handle lower tax brackets that could change by just $20-30 but we 30-40% difference.
				...
				if ( $row['provincial_deduction'] != '' ) {
					...
					if ( !in_array( $row['province'], [ '00', 'CA', 'NC', 'NY', 'OR', 'WI', 'NE', 'AR', 'LA', 'AL' ] ) && $amount_diff > 5 ) { //Some states had significant changes.
	...
	function testTaxBracketConstants() { //Double checks to make sure there isn't an error in the tax brackets or constants.
		...
		if ( !empty($provinces) ) {
			...
			foreach( $provinces as $province => $province_name ) {
				...
				$pd_obj->setDate( strtotime( '01-Jan-2022' ) );
				...
				foreach( $filing_status_arr as $filing_status ) {
					...
					foreach( $new_pd_obj->income_tax_rates as $region_name => $region_tax_brackets ) {
						...
						foreach( $region_tax_brackets as $key => $tax_bracket ) {
							...
							$this->assertEqualsWithDelta( $tax_bracket['constant'], $cumulative_constant, 2.00, 'State: '. $province .' Filing Status: '. $filing_status .' Bracket: '. $key .' Prev Income: '. $tax_bracket['prev_income']  .' Income: '. $tax_bracket['income'] .' Rate: '. $tax_bracket['rate'] .' Constant: '. $tax_bracket['constant'] .' Cummulative Constant: '. $cumulative_constant .' Diff: '. TTMath::sub( $tax_bracket['constant'], $cumulative_constant ));
	...
	function testUS_2022_Test2019W4() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '2181.42', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_TestAdditionalDeductionExcessA() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1282.33', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_TestAdditionalDeductionExcessB() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '2000.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '872.33', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Would be 174.42 without the additional deduction. Test max out of additional deduction at highest tax rate plus buffer.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_TestAdditionalDeductionExcessC() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '260.19', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Would be 11.73 without the additional deduction. Test max out of additional deduction at highest tax rate plus buffer.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_TestAdditionalDeductionExcessD() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022_Test2020W4AdditionalDeductionWithLowIncome() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022_Test2020W4AdditionalDeductionWithHighIncome() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2191.42', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //2227.24
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4AdditionalDeductionWithHighIncomeAndNonPeriodic() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022_Test2020W4SingleJob1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2181.42', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //2228.90
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4TwoJobs1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2773.34', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4OneJobWithDependents1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2085.27', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4OneJobWithOtherIncome1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2316.04', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4OneJobWithDeductions1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2114.12', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_2022_Test2020W4OneJobWithAdditionalDeduction1() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '9615.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '2181.42', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //2128.90 + 100
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket1a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '163.33', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 214 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket1b() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '163.33', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 166 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket2a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '386.07', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 395 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket2b() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '284.42', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 291 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket2c() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '351.81', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 362 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2020W4WageBracket3a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '228.29', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 236 based on 2022 W4 standard withholding tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket1a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '199.92', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 212 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket1b() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '173.25', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 177 based on tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket2a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket2b() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket2c() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	//Examples from 15-T publication.
	function testUS_2022_Test2019W4WageBracket3a() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1925.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '264.77', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) ); //Should be about 236 based on 2022 W4 standard withholding tax tables.
		$this->assertEquals( '0.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_ID_2022a_Test1() {
		...
		$this->assertEquals( '10.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_ID_2022a_Test2() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_LA_2022a_Test1() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_LA_2022a_Test2() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Test1() {
		...
		$this->assertEquals( '41.25', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '37.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
		$this->assertEquals( '35.77', $this->mf( $pd_obj->getEmployeeSocialSecurity() ) );
	}

	function testUS_AR_2022a_Test1() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '39.82', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) ); //Should be 56.83, but they round a lot.
	}

	function testUS_AR_2022a_Test2() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '425.62', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
	}

	function testUS_MS_2022a_Test1() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '80.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) ); //80.85
	}

	function testUS_MS_2022a_Test2() {
		//Example from employer guide.
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
		...
		$this->assertEquals( '1890.00', $this->mf( $pd_obj->getGrossPayPeriodIncome() ) );
		$this->assertEquals( '45.00', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) ); //46.23
	}

	//
	// US Social Security
	//
	function testUS_2022a_SocialSecurity() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_SocialSecurity_Max() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Medicare() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Additional_MedicareA() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Additional_MedicareB() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Additional_MedicareC() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_Additional_MedicareD() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_FederalUI_NoState() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_FederalUI_NoState_CustomRate() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_FederalUI_NoState_Max() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_FederalUI_State_Max() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_StateUI_Max() {
		Debug::text( 'US - SemiMonthly - Beginning of 2022 01-Jan-2022: ', __FILE__, __LINE__, __METHOD__, 10 );
	...
	function testUS_2022a_State_UI_WageBase() {
		$cf = new CompanyFactory();
		$provinces = $cf->getOptions('province', 'US');
		if ( !empty($provinces) ) {
			asort( $provinces );
