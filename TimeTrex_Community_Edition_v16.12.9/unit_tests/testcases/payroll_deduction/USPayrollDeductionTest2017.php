...
class USPayrollDeductionTest2017 extends PHPUnit\Framework\TestCase {
		...
		foreach ( $test_rows as $row ) {
			...
			if ( $row['country'] != '' && $row['gross_income'] != '' ) {
				...
				if ( $row['provincial_deduction'] != '' ) {
					...
					if ( $row['province'] != 'ME' && $row['province'] != 'NE' && $amount_diff > 5 ) { //ME had significant changes.
	...
	function testUS_2017a_Test1() {
		...
		$this->assertEquals( '55.34', $this->mf( $pd_obj->getFederalPayPeriodDeductions() ) );
		$this->assertEquals( '37.84', $this->mf( $pd_obj->getStatePayPeriodDeductions() ) );
